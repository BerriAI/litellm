        ...
        for spend_log in all_spend_logs:
            ...
            if proxy_server_request_dict:
                ...
                if isinstance(_response_input_param, str):
                    ...
                    response_input_param = [
                        cast(ResponseInputItemParam, _response_input_param)
                    ]
                elif isinstance(_response_input_param, list):
                    if response_input_param is None:
                        response_input_param = []
                    for item in _response_input_param:
                        if isinstance(item, dict):
                            response_input_param.append(
                                cast(ResponseInputItemParam, item)
                            )
                        else:
                            response_input_param.append(item)
            ...
            if isinstance(_response_output, dict):
                ...
                for choice in model_response.choices:
                    if _message := getattr(choice, "message", None):
                        chat_completion_message_history.append(
                            GenericChatCompletionMessage(**_message)
                        )
