...
class ProxyExtrasDBManager:
    ...
    def apply_autorouter_daily_coverage() -> None:
        ...
        for attempt in range(MAX_MIGRATE_DEPLOY_ATTEMPTS):
            try:
                prisma_toolchain.run_prisma(
                    [
                        _get_prisma_command(),
                ...
                if isinstance(error, OSError) or attempt + 1 == MAX_MIGRATE_DEPLOY_ATTEMPTS:
                    raise RuntimeError(
                        "Failed to install daily auto-router coverage; database setup cannot continue"
                    ) from error
                logger.warning("Daily auto-router coverage installation attempt %s failed; retrying", attempt + 1)
