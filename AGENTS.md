...
GREEN requires no UI lint, type-check, or build warnings introduced or worsened by the PR. Inspect frontend check output and GitHub annotations against the merge-base baseline. Record pre-existing warnings with evidence and leave unrelated fixes outside the ticket scope. Do not raise budgets or add suppressions to satisfy checks
