...
use crate::{
    BedrockConfig, Detection, Guardrail, GuardrailInput, GuardrailOutcome, InputType,
...
impl BedrockGuardrail {
    pub fn new(config: BedrockConfig, http: Arc<HttpClient>) -> Self {
        Self { config, http }
    }

    fn region(&self) -> Result<String, ProviderError> {
        if let Some(region) = &self.config.aws_region_name {
            return Ok(region.clone());
        }
        std::env::var("AWS_REGION")
            .or_else(|_| std::env::var("AWS_DEFAULT_REGION"))
            .map_err(|_| ProviderError::InvalidConfig {
                message: "bedrock requires aws_region_name or AWS_REGION".to_owned(),
            })
    }
    ...
    async fn credentials(&self) -> Result<Credentials, ProviderError> {
        ...
        chain
            ...
            .map_err(|e| ProviderError::InvalidConfig {
                message: format!("bedrock could not resolve AWS credentials: {e}"),
            })
    }
