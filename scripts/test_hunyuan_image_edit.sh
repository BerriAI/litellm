#!/usr/bin/env bash
#
# End-to-end integration test for Tencent Hunyuan image edit via LiteLLM proxy.
# Starts the proxy (unless --no-start-server), calls /v1/images/edits with a
# URL-based source image, downloads the returned URL, verifies it is a valid
# PNG, then cleans up.
#
# Requirements:
...
#   - curl available
#   - HUNYUAN_API_KEY set (or sourced from .env)
#
# Usage:
#   # Run with credentials from .env (starts its own proxy):
#   ./scripts/test_hunyuan_image_edit.sh
#
#   # Run against an already-running proxy:
#   BASE_URL=http://localhost:4000 LITELLM_API_KEY=sk-1234 \
#       ./scripts/test_hunyuan_image_edit.sh --no-start-server
#
set -euo pipefail
...
# ── Start proxy ────────────────────────────────────────────────────────────────
if [[ "$START_SERVER" == "true" ]]; then
  PROXY_LOG=$(mktemp /tmp/litellm-proxy-edit-XXXXXX.log)
  ...
  for i in $(seq 1 60); do
    ...
    if [[ "$i" -eq 60 ]]; then
      echo ""
      echo "Error: proxy did not start within 60 seconds."
      tail -20 "$PROXY_LOG"
      exit 1
    fi
  done
...
# ── Step 0: Generate a source image to edit ────────────────────────────────────
echo "[0/3] POST /v1/images/generations (get source image for editing)"
...
GEN_HTTP=$(echo "$GEN_RESPONSE" | tail -n1)
GEN_BODY=$(echo "$GEN_RESPONSE" | sed '$d')

if [[ "$GEN_HTTP" -ne 200 ]]; then
  echo "  FAIL: image generation for source returned HTTP $GEN_HTTP"
  echo "$GEN_BODY" | jq . 2>/dev/null || echo "$GEN_BODY"
  exit 1
fi

SOURCE_IMAGE_URL=$(echo "$GEN_BODY" | jq -r '.data[0].url // empty')
if [[ -z "$SOURCE_IMAGE_URL" ]]; then
  echo "  FAIL: generation response did not contain .data[0].url"
  exit 1
fi
echo "  OK (HTTP $GEN_HTTP)"
echo "  Source URL: ${SOURCE_IMAGE_URL:0:80}..."

# ── Step 1: Edit image ─────────────────────────────────────────────────────────
echo ""
echo "[1/3] POST /v1/images/edits"
...
EDIT_HTTP=$(echo "$EDIT_RESPONSE" | tail -n1)
EDIT_BODY=$(echo "$EDIT_RESPONSE" | sed '$d')

if [[ "$EDIT_HTTP" -ne 200 ]]; then
  echo "  FAIL: expected HTTP 200, got $EDIT_HTTP"
  echo "$EDIT_BODY" | jq . 2>/dev/null || echo "$EDIT_BODY"
  exit 1
fi
echo "  OK (HTTP $EDIT_HTTP)"

IMAGE_URL=$(echo "$EDIT_BODY" | jq -r '.data[0].url // empty')
if [[ -z "$IMAGE_URL" ]]; then
...
# ── Step 2: Download the edited image ─────────────────────────────────────────
echo ""
echo "[2/3] Downloading edited image..."
TMP_IMAGE=$(mktemp /tmp/hunyuan-edit-XXXXXX.png)
...
echo ""
echo "All image edit tests passed."
