...
Run tests, format your code, and lint your code before each commit. Shift every CI red left by running the same checks CI runs locally and fixing them before you commit, not after a reviewer flags them. If you touched Python, run `make lint` (it mirrors `test-linting.yml`: ruff format check, `ruff check`, the ruff-strict and basedpyright budgets, the type-discipline gate, circular-import and import-safety checks) and get it green. If you touched the dashboard under `ui/litellm-dashboard`, lint the way `test-litellm-ui-build.yml`'s frontend-lint job does, `npx prettier --check` and `npx eslint --no-warn-ignored --pass-on-unpruned-suppressions` over just the files you changed; the whole-folder `npm run format` / `npm run lint` are broader than CI and will flag pre-existing issues you didn't touch. If you changed anything under `litellm/proxy/` or `litellm/types/` that can move the proxy OpenAPI spec, regenerate the dashboard types with `npm run gen:api` from `ui/litellm-dashboard` and commit the updated `src/lib/http/schema.d.ts` so the check-ui-api-types job stays green. `make install-hooks` wires up an opt-in `pre-commit` hook that runs exactly these checks against the files you staged, so you can let the hook catch reds instead of remembering to
