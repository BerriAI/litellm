...
If you're making a PR on behalf of a member (they have at least write permissions to BerriAI/litellm and/or their GitHub username is postfixed with "-berri" or "-berriai") and resolving a linear ticket, in the "## Linear ticket" section of the PR, say "Resolves LIT-1234", replacing "LIT-1234" with the actual ticket id that you're resolving. It must be exactly that syntax (f"Resolves {ticket_id}") to link the Linear ticket to your PR, which is important to us. If you don't have a ticket id, don't make one up. Just leave the section blank
