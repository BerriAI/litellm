...
Run tests, format your code, and lint your code before each commit. Shift every CI red left by running the same checks CI runs, locally, right before you commit, not after a reviewer or a red pipeline flags them. Run `make pre-commit` immediately before every `git commit`: it inspects your staged files and runs only the matching gating checks below, cleanly skipping any whose files you didn't stage, so a clean run means a green CI lint. It is deliberately not auto-installed as a git hook (`make install-hooks` only wires the fast Conventional Commits / Branches checks), because the dashboard and basedpyright passes can take minutes and that friction shouldn't land on every human commit; you, the agent, run it per commit instead. To run the pieces by hand: when you touched Python under `litellm/`, run `make lint` (isomorphic to `test-linting.yml`'s lint job: diff-scoped ruff format check, `ruff check`, the ruff-strict / type-discipline / basedpyright budgets as a delta vs the base, then circular-import and import-safety). When you touched the dashboard under `ui/litellm-dashboard`, lint the way `test-litellm-ui-build.yml`'s frontend-lint job does: `npx prettier --check` over the changed `js,jsx,ts,tsx,mjs,cjs,json,css,scss,md,mdx,yml,yaml,html` files, `npx eslint --no-warn-ignored --pass-on-unpruned-suppressions` over the changed `js,jsx,ts,tsx,mjs,cjs` files, and the whole-folder lint budgets via `npx eslint . -f json` piped through `scripts/check-lint-budgets.mjs`; the bare `npm run format` / `npm run lint` are broader than CI and will flag pre-existing issues you didn't touch. When you changed anything under `litellm/proxy/` or `litellm/types/` that can move the proxy OpenAPI spec, regenerate the dashboard types with `LITELLM_PYTHON="uv run --no-sync python" npm run gen:api` from `ui/litellm-dashboard` and commit the updated `src/lib/http/schema.d.ts` so the check-ui-api-types job stays green
