...
with the gateway container, so a scrape never runs on an inference worker.
Cloud Run only routes traffic to the gateway container, so the load balancer
keeps hitting port 4000 and the sidecar port is reachable on localhost inside
the instance only. To get the series out, the stack also adds Google's
