...
# Cloud Run routes traffic to a single container port, so the separate
# Prometheus metrics port offered by the AWS module and the Helm charts has no
# equivalent here; /metrics stays on the gateway port behind the internal LB.
...
variable "sse_keepalive_ping_interval_seconds" {
  description = <<-EOT
    ...
    silent (1-300). Off by default. Set it below the load balancer backend
    timeout so a slow first token cannot get the stream closed. Needs
