...
| `jsonLogs`                  | Emit logs as JSON instead of plain text, by setting `JSON_LOGS=True` on the proxy pod. Covers uvicorn's access and error logs alongside LiteLLM's own. Defining `JSON_LOGS` in `envVars` takes precedence.                                     | `false`                   |
