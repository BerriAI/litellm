...
def format_grace_warning_comment(score: int, threshold: int) -> str:
    ...
    return (
        ...
        'a backlog. A closed PR is a soft "park this for later," not a rejection. Take your '
        "time; everything below still works after the close.\n"
        "\n"
        "**During the grace period:** push fixes that address Greptile's feedback (your "
        "existing branch is fine), then comment `@greptileai` to request a fresh review. If "
