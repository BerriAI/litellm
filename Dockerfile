...
# nodejs + npm so the prisma CLI uses Wolfi's Node/npm at runtime. `prisma
# migrate deploy` against a fresh database bootstraps the prisma npm CLI, and
# without npm on PATH prisma-python falls back to nodeenv, which downloads a
# Node build whose dynamic deps (e.g. libatomic.so.1) aren't in the Wolfi
# runtime image and crashes the migration (see #33650, #24554).
