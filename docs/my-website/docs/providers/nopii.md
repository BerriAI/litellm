...
Pass an `X-NoPII-Session-Id` header to group multi-turn conversations in your audit log and skip redundant vault calls on repeated PII. Tokenization is deterministic regardless, so multi-turn conversations stay coherent without it. See [Sessions](https://docs.nopii.co/sessions) for details.
