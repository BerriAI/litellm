...
When a provider's token counting API cannot count a model, LiteLLM falls back to
a local tiktoken estimate. The estimate can be materially lower than the real
count, and the response gives the client no way to tell the two apart.

`strict_token_count` makes a single model fail instead of estimating:
...
model_list:
  - model_name: claude-bedrock
    ...
    model_info:
      strict_token_count: true      # error instead of an estimate
```

A count that cannot be made exactly now returns an error:
...
- Default is `false`. Behaviour is unchanged for models that do not set it.
- This only applies when the provider count fails. A successful provider count
  is returned as before.
- `litellm.disable_token_counter: true` applies the same strictness to **every**
  model on the proxy. `strict_token_count` is the per-model form, for when you
  need an exact count for one model but are happy with an estimate elsewhere.
