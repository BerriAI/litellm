...
| prompt_prefix_affinity_tokens | int | Number of tokens from the canonical prompt prefix used for deterministic prompt-prefix routing. Measured against tokenization of the canonical JSON representation (not the provider-specific "raw prompt token count"). |
| prompt_prefix_affinity_min_tokens | int | Minimum canonical prompt token count before prompt-prefix affinity applies. Measured against tokenization of the canonical JSON representation (not the provider-specific "raw prompt token count"). |
