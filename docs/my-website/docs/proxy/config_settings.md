...
| LITELLM_KEY_ROTATION_GRACE_PERIOD_HOURS | Hours to keep old key valid after rotation (e.g. 24, 48, 72). Default is 0 (immediate revoke). Used for scheduled rotations and as fallback when not specified in regenerate request.
