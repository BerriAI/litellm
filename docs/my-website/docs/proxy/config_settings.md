...
general_settings:
  ...
  reject_metadata_tags: boolean  # if true, rejects requests with 'metadata.tags' to prevent users from influencing budgets
...
| reject_metadata_tags | boolean | If true, rejects requests that contain 'metadata.tags' to prevent users from influencing budgets by sending different tags. Tags can only be inherited from the API key metadata. |
