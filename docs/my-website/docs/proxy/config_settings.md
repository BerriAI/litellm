...
| LITELLM_BEDROCK_IMAGE_EDIT_UNKNOWN_MODEL_FALLBACK | When set to a truthy value (`1`, `true`, etc.), unrecognized Bedrock image-edit model IDs use the legacy Stability adapter. Default is off; unknown models raise `ValueError` so misconfigured IDs fail fast.
