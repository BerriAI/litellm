...
litellm_settings:
  ...
  # Debugging - see debugging.md for more options
...
| set_verbose | boolean | [DEPRECATED - see debugging docs](./debugging.md) Use `--debug` or `--detailed_debug` CLI flags, or set `LITELLM_LOG` env var to "INFO", "DEBUG", or "ERROR" instead. |
...
| SET_VERBOSE | [DEPRECATED] Use `LITELLM_LOG` instead with values "INFO", "DEBUG", or "ERROR". See [debugging docs](./debugging.md)
