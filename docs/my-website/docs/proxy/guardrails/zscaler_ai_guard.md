...
```
You can leverage this feature to apply multiple policies configured on the Zscaler AI Guard (ZGuard) to traffic from different applications. (Note: For consistent policy application, it is recommended to map policies using either Team or Key metadata, but not a mix of both.)
