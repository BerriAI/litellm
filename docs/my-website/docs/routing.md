...
Cooldowns apply to individual deployments, not entire model groups. This section clarifies the behavior when you have multiple deployments with the same model name.

#### Flow Diagram

```mermaid
flowchart TD
    A[Request for model_name: 'claude-sonnet-dashboard'] --> B[Router finds all deployments with model_name: 'claude-sonnet-dashboard']
    ...
    H -->|Yes| J[ONLY Deployment A goes into cooldown]
...
**Individual Deployment Cooldown**
- Only the specific failing deployment is cooled down
- Other deployments with the same model_name remain available
- Each deployment has its own failure tracking and cooldown state

**Provider Isolation**
- If Anthropic API has an outage, only Anthropic deployments are cooled down
- Vertex AI deployments remain unaffected and continue serving requests
- Different providers fail independently

**Example Scenario**

You have multiple deployments named "claude-sonnet-dashboard":

```yaml
model_list:
  - model_name: claude-sonnet-dashboard
    litellm_params:
  ...
  - model_name: claude-sonnet-dashboard
    litellm_params:
      ...
      api_base: https://different-anthropic-endpoint.com
```

**If Anthropic API experiences an outage:**
1. Deployment 1 (Anthropic) - Goes into cooldown after exceeding failure threshold
2. Deployment 2 (Vertex AI) - Remains available and handles requests
3. Deployment 3 (Anthropic #2) - May also go into cooldown if using same failing API

**Result:** Requests continue to work using Vertex AI deployment while Anthropic deployments recover.
...
router = Router(
    ...
    allowed_fails=3,        # deployment cooled down after 3 failures
    cooldown_time=60       # deployment unavailable for 60 seconds
)

# Router automatically handles failover between deployments
response = router.completion(
    model="claude-sonnet-dashboard", 
