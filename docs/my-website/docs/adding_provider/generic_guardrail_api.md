...
4. **Multi-Modal Support** - Handle text, images, and documents in requests/responses
...
1. LiteLLM extracts text, images, and documents from any request (chat messages, embeddings, image prompts, document uploads, etc.)
...
```json
{
  ...
  "documents": ["https://example.com/doc.pdf", "base64_encoded_document_data"],  // optional array of documents (URLs or base64)
...
```json
{
  ...
  "documents": ["modified_url_or_base64_document"]  // optional array of modified documents
}
```
...
- `GUARDRAIL_INTERVENED` - Request proceeds with modified texts/images/documents (provide `texts`, `images`, and/or `documents` fields)
...
The `documents` parameter contains document data from the request, which can be either URLs or base64-encoded file data.

**Format:** Array of strings (URLs or base64-encoded document data)
...
[
  ...
  "base64_encoded_text_file_content"
]
```

**Document Types:**
- **URLs:** Documents starting with `http://` or `https://` (e.g., PDF or TXT files)
- **Base64 Data:** Base64-encoded document content (can include data URI prefix or raw base64)

**Availability:**
- **Both input and output:** Documents can be present in both `input_type="request"` (documents in the user's request) and `input_type="response"` (documents generated by the LLM).
- **Supported endpoints:** The `documents` parameter is supported on endpoints that accept document inputs.

**Use cases:**
- Scan uploaded documents for sensitive information (PII, credentials, etc.)
- Validate document content before processing
- Redact sensitive data from documents
- Block documents containing prohibited content
- Log document uploads for audit purposes
- Enforce document type/size restrictions
- Extract and analyze document metadata

**Document Processing:**
When your guardrail receives documents:
1. **URL documents:** LiteLLM may fetch the document content and pass it to your API (depending on the guardrail configuration)
2. **Base64 documents:** Your API should decode the base64 data to access the document content
3. **Return format:** Return documents in the same format (URLs or base64) in your response
...
class GuardrailRequest(BaseModel):
    ...
    documents: Optional[List[str]] = None  # URLs or base64-encoded document data
...
async def apply_guardrail(request: GuardrailRequest):
    ...
    # Example: Check documents (if present in request or response)
    if request.documents:
        import base64
        for doc in request.documents:
            # Handle URL documents
            if doc.startswith("http://") or doc.startswith("https://"):
                # You could fetch and analyze the document
                # For this example, we'll just check the URL
                if "confidential" in doc.lower():
                    return GuardrailResponse(
                        action="BLOCKED",
                        blocked_reason="Document URL contains restricted terms"
                    )
            else:
                ...
                try:
                    ...
                    decoded_content = base64.b64decode(doc).decode('utf-8', errors='ignore')
                    
                    # Check document content
                    if "secret" in decoded_content.lower():
                        return GuardrailResponse(
                            action="BLOCKED",
