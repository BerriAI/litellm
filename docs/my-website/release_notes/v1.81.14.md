...
- **Major performance batch** — 20+ targeted optimizations shipping together: callback sort moved to registration time, Pydantic serialization eliminated from the hot path, OpenAI client params pre-computed at startup, cost calculator O(n²) loops removed, and more
