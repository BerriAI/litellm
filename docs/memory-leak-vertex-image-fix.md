...
### 极限压测验证（3413s→3531s，✅ 无泄漏）

> 05:40:17（3413.7s）| Heap: **1.468GB（36% of 4.093GB max）** &nbsp;→&nbsp; 05:42:15（3531.7s）| Heap: **2.625GB（64% of 4.093GB max）**

同一运行实例（max = 4.093GB 不变），118 秒间隔。视图为 Sort by Total（非 Own）。

| 指标 | Snapshot A (3413.7s) | Snapshot B (3531.7s) | Delta |
|------|---|---|---|
| Heap | 1.468GB (36%) | 2.625GB (64%) | +1.157GB |
| 总分配数 (root) | 15,007 | 37,840 | +22,833 |
| Router 调用 (`_acompletion`) | 1,734 | 4,983 | +3,249 |
| HTTP POST 调用 | 620 | 3,364 | +2,744 |
| Max heap | 4.093GB | 4.093GB | **+0（不变）** |
| 最大可见单函数 Own Bytes | 17.450MB (asyncio) | 30.368MB (asyncio) | 常驻运行时开销 |
| `convert_to_anthropic_image_obj` | 不出现 | 不出现 | Fix 11 持续有效 |

**非泄漏的 4 项证据：**

1. **Max 不变**：4.093GB 峰值发生在两张快照之前，无新峰值产生
2. **Heap 从 4.093GB 大幅回落至 1.468GB (36%)**：GC 成功回收约 2.6GB
3. **流量突增解释增长**：118 秒内 +3249 个新路由请求 ≈ 27.5 req/s
4. **所有可见函数 Own Bytes 均为 kB 级**：无任何单函数累积大量内存

2.625GB 的构成估算：~4983 个并发 Vertex 调用 × ~540kB 平均工作集/请求 ≈ 2.6GB。

> **提示**：Sort by Total 视图隐藏了叶子节点的实际持有量。按 `o` 键切换 Sort by Own 可直接看到 `aread`/`text`/`raw_decode` 等实际内存持有者。
