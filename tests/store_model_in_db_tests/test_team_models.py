...
@pytest.mark.asyncio
async def test_key_management_with_team_models():
    """
    Test key management with team models:
    1. Create a team (mock)
    2. Verify team model permissions
    3. Test key generation with team model access
    4. Verify key info shows correct permissions
    """
    client = AsyncClient(base_url=PROXY_BASE_URL)
    headers = {"Authorization": f"Bearer {TEST_MASTER_KEY}"}

    # Use a fixed team ID and model name for testing
    ...
    # Generate key for team with model access
    key_response = await client.post(
        "/key/generate",
        json={
            "team_id": team_id,
    ...
    assert key_response.status_code == 200
    team_key = key_response.json()["key"]

    # Verify key info shows correct permissions
    key_info_response = await client.get(
        "/key/info",
        headers={"Authorization": f"Bearer {team_key}"},
    )
    ...
    # Verify key has correct team and model permissions
    assert team_id == key_info.get("team_id"), "Key should be associated with correct team"
    assert model_name in key_info.get("models", []), "Key should have access to team model"
