...
TEST_BASE_URL = f"{PROXY_BASE_URL}/assemblyai"
...
def test_e2e_assemblyai_passthrough():
    ...
    make request
    """
    add_assembly_ai_model_to_db()
    virtual_key = create_virtual_key()
    # make request
    make_assemblyai_basic_transcribe_request(virtual_key=virtual_key)
...
def make_assemblyai_basic_transcribe_request(virtual_key: str):
