...
uv run python -m tests.rust-python-harness.strategies.unit_tests_mapping.runner --plain
uv run python -m tests.rust-python-harness.strategies.unit_tests_parity.runner --plain
uv run python -m tests.rust-python-harness.strategies.unit_tests_rust.runner --plain
```
...
Unit testing splits into three strategies. `unit_tests_mapping` validates Python/Rust test mappings against collect-only inventories without running the selected tests. `unit_tests_parity` runs the same existing Python unit tests in separate processes with `LITELLM_RUST=0` and `LITELLM_RUST=1` and requires the two runs to match, including on failures. `unit_tests_rust` runs the focused native Cargo tests. Native tests stay beside their Rust implementation. Existing Python tests stay at their original paths. No complete Python/native unit mapping is configured yet, so these cells remain planned
...
Unit cases use `unit_suite` instead of `selectors`, pointing to a repository-relative JSON file. Each unit strategy has its own suite schema. `unit_tests_mapping` collects both inventories and validates the mappings:
...
```json
{
  ...
  "backend": {
    ...
    "probe": "tests.rust-python-harness.shared.unit_runners.python_runner:ocr_backend"
  },
  "mappings": [{"python": "tests/test_api.py::test_decode", "rust": "ocr::test_decode"}]
}
```

`unit_tests_parity` takes only `python_selectors`; the runner toggles `LITELLM_RUST` itself and requires matching outcomes, including failures. `unit_tests_rust` takes `cargo_manifest`, `cargo_package`, and `cargo_filter`. The mapping suite keeps `backend` so collection sees the same environment as a real run; a `probe` is optional everywhere
...
```bash
uv run pytest -o consider_namespace_packages=true tests/rust-python-harness/shared tests/rust-python-harness/strategies/unit_tests_mapping tests/rust-python-harness/strategies/unit_tests_parity tests/rust-python-harness/strategies/unit_tests_rust tests/test_rust_python_harness.py -q
```
