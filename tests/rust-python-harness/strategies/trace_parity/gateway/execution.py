...
def _collect_rust(fixture: RouteFixture, route: GatewayRouteSpec) -> tuple[FunctionTraceEvent, ...]:
    ...
    entrypoint: Final = f"gateway_{route.route}"
    gateway_call: Final[object | None] = getattr(trace, entrypoint, None)
    if gateway_call is None or not callable(gateway_call):
        raise RuntimeError(f"native Rust trace bridge does not expose {entrypoint}")
    invoke_gateway: Final = cast(Callable[[str, str, str, object], Awaitable[object]], gateway_call)
