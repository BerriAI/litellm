...
async def test_custom_model_supports_vision():
    async with httpx.AsyncClient() as client:
        ...
        llava_model = next((model for model in data if model["model_name"] == "llava-hf"), None)

        assert llava_model is not None, "llava-hf model not found in response"
        assert llava_model["model_info"]["supports_vision"] == True, "llava-hf model should support vision"
