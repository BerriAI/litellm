...
async def test_proxy_failure_metrics():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        status, response_text = await make_bad_chat_completion_request(session, "sk-1234")
        ...
        for pattern in expected_patterns:
            for line in metrics.split("\n"):
                ...
                # For deprecated llm_api metric, check llm-specific fields
                    ...
                    if (
                        f'hashed_api_key="{expected_hashed_api_key}"' in line and 'model="429"' in line
...
async def test_proxy_success_metrics():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        status, response_text = await make_good_chat_completion_request(session, "sk-1234")
...
def verify_latency_metrics(metrics: str):
    ...
    for metric_name in metric_names:
        ...
        expected_buckets = [bucket.replace("inf", "+Inf") for bucket in expected_buckets]
...
async def test_proxy_fallback_metrics():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert found_successful_fallback, (
...
async def create_test_team(session: aiohttp.ClientSession, team_data: Dict[str, Any]) -> str:
    ...
    async with session.post(url, headers=headers, json=team_data) as response:
        assert response.status == 200, f"Failed to create team. Status: {response.status}"
        team_info = await response.json()
        return team_info["team_id"]


async def create_test_user(session: aiohttp.ClientSession, user_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async with session.post(url, headers=headers, json=user_data) as response:
        assert response.status == 200, f"Failed to create user. Status: {response.status}"
...
async def create_test_key(session: aiohttp.ClientSession, team_id: str) -> str:
    ...
    async with session.post(url, headers=headers, json=data) as response:
        assert response.status == 200, f"Failed to generate key. Status: {response.status}"
...
async def get_team_info(session: aiohttp.ClientSession, team_id: str) -> Dict[str, Any]:
    ...
    async with session.get(url, headers=headers) as response:
        assert response.status == 200, f"Failed to get team info. Status: {response.status}"
...
async def test_team_budget_metrics():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert first_budget["remaining"] < 10.0, "remaining budget should be less than 10.0 after first request"
        ...
        assert abs(team_info_remaining_budget - first_budget["remaining"]) <= 0.001, (
            f"Spend mismatch: Prometheus={team_info_remaining_budget}, Team Info={first_budget['remaining']}"
        )


async def create_test_key_with_budget(session: aiohttp.ClientSession, budget_data: Dict[str, Any]) -> str:
...
async def get_key_info(session: aiohttp.ClientSession, key: str) -> Dict[str, Any]:
    ...
    async with session.get(url, headers=headers) as response:
        assert response.status == 200, f"Failed to get key info. Status: {response.status}"
        return await response.json()
...
async def get_user_info(session: aiohttp.ClientSession, user_id: str) -> Dict[str, Any]:
    ...
    async with session.get(url, headers=headers) as response:
        assert response.status == 200, f"Failed to get user info. Status: {response.status}"
        return await response.json()
...
def extract_key_budget_metrics(metrics_text: str, key_id: str) -> Dict[str, float]:
    ...
    # Get remaining budget
    remaining_pattern = (
        f'litellm_remaining_api_key_budget_metric{{api_key_alias="[^"]*",hashed_api_key="{key_id}"}} ([0-9.]+)'
    )
    remaining_match = re.search(remaining_pattern, metrics_text)
    metrics["remaining"] = float(remaining_match.group(1)) if remaining_match else None
    ...
    # Get remaining hours
    hours_pattern = (
        f'litellm_api_key_budget_remaining_hours_metric{{api_key_alias="[^"]*",hashed_api_key="{key_id}"}} ([0-9.]+)'
    )
    hours_match = re.search(hours_pattern, metrics_text)
    metrics["remaining_hours"] = float(hours_match.group(1)) if hours_match else None
...
async def test_key_budget_metrics():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert 0 <= first_budget["remaining_hours"] <= 168, (
        ...
        # Verify spends match within a small delta (floating point comparison)
        assert abs(key_info_remaining_budget - first_budget["remaining"]) <= 0.001, (
...
async def test_user_budget_metrics():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print(f"Budget after 1 request: {first_budget}")
        assert first_budget["remaining"] is not None, "remaining budget metric should be present"
        assert first_budget["total"] is not None, "total budget metric should be present"
        assert first_budget["remaining"] < 10.0, "remaining budget should be less than 10.0 after first request"
        assert first_budget["total"] == 10.0, "Total budget metric is incorrect"
        print("first_budget['remaining_hours']", first_budget["remaining_hours"])
        # The budget reset time is now standardized - for "7d" it resets on Monday at midnight
        # So we'll check if it's within a reasonable range (0-7 days depending on current day of week)
        assert first_budget["remaining_hours"] is not None, "remaining hours metric should be present"
        assert 0 <= first_budget["remaining_hours"] <= 168, (
            "Budget remaining hours should be within a reasonable range (0-7 days depending on day of week)"
        )
        ...
        # Verify spends match within a small delta (floating point comparison)
        assert abs(user_info_remaining_budget - first_budget["remaining"]) <= 0.001, (
...
async def test_user_email_metrics():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        metrics_after_first = await get_prometheus_metrics(session)
        print("metrics_after_first request", metrics_after_first)
        assert user_email in metrics_after_first, "user_email should be tracked correctly"
...
async def test_user_email_in_all_required_metrics():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        failure_pattern = (
            rf'litellm_proxy_failed_requests_metric_total{{[^}}]*user_email="{re.escape(user_email)}"[^}}]*}}'
        )
        failure_matches = re.findall(failure_pattern, metrics_text)
        assert len(failure_matches) > 0, (
