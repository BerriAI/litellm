...
def test_completions_request_creation(client, base_url, api_key, sample_messages):
    ...
    request = client.completions(
        model="gpt-4", messages=sample_messages, temperature=0.7, max_tokens=100, return_request=True
    )
    ...
    # Check request body
    assert request.json == {"model": "gpt-4", "messages": sample_messages, "temperature": 0.7, "max_tokens": 100}
...
def test_completions_unauthorized_error(client, sample_messages, requests_mock):
    ...
    # Mock a 401 response
    requests_mock.post(f"{client._base_url}/chat/completions", status_code=401, json={"error": "Unauthorized"})
...
def test_completions_other_errors(client, sample_messages, requests_mock):
    ...
    # Mock a 500 response
    requests_mock.post(f"{client._base_url}/chat/completions", status_code=500, json={"error": "Internal Server Error"})
