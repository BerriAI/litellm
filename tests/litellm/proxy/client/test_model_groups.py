...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("http://localhost:8000/", "http://localhost:8000/model_group/info"),  # With trailing slash
    ...
    mock_data = {
        "data": [
            ...
            },
            {
                ...
                "litellm_params": {"api_base": "https://azure-endpoint.com", "api_version": "2023-05-15"},
            },
        ]
    }
...
def test_info_unauthorized_error(client, requests_mock):
    ...
    requests_mock.get(f"{client._base_url}/model_group/info", status_code=401, json={"error": "Invalid API key"})
...
def test_info_other_errors(client, requests_mock):
    ...
    requests_mock.get(f"{client._base_url}/model_group/info", status_code=500, json={"error": "Internal Server Error"})
