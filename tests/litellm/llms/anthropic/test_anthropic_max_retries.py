...
OpenAI/Azure (see the TODO in `litellm/utils.py`). These tests lock in the
fix that:

1. Declares `max_retries` as a supported param for all providers.
...
class TestMaxRetriesHttpTransport:
    ...
    def test_default_handler_does_not_change_transport(self):
        # Without max_retries the default (aiohttp) transport path is untouched.
