...
@asynccontextmanager
async def s3_cache_context(bucket_name, s3_path):
    with patch("aioboto3.Session"):
        cache = S3Cache(s3_bucket_name=bucket_name, s3_path=s3_path)
...
async def test_s3_client_initialization():
    ...
    with patch("aioboto3.Session", return_value=mock_session):
        ...
        client = await cache.get_s3_client()
...
@pytest.mark.asyncio
async def test_async_set_cache_with_ttl(bucket_name, s3_path):
    """Test setting cache with TTL"""
    async with s3_cache_context(bucket_name, s3_path) as s3_cache:
        key = "test_key"
...
@pytest.mark.asyncio
async def test_async_set_cache_without_ttl(bucket_name, s3_path):
...
@pytest.mark.asyncio
async def test_async_get_cache_success(bucket_name, s3_path):
    ...
    async with s3_cache_context(bucket_name, s3_path) as s3_cache:
        ...
        s3_cache._s3_client.get_object.return_value = {"Body": mock_body}
...
@pytest.mark.asyncio
async def test_async_get_cache_no_key(bucket_name, s3_path):
...
@pytest.mark.asyncio
async def test_async_get_cache_error(bucket_name, s3_path):
...
@pytest.mark.asyncio
async def test_disconnect(bucket_name, s3_path):
...
@pytest.mark.asyncio
async def test_sync_methods_raise_error(bucket_name, s3_path):
