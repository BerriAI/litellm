...
# Mock pydantic EmailStr to avoid needing email_validator locally
import pydantic
pydantic.networks.EmailStr = str
pydantic.EmailStr = str

# No proxy mock hacks here.
import litellm
from litellm.proxy.auth.auth_checks import get_end_user_object
from litellm.proxy.utils import ProxyUpdateSpend
from litellm.proxy.management_endpoints.customer_endpoints import new_end_user
from litellm.proxy._types import UserAPIKeyAuth, NewCustomerRequest, Litellm_EntityType
from litellm.caching import DualCache


# Define a real class for type checks that doesn't conflict
class LocalMockPrismaClient:
...
def mock_proxy_logging_obj():
    mock_obj = MagicMock()
    mock_obj.service_logging_obj.async_service_success_hook = AsyncMock()
    mock_obj.service_logging_obj.async_service_failure_hook = AsyncMock()
    mock_obj.db_spend_update_writer.spend_update_queue.add_update = AsyncMock()
    return mock_obj


@pytest.mark.asyncio
async def test_apply_default_budget_persistence(mock_proxy_logging_obj):
    """
    ...
    mock_prisma_db = MagicMock()
    mock_prisma_db.litellm_endusertable.find_unique = AsyncMock(
        return_value=mock_record(mock_end_user_data)
    )
    mock_prisma_db.litellm_budgettable.find_unique = AsyncMock(
        return_value=mock_record(default_budget)
    )

    mock_prisma_client = MagicMock()
    mock_prisma_client.db = mock_prisma_db

    mock_cache = AsyncMock(spec=DualCache)
    mock_cache.async_get_cache = AsyncMock(return_value=None)
    mock_cache.async_set_cache = AsyncMock()

    # Use patch to avoid global side effects
    with patch("litellm.proxy.auth.auth_checks.PrismaClient", LocalMockPrismaClient):
...
@pytest.mark.asyncio
async def test_update_end_user_spend_persistence(mock_proxy_logging_obj):
    """
    ...
    mock_batch.litellm_endusertable.upsert = MagicMock()
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.tx.return_value = mock_tx_context
...
async def test_manual_user_creation_persistence():
    ...
    # Mock modules correctly within the test
    with patch(
        "litellm.proxy.management_endpoints.customer_endpoints.prisma_client",
        mock_prisma_client,
    ), patch(
        "litellm.proxy.management_endpoints.customer_endpoints.litellm_proxy_admin_name",
        "admin",
    ), patch(
    ...
    mock_prisma_client.db.litellm_endusertable.create.assert_called()
    args, kwargs = mock_prisma_client.db.litellm_endusertable.create.call_args
...
@pytest.mark.asyncio
async def test_get_end_user_object_cache_sync(mock_proxy_logging_obj):
    """
...
@pytest.mark.asyncio
async def test_update_end_user_spend_zero_cost_persistence(mock_proxy_logging_obj):
    """
    [Budget Reset Fix]
    ...
    assert "spend" not in kwargs["data"]["update"]


@pytest.mark.asyncio
async def test_update_end_user_spend_none_transactions(mock_proxy_logging_obj):
    """
    Verifies that update_end_user_spend handles end_user_list_transactions=None gracefully.
    """
    prisma_client = MagicMock()
    prisma_client.db = MagicMock()
    ...
    mock_proxy_logging_obj.alerting_handler = AsyncMock()
    mock_proxy_logging_obj.failure_handler = AsyncMock()
...
async def test_apply_default_budget_to_end_user_redundant_check():
    ...
    mock_proxy_logging_obj.alerting_handler = AsyncMock()
    mock_proxy_logging_obj.failure_handler = AsyncMock()
    mock_proxy_logging_obj.db_spend_update_writer = MagicMock()
    ...
    mock_proxy_logging_obj.db_spend_update_writer.spend_update_queue.add_update.assert_not_called()


@pytest.mark.asyncio
async def test_update_end_user_spend_zero_spend_budget_update(mock_proxy_logging_obj):
    """
