...
async def test_logging_with_knowledge_base_hook(setup_vector_store_registry):
    ...
    litellm.callbacks = [
        BedrockVectorStore(aws_region_name="us-west-2"),
        test_custom_logger,
    ]
    litellm.set_verbose = True
    await litellm.acompletion(
    ...
    standard_logging_payload: Optional[
        StandardLoggingPayload
    ] = test_custom_logger.standard_logging_payload
    assert standard_logging_payload is not None

    metadata = standard_logging_payload["metadata"]
    standard_logging_vector_store_request_metadata: Optional[
        List[StandardLoggingVectorStoreRequest]
    ] = metadata["vector_store_request_metadata"]

    print(
        "standard_logging_vector_store_request_metadata:",
        json.dumps(
            standard_logging_vector_store_request_metadata, indent=4, default=str
        ),
    )
...
@pytest.mark.asyncio
async def test_logging_with_knowledge_base_hook_no_vector_store_registry(
    setup_vector_store_registry,
):
    """
    Test that the knowledge base request was logged in standard logging payload
