...
async def test_get_users_filters_dashboard_keys(prisma_client):
    ...
    dashboard_key = await generate_key_helper_fn(
        user_id=test_user.user_id,
        request_type="key",
        team_id="litellm-dashboard",  # This key should be filtered out
        models=[],
        aliases={},
        config={},
        spend=0,
    ...
    assert user.key_count == 1  # Only count the regular key, not the dashboard key
