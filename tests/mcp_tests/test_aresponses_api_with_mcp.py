...
async def test_streaming_mcp_events_validation():
    ...
    with patch.object(
        LiteLLM_Proxy_MCP_Handler, "_get_mcp_tools_from_manager", new_callable=AsyncMock
    ) as mock_get_tools, patch.object(
        LiteLLM_Proxy_MCP_Handler, "_execute_tool_calls", new_callable=AsyncMock
        ...
        try:
            async for chunk in response:
                ...
                if hasattr(chunk, "mcp_servers"):
                    ...
                    print(f"     MCP Tools: {len(chunk.mcp_tools)} tools discovered")
        ...
        if mcp_discovery_events:
            ...
            started_events = [
                e
                for e in mcp_discovery_events
                if e.type == ResponsesAPIStreamEvents.MCP_TOOLS_DISCOVERY_STARTED
            ]
            if started_events:
            ...
            completed_events = [
                e
                for e in mcp_discovery_events
                if e.type == ResponsesAPIStreamEvents.MCP_TOOLS_DISCOVERY_COMPLETED
            ]
            if completed_events:
            ...
            print("  No MCP discovery events found")
        ...
        if mcp_execution_events:
            ...
            execution_started = [
                e
                for e in mcp_execution_events
                if e.type == ResponsesAPIStreamEvents.MCP_TOOL_EXECUTION_STARTED
            ]
            execution_completed = [
                e
                for e in mcp_execution_events
                if e.type == ResponsesAPIStreamEvents.MCP_TOOL_EXECUTION_COMPLETED
            ]
...
async def test_streaming_responses_api_with_mcp_tools(
    ...
    # Skip test if ANTHROPIC_API_KEY is not set for anthropic models
    if ("anthropic" in model.lower() or "claude" in model.lower()) and not os.getenv(
        "ANTHROPIC_API_KEY"
    ):
    ...
    assert not lite_errors, (
        ...
        + "\n".join(f"{record.name}: {record.getMessage()}" for record in lite_errors)
        + "\nAll caplog records:\n"
...
async def test_no_duplicate_mcp_tools_in_streaming_e2e():
    ...
    with patch.object(
        ...
        "litellm.aresponses", side_effect=capture_llm_tools
    ) as mock_aresponses:
        ...
        for call_idx, tools_in_call in enumerate(llm_call_tools):
            ...
            if tools_in_call:
                ...
                if openai_format_tools:
                    ...
                    print("   All tools have proper OpenAI format")
        ...
        print("   - No duplicate tools detected")
