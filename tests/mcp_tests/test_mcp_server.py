...
async def test_mcp_http_transport_list_tools_mock():
    ...
    with patch('litellm.proxy._experimental.mcp_server.mcp_server_manager.MCPClient', mock_client_constructor):
        ...
        # Verify client methods were called
        mock_client.__aenter__.assert_called()
        # Note: list_tools is called 1-2 times depending on environment:
        # - 1 time: when initialization is skipped due to no running event loop (local test env)
        # - 2 times: when initialization runs + actual list_tools call (CI environment)
