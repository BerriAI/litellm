...
@pytest.mark.asyncio
async def test_call_mcp_tool_uses_post_mcp_hook_response():
    from litellm.proxy._experimental.mcp_server.server import (
        call_mcp_tool,
        global_mcp_server_manager,
    )
    ...
    replacement_response = CallToolResult(
    ...
    logging_obj.async_post_mcp_tool_call_hook = AsyncMock(
        return_value=replacement_response
    )
    ...
    with (
        ...
        mock_get_allowed.return_value = [mock_server.server_id]
        mock_tool_registry.get_tool.return_value = None
        mock_handle_managed.return_value = original_response
