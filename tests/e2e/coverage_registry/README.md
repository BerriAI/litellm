...
grouped `module > feature > test`, with LLM cells split into `core_llms` and
`non_core_llms` for dashboarding. Each cell carries a tier (P0/P1/P2), a source, and a
...
`responses` roll up to `core_llms`; all other LLM endpoints roll up to `non_core_llms`.
...
that roll up into the single `logging_guardrails` dashboard module.
...
in `MODULE_ORDER`. Use `--format prometheus` or `--format json` for jobs that publish
coverage through other ingestion paths.
