...
def test_logging_and_guardrail_roll_up_into_one_module() -> None:
    ...
    logging_and_guardrails = next(
        m for m in report.modules if m.module == "logging_guardrails"
    )
...
def test_llm_cells_roll_up_by_core_endpoint() -> None:
    ...
    non_core = next(m for m in report.modules if m.module == "non_core_llms")
...
def test_json_render_exposes_module_coverage_for_grafana_jobs() -> None:
    ...
    assert '"module": "non_core_llms"' in payload
...
def test_prometheus_render_exposes_module_coverage_timeseries() -> None:
    ...
    assert 'litellm_e2e_coverage_cells{module="core_llms",state="covered"} 1' in metrics
    assert 'litellm_e2e_coverage_percent{module="core_llms"} 100.000000' in metrics
    assert 'litellm_e2e_coverage_percent{module="non_core_llms"} 0.000000' in metrics
...
def test_loki_render_exposes_exact_stdout_lines_for_loki() -> None:
    ...
    assert [line.split("module=", 1)[1].split(" ", 1)[0] for line in lines[1:]] == [
        module.module for module in report.modules
    ]
    assert all(
