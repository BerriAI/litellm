...
def _assert_trips_then_recovers(
    ...
    visible_after = _seconds_until_first_backup(client, key, group, backup, failure_status, tripped_at)
    ...
    while time.monotonic() < bench_until:
        _assert_served_by_backup(
            ...
            f"after {visible_after:.1f}s,",
        )

    recovery_deadline = tripped_at + COOLDOWN_SECONDS + RECOVERY_GRACE_SECONDS
    ...
    pytest.fail(
        ...
        f"{COOLDOWN_SECONDS + RECOVERY_GRACE_SECONDS:.0f}s of the trip, so the cooldown never lapsed"
    )
