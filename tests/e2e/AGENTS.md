...
- `client_apps/` - real client applications driven headlessly against the proxy on a virtual key, one streamed turn with a tool call each: the Claude Code CLI (`/v1/messages`), the Codex CLI (`/v1/responses`), and a Vercel AI SDK script (`vercel_ai_turn.mjs`, over both `/v1/responses` and `/v1/chat/completions`). The CLIs and the SDK are pinned npm dependencies of the suite's own `package.json` (`npm ci` in this folder before running; `test-e2e-changed.yml` does it in CI), the Claude Code path reuses `claude_code/cli_driver.py`, and every `.py` file here follows the Python rules in this file
