...
- `secret_manager/` - the gateway's `key_management_system` against a real secret manager: deployment keys resolved from it (`os.environ/<name>` where the name exists only in the manager) and virtual keys written to and deleted from it. Each manager is its own opt-in lane, because the setting is global to the proxy: HashiCorp Vault needs a proxy booted from `gateway/secret_manager_vault_ci_config.yml` against a live Vault, is marked `secret_manager_vault`, is deselected unless `E2E_SECRET_MANAGER_VAULT` is set, and stays out of the per-PR selector
