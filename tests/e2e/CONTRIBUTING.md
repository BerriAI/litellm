...
Tests marked `@pytest.mark.e2e` hard-fail when no proxy answers `/health/liveliness`, so a run that goes red with `No live proxy` at setup means the proxy isn't up; they never skip for a missing proxy, so an absent proxy can't be mistaken for a pass. The Redis chaos test under `load/` needs a proxy it can pause the Redis of on the same host, using `gateway/redis_chaos_ci_config.yml`, with `E2E_PROXY_PID` and `E2E_PROXY_LOG` set, and runs only when `E2E_REDIS_CHAOS` is set
