...
from models_mgmt_client import (
...
BACKEND_MODEL = "gemini/gemini-3.5-flash"
...
def _poll[T](client: ModelsMgmtClient, attempt: Callable[[], T | None], failure: str) -> T:
...
def _delete_if_present(client: ModelsMgmtClient, model_name: str) -> None:
    if client.find_deployment(model_name) is not None:
        client.gateway.delete_model(model_name)


def _provision(
    client: ModelsMgmtClient, resources: ResourceManager, *, tpm: int | None = None
) -> str:
    """Register a fresh deployment (id == name, cleaned up on teardown) and return
    its unique model name."""
    model_name = f"e2e-mgmt-{unique_marker()}"
    _ = client.gateway.create_model(
        model_name, LiteLLMParamsBody(model=BACKEND_MODEL, api_key=GEMINI_API_KEY, tpm=tpm)
    )
    resources.defer(lambda: _delete_if_present(client, model_name))
    return model_name


def _poll_chat_ok(client: ModelsMgmtClient, key: str, model: str) -> ChatResponse:
...
def _poll_chat_rejects_deleted_model(client: ModelsMgmtClient, key: str, model: str) -> None:
...
class TestModelManagementRoutes:
    ...
    def test_add_persists_to_model_info_and_serves_chat(
        self, client: ModelsMgmtClient, resources: ResourceManager, scoped_key: str
    ) -> None:
        ...
        assert entry.litellm_params.tpm == CREATED_TPM, (
            f"/model/info reports tpm {entry.litellm_params.tpm}, configured {CREATED_TPM}"
        )
    ...
    def test_update_tpm_persists_and_deployment_still_serves(
        ...
        client.update_model_tpm(model, UPDATED_TPM)

        def updated() -> ModelInfoEntry | None:
            entry = client.find_deployment(model)
            if entry is not None and entry.litellm_params.tpm == UPDATED_TPM:
                return entry
            return None
        ...
        entry = _poll(
            client,
            updated,
            f"/model/info never reflected tpm {UPDATED_TPM} for {model} after /model/update",
        )
        assert entry.litellm_params.model == BACKEND_MODEL, (
    ...
    def test_add_forbidden_for_llm_only_key(
        self, client: ModelsMgmtClient, resources: ResourceManager
    ) -> None:
        key = client.llm_only_key()
        resources.defer(lambda: client.gateway.delete_key(key))
        model = f"e2e-mgmt-forbidden-{unique_marker()}"

        outcome = client.create_model_status(key, model, LiteLLMParamsBody(model=BACKEND_MODEL))
