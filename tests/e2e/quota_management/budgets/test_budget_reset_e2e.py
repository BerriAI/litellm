...
spend, walking the same ladder: bare key (E2E-6), team key (E2E-7), org (E2E-8),
personal key (E2E-10), and a team-member key frozen by its owner's user budget
(E2E-11, the #32005 draw-down). The per-team-member rung (E2E-9) lives in
test_team_member_budget_reset_e2e.py; the strict before<after budget_reset_at
ladder (the #25109 guard) lives in test_budget_reset_advances_e2e.py.
...
block, never a provider or server error - which pins both #25109 failure modes
(a reset that no-ops leaves the entity blocked forever; a reset that crashes leaks
a non-budget error).
"""
...
# Keep the drive phase comfortably under one window so a block is observed before
# the reset job can fire: one haiku call at max_tokens=16 costs several times
# TINY_CAP, so the cap trips within the first couple of calls.
...
def _poll_until_serves_again(client: BudgetClient, key: str) -> None:
    ...
    Every refusal during the wait must stay a budget block; anything else (a provider
    429, a network blip, or a reset path that crashed into a 5xx) fails loudly with
    its status so the cause is unambiguous."""
...
class TestBudgetResetDiagonal:
    ...
    def test_bare_key_budget_resets_after_window(self, client: BudgetClient, resources: ResourceManager) -> None:
        """E2E-6: a bare key (no team_id / user_id) with max_budget + budget_duration
        blows its budget, then serves again once the window rolls and the reset job
        zeroes key.spend. The key-level budget is the only cap in play."""
