...
The set of behaviors we want covered is the denominator, and it is derived from the product surface rather than hand-listed, so it grows itself when the product gains a surface and cannot be gamed by adding a row in the same PR that adds the test. `product_surface.py` generates the conversational-core LLM cells by crossing the typed vocabularies in `schema.py` with the `supports_*` capability metadata in `model_prices_and_context_window.json`, and `overlay.yaml` is a small owner-gated overlay that carries the human judgement (tier, source, rationale, fail-before-fix, support) plus the ids generation does not yet produce (the non-core LLM operations and the behavior modules). Each e2e test declares what it covers with `@pytest.mark.covers("...")`, and a small collector diffs the denominator against the tests and ships coverage to the existing Grafana. Adding coverage is only a marker, never a registry row. No Allure, no new dependencies
...
Tests do not declare a dashboard module directly. They only declare the cell id with `@pytest.mark.covers("...")`; the id itself decides the module and dashboard rollup (endpoint, route, capability, and streaming are parsed back out of the id), and the overlay decides tier and source. Run `python -m coverage_registry.collector --strict` when you want CI to reject marker ids that are not in the denominator. Add `--fail-on-collection-errors` when the job should also fail on pytest collection errors.
