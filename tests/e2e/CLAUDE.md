...
The recording workflow uses the existing `e2e-changed` environment and OIDC secrets reader (`E2E_AWS_ROLE_TO_ASSUME`, secret `litellm-e2e-changed-provider-keys`), and fails before starting the proxy if any required provider credential is missing. Replay supplies only test credentials and includes the four supported Bedrock endpoints in the egress sentinel

HTTP assertions classify a Bedrock 503 as `ProviderUnavailableError` only when AWS headers or a recognized Bedrock error envelope support that attribution. The default live lane allows one test rerun after ten seconds for that error or a transport failure. Generic proxy 5xx, authentication errors and assertion failures do not qualify. Persistent provider failures remain failed, and JUnit properties retain AWS and proxy request IDs from the failed attempt even when the rerun passes. Record and replay continue to use `--reruns 0`
