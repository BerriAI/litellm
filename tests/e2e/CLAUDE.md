...
Bedrock tests use `bedrock_config.bedrock_params()` for Chat Completions, Responses, Converse and Invoke. Live mode retains the normal AWS credential path. Record and replay use a `/bedrock-us-east-1` mount by default (`E2E_BEDROCK_REGION` selects one of `us-east-1`, `us-east-2`, `us-west-1`, `us-west-2`; use the same region in both runs). The proxy signs each edge request with public test credentials. The edge verifies that signature over the host, path and body before accepting it. Recording re-signs the unchanged payload for the AWS endpoint using the runner's `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` and optional `AWS_SESSION_TOKEN`, assuming `AWS_ROLE_NAME` with the SDK when configured. Start the proxy without ambient `AWS_ROLE_NAME`, `AWS_PROFILE_NAME`, web identity or Bedrock bearer credentials in these modes so its public test credentials cannot trigger STS or bypass signature verification. The workflow isolates that proxy environment while retaining real credentials and role settings in the recording runner. Replay requires no real AWS credentials and never forwards a miss. This checks request signing and protocol behavior in replay; live runs remain necessary to check real IAM access and model availability. Binary AWS EventStream responses preserve transfer chunks in the bundle, and native stream assertions decode events with botocore and require complete frames, text, ordered completion and usage. The original Responses tool test keeps default tool selection, and a separate test covers `tool_choice="required"`

The recording workflow uses the existing `e2e-changed` environment and OIDC secrets reader (`E2E_AWS_ROLE_TO_ASSUME`, secret `litellm-e2e-changed-provider-keys`), and fails before starting the proxy if any required provider credential is missing. Recording requires approval under that environment's existing protection policy, including scheduled runs. Replay supplies only test credentials and includes the four supported Bedrock endpoints in the egress sentinel

HTTP assertions classify a Bedrock 503 as `ProviderUnavailableError` only when AWS headers with Bedrock route context, or a recognized Bedrock error envelope support that attribution. A decoded native `serviceUnavailableException` receives the same classification even when the committed HTTP status is 200, with `stream_event` recorded as its evidence. Interrupted binary or SSE transfers produce `NetworkFailureError` with the received status and request IDs preserved. Both diagnostic exceptions hard-fail without a test rerun, including when their messages contain a legacy retry marker. Incomplete binary frames hard-fail even after a valid completion event. The existing typed `NetworkError` retry remains, with its filter anchored to that assertion rather than matching a marker embedded in another response, header or assertion message. No newly classified failure becomes eligible for a rerun. The exercised proxy 5xx, authentication and behavior failures do not qualify. This retains the existing text-based retry convention: an arbitrary assertion that deliberately imitates the exact typed network-error prefix can still match. JUnit retains AWS and proxy request IDs from the failed attempt. Record and replay continue to use `--reruns 0`

Current limits: deployments baked into the proxy's config file cannot be edge-wired (only `/model/new` registrations can carry the edge api_base), and a file upload routed by `custom_llm_provider` through the proxy's `files_settings` block never passes a deployment at all, so the batches `model_param` and `provider_fallback` scenarios keep uploading live in every mode
