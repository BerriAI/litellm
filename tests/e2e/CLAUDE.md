...
LLMs - endpoint features (subject = the route), seeded from the Claude Code compat matrix. `chat_completions`, `messages`, and `responses` roll up to `core_llms`. Other LLM endpoints, including `batches` and `realtime`, roll up to `non_core_llms`.
