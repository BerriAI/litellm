...
Request and response bodies are typed pydantic models in `models.py`; only the fields a test reads are modelled, and nothing passes raw dicts. Outcomes come back as a `Result[R]` tagged union (`Success`, `NetworkError`, `UnauthorizedError`, `RateLimitedError`, `ValidationError`, `InfraUnavailable`, `UnknownApiError`). Handle them with `match`, or call `unwrap(...)` when a non-success should fail the test. `InfraUnavailable` is the environment's fault, not the product's: an edge proxy (ALB, nginx) answered instead of litellm, proven by an edge status plus no `x-litellm-call-id`, no JSON body, and an HTML error page. Anything ambiguous stays `UnknownApiError`, because a false "bad environment" silently discards a real regression. When you need to know whether a failure is worth retrying, call `is_transient_failure(result)` rather than writing your own status list; discriminating on a single variant by class is how a new variant silently empties a retry budget. The harness hard-fails and never skips: a test marked `e2e` fails when no proxy answers its liveness probe, and once a request reaches the proxy any wrong behavior is likewise a hard failure, so a missing proxy turns the run red instead of being mistaken for a pass
