...
def test_vercel_ai_sdk_streams_a_tool_turn(client: ClientAppsClient, scoped_key: str, api: VercelApi) -> None:
    secret_word = f"pong-{unique_marker()}"
    turn = unwrap_run(
        client.run_vercel_ai_sdk(key=scoped_key, model=CHEAP_OPENAI_MODEL, api=api, secret_word=secret_word)
    )
    ...
    assert turn.text_deltas >= 2, turn
