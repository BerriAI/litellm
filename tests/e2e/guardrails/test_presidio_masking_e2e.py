...
def _contains_card_number(text: str) -> bool:
    return any(13 <= len(re.sub(r"[ -]", "", match.group())) <= 19 for match in _CARD_DIGIT_RUN.finditer(text))
    ...
    while True:
        ...
        if content is not None:
            ...
            if _contains_card_number(content):
                pytest.fail(f"the post_call output masking let a card number through: {content[:300]!r}")
...
class TestPresidioSpendLogRecord:
    ...
    def test_masking_run_is_recorded_on_the_spend_log(
        ...
        assert {entity.entity_type for entity in entities} >= _LOGGED_ENTITIES.keys(), (
            f"the detected entities must cover what was masked, got {sorted(entity.entity_type for entity in entities)}"
        )
