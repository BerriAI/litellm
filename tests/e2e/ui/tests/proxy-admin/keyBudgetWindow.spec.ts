...
test.describe("Proxy Admin - Key budget window", () => {
  ...
  test.beforeEach(async ({ page }) => {
    alias = `e2e-budget-window-${uniqueSuffix()}`;
  ...
  test("a monthly spend cap survives a reload, and clearing the window keeps the cap", async ({
    page,
  }) => {
    const before = await readKeyInfo(page.request, token);
    expect(
      before.max_budget,
      "a freshly generated key starts with no budget",
    ).toBeNull();
    ...
    await page
      .getByRole("spinbutton", { name: "Max Budget (USD)" })
      .fill("12.5");
    ...
    await expect
      .poll(
        async () => (await readKeyInfo(page.request, token)).budget_duration,
        {
    ...
    expect(
      ...
      "a monthly window left the key with no budget_reset_at",
    ).toBe(false);
    expect(
      resetAt.getTime(),
      "budget_reset_at was set in the past",
    ).toBeGreaterThan(Date.now());
    expect(
      resetAt.getUTCDate(),
      "a monthly window resets on the 1st, a daily one would not",
    ).toBe(1);
    ...
    await page.getByLabel("Reset Budget", { exact: true }).click();
    await page
      .getByRole("option", { name: "Never resets", exact: true })
      .click();
    ...
    expect(
      cleared.budget_duration,
      "clearing the window must send budget_duration: null explicitly",
    ).toBeNull();
    ...
    expect(
      after.max_budget,
      "clearing the reset window also wiped the spend cap",
    ).toBe(12.5);
    expect(
      after.models,
      "editing the budget left the key's models untouched",
    ).toEqual(before.models);
    expect(
      after.team_id,
      "editing the budget left the key's team untouched",
    ).toEqual(before.team_id);
  });
});
