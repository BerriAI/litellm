...
test("team logging settings persist otel_internal_spans exclude through the callback editor", async ({
  page,
  request,
}) => {
  ...
  try {
    ...
    await page
      ...
      .click();
    const internalSpans = page.getByRole("combobox", {
      name: "otel internal spans",
    });
    await internalSpans.click();
    await expect(
      page.getByRole("option", { name: "include", exact: true }),
    ).toBeVisible();
    await page
      .getByRole("option", { name: "exclude", exact: true })
      .click();
    ...
    expect(logging).toEqual([
      {
        ...
        callback_vars: { otel_internal_spans: "exclude" },
      },
    ]);
    await expect(
    ...
    expect(
      info.team_info.metadata.logging[0].callback_vars.otel_internal_spans,
    ).toBe("exclude");
  } finally {
