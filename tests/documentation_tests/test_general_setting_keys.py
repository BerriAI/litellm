...
get_pattern = re.compile(r'general_settings\.get\(\s*[\'"]([^\'"]+)[\'"](,?\s*[^)]*)?\)')
    ...
    for file in files:
        if file.endswith(".py"):  # Only process Python files
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                general_settings_keys.update(match[0] for match in get_matches)  # Extract only the key part
...
try:
    with open(docs_path, "r", encoding="utf-8") as docs_file:
        ...
        general_settings_section = re.search(r"### general_settings - Reference(.*?)###", content, re.DOTALL)
