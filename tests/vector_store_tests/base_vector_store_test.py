...
class BaseVectorStoreTest(ABC):
    ...
    async def test_basic_search_vector_store(self, sync_mode):
        ...
        try:
            if sync_mode:
                response = litellm.vector_stores.search(query=default_query, **base_request_args)
            else:
                response = await litellm.vector_stores.asearch(query=default_query, **base_request_args)
    ...
    async def test_basic_create_vector_store(self, sync_mode):
        ...
        try:
            if sync_mode:
                response = litellm.vector_stores.create(name="Test Vector Store", **create_args)
            else:
                response = await litellm.vector_stores.acreate(name="Test Vector Store", **create_args)
        except litellm.InternalServerError:
            pytest.skip("Skipping test due to litellm.InternalServerError")
    ...
    def _validate_vector_store_response(self, response):
        ...
        # Validate data field
        assert isinstance(response["data"], list), f"data should be a list, got {type(response['data'])}"
    ...
    def _validate_vector_store_create_response(self, response):
        ...
        assert response["id"].startswith("vs_"), f"id should start with 'vs_', got '{response['id']}'"
        ...
        if "name" in response:
            assert isinstance(response["name"], str), f"name should be a string, got {type(response['name'])}"

        if "bytes" in response:
            assert isinstance(response["bytes"], int), f"bytes should be an integer, got {type(response['bytes'])}"
    ...
    def _validate_file_counts(self, file_counts):
        ...
        assert isinstance(file_counts, dict), f"file_counts should be a dict, got {type(file_counts)}"
        ...
        for field in required_count_fields:
            ...
            assert isinstance(file_counts[field], int), f"{field} should be an integer, got {type(file_counts[field])}"
            assert file_counts[field] >= 0, f"{field} should be non-negative"

        # Validate that total equals sum of other counts
        calculated_total = (
            file_counts["in_progress"] + file_counts["completed"] + file_counts["failed"] + file_counts["cancelled"]
        )
    ...
    def _validate_search_result(self, result, index):
        ...
        for j, content_item in enumerate(result["content"]):
            ...
            assert "type" in content_item, f"Content item {j} in result {index} missing 'type' field"
            assert "text" in content_item, f"Content item {j} in result {index} missing 'text' field"
            assert isinstance(content_item["text"], str), (
                f"Content text should be a string in item {j} of result {index}"
            )
            assert len(content_item["text"]) > 0, f"Content text should not be empty in item {j} of result {index}"
