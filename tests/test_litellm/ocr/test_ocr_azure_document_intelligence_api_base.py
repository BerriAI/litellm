...
class TestDocIntelligenceApiBaseResolution:
    """Both the Python and boundary routes resolve endpoints through the provider
    config's ``get_complete_url``; these pin that resolution."""
    ...
    def test_generic_azure_ai_base_does_not_hijack_doc_intelligence(self, monkeypatch):
        ...
        url = AzureDocumentIntelligenceOCRConfig().get_complete_url(
            api_base=prepared.api_base,
            model=prepared.model,
            optional_params=prepared.optional_params,
            litellm_params=prepared.litellm_params,
        )
        assert url.startswith(_DOC_INTELLIGENCE_ENDPOINT)
    ...
    def test_generic_azure_ai_base_still_applies_to_mistral_ocr(self, monkeypatch):
        ...
        url = AzureAIOCRConfig().get_complete_url(
            api_base=prepared.api_base,
            model=prepared.model,
            optional_params=prepared.optional_params,
            litellm_params=prepared.litellm_params,
        )
        assert url.startswith(_AZURE_AI_API_BASE)
