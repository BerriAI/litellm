...
from litellm.translation import Route, ported_providers, route

_PORTED = frozenset({"anthropic", "openai_compat"})


def test_flag_off_stays_on_v1_even_for_a_ported_provider() -> None:
    decision = route(
...
def test_unported_provider_stays_on_v1_with_the_flag_on() -> None:
    decision = route(
        schema="openai_chat",
...
def test_ported_providers_is_the_serializer_registry() -> None:
    assert ported_providers() == frozenset({"anthropic"})
