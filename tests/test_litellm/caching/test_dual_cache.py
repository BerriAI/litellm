...
@pytest.mark.asyncio
async def test_async_increment_cache_can_fail_closed_on_redis_error():
    dc = DualCache()
    dc.redis_cache = MagicMock()
    dc.redis_cache.async_increment = AsyncMock(side_effect=PermissionError("EVAL denied"))

    with pytest.raises(PermissionError, match="EVAL denied"):
        await dc.async_increment_cache("rate-limit", 1.0, ttl=60, fail_on_redis_error=True)


@pytest.mark.asyncio
async def test_async_increment_cache_pipeline_can_fail_closed_on_redis_error():
    dc = DualCache()
    dc.redis_cache = MagicMock()
    dc.redis_cache.async_increment_pipeline = AsyncMock(side_effect=PermissionError("EVAL denied"))
    increment_list = [{"key": "rate-limit", "increment_value": 1.0, "ttl": 60, "refresh_ttl": False}]

    with pytest.raises(PermissionError, match="EVAL denied"):
        await dc.async_increment_cache_pipeline(increment_list, fail_on_redis_error=True)
