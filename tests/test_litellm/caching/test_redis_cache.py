...
async def _accept_and_never_answer(reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
    try:
...
@pytest.mark.asyncio
async def test_real_client_timeout_is_the_failure_the_fault_injecting_fake_claims():
    """Contract check for tests/_fault_injecting_redis.py.

    The fake raises ``redis.exceptions.TimeoutError`` to stand for a Redis that accepts the
    connection and never answers. This drives the real redis-py client into exactly that
    situation, a listener that swallows every byte, and asserts the exception that reaches
    ``async_increment``'s caller is the same type and that the breaker counts it the same
    way. If redis-py ever surfaces a stall differently, this fails before the fake lies.
    """
    from redis.exceptions import TimeoutError as RedisTimeoutError

    from litellm.caching.redis_cache import _is_redis_health_failure

    server = await asyncio.start_server(_accept_and_never_answer, host="127.0.0.1", port=0)
    port = server.sockets[0].getsockname()[1]
    try:
        cache = RedisCache(host="127.0.0.1", port=port, socket_timeout=0.05)
        with pytest.raises(RedisTimeoutError) as raised:
            await cache.async_increment(key="spend:key:contract", value=1.0)
        assert _is_redis_health_failure(raised.value), "a stalled socket must count against the breaker"
