...
import pytest
moke_response = {'request_id': 'e86a0b4e-53e3-97dc-a5f7-82e451376b23',
                 'intermediate_results': {
                     'templating': [
                         ...
                         'choices': [
                             {'index': 0,
                              'message': {
                                  'role': 'assistant', 'content': "Hello from SAP!"
                              },
                              'finish_reason': 'stop'}
                         ],
                         'usage': {
...
async def test_sap_chat(
    respx_mock, monkeypatch, sap_api_response, fake_token_creator, fake_deployment_url, sync_mode
):
    import litellm
    litellm.disable_aiohttp_transport = True
    with (
        patch("litellm.llms.sap.chat.handler.GenAIHubOrchestration.deployment_url",
              new_callable=PropertyMock,
              return_value=fake_deployment_url),
        patch("litellm.llms.sap.chat.handler.get_token_creator",
...
async def test_sap_streaming(
        respx_mock, monkeypatch, sap_api_stream_response, fake_token_creator, fake_deployment_url
):
    import litellm
    litellm.disable_aiohttp_transport = True
    with (
