...
def _sync_fake_stream_text(**extra) -> str:
    import json

    body = _json_tool_use_body()
    response = MagicMock()
    response.status_code = 200
...
def test_make_sync_call_unwraps_json_object_wrapper_on_fake_stream():
    ...
    The tool call becomes content inside MockResponseIterator, so the unwrap key
    has to reach it; otherwise a non-async caller streaming with
