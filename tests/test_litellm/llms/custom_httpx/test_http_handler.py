...
class _FrameStream(httpx.AsyncByteStream):
    ...
    async def __aiter__(self):
        for index in range(self._frame_count):
            ...
            yield b"data: frame-%d\n\n" % index


def _retry_transport(status_code: int = 200, stream: bool = True) -> httpx.MockTransport:
    async def handler(request: httpx.Request) -> httpx.Response:
        if stream:
            return httpx.Response(status_code, request=request, stream=_FrameStream())
        return httpx.Response(status_code, request=request, json={"ok": True})

    return httpx.MockTransport(handler)


def _handler_that_retries(retry_transport: httpx.MockTransport, retry_clients: list) -> AsyncHTTPHandler:
    """Build a handler whose first send fails with ConnectError, forcing the retry path."""
    handler = AsyncHTTPHandler()

    def create_client(*args, **kwargs) -> httpx.AsyncClient:
        client = httpx.AsyncClient(transport=retry_transport)
        retry_clients.append(client)
        return client

    async def failing_send(request: httpx.Request, **kwargs) -> httpx.Response:
        raise httpx.ConnectError("simulated connection error", request=request)

    handler.create_client = create_client
    handler.client.send = failing_send
    return handler


async def _wait_until_closed(client: httpx.AsyncClient, timeout: float = 5.0) -> bool:
...
@pytest.mark.asyncio
async def test_async_post_connection_error_retry_keeps_streaming_client_open():
    retry_clients: list = []
    handler = _handler_that_retries(_retry_transport(), retry_clients)
    try:
        response = await handler.post("https://example.invalid/v1/chat", data=b"{}", stream=True)

        assert len(retry_clients) == 1
        assert retry_clients[0].is_closed is False

        chunks = [chunk async for chunk in response.aiter_bytes()]
        assert b"".join(chunks).count(b"data: frame-") == 4
        assert await _wait_until_closed(retry_clients[0]) is True
    finally:
        await handler.close()


@pytest.mark.asyncio
async def test_async_post_connection_error_retry_closes_client_on_abandoned_body():
    retry_clients: list = []
    handler = _handler_that_retries(_retry_transport(), retry_clients)
    try:
        response = await handler.post("https://example.invalid/v1/chat", data=b"{}", stream=True)
...
@pytest.mark.asyncio
async def test_async_post_connection_error_retry_closes_client_when_body_never_read():
    retry_clients: list = []
    handler = _handler_that_retries(_retry_transport(), retry_clients)
    try:
        response = await handler.post("https://example.invalid/v1/chat", data=b"{}", stream=True)
...
@pytest.mark.asyncio
async def test_async_post_connection_error_retry_non_streaming_closes_client():
    retry_clients: list = []
    handler = _handler_that_retries(_retry_transport(stream=False), retry_clients)
    try:
        response = await handler.post("https://example.invalid/v1/chat", data=b"{}", stream=False)

        assert response.json() == {"ok": True}
        assert retry_clients[0].is_closed is True
    finally:
        await handler.close()


@pytest.mark.asyncio
async def test_async_post_connection_error_retry_closes_client_on_error_status():
    retry_clients: list = []
    handler = _handler_that_retries(_retry_transport(status_code=429, stream=False), retry_clients)
    try:
        with pytest.raises(httpx.HTTPStatusError):
            await handler.post("https://example.invalid/v1/chat", data=b"{}", stream=False)
...
class _ChunkedStreamServer:
    """Local HTTP server that streams a chunked body, so a torn-down client is observable."""

    def __init__(self, chunk_count: int = 6):
...
def _force_connection_error_retry(handler, retry_clients, retry_transport=None):
    """Make the handler's first send fail with ConnectError and record the retry client."""
    real_create_client = handler.create_client

    def tracking_create_client(*args, **kwargs):
        if retry_transport is not None:
            client = httpx.AsyncClient(transport=retry_transport)
        else:
            client = real_create_client(*args, **kwargs)
        retry_clients.append(client)
        return client

    handler.create_client = tracking_create_client

    real_send = handler.client.send
    attempts = {"count": 0}

    async def failing_first_send(request, **kwargs):
        attempts["count"] += 1
        if attempts["count"] == 1:
            raise httpx.ConnectError("simulated connection error", request=request)
        return await real_send(request, **kwargs)

    handler.client.send = failing_first_send


@pytest.mark.asyncio
async def test_async_post_connection_error_retry_streams_full_body_over_a_real_socket():
    """
    The connection-error retry returns the streaming response before any body byte is
    read, so closing its single-use client at that point kills the stream.
    """
    retry_clients = []
    async with _ChunkedStreamServer(chunk_count=6) as server:
        handler = AsyncHTTPHandler()
        _force_connection_error_retry(handler, retry_clients)
        try:
            ...
            assert b"".join(chunks).count(b"data: chunk-") == 6
            assert retry_clients[0].is_closed
        finally:
            await handler.close()
