...
@pytest.mark.asyncio
@pytest.mark.parametrize("use_http2", [True, False])
@pytest.mark.parametrize("handler_kind", ["async", "sync"])
async def test_http_version_negotiated_over_tls(monkeypatch, http2_tls_server, handler_kind, use_http2):
    monkeypatch.setattr(litellm, "http2", use_http2)
    monkeypatch.delenv("LITELLM_HTTP2", raising=False)
    monkeypatch.setattr(litellm, "disable_aiohttp_transport", True)
    monkeypatch.setattr(litellm, "force_ipv4", False)

    expected_version = "HTTP/2" if use_http2 else "HTTP/1.1"

    if handler_kind == "async":
        handler = AsyncHTTPHandler(ssl_verify=False)
        try:
            response = await handler.post(f"{http2_tls_server}/echo", json={"ping": "pong"})
            assert response.status_code == 200
            assert response.http_version == expected_version

            stream_response = await handler.post(f"{http2_tls_server}/stream", stream=True)
            assert stream_response.http_version == expected_version
            chunks = [chunk async for chunk in stream_response.aiter_bytes()]
            assert chunks
            await stream_response.aclose()
        finally:
            await handler.close()
    else:
        handler = HTTPHandler(ssl_verify=False)
        try:
            response = handler.post(f"{http2_tls_server}/echo", json={"ping": "pong"})
            assert response.status_code == 200
            assert response.http_version == expected_version

            stream_response = handler.post(f"{http2_tls_server}/stream", stream=True)
            assert stream_response.http_version == expected_version
