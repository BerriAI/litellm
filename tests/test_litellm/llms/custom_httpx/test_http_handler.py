...
@pytest.mark.asyncio
async def test_async_http_handler_with_custom_adapter():
    """
    Test AsyncHTTPHandler with a custom adapter implementation.
    Verifies that the abstraction works as intended for future implementations.
    """
    ...
    # AsyncHTTPHandler.post calls build_request then send
    mock_adapter.build_request.return_value = {
        "method": "POST",
        "url": "https://example.com",
    }
    mock_adapter.send.return_value = mock_response

    handler = AsyncHTTPHandler(client=mock_adapter)

    # Act
    response = await handler.post(url="https://example.com", data={"test": "data"})

    # Assert
    assert response.status_code == 200
    mock_adapter.build_request.assert_called_once()
    mock_adapter.send.assert_called_once()


@pytest.mark.asyncio
async def test_async_http_handler_adapter_close():
    """
    Test that AsyncHTTPHandler.close() calls the adapter's close() method.
    """
    from litellm.llms.custom_httpx.http_handler import HTTPClientAdapterAsync
    from unittest.mock import AsyncMock
