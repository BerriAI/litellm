"""
Tests for request translation timing tracking.

Verifies that request_translation_time_ms is correctly stored in model_call_details
when transform_request is called during completion.
"""
...
class TestRequestTranslationTiming:
    """Test suite for request translation timing tracking."""

    def test_request_translation_time_is_tracked(self):
        """
        Test that request_translation_time_ms is stored in model_call_details
        when transform_request completes successfully.
        """
        handler = BaseLLMHTTPHandler()
        
        # Create a mock config that simulates work during transform_request
        mock_config = Mock(spec=BaseConfig)
        
        # Simulate a transform_request that takes measurable time
        def transform_with_delay(*args, **kwargs):
            time.sleep(0.005)  # 5ms delay to ensure measurable time
            return {"model": "gpt-4", "messages": []}
        
        mock_config.transform_request = transform_with_delay
        ...
        assert "request_translation_time_ms" in mock_logging_obj.model_call_details
        translation_time = mock_logging_obj.model_call_details["request_translation_time_ms"]
    ...
    def test_request_translation_time_is_zero_for_instant_transform(self):
        """
        Test that request_translation_time_ms is still tracked even when
        transform_request is very fast (near-zero time).
        """
        handler = BaseLLMHTTPHandler()
        
        mock_config = Mock(spec=BaseConfig)
        # Very fast transform_request (no delay)
    ...
    def test_request_translation_time_not_set_on_transform_error(self):
        """
        Test that request_translation_time_ms is not set if transform_request
        raises an exception before completion.
        """
        handler = BaseLLMHTTPHandler()
        
        mock_config = Mock(spec=BaseConfig)
        # transform_request that raises an error
        mock_config.transform_request = Mock(side_effect=ValueError("Transform error"))
        mock_config.get_complete_url = Mock(return_value="https://api.openai.com/v1/chat/completions")
        mock_config.validate_environment = Mock(return_value={})
        ...
        assert "request_translation_time_ms" not in mock_logging_obj.model_call_details
