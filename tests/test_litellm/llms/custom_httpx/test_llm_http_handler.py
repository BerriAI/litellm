...
def test_embedding_strips_internal_params_from_request_body():
    ...
    seeded["extra_body"] = {"skip_mcp_handler": True, "input_type": "query"}
    ...
    for param in LiteLLMInternalParam:
        assert param.value not in body, f"{param.value} leaked into voyage embedding body"
    assert "output_dimension" in body
    assert json.loads(body)["input_type"] == "query"
...
def test_chat_boundary_preserves_cache_control_injection_points():
    ...
    for param in LiteLLMInternalParam:
        if param is LiteLLMInternalParam.CACHE_CONTROL_INJECTION_POINTS:
            continue
        assert param.value not in forwarded, f"{param.value} leaked past the chat-completion strip"
...
def test_chat_boundary_strips_internal_params_from_splat_body():
    ...
    body = captured["body"]
    for param in LiteLLMInternalParam:
        assert param.value not in body, f"{param.value} leaked into the wire body past the splat transform"
...
async def test_internal_params_are_filtered_before_signing_both_transform_paths(transform_async):
    ...
    class SplatConfig(_TransformRecordingConfig):
        def transform_request(self, model, messages, optional_params, litellm_params, headers):
            return {"transformed_by": "sync", **optional_params}

        async def async_transform_request(self, model, messages, optional_params, litellm_params, headers):
            return {"transformed_by": "async", **optional_params}

        def sign_request(self, headers, optional_params, request_data, **kwargs):
            assert request_data == {"transformed_by": "async" if transform_async else "sync", "temperature": 0.5}
            assert optional_params["stream_chunk_size"] == 7
            return headers, json.dumps(request_data).encode()

    config = SplatConfig(transform_async=transform_async)
    captured = {}
    client = AsyncHTTPHandler()
    client.client = httpx.AsyncClient(transport=_echo_json_transport(captured))
    seeded = {param.value: "internal" for param in LiteLLMInternalParam}
    seeded.update({"temperature": 0.5, "stream_chunk_size": 7})
    response = await BaseLLMHTTPHandler().completion(
        model="stub-model",
        messages=[{"role": "user", "content": "hi"}],
        api_base="https://llm.example/v1/chat",
        custom_llm_provider="openai",
    ...
    assert response.choices[0].message.content == ("async" if transform_async else "sync")
    assert captured["body"] == {"transformed_by": response.choices[0].message.content, "temperature": 0.5}
