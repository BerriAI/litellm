...
def test_flux2_flex_reference_cost_honors_explicit_zero_deployment_rate() -> None:
    response: Final = ImageResponse(
        data=[ImageObject(b64_json="aW1n")], hidden_params={"reference_pixels": 1048576}, size="1024x1024"
    )
    ...
    cost: Final = CostCalculatorUtils.route_image_generation_cost_calculator(
        ...
        size="1024x1024",
        call_type="image_edit",
        model_info={"input_cost_per_pixel": 2e-07, "input_cost_per_reference_pixel": 0.0},
    )

    assert cost == pytest.approx(2e-07 * 1048576)


def test_flux2_flex_generated_cost_honors_explicit_zero_deployment_rate() -> None:
    response: Final = ImageResponse(
        data=[ImageObject(b64_json="aW1n")], hidden_params={"reference_pixels": 1048576}, size="1024x1024"
    )
    ...
    cost: Final = CostCalculatorUtils.route_image_generation_cost_calculator(
        ...
        size="1024x1024",
        call_type="image_edit",
        model_info={"input_cost_per_pixel": 0.0, "input_cost_per_reference_pixel": 1e-07},
    )

    assert cost == pytest.approx(1e-07 * 1048576)


@pytest.mark.parametrize("reference_pixels", (1048576.0, "1048576"))
def test_flux2_flex_bills_only_integer_reference_pixels(reference_pixels: object) -> None:
    response: Final = ImageResponse(
        data=[ImageObject(b64_json="aW1n")], hidden_params={"reference_pixels": reference_pixels}, size="1024x1024"
    )

    cost: Final = _flux2_flex_cost(response)

    assert cost == pytest.approx(litellm.model_cost["azure_ai/FLUX.2-flex"]["input_cost_per_pixel"] * 1048576)


def test_flux2_flex_bills_zero_reference_cost_when_catalog_rate_is_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setitem(litellm.model_cost["azure_ai/FLUX.2-flex"], "input_cost_per_reference_pixel", None)
    litellm.get_model_info.cache_clear()
    _invalidate_model_cost_lowercase_map()
    response: Final = ImageResponse(
        data=[ImageObject(b64_json="aW1n")], hidden_params={"reference_pixels": 1048576}, size="1024x1024"
