...
def test_azure_ai_kimi_k27_code_cached_prompt_cost(use_local_model_cost_map):
    from litellm.llms.azure_ai.cost_calculator import cost_per_token
    ...
    usage = Usage(
        ...
        prompt_tokens_details=PromptTokensDetailsWrapper(cached_tokens=1_000_000),
    )

    prompt_cost, completion_cost = cost_per_token(model="kimi-k2.7-code", usage=usage)

    assert prompt_cost == pytest.approx(0.19)
