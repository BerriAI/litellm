...
class TestOllamaChatVideoBlockUsage:
    """Usage counting for requests that carry a non-text content block."""
    ...
    def test_missing_counts_with_a_video_block_do_not_raise(self):
        """
        End-to-end of the two fixes on this route: Ollama omits the counts, so the
        estimator runs over a message that carries a `video_url` block. It must
        produce usage instead of raising, because the response has already been
        generated by the time the counter runs.
        """
        ollama_response = {
            "model": "qwen3:14b",
            "created_at": "2025-01-11T00:00:00.000000Z",
