"""
xAI retired eight slugs on 2026-05-15 but kept them resolvable: requests to them are
redirected to grok-4.3 and billed at grok-4.3's rates, so the registry must price them
that way or spend tracking is wrong.
...
def test_both_cost_maps_agree_on_the_redirected_slugs():
    ...
    for slug in (*REDIRECTED_SLUGS, REDIRECT_TARGET):
...
def test_every_retired_chat_slug_is_covered(cost_map: dict):
    ...
    assert marked == set(REDIRECTED_SLUGS)
