...
class TestCacheControlPreservationForCustomEndpoint:
    ...
    def test_remove_cache_control_does_not_mutate_caller_input(self):
        """
        Test that remove_cache_control_flag_from_messages_and_tools does not
        mutate the dictionary objects passed in by the caller.
        """
        input_messages = [
            {
                "role": "user",
        ...
        filtered_messages, filtered_tools = self.config.remove_cache_control_flag_from_messages_and_tools(
            model="gpt-4o",
            messages=input_messages,
            tools=input_tools,
        )

        # Verify filtered copies do not have cache_control
        assert all("cache_control" not in m for m in filtered_messages)
        assert all("cache_control" not in t for t in filtered_tools)

        # Verify original inputs are untouched/unmutated
        assert "cache_control" in input_messages[0]
        assert "cache_control" in input_tools[0]
