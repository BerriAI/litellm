...
def test_translate_anthropic_messages_to_openai_tool_message_placement():
    ...
    assert tool_message_idx < user_message_idx, (
...
def test_translate_anthropic_messages_to_openai_thinking_only_sets_reasoning_content():
    anthropic_messages = [
        AnthropicMessagesUserMessageParam(
            role="user",
            content=[{"type": "text", "text": "Think through this carefully."}],
        ),
        AnthopicMessagesAssistantMessageParam(
            role="assistant",
            content=[
    ...
    assert result[1]["reasoning_content"] == "First step. Second step."
    assert len(result[1]["thinking_blocks"]) == 2
...
def test_translate_anthropic_messages_to_openai_tool_result_with_multiple_content_items():
    ...
    assert len(tool_message["content"]) == 3, (
