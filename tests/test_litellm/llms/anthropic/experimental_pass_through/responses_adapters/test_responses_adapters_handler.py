...
async def test_streaming_headers_are_carried_on_the_returned_response_object():
    ...
    # the handler's only seam is litellm.aresponses; faking the HTTP boundary
    # would re-test the responses client instead of this bridge.
    aresponses_mock = AsyncMock(return_value=stream)
    with patch.object(litellm, "aresponses", aresponses_mock):  # test-quality-ok: only seam is the SDK call
