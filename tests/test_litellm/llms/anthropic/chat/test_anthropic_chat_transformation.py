...
def test_get_supported_openai_params_hides_temperature_for_models_without_sampling_params():
    """Models whose API removed sampling params (Opus 4.7+/Fable 5) must not
    advertise temperature/top_p, otherwise callers send values that 400.

    Regression for https://github.com/BerriAI/litellm/issues/42910.
    """
    config = AnthropicConfig()

    # Model name matches the name-based fallback (no bundled cost-map entry needed).
    no_sampling = config.get_supported_openai_params(model="claude-opus-4-7-20250101")
    assert "temperature" not in no_sampling
    assert "top_p" not in no_sampling

    # A normal Claude model still advertises both.
