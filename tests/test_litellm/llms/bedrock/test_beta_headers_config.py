...
class TestModelVersionExtraction:
    ...
    def test_extract_version_suffix_less_model_id(self):
        """Test version extraction for suffix-less model IDs (no date/v1 suffix)."""
        filter_obj = get_bedrock_beta_filter(BedrockAPI.INVOKE_CHAT)
        # These bare IDs are valid Bedrock model identifiers
        assert filter_obj._extract_model_version("anthropic.claude-sonnet-4-6") == 4.6
        assert filter_obj._extract_model_version("anthropic.claude-opus-4-5") == 4.5
        assert filter_obj._extract_model_version("anthropic.claude-haiku-4-5") == 4.5
...
class TestVersionBasedFiltering:
    ...
    def test_context_management_requires_claude_4_5(self):
        ...
        # Haiku 4.5 should support context management
        result = filter_obj.filter_beta_headers(
            ["context-management-2025-06-27"],
            "anthropic.claude-haiku-4-5-20250514-v1:0",
            translate=False,
        ...
        # Opus 4.5 should NOT support context management (wrong family)
        result = filter_obj.filter_beta_headers(
            ["context-management-2025-06-27"],
            "anthropic.claude-opus-4-5-20250514-v1:0",
            translate=False,
    ...
    def test_future_model_supports_all_headers(self):
        ...
        # Opus 5 should support thinking/context headers (no family restriction on those)
        result = filter_obj.filter_beta_headers(
            [
        ...
        # Sonnet 5 should support context-management (sonnet/haiku family only)
        result = filter_obj.filter_beta_headers(
            ["context-management-2025-06-27"],  # Requires 4.5+, sonnet/haiku only
            "anthropic.claude-sonnet-5-20270101-v1:0",
            translate=False,
        )
        assert "context-management-2025-06-27" in result
...
class TestFamilyRestrictions:
    ...
    def test_latest_computer_use_version_and_family_restriction(self):
        """Test that computer-use-2025-11-24 requires Opus/Sonnet 4.5+."""
        filter_obj = get_bedrock_beta_filter(BedrockAPI.INVOKE_CHAT)

        # Opus 4.5 should support latest computer use
        result = filter_obj.filter_beta_headers(
            ["computer-use-2025-11-24"],
            "anthropic.claude-opus-4-5-20250514-v1:0",
            translate=False,
        )
        assert "computer-use-2025-11-24" in result

        # Sonnet 4.5 should support latest computer use
        result = filter_obj.filter_beta_headers(
            ["computer-use-2025-11-24"],
            "anthropic.claude-sonnet-4-5-20250514-v1:0",
            translate=False,
        ...
        # Haiku 4.5 should NOT support latest computer use (wrong family)
        result = filter_obj.filter_beta_headers(
            ["computer-use-2025-11-24"],
            "anthropic.claude-haiku-4-5-20251001-v1:0",
            translate=False,
        )
        assert "computer-use-2025-11-24" not in result

        # Sonnet 3.7 should NOT support latest computer use (version too old)
        result = filter_obj.filter_beta_headers(
            ["computer-use-2025-11-24"],
        ...
        # Claude 3.5 Sonnet should NOT support latest computer use (version too old)
        result = filter_obj.filter_beta_headers(
            ["computer-use-2025-11-24"],
...
class TestCrossAPIConsistency:
    ...
    def test_same_headers_work_on_all_apis(self):
        ...
        result_chat = set(
            filter_chat.filter_beta_headers(headers, model, translate=False)
        )
        result_messages = set(
