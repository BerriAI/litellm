...
def test_map_openai_params_forwards_anthropic_shape():
    ...
    cfg.map_openai_params(
        non_default_params={"context_management": {"edits": [{"type": "compact_20260112"}]}},
        optional_params=optional_params,
        model=CLAUDE_MODEL,
        drop_params=False,
    )
    assert optional_params.get("context_management") == {"edits": [{"type": "compact_20260112"}]}
...
def test_filter_keeps_only_compact_edits_and_adds_beta_header():
    ...
    betas: list = []
    AmazonConverseConfig._filter_context_management_for_bedrock_converse(additional, betas)
