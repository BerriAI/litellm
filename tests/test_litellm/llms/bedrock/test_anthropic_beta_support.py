...
class TestBedrockBetaHeaderFiltering:
    ...
    def test_messages_filters_unsupported_headers(self):
        ...
        config = AmazonAnthropicClaudeMessagesConfig()
        headers = {"anthropic-beta": "output-128k-2025-02-19,unknown-beta-2099-01-01"}
    ...
    def test_backward_compatibility_existing_headers(self):
        ...
        No single model supports every header (e.g. effort is opus-only,
        context-management is sonnet/haiku-only), so we test two model families.
        """
        config = AmazonAnthropicClaudeConfig()

        # Headers supported across all Claude 4.5+ models
        universal_headers = [
            "computer-use-2024-10-22",
        ...
        # --- Opus 4.5: universal + opus-only headers ---
        opus_headers = universal_headers + [
            "effort-2025-11-24",
        ...
        result = config.transform_request(
            ...
            optional_params={},
            litellm_params={},
            headers={"anthropic-beta": ",".join(opus_headers)},
        )
        assert "anthropic_beta" in result
        opus_betas = set(result["anthropic_beta"])
        for header in opus_headers:
            assert header in opus_betas, f"Opus: {header} was filtered out unexpectedly"

        # --- Sonnet 4.5: universal + sonnet-compatible headers ---
        sonnet_headers = universal_headers + [
        ...
        result = config.transform_request(
            ...
            optional_params={},
            litellm_params={},
            headers={"anthropic-beta": ",".join(sonnet_headers)},
        )
        assert "anthropic_beta" in result
        sonnet_betas = set(result["anthropic_beta"])
        for header in sonnet_headers:
            assert (
                header in sonnet_betas
            ), f"Sonnet: {header} was filtered out unexpectedly"
    ...
    def test_messages_advanced_tool_use_translation_opus_4_5(self):
        ...
        assert (
            "advanced-tool-use-2025-11-20" not in beta_headers
        ...
        assert (
            "tool-search-tool-2025-10-19" in beta_headers
        ), "tool-search-tool-2025-10-19 should be added for Opus 4.5"
        assert (
            "tool-examples-2025-10-29" in beta_headers
