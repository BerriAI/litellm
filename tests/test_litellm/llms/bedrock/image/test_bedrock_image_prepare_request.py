...
def test_bedrock_image_prepare_request_with_arn() -> None:
    ...
    with (
        patch(
            "litellm.llms.bedrock.image.image_handler.BedrockImageGeneration._get_boto_credentials_from_optional_params"
        ),
        patch(
            "litellm.llms.bedrock.image.image_handler.BedrockImageGeneration.get_request_headers"
        ),
    ):
