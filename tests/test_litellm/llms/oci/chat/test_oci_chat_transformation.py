...
class TestOCIGeminiResponseHandling:
    """Tests for Gemini-specific response edge cases on OCI."""

    def test_transform_response_missing_message(self):
        """
        Gemini may omit choices[0].message when reasoning tokens exhaust maxTokens.
        Should return empty content instead of crashing.
        """
        config = OCIChatConfig()
        created_time = (
            datetime.datetime.now(datetime.timezone.utc)
        ...
        assert isinstance(result, ModelResponse)
        assert len(result.choices) == 1
        assert result.usage.prompt_tokens == 6  # type: ignore
        assert result.usage.completion_tokens == 0  # type: ignore
        assert result.usage.total_tokens == 12  # type: ignore

    def test_transform_response_gemini_with_message(self):
        """
        Gemini with sufficient maxTokens returns message normally.
        """
        config = OCIChatConfig()
        created_time = (
            datetime.datetime.now(datetime.timezone.utc)
        ...
        assert isinstance(result, ModelResponse)
        assert result.choices[0].message.content == "Hello"
        assert result.usage.completion_tokens == 1  # type: ignore
        assert result.usage.total_tokens == 29  # type: ignore
