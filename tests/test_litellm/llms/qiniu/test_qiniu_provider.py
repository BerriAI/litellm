...
def test_qiniu_models_in_model_cost(monkeypatch):
...
def test_qiniu_cost_calculation(monkeypatch):
    monkeypatch.setenv("LITELLM_LOCAL_MODEL_COST_MAP", "True")
    original = litellm.model_cost
    litellm.model_cost = litellm.get_model_cost_map(url="")
    try:
        ...
        # $0.56/M input, $1.67/M output
        assert math.isclose(prompt_cost, 0.56, rel_tol=1e-6)
        assert math.isclose(completion_cost, 1.67, rel_tol=1e-6)
    finally:
        litellm.model_cost = original
