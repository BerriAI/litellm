...
def test_add_object_type_preserves_empty_schema():
    """
    Test that add_object_type does NOT add type: object to a truly empty schema {}
    in nested (property) context.

    An empty schema {} in JSON Schema means 'any value is valid'. This is
    generated by Pydantic's JsonValue type. Converting it to {type: object}
    incorrectly narrows semantics to only accept JSON objects.

    See: https://github.com/BerriAI/litellm/issues/22391
    """
    from litellm.llms.vertex_ai.common_utils import add_object_type

    input_schema = {}
    add_object_type(input_schema, _nested=True)

    assert "type" not in input_schema, (
        "Empty schema {} means 'any type' and should not be narrowed to type: object"
    )


def test_build_vertex_schema_jsonvalue_preserved():
    """
    End-to-end test: Pydantic JsonValue type should NOT be coerced to type: object
    when processed by _build_vertex_schema for Gemini.

    JsonValue generates {"$defs": {"JsonValue": {}}} where {} means any JSON value.
    After $ref resolution, the empty schema must remain untyped so Gemini treats it
    as TYPE_UNSPECIFIED (accepts any value).
    ...
    # Simulates Pydantic's JsonValue output
    input_schema = {
        "$defs": {
            "JsonValue": {}
        },
    ...
    result = _build_vertex_schema(copy.deepcopy(input_schema))

    # The 'value' property should NOT have type: object
    value_schema = result["properties"]["value"]
    assert value_schema.get("type") != "object", (
        f"JsonValue property should not be coerced to type: object, got: {value_schema}"
    )


def test_build_vertex_schema_nested_jsonvalue_in_anyof():
    """
    Test that JsonValue inside anyOf (e.g. Optional[JsonValue]) is not coerced.

    Pydantic generates anyOf: [{$ref: #/$defs/JsonValue}, {type: null}] for
    Optional[JsonValue]. After $ref resolution the empty {} member should not
    become {type: object}.
    ...
    # A strong assertion: 'type' key should not be present at all for 'any JSON value'
    assert "type" not in data_schema, (
        f"Optional[JsonValue] should not have 'type' set (any value semantics), got: {data_schema}"
    )


def test_add_object_type_preserves_metadata_only_schema_nested():
    """
    Test that add_object_type does NOT add type: object to metadata-only
    schemas in nested context. Schemas like {"title": "X"} (from typing.Any)
    mean 'any value is valid' in JSON Schema.

    See: https://github.com/BerriAI/litellm/issues/22391
    """
    from litellm.llms.vertex_ai.common_utils import add_object_type

    # Simulates typing.Any property schema from Pydantic: {"title": "X"}
    input_schema = {"title": "X"}
    add_object_type(input_schema, _nested=True)

    assert "type" not in input_schema, (
        "Metadata-only schema {'title': 'X'} means 'any type' and should not "
        "be narrowed to type: object in nested context"
    )


def test_add_object_type_root_still_gets_type_object():
    """
    Root-level tool parameter schemas should still get type: object even when
    they only have metadata keys. This ensures tools with no arguments work.
    """
    from litellm.llms.vertex_ai.common_utils import add_object_type

    # Root level call (default _nested=False) — should add type: object
    input_schema = {"$schema": "https://json-schema.org/draft/2020-12/schema"}
    add_object_type(input_schema)

    assert input_schema.get("type") == "object", (
        "Root-level schemas should still default to type: object"
    )
