...
class TestProcessEmbedContentResponseUsage:
    ...
    Uses ``local_model_cost_map`` so ``litellm.model_cost`` and the
    ``get_model_info`` lru_cache start from the bundled cost map on every test in
    this class. Without it, an earlier test in the same xdist worker can warm the
    cache with a different model's rates and drop prompt_cost to 0. Cost
    assertions read the rates back out of ``litellm.model_cost`` rather than
    pinning vendor prices, per CLAUDE.md's "never pin third-party facts" rule.
    """
