...
class TestProcessEmbedContentResponseUsage:
    ...
    lru_cache start from the bundled cost map on every test in this class. Without
    it, an earlier test in the same xdist worker can warm the cache with a
    different model's rates (e.g. mixing gemini-embedding-2's audio_per_second
    with gemini-3.5-flash's audio_per_token) and drive prompt_cost to 0.

    Cost assertions on this class read the rates back out of ``litellm.model_cost``
    rather than pinning vendor prices, per CLAUDE.md's "never pin third-party
    facts" rule.
    """
    ...
    def test_file_reference_non_image_not_counted_as_image(self):
        ...
        expected_per_audio_second = litellm.model_cost[self.MODEL][
