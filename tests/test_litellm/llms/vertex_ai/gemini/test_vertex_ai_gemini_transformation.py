...
def test_convert_tool_response_with_base64_image():
    ...
    assert not isinstance(result, list), "Should return one function_response part"
...
def test_convert_tool_response_with_anthropic_image_block():
    """Anthropic tool_result image blocks must be nested in functionResponse.parts."""
    test_image_base64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=="
    tool_message = {
        "role": "tool",
        "tool_call_id": "call_anthropic_image",
        "content": [
    ...
    function_response = result["function_response"]
    assert function_response["name"] == "get_image"
