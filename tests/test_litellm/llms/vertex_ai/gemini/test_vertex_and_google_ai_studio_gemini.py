...
def test_streaming_iterator_handles_partial_json_after_first_chunk_sync():
    """
    Tests that the synchronous ModelResponseIterator can correctly handle a fragmented
    JSON chunk that arrives after a complete chunk has already been processed.

    This specifically tests the fix for the bug where the iterator would only
    enter JSON accumulation mode on the very first chunk, raising a JSONDecodeError
    on any subsequent partial chunks.
    """
    from litellm.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import (
        ModelResponseIterator,
    )
    ...
    # 1. A complete, valid JSON chunk.
    # 2. A second JSON object, split into two partial chunks.
    mock_stream_chunks = [
        '{"candidates": [{"content": {"parts": [{"text": "This is the first complete chunk."}]}}]}',
        '{"candidates": [{"content": {"parts": [{"text": "This is the second, fragmented chunk..."',  # Partial JSON
        "}]}}]}",  # Remainder of the JSON
    ]

    iterator = ModelResponseIterator(
        streaming_response=iter(mock_stream_chunks),
        sync_stream=True,
        logging_obj=mock_logging_obj,
    )

    # it should complete successfully.
    parsed_chunks = [chunk for chunk in iterator if chunk is not None]

    # Assert that both messages were correctly parsed.
    assert len(parsed_chunks) == 2
    assert "first complete chunk" in parsed_chunks[0].choices[0].delta.content
    assert "second, fragmented chunk" in parsed_chunks[1].choices[0].delta.content


@pytest.mark.asyncio
async def test_streaming_iterator_handles_partial_json_after_first_chunk_async():
    """
    Tests that the asynchronous ModelResponseIterator can correctly handle a fragmented
    JSON chunk that arrives after a complete chunk has already been processed.
    ...
    # Simulate an async stream with a complete chunk followed by a fragmented one.
    async def mock_async_iterator():
        yield '{"candidates": [{"content": {"parts": [{"text": "First async chunk."}]}}]}'
        await asyncio.sleep(0.01)
        yield '{"candidates": [{"content": {"parts": [{"text": "Second async fragmen'  # Partial
        await asyncio.sleep(0.01)
        yield 'ted chunk."}]}}]}'  # Remainder

    iterator = ModelResponseIterator(
        streaming_response=mock_async_iterator(),
        sync_stream=False,
        logging_obj=mock_logging_obj,
    )
    ...
    # Assert that both messages were correctly parsed.
    assert len(parsed_chunks) == 2
    assert "First async chunk" in parsed_chunks[0].choices[0].delta.content
    assert "Second async fragmented chunk" in parsed_chunks[1].choices[0].delta.content
