...
class TestVertexAIRerankIntegration:
    ...
    @patch(
        "litellm.llms.vertex_ai.rerank.transformation.VertexAIRerankConfig._ensure_access_token"
    )
    def test_end_to_end_rerank_flow(self, mock_ensure_access_token):
        ...
        with patch.object(
            self.config, "get_vertex_ai_credentials", return_value=None
        ), patch.object(
            self.config, "get_vertex_ai_project", return_value="test-project-123"
        ):
