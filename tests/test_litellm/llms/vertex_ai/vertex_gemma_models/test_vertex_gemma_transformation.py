...
@pytest.fixture(autouse=True)
def _mock_vertexai_import(monkeypatch):
    vertex_preview = Mock()
    vertex_preview.language_models = []
    vertex_ai_mock = Mock()
    vertex_ai_mock.preview = vertex_preview
    with patch.dict(sys.modules, {"vertexai": vertex_ai_mock}):
        yield
