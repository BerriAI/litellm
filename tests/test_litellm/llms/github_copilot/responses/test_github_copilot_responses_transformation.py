...
from litellm.llms.github_copilot.responses.transformation import (
    ...
    _is_valid_regex,
    _sanitize_json_schema_regex_patterns,
)
...
class TestGithubCopilotToolRegexSanitization:
    ...
    def test_preserve_valid_regex_pattern(self):
        ...
        assert id_prop.get("pattern") == valid_pattern
    ...
    def test_parameter_named_pattern_preserved(self):
        ...
        assert "pattern" not in pattern_arg
    ...
    def test_pattern_properties_incompatible_key(self):
        ...
        assert invalid_regex_key not in pp
        assert valid_regex_key in pp
    ...
    def test_tools_passthrough_edge_cases(self):
        ...
        non_dict_tools: list[object] = ["not_a_dict", 123]
        res3 = config.transform_responses_api_request(
            model="github_copilot/gpt-5.5",
            input="Hi",
            response_api_optional_request_params={"tools": non_dict_tools},  # pyright: ignore[reportArgumentType]  # runtime defense against malformed tools
            litellm_params=MagicMock(),
            headers={},
        )
    ...
    def test_sanitize_json_schema_non_dict_and_cycle_handling(self):
        assert _sanitize_json_schema_regex_patterns("not_a_schema") == "not_a_schema"
        assert _sanitize_json_schema_regex_patterns(42) == 42
        assert _sanitize_json_schema_regex_patterns(None) is None

        cyclic: dict[str, object] = {"type": "object"}
        cyclic["self"] = cyclic
        cleaned = _sanitize_json_schema_regex_patterns(cyclic)
