...
class TestGitHubCopilotAuthenticator:
    ...
    def test_get_access_token_login(self, authenticator):
        ...
        with patch.object(authenticator, "_login", return_value=mock_token), patch(
    ...
    def test_get_api_key_expired(self, authenticator):
        ...
        with patch(
            "builtins.open", mock_open(read_data=mock_expired_data)
        ), patch.object(
            authenticator, "_refresh_api_key", return_value=mock_new_data
        ), patch(
            "json.dump"
        ) as mock_json_dump:
            api_key = authenticator.get_api_key()
    ...
    def test_refresh_api_key(self, authenticator, mock_http_client):
        ...
        with patch.object(
            authenticator, "get_access_token", return_value=mock_token
        ), patch(
            "litellm.llms.github_copilot.authenticator._get_httpx_client",
            return_value=mock_client,
        ), patch.object(
            mock_response, "json", return_value=mock_api_key_data
        ):
    ...
    def test_poll_for_access_token(self, authenticator, mock_http_client):
        ...
        with patch(
            "litellm.llms.github_copilot.authenticator._get_httpx_client",
            return_value=mock_client,
        ), patch.object(mock_response, "json", return_value=mock_token_data), patch(
            "time.sleep"
        ):
