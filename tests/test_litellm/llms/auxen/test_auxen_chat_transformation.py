...
class TestAuxenProviderRouting:
    ...
    def test_auxen_models_registered(self):
        assert len(litellm.auxen_models) > 0
        assert "auxen/llama-3.1-8b" in litellm.auxen_models

    def test_models_by_provider_contains_auxen(self):
        assert "auxen" in litellm.models_by_provider
        assert len(litellm.models_by_provider["auxen"]) > 0
