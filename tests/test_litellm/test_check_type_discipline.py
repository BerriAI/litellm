...
def test_two_for_clauses_are_flagged(tmp_path):
    assert "LIT013" in _codes(tmp_path, "y = [x for a in xs for x in a]\n")


def test_two_ifs_on_one_generator_are_flagged(tmp_path):
    assert "LIT013" in _codes(tmp_path, "y = [x for x in xs if x if x > 1]\n")


def test_one_if_on_each_of_two_generators_is_flagged(tmp_path):
    assert "LIT013" in _codes(tmp_path, "y = [x for a in xs if a for x in a if x]\n")


def test_one_for_and_one_if_is_clean(tmp_path):
    assert "LIT013" not in _codes(tmp_path, "y = tuple(x for x in xs if x)\n")


def test_dict_set_and_generator_two_fors_are_each_flagged(tmp_path):
    assert "LIT013" in _codes(tmp_path, "d = {k: v for a in xs for k, v in a}\n")
    assert "LIT013" in _codes(tmp_path, "s = {x for a in xs for x in a}\n")
    assert "LIT013" in _codes(tmp_path, "g = (x for a in xs for x in a)\n")


def test_nested_comprehension_in_element_is_judged_separately(tmp_path):
    assert "LIT013" not in _codes(tmp_path, "y = [[v for v in a] for a in xs]\n")


def test_comprehension_ok_with_reason_suppresses_lit013(tmp_path):
    codes = _codes(
        tmp_path,
...
def test_violation_message_names_the_clause_counts(tmp_path):
    ...
    messages = [v.message for v in checker.check_file(f) if v.code == "LIT013"]
    assert len(messages) == 1
