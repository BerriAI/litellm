...
def test_custom_pricing_preferred_over_provider_dispatch(monkeypatch):
    ...
    def raise_if_provider_dispatch_is_used(*args, **kwargs):
        raise AssertionError("provider-specific cost calculator should not run when custom pricing is configured")
...
def test_provider_dispatch_preserved_without_custom_pricing(monkeypatch):
    ...
    prompt_cost, completion_cost_value = cost_per_token(
        model=model,
        custom_llm_provider="anthropic",
        usage_object=Usage(prompt_tokens=1000, completion_tokens=100, total_tokens=1100),
        custom_pricing=False,
    )
...
def test_custom_pricing_preferred_over_bedrock_dispatch(monkeypatch):
    ...
    def raise_if_provider_dispatch_is_used(*args, **kwargs):
        raise AssertionError("bedrock_cost_per_token should not run when custom pricing is configured")
...
def test_custom_pricing_preferred_with_cache_only_pricing(monkeypatch):
    ...
    def raise_if_provider_dispatch_is_used(*args, **kwargs):
        raise AssertionError(
            "provider-specific cost calculator should not run when cache-only custom pricing is configured"
        )
...
def test_has_token_based_custom_pricing_helper():
    ...
    assert _has_token_based_custom_pricing({"cache_creation_input_token_cost": 0.0}) is True
    ...
    assert _has_token_based_custom_pricing({"output_cost_per_reasoning_token": 0.0}) is True
