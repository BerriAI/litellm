...
async def test_retry_breadcrumbs_do_not_carry_the_walk_state():
    ...
    assert any("fallback_depth" in breadcrumb for breadcrumb in breadcrumbs), (
...
async def test_retry_breadcrumbs_never_carry_a_forwarded_credential(container_key, request_kwargs):
    ...
    assert container_key in dumped, (
        "the credential-bearing kwarg never reached the breadcrumb, so this test cannot see the leak"
    )
