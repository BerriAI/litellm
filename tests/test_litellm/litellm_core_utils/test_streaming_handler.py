...
async def _advance_to_end_of_stream(response: CustomStreamWrapper) -> None:
    """Drive the wrapper until its source iterator is exhausted.

    The next ``__anext__`` after this is the one that runs end-of-stream
    finalization. Keyed to the source iterator rather than to
    ``sent_last_chunk``, which ``chunk_creator`` can set while chunks remain.
    """
    source = response.completion_stream
    while source.index < len(source.model_responses):
        await response.__anext__()
    assert response.sent_last_chunk is True
...
async def test_async_stream_completion_accounts_before_repeated_cancellation(
    ...
    await _advance_to_end_of_stream(response)
