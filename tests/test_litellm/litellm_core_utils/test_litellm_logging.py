...
async def test_prompt_hook_injection_marker_recorded_for_every_surface(logging_obj):
    """The savings gate reads litellm_injected_cache_breakpoints from the request's
    ...
    assert request_kwargs["metadata"]["litellm_injected_cache_breakpoints"] == 1
    ...
    assert "litellm_injected_cache_breakpoints" not in untouched["metadata"]
