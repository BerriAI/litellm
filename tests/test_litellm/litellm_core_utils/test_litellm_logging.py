...
def test_get_assembled_streaming_response_handles_dict_response_usage(logging_obj):
    """#29913: on the streaming /v1/responses spend-log path result.response can
    arrive as a dict rather than a ResponsesAPIResponse. The unified-usage step
    must not assume a `.usage` attribute (it raised AttributeError before)."""
