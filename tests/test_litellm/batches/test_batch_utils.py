...
def test_bedrock_embedding_batch_usage_is_parsed():
    body = {"embedding": [0.1, 0.2], "inputTextTokenCount": 17}
    usage = bu._get_batch_job_usage_from_response_body(body, custom_llm_provider="bedrock")
    assert (usage.prompt_tokens, usage.completion_tokens, usage.total_tokens) == (17, 0, 17)


def test_bedrock_batch_usage_without_usage_or_embedding_tokens_is_zero():
