...
def test_transform_response_recovers_output_text_delta_from_raw_sse():
    from litellm.completion_extras.litellm_responses_transformation.transformation import (
        LiteLLMResponsesTransformationHandler,
    )
    ...
    raw_sse = "\n".join(
        [
            'data: {"type":"response.output_item.added","output_index":0,"item":{"type":"message","id":"msg_from_delta","role":"assistant","status":"in_progress","content":[]}}',
            'data: {"type":"response.content_part.added","output_index":0,"content_index":0,"item_id":"msg_from_delta","part":{"type":"output_text","text":"","annotations":[]}}',
            'data: {"type":"response.output_text.delta","output_index":0,"content_index":0,"item_id":"msg_from_delta","delta":"Recovered "}',
            'data: {"type":"response.output_text.delta","output_index":0,"content_index":0,"item_id":"msg_from_delta","delta":"from delta"}',
            'data: {"type":"response.completed","response":{"id":"resp_from_stream","object":"response","created_at":1760144904,"status":"completed","model":"gpt-5.4","output":[]}}',
            "data: [DONE]",
            "",
        ]
    ...
    assert len(result.choices) == 1
    assert result.choices[0].message.content == "Recovered from delta"
