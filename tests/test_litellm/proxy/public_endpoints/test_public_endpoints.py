...
def test_get_provider_create_fields_exposes_inception():
    app_instance = FastAPI()
    app_instance.include_router(router)
    client = TestClient(app_instance)
    ...
    inception = [
        provider for provider in response.json() if provider["litellm_provider"] == LlmProviders.INCEPTION.value
    ]

    assert len(inception) == 1, (
        "Inception must be served by /public/providers/fields; the Admin UI Add Model "
        "dropdown is built exclusively from this response, so a missing entry makes the "
        "provider unselectable even though it is a supported LlmProviders member"
    )
    assert inception[0]["provider"] == "INCEPTION"
    assert inception[0]["provider_display_name"] == "Inception"
    assert {field["key"] for field in inception[0]["credential_fields"]} == {
        "api_base",
        "api_key",
    }


def test_get_provider_create_fields_only_lists_supported_providers():
    app_instance = FastAPI()
    app_instance.include_router(router)
    client = TestClient(app_instance)
    ...
    supported = {provider.value for provider in LlmProviders}
    unknown = sorted(
        provider["litellm_provider"] for provider in response.json() if provider["litellm_provider"] not in supported
    )

    assert unknown == [], f"Unknown litellm_provider values in provider_create_fields.json: {unknown}"
