...
def test_get_sanitized_user_information_from_key_does_not_alias_metadata():
    """Regression: user_api_key_auth_metadata must be a copy of user_api_key_dict.metadata,
    not the same dict object. If it is aliased, add_management_endpoint_metadata_to_request_metadata
    mutates user_api_key_dict.metadata in place when merging team metadata, causing
    user_api_key_metadata to receive team metadata instead of key-only metadata."""
    key_metadata = {"owner": "alice", "cost_center": "eng-123"}
    user_api_key_dict = UserAPIKeyAuth(
        api_key="hashed-key",
        metadata=key_metadata,
        team_metadata={"cost_center": "team-456", "department": "Platform"},
    )

    result = LiteLLMProxyRequestSetup.get_sanitized_user_information_from_key(user_api_key_dict=user_api_key_dict)

    assert result["user_api_key_auth_metadata"] is not user_api_key_dict.metadata, (
        "user_api_key_auth_metadata must not be aliased to user_api_key_dict.metadata; "
        "mutating it (via .update() when merging team metadata) would corrupt user_api_key_metadata"
    )
    assert result["user_api_key_auth_metadata"] == key_metadata


@pytest.mark.asyncio
async def test_key_metadata_not_contaminated_by_team_metadata_after_add_litellm_data_to_request():
    """Regression: when a key has non-None metadata and belongs to a team that also has metadata,
    user_api_key_metadata in the request metadata dict must contain only the key's own fields,
    not team metadata merged in."""
    from litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request
    ...
    key_metadata = {"owner": "alice", "cost_center": "eng-123"}
    team_metadata = {"cost_center": "team-456", "department": "Platform", "manager": "bob"}
    ...
    key_metadata_in_request = updated["metadata"]["user_api_key_metadata"]

    assert "department" not in key_metadata_in_request, (
        "team metadata field 'department' must not appear in user_api_key_metadata"
    )
    assert "manager" not in key_metadata_in_request, (
        "team metadata field 'manager' must not appear in user_api_key_metadata"
    )
    assert key_metadata_in_request.get("owner") == "alice"
    assert key_metadata_in_request.get("cost_center") == "eng-123"
