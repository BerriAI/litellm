...
class TestTokenUtilities:
    ...
    def test_save_token(self):
        ...
        ) as mock_path, patch("os.chmod") as mock_chmod:
    ...
    def test_load_token_success(self):
        ...
        with patch("builtins.open", mock_open(read_data=json.dumps(token_data))), patch(
            "litellm.proxy.client.cli.commands.auth.get_token_file_path"
        ) as mock_path, patch("os.path.exists", return_value=True):
            mock_path.return_value = "/test/path/token.json"
    ...
    def test_load_token_file_not_exists(self):
        ...
        with patch(
            "litellm.proxy.client.cli.commands.auth.get_token_file_path"
        ) as mock_path, patch("os.path.exists", return_value=False):
            mock_path.return_value = "/test/path/token.json"
    ...
    def test_load_token_io_error(self):
        ...
        with patch("builtins.open", side_effect=IOError("Permission denied")), patch(
            "litellm.proxy.client.cli.commands.auth.get_token_file_path"
        ) as mock_path, patch("os.path.exists", return_value=True):
            mock_path.return_value = "/test/path/token.json"
    ...
    def test_clear_token_file_exists(self):
        ...
        with patch(
            "litellm.proxy.client.cli.commands.auth.get_token_file_path"
        ) as mock_path, patch("os.path.exists", return_value=True), patch(
            "os.remove"
        ) as mock_remove:
            mock_path.return_value = "/test/path/token.json"
    ...
    def test_clear_token_file_not_exists(self):
        ...
        with patch(
            "litellm.proxy.client.cli.commands.auth.get_token_file_path"
        ) as mock_path, patch("os.path.exists", return_value=False), patch(
            "os.remove"
        ) as mock_remove:
            mock_path.return_value = "/test/path/token.json"
...
class TestLoginCommand:
    ...
    def test_login_success(self):
        ...
        with patch("webbrowser.open") as mock_browser, patch(
            "requests.get", return_value=mock_response
        ) as mock_get, patch(
            "litellm.proxy.client.cli.commands.auth.save_token"
        ) as mock_save, patch(
            "litellm.proxy.client.cli.interface.show_commands"
        ) as mock_show_commands, patch(
            "uuid.uuid4", return_value="test-uuid-123"
        ):
    ...
    def test_login_http_error(self):
        ...
        with patch("webbrowser.open"), patch(
            "requests.get", return_value=mock_response
        ), patch("time.sleep"), patch("uuid.uuid4", return_value="test-uuid-123"):
            result = self.runner.invoke(login, obj=mock_context.obj)
    ...
    def test_login_general_exception(self):
        ...
        with patch("webbrowser.open"), patch(
            "requests.get", side_effect=ValueError("Invalid value")
        ), patch("uuid.uuid4", return_value="test-uuid-123"):
            result = self.runner.invoke(login, obj=mock_context.obj)
...
class TestCLIKeyRegenerationFlow:
    ...
    def test_login_with_existing_key_regeneration_flow(self):
        ...
        with patch("webbrowser.open") as mock_browser, patch(
            ...
            "litellm.proxy.client.cli.commands.auth.get_stored_api_key",
            return_value=existing_key,
        ) as mock_get_stored, patch(
            "litellm.proxy.client.cli.commands.auth.save_token"
        ) as mock_save, patch(
            "litellm.proxy.client.cli.interface.show_commands"
        ) as mock_show_commands, patch(
            "uuid.uuid4", return_value="new-session-uuid-789"
        ):
            result = self.runner.invoke(login, obj=mock_context.obj)
    ...
    def test_login_without_existing_key_creation_flow(self):
        ...
        with patch("webbrowser.open") as mock_browser, patch(
            ...
            "uuid.uuid4", return_value="new-session-uuid-999"
        ):
            result = self.runner.invoke(login, obj=mock_context.obj)
            ...
            assert (
                "existing_key=" not in call_args
            )  # Should not include existing_key param
