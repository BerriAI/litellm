...
def test_create_model_info_response_includes_max_tokens_when_available():
    ...
    response = create_model_info_response(model_id="qwen-vllm", provider="openai", llm_router=router)
...
def test_create_model_info_response_omits_unknown_individual_limit():
    ...
    response = create_model_info_response(model_id="partial", provider="openai", llm_router=router)
...
def test_create_model_info_response_omits_limits_when_both_none():
    ...
    response = create_model_info_response(model_id="no-limits", provider="openai", llm_router=router)
...
def test_create_model_info_response_omits_limits_when_group_unknown():
    ...
    response = create_model_info_response(model_id="openai/*", provider="openai", llm_router=router)
...
def test_create_model_info_response_degrades_when_group_info_raises():
    ...
    response = create_model_info_response(model_id="broken", provider="openai", llm_router=router)
...
def test_create_model_info_response_no_router_keeps_base_fields():
    response = create_model_info_response(model_id="some-model", provider="openai", llm_router=None)

    # Base identity fields are always present and no router-derived token limits
    # leak in when llm_router is None. The S1-06 ``capabilities`` sub-object is an
    # additive, always-present field, so this is a subset check, not exact equality.
