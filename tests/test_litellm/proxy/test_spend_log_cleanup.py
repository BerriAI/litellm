...
async def test_cleanup_old_spend_logs_batch_deletion():
    ...
    # first batch of each derived table's cleanup returning 0)
    ...
    # So do the auto-router session rollups, keyed on last activity rather than
    # session start so a live conversation is not pruned while still being written to
