...
@pytest.mark.asyncio
async def test_acquire_lock_extends_the_lease_this_pod_already_holds(
    pod_lock_manager, mock_redis
):
    """
    A job that outlives its TTL keeps re-acquiring its own lock. SET NX cannot
    refresh an existing key, so without an explicit extend the lock expires
    underneath the running job and a second pod is free to start the same work.
    """
    mock_redis.async_set_cache.return_value = False
    mock_redis.async_get_cache.return_value = pod_lock_manager.pod_id
    script_callable = AsyncMock(return_value=1)
    ...
    result = await pod_lock_manager.acquire_lock(cronjob_id="test_job", ttl=300)

    assert result is True
    lock_key = pod_lock_manager.get_redis_lock_key(cronjob_id="test_job")
    mock_redis.async_register_script.assert_called_once_with(
        PodLockManager._COMPARE_AND_EXPIRE_LOCK_SCRIPT
    )
    # acquire_lock stores the pod_id JSON-encoded, so the compare arm of the
    # script has to use the same encoding or it never matches.
    script_callable.assert_called_once_with(
        keys=[lock_key], args=[json.dumps(pod_lock_manager.pod_id), 300]
    )


@pytest.mark.asyncio
async def test_acquire_lock_extends_the_lease_with_set_when_scripting_is_unavailable(
    pod_lock_manager, mock_redis
):
    """
    Caches that cannot register Lua still have to get their lease renewed.
    """
    mock_redis.async_set_cache.return_value = False
    mock_redis.async_get_cache.return_value = pod_lock_manager.pod_id
    ...
    assert result is True
    lock_key = pod_lock_manager.get_redis_lock_key(cronjob_id="test_job")
    assert mock_redis.async_set_cache.call_args_list[-1] == call(
        lock_key, pod_lock_manager.pod_id, ttl=300
    )


@pytest.mark.asyncio
async def test_acquire_lock_still_succeeds_when_the_lease_extension_fails(
    pod_lock_manager, mock_redis
):
    """
    Ownership is already established by the time the extension runs. Reporting
    a failed renewal as a failed acquire would make the holder skip its own run.
    """
    mock_redis.async_set_cache.side_effect = [False, Exception("redis is down")]
    mock_redis.async_get_cache.return_value = pod_lock_manager.pod_id
...
@pytest.mark.asyncio
async def test_acquire_lock_does_not_extend_a_lease_held_by_another_pod(
    pod_lock_manager, mock_redis
):
    """
    Renewing someone else's lock would let a follower hold the leader's lease
    open indefinitely.
    """
    mock_redis.async_set_cache.return_value = False
    mock_redis.async_get_cache.return_value = "different_pod_id"
    ...
    script_callable.assert_not_called()
    assert mock_redis.async_set_cache.call_count == 1


def test_compare_and_expire_script_guards_the_expire_on_ownership():
    """
    The renewal has to stay conditional on this pod still owning the key. An
    unconditional EXPIRE would let a pod that lost the lock keep pushing out
    the new holder's lease, so the two of them run the job together.

    The Lua body is not executed by these tests (no live Redis), so its compare
    arm is asserted directly.
    """
    script = PodLockManager._COMPARE_AND_EXPIRE_LOCK_SCRIPT

    assert 'redis.call("get", KEYS[1]) == ARGV[1]' in script
    assert 'redis.call("expire", KEYS[1], ARGV[2])' in script
    assert script.index('redis.call("get"') < script.index('redis.call("expire"')


class FakeRedisLeaseStore(FakeRedisLockStore):
    """FakeRedisLockStore that also tracks expiry, so a renewal is observable."""
    ...
    async def async_set_cache(self, key, value, nx=False, ttl=None, **kwargs):
        result = await super().async_set_cache(key, value, nx=nx, ttl=ttl, **kwargs)
        if result:
            self.ttls[key] = ttl
        return result
    ...
    def async_register_script(self, script):
        async def _run(keys, args):
            ...
            if self.store.get(key) != args[0]:
                return 0
            if script == PodLockManager._COMPARE_AND_EXPIRE_LOCK_SCRIPT:
...
@pytest.mark.asyncio
async def test_reacquiring_your_own_lock_pushes_the_expiry_out():
    """
    End to end: the second acquire finds its own pod_id and has to move the
    expiry, which is what keeps a long reset run's lease alive.
    """
    redis = FakeRedisLeaseStore()
    pod = PodLockManager(redis_cache=redis)
    lock_key = PodLockManager.get_redis_lock_key("reset_budget_job")

    assert await pod.acquire_lock(cronjob_id="reset_budget_job", ttl=300) is True
    assert redis.ttls[lock_key] == 300

    assert await pod.acquire_lock(cronjob_id="reset_budget_job", ttl=600) is True
    assert redis.ttls[lock_key] == 600


@pytest.mark.asyncio
async def test_a_second_pod_cannot_push_out_the_holders_expiry():
    """
    The loser of the election must leave the holder's lease exactly as it was.
    """
    redis = FakeRedisLeaseStore()
    holder = PodLockManager(redis_cache=redis)
    other = PodLockManager(redis_cache=redis)
    ...
    assert await holder.acquire_lock(cronjob_id="reset_budget_job", ttl=300) is True
    assert await other.acquire_lock(cronjob_id="reset_budget_job", ttl=900) is False

    assert redis.ttls[lock_key] == 300
    assert redis.store[lock_key] == json.dumps(holder.pod_id)
