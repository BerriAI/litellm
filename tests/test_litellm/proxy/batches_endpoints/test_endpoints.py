...
def _creds_lookup(*, model_id: str, user_api_key_auth: object = None) -> Dict[str, str]:
...
async def test_create__model_encoded_file_id(harness):
    ...
    harness.creds_resolver.assert_called_once_with(model_id="azure/gpt-4o", user_api_key_auth=ANY)
...
async def test_create__model_from_body(harness):
    ...
    assert harness.litellm_acreate.call_count == 1
    harness.router_acreate.assert_not_called()
    harness.creds_resolver.assert_called_once_with(model_id="vertex-model", user_api_key_auth=ANY)
...
async def test_retrieve__model_encoded_id(retrieve_harness):
    ...
    retrieve_harness.creds_resolver.assert_called_once_with(model_id="azure/gpt-4o", user_api_key_auth=ANY)
...
async def test_list__model_from_body_routes_and_encodes(list_harness):
    ...
    list_harness.router_alist.assert_not_called()
    list_harness.creds_resolver.assert_called_once_with(model_id="azure/gpt-4o", user_api_key_auth=ANY)
...
async def test_cancel__model_encoded_id(cancel_harness):
    ...
    cancel_harness.creds_resolver.assert_called_once_with(model_id="azure/gpt-4o", user_api_key_auth=ANY)
...
async def test_create__header_model_allows_key_with_model_grant(harness):
    ...
    key = _key_restricted_to("vertex-model")
    await call_create(harness, user=key, headers={"x-litellm-model": "vertex-model"})

    harness.creds_resolver.assert_called_once_with(model_id="vertex-model", user_api_key_auth=key)
