...
@pytest.mark.asyncio
async def test_agent_with_unflushed_spend_is_included_in_budget_reset(reset_budget_job, mock_prisma_client):
    agent: Final = types.SimpleNamespace(agent_id="unflushed-agent", budget_id="budget-1", spend=0.0)

    async def matching_agents(*, where):
        return [] if "spend" in where else [agent]

    mock_prisma_client.db.litellm_agentstable.find_many = matching_agents
    cascade: Final = await reset_budget_job._collect_budget_cascade([_budget_row(budget_id="budget-1")])
    assert ("spend:agent:unflushed-agent", 0.0) in cascade.counter_resets


@pytest.mark.asyncio
@pytest.mark.parametrize("warm_floor", [False, True])
async def test_agent_reset_replaces_stale_floor_and_guards_inflight_db_read(
    reset_budget_job, monkeypatch, warm_floor: bool
):
    ...
    key: Final = "spend:agent:reset-race"
    marker: Final = f"spend_db_floor:{key}"
    entered: Final = asyncio.Event()
    release: Final = asyncio.Event()
    monkeypatch.setattr(proxy_server, "spend_counter_cache", cache)
    cache.in_memory_cache.set_cache(key=key, value=12.0)
    if warm_floor:
        cache.in_memory_cache.set_cache(key=marker, value=12.0)
    ...
    monkeypatch.setattr(proxy_server, "_floor_spend_from_db", stale_read)
    pending: Final = None if warm_floor else asyncio.create_task(proxy_server._authoritative_floor_spend(key))
    if pending is not None:
        await entered.wait()
    try:
        await reset_budget_job._invalidate_budget_cascade_caches(_BudgetCascade(counter_resets=((key, 0.0),)))
    finally:
        release.set()
    actual: Final = await pending if pending is not None else await proxy_server._authoritative_floor_spend(key)
    assert actual == 0.0
    assert cache.in_memory_cache.get_cache(key=key) == 0.0
    assert cache.in_memory_cache.get_cache(key=marker) == 0.0
