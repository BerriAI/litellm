...
def test_build_ui_login_form_uses_brand_name():
    html = build_ui_login_form()

    assert BRAND_NAME in html
    assert "LiteLLM" not in html
    assert f"{BRAND_NAME} Login" in html
    assert f"Access your {BRAND_NAME} Admin UI" in html
