...
@pytest.mark.asyncio
async def test_team_budget_alert_fires_at_each_crossed_threshold():
    """Each threshold in the list should fire a separate budget_alerts call when crossed."""
    from litellm.proxy.utils import ProxyLogging
    ...
    valid_token = UserAPIKeyAuth(token="hashed-token", team_id="team-abc")
    ...
    thresholds_fired = {call.kwargs["user_info"].budget_percentage_used for call in call_args_list}
    assert thresholds_fired == {0.8, 0.85}
    for call in call_args_list:
        assert call.kwargs["type"] == "team_budget"
        assert call.kwargs["user_info"].event_group == Litellm_EntityType.TEAM
        assert call.kwargs["user_info"].team_id == "team-abc"


@pytest.mark.asyncio
async def test_team_budget_alert_silent_below_lowest_threshold():
    """No alert fires when spend is below the lowest configured threshold."""
    from litellm.proxy.utils import ProxyLogging
...
@pytest.mark.asyncio
async def test_team_budget_alert_silent_when_no_max_budget():
    """No alert fires when team has no max_budget set."""
    from litellm.proxy.utils import ProxyLogging

    team_object = LiteLLM_TeamTable(team_id="team-free", spend=50.0, max_budget=None)
...
@pytest.mark.asyncio
async def test_team_budget_alert_silent_when_budget_fully_exceeded():
    """No threshold alert fires when spend >= max_budget (handled by hard enforcement)."""
    from litellm.proxy.utils import ProxyLogging
...
@pytest.mark.asyncio
async def test_user_budget_alert_fires_at_each_crossed_threshold():
    """Each threshold in the list should fire a separate budget_alerts call for users."""
    from litellm.proxy.utils import ProxyLogging
    ...
    valid_token = UserAPIKeyAuth(token="hashed-token", user_id="user-123")
    ...
    assert thresholds_fired == {0.8, 0.85}
    for call in call_args_list:
        assert call.kwargs["type"] == "user_budget"
        assert call.kwargs["user_info"].event_group == Litellm_EntityType.USER
        assert call.kwargs["user_info"].user_id == "user-123"


@pytest.mark.asyncio
async def test_user_budget_alert_silent_below_threshold():
    """No alert fires when user spend is below all configured thresholds."""
    from litellm.proxy.utils import ProxyLogging

    user_object = LiteLLM_UserTable(user_id="user-low", spend=0.0, max_budget=50.0)
...
@pytest.mark.asyncio
async def test_user_budget_alert_silent_when_no_max_budget():
    """No alert fires when user has no max_budget set."""
    from litellm.proxy.utils import ProxyLogging

    user_object = LiteLLM_UserTable(user_id="user-no-budget", spend=100.0, max_budget=None)
...
@pytest.mark.asyncio
async def test_virtual_key_budget_alert_multi_threshold():
    """Key alert fires for each crossed threshold with correct budget_percentage_used."""
    from litellm.proxy.utils import ProxyLogging
    ...
    # 88% crosses 0.8 and 0.85 but not 0.95
    assert proxy_logging_obj.budget_alerts.call_count == 2
    thresholds_fired = {
        call.kwargs["user_info"].budget_percentage_used for call in proxy_logging_obj.budget_alerts.call_args_list
    }
    assert thresholds_fired == {0.8, 0.85}
    for call in proxy_logging_obj.budget_alerts.call_args_list:
        assert call.kwargs["user_info"].event_group == Litellm_EntityType.KEY
