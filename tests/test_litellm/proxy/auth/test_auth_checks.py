...
# ---------------------------------------------------------------------------
# Tests for _copy_user_api_key_auth_for_cache and
# update_valid_token_with_end_user_params (issue #29142)
# ---------------------------------------------------------------------------

def test_copy_user_api_key_auth_for_cache_strips_end_user_fields():
    """_copy_user_api_key_auth_for_cache must not carry per-request end-user
    budget fields into the cache so that subsequent requests sharing the same
    virtual key are never checked against a different end-user's budget.

    Regression test for https://github.com/BerriAI/litellm/issues/29142
    """
    from litellm.proxy.auth.auth_checks import _copy_user_api_key_auth_for_cache
    from litellm.proxy._types import UserAPIKeyAuth

    token = UserAPIKeyAuth(
        token="hashed-key-abc",
        end_user_id="user-A",
        end_user_max_budget=2.0,
        end_user_tpm_limit=100,
        end_user_rpm_limit=50,
        end_user_model_max_budget={"gpt-4": 1.0},
    )

    cached = _copy_user_api_key_auth_for_cache(token)

    # Per-request end-user fields must be cleared so they don't bleed
    # into unrelated requests for a different end-user.
    assert cached.end_user_id is None, "end_user_id must be cleared before caching"
    assert cached.end_user_max_budget is None, (
        "end_user_max_budget must be cleared before caching"
    )
    assert cached.end_user_tpm_limit is None, (
        "end_user_tpm_limit must be cleared before caching"
    )
    assert cached.end_user_rpm_limit is None, (
        "end_user_rpm_limit must be cleared before caching"
    )
    assert cached.end_user_model_max_budget is None, (
        "end_user_model_max_budget must be cleared before caching"
    )

    # The virtual key identifier itself must be preserved.
    assert cached.token == "hashed-key-abc"


def test_update_valid_token_with_end_user_params_writes_max_budget():
    """update_valid_token_with_end_user_params must write end_user_max_budget
    onto the token so that the freshly-resolved DB value wins over any stale
    cached value.
    ...
    # Simulate a cache-hit token that carries stale end-user budget from a
    # *different* end-user (user-A's $2 budget leaked into the cached entry).
    token = UserAPIKeyAuth(
        token="hashed-key-abc",
        end_user_max_budget=2.0,  # stale value from a previous cache seed
    )

    # The DB resolved user-B's budget as $20.
    end_user_params = {
        "end_user_id": "user-B",
        "end_user_max_budget": 20.0,
    }

    updated = update_valid_token_with_end_user_params(token, end_user_params)

    assert updated.end_user_id == "user-B"
    assert updated.end_user_max_budget == 20.0, (
        "end_user_max_budget must be overwritten with the freshly-resolved DB "
        "value; stale cached budget must not survive the update"
    )
