...
def test_xff_ignored_when_forwarding_disabled():
    config = TrustedProxyConfig(use_forwarded_for=False, trusted_proxy_cidrs=["10.0.0.0/8"])
    request = make_request(headers={"x-forwarded-for": "203.0.113.9"}, client=("10.0.0.1", 1))
...
def test_xff_honored_from_trusted_peer():
    request = make_request(headers={"x-forwarded-for": "203.0.113.9, 10.0.0.5"}, client=("10.0.0.1", 1))
...
def test_spoofed_xff_from_untrusted_peer_is_ignored():
    request = make_request(headers={"x-forwarded-for": "203.0.113.9"}, client=("8.8.8.8", 1))
    ip, via_proxy = resolve_client_ip(request, TRUSTED)
    assert ip == "8.8.8.8"
    assert via_proxy is False
...
def test_all_trusted_hops_fall_back_to_peer():
    request = make_request(headers={"x-forwarded-for": "10.1.1.1, 10.0.0.9"}, client=("10.0.0.1", 1))
    ip, via_proxy = resolve_client_ip(request, TRUSTED)
    assert ip == "10.0.0.1"
    assert via_proxy is True
...
def test_invalid_xff_token_is_skipped():
    request = make_request(headers={"x-forwarded-for": "not-an-ip, 203.0.113.50"}, client=("10.0.0.1", 1))
