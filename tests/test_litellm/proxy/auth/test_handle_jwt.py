...
async def test_auth_builder_proxy_admin_user_role():
    ...
    with (
        ...
        ) as mock_check_rbac,  # test-quality-ok: [TQ008] collaborator injected via its import site; there is no seam to patch otherwise
        ...
        ) as mock_get_all_team_ids,  # test-quality-ok: [TQ008] collaborator injected via its import site; there is no seam to patch otherwise
        patch.object(
            JWTAuthManager,
        ...
        ) as mock_map_user,  # test-quality-ok: [TQ008] collaborator injected via its import site; there is no seam to patch otherwise
        patch.object(
            JWTAuthManager, "validate_object_id", return_value=True
        ) as mock_validate_object,  # test-quality-ok: [TQ008] collaborator injected via its import site; there is no seam to patch otherwise
    ):
...
async def test_auth_builder_returns_team_membership_object():
    ...
    with (
        ...
        ) as mock_sync_user,  # test-quality-ok: [TQ008] collaborator injected via its import site; there is no seam to patch otherwise
    ):
        # Set up the mock return values
...
async def test_auth_builder_single_team_fallback_membership_error_skips_no_raise():
    ...
    with (
        ...
        mock_get_membership.side_effect = HTTPException(status_code=500, detail="membership lookup failed")
