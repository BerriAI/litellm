...
@pytest.mark.asyncio
async def test_common_checks_runs_user_multi_budget_for_team_keys_by_default():
    user = LiteLLM_UserTable(
...
@pytest.mark.asyncio
async def test_common_checks_skips_user_multi_budget_for_team_keys_when_configured():
    user = LiteLLM_UserTable(
        user_id="user-with-windows",
        budget_limits=[
            {"budget_duration": "1d", "max_budget": 1.0, "reset_at": None},
