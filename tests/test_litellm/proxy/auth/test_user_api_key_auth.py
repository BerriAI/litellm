...
@pytest.mark.asyncio
async def test_blocked_user_key_is_rejected():
    """A virtual key whose owning user has metadata.blocked=True must be
    rejected by user_api_key_auth.
    """
    from fastapi import Request
    from starlette.datastructures import URL
    ...
    api_key = "sk-blocked-user-key"
    hashed_key = hash_token(api_key)

    valid_token = UserAPIKeyAuth(
        api_key=api_key,
        token=hashed_key,
        user_id="blocked-target-user",
    )
    blocked_user = LiteLLM_UserTable(
        user_id="blocked-target-user",
    ...
    try:
        ...
        assert "is blocked" in str(exc_info.value.message)
    finally:
        for attr, val in _original_values.items():
            setattr(_proxy_server_mod, attr, val)
