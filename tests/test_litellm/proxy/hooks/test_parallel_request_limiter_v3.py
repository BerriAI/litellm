...
async def test_max_parallel_requests_zset_acquire_allows_and_rejects():
    ...
    async def mock_max_parallel_requests_acquire(*args, **kwargs):
        ...
        if request_id in active_leases:
            return [1, current_count, current_count, 0, 0, 1]
    ...
    first_request_data = {
        "litellm_call_id": "client-controlled-call-id",
        "metadata": {},
    }
    ...
    first_lease_id = first_request_data["metadata"][
        MAX_PARALLEL_REQUESTS_LEASE_ID_FIELD
    ]
    assert first_lease_id != "client-controlled-call-id"
    uuid.UUID(first_lease_id)
    ...
    assert acquire_calls[1][1] != acquire_calls[0][1]
    assert len(active_leases) == 1


def test_max_parallel_requests_acquire_ignores_client_controlled_lease_inputs():
    """
    Acquire lease ids must be generated by the proxy, not copied from headers or
    ...
    attacker_values = {
        ...
        "top-level-lease",
        "metadata-lease",
        "litellm-metadata-lease",
        "params-lease",
        "params-metadata-lease",
        "params-litellm-metadata-lease",
        "standard-logging-lease",
    }
    request_data = {
        "litellm_call_id": "client-call-id",
        MAX_PARALLEL_REQUESTS_LEASE_ID_FIELD: "top-level-lease",
        "metadata": {
            MAX_PARALLEL_REQUESTS_LEASE_ID_FIELD: "metadata-lease",
        },
        "litellm_metadata": {
            MAX_PARALLEL_REQUESTS_LEASE_ID_FIELD: "litellm-metadata-lease",
        },
        "litellm_params": {
            MAX_PARALLEL_REQUESTS_LEASE_ID_FIELD: "params-lease",
            "metadata": {
                MAX_PARALLEL_REQUESTS_LEASE_ID_FIELD: "params-metadata-lease",
            },
            "litellm_metadata": {
                MAX_PARALLEL_REQUESTS_LEASE_ID_FIELD: "params-litellm-metadata-lease",
            },
        },
        "standard_logging_object": {
            "metadata": {
                MAX_PARALLEL_REQUESTS_LEASE_ID_FIELD: "standard-logging-lease",
            },
        },
    }

    lease_id = parallel_request_handler._create_max_parallel_requests_lease_id(
        request_data
    )

    assert lease_id not in attacker_values
    uuid.UUID(lease_id)
