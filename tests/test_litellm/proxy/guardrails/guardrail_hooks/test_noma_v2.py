...
class TestNomaV2Configuration:
    ...
    def test_build_scan_payload_survives_unpicklable_request_data(
        ...
        # safe_deep_copy keeps the original reference for un-deepcopy-able values
        # (per-key fallback). JSON sanitization happens later in _call_noma_scan.
        assert payload["request_data"]["event_loop"] is unpicklable
