...
def _output_request(messages: list, response=None, from_messages: bool = False) -> dict:
    ...
    return asyncio.run(
        _grounding_guardrail(from_messages).convert_to_bedrock_format(
...
def test_grounding_input_ignores_plain_message_derivation():
    ...
    actual_request = asyncio.run(
        _grounding_guardrail(from_messages=True).convert_to_bedrock_format(source="INPUT", messages=messages)
    )
...
class TestBedrockGuardrailImageInput:
    ...
    @pytest.mark.asyncio
    async def test_image_at_exactly_the_limit_is_scanned(self):
        """4 MB is what ApplyGuardrail accepts, so an image of exactly that size belongs
        in the scan. Sizing it from the base64 length alone reads 2 bytes over, because
        padding rounds the string up to a multiple of four, and the image gets refused.
        """
        exact_png = base64.b64encode(b"\x89PNG\r\n\x1a\n" + b"\x00" * (4 * 1024 * 1024 - 8)).decode()
        ...
        request = await self._guardrail().convert_to_bedrock_format(
            ...
            messages=[
                {
                    "role": "user",
                    "content": [{"type": "image_url", "image_url": {"url": f"data:image/png;base64,{exact_png}"}}],
                }
            ],
        )

        assert request["content"] == [{"image": {"format": "png", "source": {"bytes": exact_png}}}]
