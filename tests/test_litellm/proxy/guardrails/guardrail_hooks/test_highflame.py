...
def test_javelin_alias_routes_to_highflame():
    """Deprecated `javelin` guardrail routes to HighflameGuardrail (non-breaking)."""
    from litellm.proxy.guardrails.guardrail_hooks.highflame import (
    ...
    jav = SupportedGuardrailIntegrations.JAVELIN.value
    assert guardrail_class_registry[jav] is HighflameGuardrail
    assert guardrail_initializer_registry[jav] is initialize_guardrail

    # An existing javelin config still loads and registers a Highflame callback.
    params = LitellmParams(
        guardrail="javelin",
        mode="pre_call",
        api_key="hf_sk_test",
    ...
    cb = initialize_guardrail(params, {"guardrail_name": "javelin-legacy"})
    assert isinstance(cb, HighflameGuardrail)
    assert cb.application == "legacy-app"
