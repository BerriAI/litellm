...
def _make_guardrail(**kwargs: object) -> AiriaGuardrail:
...
def _make_guardrail_with_response(payload: object, status_code: int = 200, **kwargs: object) -> AiriaGuardrail:
...
def _inputs(**overrides: object) -> GenericGuardrailAPIInputs:
    return GenericGuardrailAPIInputs(**{"texts": ["hello"], "model": "gpt-4o", **overrides})


def test_airia_guardrail_config(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_during_call_is_not_a_supported_event_hook() -> None:
...
def test_trailing_slash_is_stripped_from_api_base() -> None:
    guardrail = _make_guardrail(api_base=f"{API_BASE}/")

    assert guardrail.api_base == API_BASE


def test_custom_timeout_from_kwargs() -> None:
...
def test_timeout_falls_back_to_environment(monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.asyncio
async def test_request_payload_and_auth_header() -> None:
...
async def test_input_type_is_forwarded_verbatim(input_type: Literal["request", "response"]) -> None:
    guardrail = _make_guardrail_with_response({"action": "NONE"})
...
@pytest.mark.asyncio
async def test_call_id_falls_back_to_a_generated_uuid() -> None:
    guardrail = _make_guardrail_with_response({"action": "NONE"})
...
@pytest.mark.asyncio
async def test_action_none_returns_inputs_unchanged() -> None:
    guardrail = _make_guardrail_with_response({"action": "NONE"})
...
@pytest.mark.asyncio
async def test_action_blocked_raises_with_blocked_content_set() -> None:
...
@pytest.mark.asyncio
async def test_action_blocked_without_a_reason_uses_the_default_message() -> None:
...
@pytest.mark.asyncio
async def test_intervened_replaces_both_text_bearing_fields() -> None:
...
@pytest.mark.asyncio
async def test_intervened_leaves_a_field_alone_when_airia_omits_it() -> None:
...
@pytest.mark.asyncio
async def test_intervened_with_a_non_list_field_is_treated_as_a_block() -> None:
...
@pytest.mark.asyncio
async def test_intervened_with_a_non_list_structured_messages_is_treated_as_a_block() -> None:
    guardrail = _make_guardrail_with_response(
...
@pytest.mark.asyncio
async def test_intervened_without_any_rewritten_field_is_treated_as_a_block() -> None:
...
async def test_a_non_object_response_body_fails_closed_but_is_not_reported_as_a_verdict(body: object) -> None:
...
@pytest.mark.asyncio
async def test_intervened_with_a_texts_rewrite_of_the_wrong_length_is_treated_as_a_block() -> None:
...
@pytest.mark.asyncio
async def test_streaming_redaction_actually_reaches_the_client() -> None:
    ...
    async def source_stream() -> AsyncGenerator[ModelResponseStream, None]:
...
def test_config_model_is_exposed() -> None:
...
@pytest.mark.asyncio
async def test_unrecognized_action_is_treated_as_a_block() -> None:
...
@pytest.mark.asyncio
async def test_missing_action_is_treated_as_a_block() -> None:
...
@pytest.mark.asyncio
async def test_transport_failure_fails_closed_but_is_not_reported_as_a_verdict() -> None:
...
@pytest.mark.asyncio
async def test_non_2xx_fails_closed_but_is_not_reported_as_a_verdict() -> None:
