...
class TestPanwAirsScanIdExposure:
    ...
    @pytest.mark.asyncio
    async def test_pre_call_allow_records_scan_id(self, base_handler, user_api_key_dict):
    ...
    @pytest.mark.asyncio
    async def test_post_call_allow_records_response_scan_id(self, base_handler, user_api_key_dict):
    ...
    @pytest.mark.asyncio
    async def test_apply_guardrail_allow_records_scan_id(self, base_handler):
        inputs: GenericGuardrailAPIInputs = {"texts": ["Hello world"]}
        ...
        with patch.object(base_handler, "_call_panw_api", new_callable=AsyncMock) as mock_api:
            ...
            await base_handler.apply_guardrail(inputs=inputs, request_data=request_data, input_type="request")

        assert self._recorded_scan_ids(request_data) == ("scan-abc-123",)

    @pytest.mark.asyncio
    async def test_allowed_scan_id_becomes_response_header(self, base_handler, user_api_key_dict):
    ...
    @pytest.mark.asyncio
    async def test_repeated_scan_id_is_not_duplicated(self, base_handler, user_api_key_dict):
        data = _simple_data(litellm_call_id="test-call-id", metadata={})
        response = ModelResponse(
            id="test_id",
            choices=[Choices(index=0, message=Message(role="assistant", content="hi"))],
