...
def test_split_text_by_words():
    ...
    chunks = guardrail._split_text_by_words(short_text, 100)
    assert len(chunks) == 1
    assert chunks[0] == short_text
    
    # Test text that needs splitting
    text = "word1 word2 word3 word4 word5"
    chunks = guardrail._split_text_by_words(text, 20)
    assert len(chunks) > 1
    ...
    chunks = guardrail._split_text_by_words(long_word, 50)
    assert len(chunks) > 1
    ...
    # Test empty string
    chunks = guardrail._split_text_by_words("", 100)
    assert chunks == [""]
    
    # Test with punctuation and special characters
    text_with_punctuation = "Hello, world! How are you? I'm fine."
    chunks = guardrail._split_text_by_words(text_with_punctuation, 30)
...
def test_split_text_preserves_content():
    ...
    chunks = guardrail._split_text_by_words(original_text, 1000)
