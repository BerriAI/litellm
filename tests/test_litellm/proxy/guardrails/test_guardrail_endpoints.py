...
async def test_bedrock_guardrail_make_api_request_passes_api_key():
    """Test make_bedrock_api_request method correctly passes api_key from guardrail_config"""
    ...
    test_request_data = {"guardrail_config": {"api_key": "test-api-key-789"}}
...
@pytest.mark.asyncio
async def test_bedrock_guardrail_uses_guardrail_config_api_key_not_request_api_key():
    """Test that guardrail uses api_key from guardrail_config, not from request_data"""
    from unittest.mock import AsyncMock, Mock, patch
    ...
    # Request data with BOTH api_key (for LLM) and guardrail_config.api_key (for guardrail)
    test_request_data = {
        "api_key": "openai-key-should-not-be-used",  # LLM provider key
        "guardrail_config": {
            "api_key": "bedrock-guardrail-key-789"  # Guardrail-specific key
        },
    }
    ...
    with patch.object(
        ...
        mock_request_instance.headers = {
            "Content-Type": "application/json",
            "Authorization": "Bearer bedrock-guardrail-key-789",  # Should use guardrail key
        }
        mock_request_instance.prepare.return_value = Mock()
        mock_aws_request.return_value = mock_request_instance
        ...
        # Verify the guardrail-specific API key was used, NOT the LLM provider key
        mock_aws_request.assert_called_once()
        call_args = mock_aws_request.call_args
        headers = call_args[1]["headers"]
        assert headers["Authorization"] == "Bearer bedrock-guardrail-key-789"
        assert "openai-key-should-not-be-used" not in headers["Authorization"]
...
<<<<<<< fix/bedrock-guardrail-incorrect-auth
