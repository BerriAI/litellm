...
def test_get_provider_specific_params():
    ...
    assert fields["optional_params"]["required"] is True
    ...
    assert nested_fields["severity_threshold"]["required"] is False
    assert (
...
async def test_bedrock_guardrail_prepare_request_without_api_key():
    ...
    ) as mock_get_secret, patch("botocore.auth.SigV4Auth") as mock_sigv4_auth, patch(
...
async def test_bedrock_guardrail_prepare_request_with_bearer_token_env():
    ...
    with patch(
        "litellm.proxy.guardrails.guardrail_hooks.bedrock_guardrails.get_secret_str"
    ) as mock_get_secret, patch("botocore.awsrequest.AWSRequest") as mock_aws_request:
...
async def test_bedrock_guardrail_make_api_request_passes_api_key():
    ...
    with patch.object(
        ...
        guardrail_hook, "_load_credentials"
    ) as mock_load_creds, patch.object(
        guardrail_hook, "convert_to_bedrock_format"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("success_sync_fails", None, HTTPException),
...
async def test_create_guardrail_endpoint(
    ...
    if expected_exception:
        ...
        if scenario == "database_failure":
            ...
            assert "failed to initialize" in str(exc_info.value.detail)
            # Verify rollback
            mock_guardrail_registry.delete_guardrail_from_db.assert_called_once()
    else:
...
@pytest.mark.parametrize(
    ...
    [
        ("success_with_sync", "test-db-guardrail", None),
        ("success_sync_fails", "test-db-guardrail", HTTPException),
    ],
...
@pytest.mark.asyncio
async def test_create_guardrail_with_invalid_pattern(mocker):
    """
    Test that creating a guardrail with an invalid pattern returns 422
    and rolls back the DB entry.
    """
    from litellm.proxy.guardrails.guardrail_endpoints import create_guardrail
    ...
    mock_prisma.db.litellm_guardrailstable.create = AsyncMock(
        return_value=mocker.Mock(
            guardrail_id="failed-id",
            guardrail_name="invalid-guard",
    ...
    mock_prisma.db.litellm_guardrailstable.delete = AsyncMock()
    mocker.patch("litellm.proxy.proxy_server.prisma_client", mock_prisma)

    # Request with fake pattern
    invalid_guardrail = Guardrail(
        guardrail_name="invalid-guard",
    ...
    request = CreateGuardrailRequest(guardrail=invalid_guardrail)

    with pytest.raises(HTTPException) as exc_info:
        await create_guardrail(request, user_api_key_dict=MOCK_ADMIN_USER)

    assert exc_info.value.status_code == 422
    assert "Unknown pattern name: 'FAKE-PATTERN'" in str(exc_info.value.detail)

    # Verify rollback was called (delete from DB)
    mock_prisma.db.litellm_guardrailstable.delete.assert_called_once()


@pytest.mark.asyncio
async def test_get_policy_templates_filters_unavailable_patterns(mocker):
    """
    Test that get_policy_templates filters out templates containing patterns
    ...
    # Mock prebuilt patterns to only have one
    mocker.patch(
        "litellm.proxy.guardrails.guardrail_hooks.litellm_content_filter.patterns.PREBUILT_PATTERNS",
        {"existing_pattern": ".*"},
    )
            ...
            "guardrailDefinitions": [
                {
                    "litellm_params": {
                        ...
                        "patterns": [
                            {
                                "pattern_type": "prebuilt",
                                "pattern_name": "non_existent_pattern",
                            }
                        ],
                    }
                }
    ...
    mocker.patch(
        "litellm.proxy.management_endpoints.policy_endpoints.endpoints._load_policy_templates_from_local_backup",
        return_value=mock_templates,
    )
    # Force use of local backup
    mocker.patch("os.getenv", return_value="true")

    result = await get_policy_templates(
        mocker.Mock(), user_api_key_dict=MOCK_ADMIN_USER
    )

    assert len(result) == 1
    assert result[0]["id"] == "valid-template"
