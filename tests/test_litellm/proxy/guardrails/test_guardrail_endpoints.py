...
sys.path.insert(
    0, os.path.abspath("../../../../..")
)  # Adds the repo root directory to the system path
...
async def test_list_guardrails_v2_with_db_and_config(
    mocker, mock_prisma_client, mock_in_memory_handler, mock_admin_user_auth
):
    ...
    response = await list_guardrails(user_api_key_dict=mock_admin_user_auth)
...
@pytest.mark.asyncio
async def test_get_guardrail_info_from_db(
    mocker, mock_prisma_client, mock_admin_user_auth
):
    ...
    response = await get_guardrail_info(
        "test-db-guardrail", user_api_key_dict=mock_admin_user_auth
    )
...
async def test_get_guardrail_info_from_config(
    ...
    response = await get_guardrail_info(
        "test-config-guardrail", user_api_key_dict=mock_admin_user_auth
    )
...
async def test_get_guardrail_info_not_found(
    ...
    with pytest.raises(HTTPException) as exc_info:
        await get_guardrail_info(
            "non-existent-guardrail", user_api_key_dict=mock_admin_user_auth
        )
...
def test_get_provider_specific_params():
    ...
    assert fields["api_key"]["type"] == "string"
    ...
    assert nested_fields["outputType"]["type"] == "select"
    ...
    if expected_exception:
        with pytest.raises(expected_exception) as exc_info:
            await create_guardrail(
                MOCK_CREATE_REQUEST, user_api_key_dict=mock_admin_user_auth
            )
    ...
    if expected_exception:
        with pytest.raises(expected_exception) as exc_info:
            await update_guardrail(
                "test-guardrail-id",
                MOCK_UPDATE_REQUEST,
                user_api_key_dict=mock_admin_user_auth,
            )
    ...
    if expected_exception:
        with pytest.raises(expected_exception):
            await delete_guardrail(
                guardrail_id=expected_result, user_api_key_dict=mock_admin_user_auth
            )
    else:
...
@pytest.mark.asyncio
async def test_get_guardrail_info_endpoint_config_guardrail(
    mocker, mock_admin_user_auth
):
    """
    ...
    result = await get_guardrail_info(
        guardrail_id="test-config-guardrail", user_api_key_dict=mock_admin_user_auth
    )
...
@pytest.mark.asyncio
async def test_get_guardrail_info_endpoint_db_guardrail(mocker, mock_admin_user_auth):
    """
    ...
    # Call endpoint and expect GuardrailInfoResponse
    result = await get_guardrail_info(
        guardrail_id="test-db-guardrail", user_api_key_dict=mock_admin_user_auth
    )
