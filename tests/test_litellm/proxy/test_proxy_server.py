...
def test_get_image_returns_default_brand_svg(client_no_auth, monkeypatch):
    monkeypatch.delenv("UI_LOGO_PATH", raising=False)

    response = client_no_auth.get("/get_image")

    assert response.status_code == 200
    assert response.headers["content-type"] == "image/svg+xml"
    assert "ArcheOps" in response.text


def test_get_favicon_returns_default_brand_svg(client_no_auth, monkeypatch):
    monkeypatch.delenv("LITELLM_FAVICON_URL", raising=False)

    response = client_no_auth.get("/get_favicon")
...
def test_openapi_title_uses_brand_name(client_no_auth):
    response = client_no_auth.get("/openapi.json")

    assert response.status_code == 200
    assert response.json()["info"]["title"] == "ArcheOps API"
...
@pytest.mark.asyncio
async def test_get_image_no_custom_logo_returns_default_brand_svg(monkeypatch):
    """
    Test that get_image returns the default text-based SVG brand logo when
    UI_LOGO_PATH is not set.
    """
    from litellm.proxy.proxy_server import _default_brand_logo_svg, get_image

    monkeypatch.delenv("UI_LOGO_PATH", raising=False)

    response = await get_image()

    assert response.media_type == "image/svg+xml"
    assert _default_brand_logo_svg().encode() == response.body


@pytest.mark.asyncio
async def test_get_image_remote_logo_url_redirects(monkeypatch):
    """
    Test that get_image redirects to the configured remote UI_LOGO_PATH URL.
    """
    from litellm.proxy.proxy_server import get_image

    monkeypatch.setenv("UI_LOGO_PATH", "https://example.com/logo.png")

    response = await get_image()

    assert response.status_code == 307
    assert response.headers["location"] == "https://example.com/logo.png"
...
async def test_get_image_default_logo_ignores_stale_cache(monkeypatch, tmp_path):
    """
    Test that when UI_LOGO_PATH is NOT set, get_image returns the default
    text-based SVG logo and does not serve a stale cached logo file.
    """
    from litellm.proxy.proxy_server import _default_brand_logo_svg, get_image
    ...
    assert response.media_type == "image/svg+xml"
    assert response.body == _default_brand_logo_svg().encode()


@pytest.mark.asyncio
async def test_get_image_missing_custom_logo_returns_default_brand_svg(monkeypatch, tmp_path):
    """
    Test that when UI_LOGO_PATH points to a non-existent local file,
    get_image falls back to the default text-based SVG logo instead of failing.
    """
    from litellm.proxy.proxy_server import _default_brand_logo_svg, get_image
