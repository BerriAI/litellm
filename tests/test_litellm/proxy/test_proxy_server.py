...
async def test_increment_spend_counters_user_budget_window():
    ...
    try:
        ...
        assert counter_cache.in_memory_cache.get_cache(key="spend:user:user-window-counter") == pytest.approx(1.30)
        assert counter_cache.in_memory_cache.get_cache(key="spend:user:user-window-counter:window:1h") == pytest.approx(
            0.30
        )
...
async def test_user_window_spend_counter_reseeds_from_spend_logs_on_counter_miss():
    ...
    try:
        ...
        assert counter_cache.in_memory_cache.get_cache(key="spend:user:user-window:window:1h") == pytest.approx(2.75)
    finally:
        ps.spend_counter_cache = orig_counter
        ps.prisma_client = orig_prisma
