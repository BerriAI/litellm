...
@pytest.mark.asyncio
async def test_user_window_spend_row_is_enqueued_from_carried_windows_without_user_lookup():
    """The windows auth resolved ride on the request, so a bounded-cache miss neither drops
    the row nor sends the cost callback back to the user table."""
    import litellm.proxy.proxy_server as ps
    from litellm.models.team import BudgetLimitEntry
    from litellm.proxy.proxy_server import increment_spend_counters

    reset_at = datetime.now(timezone.utc) + timedelta(days=3)
    carried = (BudgetLimitEntry(budget_duration="7d", max_budget=50.0, reset_at=reset_at),)

    with _window_spend_enqueue_env({}) as queue:
        ps.prisma_client = MagicMock()
        ps.prisma_client.db.litellm_usertable.find_unique = AsyncMock(
            side_effect=AssertionError("user table must not be read on the cost path")
        )
        await increment_spend_counters(
            token=None, team_id=None, user_id="user-1", response_cost=1.5, user_budget_limits=carried
        )
        enqueued = await _drain(queue)
    ...
    assert enqueued[0]["window_duration"] == "7d"
    assert enqueued[0]["spend"] == pytest.approx(1.5)
    assert enqueued[0]["window_start"] == (reset_at - timedelta(days=7)).astimezone(timezone.utc).replace(
        tzinfo=None
    ).isoformat(timespec="microseconds")
