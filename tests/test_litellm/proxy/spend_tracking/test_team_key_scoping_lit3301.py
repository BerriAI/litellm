...
def three_team_logs():
    ...
    return [
        {"id": "l1", "request_id": "req1", "api_key": "sk-team1",
         "user": "admin_user", "team_id": "team-1", "spend": 0.05,
         "startTime": now, "model": "gpt-4o"},
        {"id": "l2", "request_id": "req2", "api_key": "sk-team2",
         "user": "other", "team_id": "team-2", "spend": 0.10,
         "startTime": now, "model": "gpt-4o"},
        {"id": "l3", "request_id": "req3", "api_key": "sk-team3",
         "user": "third", "team_id": "team-3", "spend": 0.20,
         "startTime": now, "model": "gpt-4o"},
    ]
...
def test_get_calling_key_team_id_returns_team_id_when_present():
    auth = UserAPIKeyAuth(user_role=LitellmUserRoles.PROXY_ADMIN,
                          user_id="admin_user", team_id="team-1")
...
@pytest.mark.asyncio
async def test_admin_team_key_is_restricted_to_own_team_logs(three_team_logs, monkeypatch):
    monkeypatch.setattr(
        "litellm.proxy.proxy_server.prisma_client",
        make_ui_spend_logs_mock_prisma(three_team_logs, _filter_by_team(three_team_logs)),
    )
    app.dependency_overrides[ps.user_api_key_auth] = lambda: UserAPIKeyAuth(
        user_role=LitellmUserRoles.PROXY_ADMIN, user_id="admin_user",
        team_id="team-1", api_key="sk-team1",
    )
    try:
...
@pytest.mark.asyncio
async def test_admin_team_key_team_id_filter_must_match_calling_key(three_team_logs, monkeypatch):
    monkeypatch.setattr(
        "litellm.proxy.proxy_server.prisma_client",
        make_ui_spend_logs_mock_prisma(three_team_logs, _filter_by_team(three_team_logs)),
    )
...
@pytest.mark.asyncio
async def test_admin_key_without_team_id_still_sees_everything(three_team_logs, monkeypatch):
    monkeypatch.setattr(
        "litellm.proxy.proxy_server.prisma_client",
        make_ui_spend_logs_mock_prisma(three_team_logs, _filter_by_team(three_team_logs)),
    )
    app.dependency_overrides[ps.user_api_key_auth] = lambda: UserAPIKeyAuth(
        user_role=LitellmUserRoles.PROXY_ADMIN, user_id="admin_user", api_key="sk-admin",
    )
    try:
        client = TestClient(app)
        start, end = _default_date_range()
...
async def test_internal_user_team_key_also_restricted(three_team_logs, monkeypatch):
    ...
    app.dependency_overrides[ps.user_api_key_auth] = lambda: UserAPIKeyAuth(
        user_role=LitellmUserRoles.INTERNAL_USER, user_id="regular_user",
        team_id="team-1", api_key="sk-team1",
    )
    try:
        client = TestClient(app)
...
async def test_assert_calling_key_team_can_view_request_id_blocks_rowless_teams():
    ...
    with pytest.raises(HTTPException) as exc:
        await sme._assert_calling_key_team_can_view_request_id(
            prisma_client=prisma, calling_key_team_id="team-1",
...
async def test_assert_calling_key_team_can_view_request_id_missing_row_is_noop():
    ...
    await sme._assert_calling_key_team_can_view_request_id(
        prisma_client=prisma, calling_key_team_id="team-1", request_id="no-such-id",
    )
