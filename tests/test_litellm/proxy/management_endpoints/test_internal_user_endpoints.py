...
async def test_change_password_success(mocker):
    """Self-service password change verifies current password and updates fields."""
    from litellm.proxy.management_endpoints.internal_user_endpoints import (
        change_password,
    ...
    mock_user.password = "scrypt:salt:hash"

    async def mock_find_unique(*args, **kwargs):
        return mock_user

    mock_prisma_client.db.litellm_usertable.find_unique = mocker.AsyncMock(side_effect=mock_find_unique)
    mock_prisma_client.db.litellm_usertable.update = mocker.AsyncMock(return_value=mock_user)
    mocker.patch("litellm.proxy.proxy_server.prisma_client", mock_prisma_client)
    mocker.patch(
    ...
    response = await change_password(
        data=ChangePasswordRequest(current_password="old-password", new_password="new-password"),
        user_api_key_dict=caller,
    )

    assert response.success is True
    mock_prisma_client.db.litellm_usertable.update.assert_awaited_once()
    update_call = mock_prisma_client.db.litellm_usertable.update.await_args
    assert update_call.kwargs["data"]["password"] == "new-hash"
    assert update_call.kwargs["data"]["reset_password_required"] is False
    assert update_call.kwargs["data"]["password_updated_at"] is not None


@pytest.mark.asyncio
async def test_change_password_wrong_current(mocker):
    """Self-service password change fails when current password is wrong."""
    from litellm.proxy.management_endpoints.internal_user_endpoints import (
        change_password,
        ChangePasswordRequest,
    )
    ...
    mock_prisma_client.db.litellm_usertable.find_unique = mocker.AsyncMock(return_value=mock_user)
    mocker.patch("litellm.proxy.proxy_server.prisma_client", mock_prisma_client)
    mocker.patch(
        "litellm.proxy.management_endpoints.internal_user_endpoints.verify_password",
        return_value=False,
    ...
    with pytest.raises(ProxyException) as exc_info:
        await change_password(
            data=ChangePasswordRequest(current_password="wrong-password", new_password="new-password"),
            user_api_key_dict=caller,
        )
...
@pytest.mark.asyncio
async def test_reset_password_admin_only(mocker):
    """Only proxy admins can reset another user's password."""
    from litellm.proxy.management_endpoints.internal_user_endpoints import (
        reset_password,
    ...
    with pytest.raises(ProxyException) as exc_info:
        await reset_password(
            data=ResetPasswordRequest(user_id="target-user"),
            user_api_key_dict=caller,
        )
...
async def test_reset_password_success(mocker):
    """Admin reset password sets flag and optional temporary password."""
    from litellm.proxy.management_endpoints.internal_user_endpoints import (
        reset_password,
        ResetPasswordRequest,
    )
    ...
    mock_prisma_client = mocker.MagicMock()
    mock_user = mocker.MagicMock()
    mock_user.user_id = "target-user"

    mock_prisma_client.db.litellm_usertable.find_first = mocker.AsyncMock(return_value=mock_user)
    mock_prisma_client.db.litellm_usertable.update = mocker.AsyncMock(return_value=mock_user)
    mocker.patch("litellm.proxy.proxy_server.prisma_client", mock_prisma_client)
    mocker.patch(
        "litellm.proxy.management_endpoints.internal_user_endpoints.hash_password",
        return_value="temp-hash",
    )
    ...
    assert response.success is True
    update_call = mock_prisma_client.db.litellm_usertable.update.await_args
    assert update_call.kwargs["data"]["password"] == "temp-hash"
    assert update_call.kwargs["data"]["reset_password_required"] is True
