...
@pytest.mark.asyncio
async def test_regenerate_key_with_grace_period_flow(monkeypatch):
    """
    Test that regenerating a key with grace period:
    1. Sets expiry on old key
    2. Creates a new key with same properties
    """
    ...
    # Mock existing key
    existing_key = MagicMock()
    existing_key.token = "hashed_old_token"
    existing_key.key_name = "sk-...old1"
    existing_key.user_id = "test-user"
    existing_key.team_id = "test-team"
    ...
    with patch("litellm.proxy.proxy_server.prisma_client", mock_prisma_client), \
         ...
         patch("litellm.proxy.proxy_server.hash_token", lambda x: f"hashed_{x}" if x.startswith("sk-") else x), \
         patch("litellm.proxy.management_endpoints.key_management_endpoints.TeamMemberPermissionChecks.can_team_member_execute_key_management_endpoint", AsyncMock()), \
         patch("litellm.proxy.management_endpoints.key_management_endpoints._delete_cache_key_object", AsyncMock()), \
         patch("litellm.proxy.management_endpoints.key_management_endpoints.KeyManagementEventHooks.async_key_rotated_hook", AsyncMock()):
        ...
        await regenerate_key_fn(
            key="sk-oldkey",
            data=data,
            user_api_key_dict=UserAPIKeyAuth(user_role="proxy_admin", api_key="sk-admin", user_id="admin"),
        )

        # Verify: old key was updated with expiry
        assert len(update_calls) == 1
        assert "expires" in update_calls[0]["data"]
...
async def test_regenerate_key_without_grace_period_uses_standard_flow(monkeypatch):
    """
    Test that regenerating without grace period does in-place update (standard flow).
    """
    from unittest.mock import AsyncMock, MagicMock, patch
    ...
    async def mock_update(**kwargs):
        ...
        update_called = True
        result = MagicMock()
        result.__iter__ = lambda self: iter({"token": "new", "key_name": "sk-...new"}.items())
        return result
    ...
    with patch("litellm.proxy.proxy_server.prisma_client", mock_prisma_client), \
        ...
        data = RegenerateKeyRequest(key="sk-oldkey")  # No grace period
        ...
        # Standard flow: update only, no create
