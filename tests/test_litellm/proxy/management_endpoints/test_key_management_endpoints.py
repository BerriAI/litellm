...
async def test_info_key_fn_includes_model_max_budget_usage(monkeypatch):
    ...
    mock_prisma_client.db.litellm_verificationtoken.find_unique = AsyncMock(return_value=mock_key_info)
    mock_prisma_client.db.query_raw = AsyncMock(return_value=[{"model": "gpt-4o", "total_spend": 0.23}])
...
async def test_info_key_fn_v2_includes_model_max_budget_usage(monkeypatch):
    ...
    mock_prisma_client.db.query_raw = AsyncMock(return_value=[{"model": "gpt-4o", "total_spend": 0.55}])
...
async def test_info_key_fn_provider_prefix_spend_fallback(monkeypatch):
    ...
    mock_prisma_client.db.query_raw = AsyncMock(return_value=[{"model": "gpt-4o", "total_spend": 0.75}])
...
async def test_query_model_spend_for_period_db_error_returns_zero():
    ...
    mock_prisma_client = AsyncMock()
    mock_prisma_client.db.query_raw = AsyncMock(side_effect=Exception("connection lost"))
...
async def test_build_model_max_budget_usage_different_durations_per_model_windows():
    ...
    assert timedelta(days=6, hours=23) <= sorted(windows)[1] <= timedelta(days=7, hours=1)
