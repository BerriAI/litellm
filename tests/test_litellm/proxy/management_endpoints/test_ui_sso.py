...
class TestDirectorySearchConfiguredSafe:
    """_is_directory_search_configured_safe() must never let a secret
    manager failure break /sso/get_ui_settings for every user."""

    def test_returns_true_when_configured(self, mocker):
        from litellm.proxy.management_endpoints.ui_sso import (
            _is_directory_search_configured_safe,
        )

        mocker.patch(
            "litellm.proxy.management_endpoints.ui_sso.get_configured_directory_search_provider",
            return_value=mocker.MagicMock(),
        )
        assert _is_directory_search_configured_safe() is True
    ...
    def test_returns_false_when_not_configured(self, mocker):
        ...
        assert _is_directory_search_configured_safe() is False
    ...
    @pytest.mark.asyncio
    async def test_get_ui_settings_survives_directory_search_config_failure(self, mocker):
