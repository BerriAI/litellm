...
@pytest.mark.asyncio
async def test_update_team_converts_organization_id_to_relation_syntax(
    mock_db_client, mock_admin_auth
):
    """
    Test that /team/update converts organization_id to Prisma relation
    connect/disconnect syntax instead of passing it as a scalar field,
    which causes 'Could not find field at
    updateOneLiteLLM_TeamTable.data.organization_id'.

    Regression test for https://github.com/BerriAI/litellm/issues/22201
    """
    mock_db_client.jsonify_team_object = lambda db_data: db_data
    mock_db_client.db = MagicMock()

    existing_team_mock = MagicMock()
    existing_team_mock.team_id = "team-org-update-123"
    existing_team_mock.organization_id = None
    existing_team_mock.max_budget = None
    existing_team_mock.soft_budget = None
    existing_team_mock.models = []
    existing_team_mock.members_with_roles = []
    existing_team_mock.model_id = None
    existing_team_mock.model_dump.return_value = {
        "team_id": "team-org-update-123",
        "organization_id": None,
        "models": [],
        "members_with_roles": [],
    }

    updated_team_result = MagicMock(team_id="team-org-update-123")
    updated_team_result.model_dump.return_value = {
        "team_id": "team-org-update-123",
    }
    mock_db_client.db.litellm_teamtable = MagicMock()
    mock_db_client.db.litellm_teamtable.find_unique = AsyncMock(
        return_value=existing_team_mock
    )
    mock_db_client.db.litellm_teamtable.update = AsyncMock(
        return_value=updated_team_result
    )
    ...
    # Test 1: setting organization_id should use connect
    team_update_request = UpdateTeamRequest(
        team_id="team-org-update-123",
    ...
    with patch(
        "litellm.proxy.management_endpoints.team_endpoints.fetch_and_validate_organization",
        new_callable=AsyncMock,
    ), patch(
        "litellm.proxy.management_endpoints.team_endpoints.get_org_object",
        new_callable=AsyncMock,
    ...
    update_call_kwargs = mock_db_client.db.litellm_teamtable.update.call_args.kwargs
    team_data = update_call_kwargs["data"]

    # organization_id should NOT be a direct scalar field
    assert "organization_id" not in team_data
    # Should use Prisma relation connect syntax
    assert "litellm_organization_table" in team_data
    assert team_data["litellm_organization_table"] == {
        "connect": {"organization_id": "org-456"}
    }

    # Test 2: unsetting organization_id (empty string) should use disconnect
    mock_db_client.db.litellm_teamtable.update.reset_mock()
    mock_db_client.db.litellm_teamtable.update = AsyncMock(
        return_value=updated_team_result
    )

    team_update_request_unset = UpdateTeamRequest(
        team_id="team-org-update-123",
        organization_id="",
    )

    await update_team(
        data=team_update_request_unset,
        http_request=dummy_request,
        user_api_key_dict=mock_admin_auth,
    )
    ...
    assert "organization_id" not in team_data
    assert "litellm_organization_table" in team_data
    assert team_data["litellm_organization_table"] == {"disconnect": True}
