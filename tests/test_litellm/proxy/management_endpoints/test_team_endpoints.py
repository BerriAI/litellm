...
# ---------------------------------------------------------------------------
# metadata.team_member_max_budget_alert_emails is validated on every team write
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("request_cls_name", ["NewTeamRequest", "UpdateTeamRequest"])
def test_team_member_budget_alert_emails_stored_canonically(request_cls_name):
    import litellm.proxy._types as proxy_types

    parsed = getattr(proxy_types, request_cls_name).model_validate(
        {
            "team_id": "t",
            "metadata": {
                "cost_center": "1",
                "team_member_max_budget_alert_emails": {
                    "050": None,
                    "100": [" finance@example.com", "finance@example.com"],
                },
            },
        }
    )
    ...
    assert parsed.metadata == {
        "cost_center": "1",
        "team_member_max_budget_alert_emails": {"50": [], "100": ["finance@example.com"]},
    }
...
def test_team_member_budget_alert_emails_rejects_malformed(request_cls_name, alert_emails):
    from pydantic import ValidationError

    import litellm.proxy._types as proxy_types

    with pytest.raises(ValidationError, match="team_member_max_budget_alert_emails"):
        getattr(proxy_types, request_cls_name).model_validate(
            {"team_id": "t", "metadata": {"team_member_max_budget_alert_emails": alert_emails}}
        )


def test_team_row_with_malformed_alert_emails_still_loads():
    """Validation is write-side only; reading a stored team never fails on this key."""
    from litellm.proxy._types import LiteLLM_TeamTable

    row = LiteLLM_TeamTable(team_id="t", metadata={"team_member_max_budget_alert_emails": {"0": "x"}})

    assert row.metadata == {"team_member_max_budget_alert_emails": {"0": "x"}}


@pytest.mark.asyncio
async def test_patch_null_deletes_one_alert_threshold():
    """The PATCH body is a merge patch, so {"50": null} must delete that threshold rather than
    being coerced to an empty recipient list before the merge."""
    _, update_mock = await _drive_team_write(
        "patch",
        existing_metadata={"team_member_max_budget_alert_emails": {"50": [], "100": ["finance@example.com"]}},
        raw_body={"metadata": {"team_member_max_budget_alert_emails": {"50": None}}},
    )

    written_metadata = update_mock.call_args.kwargs["data"]["metadata"]
    if isinstance(written_metadata, str):
        written_metadata = json.loads(written_metadata)
    assert written_metadata["team_member_max_budget_alert_emails"] == {"100": ["finance@example.com"]}


@pytest.mark.asyncio
async def test_patch_rejects_malformed_alert_threshold():
    from litellm.proxy._types import ProxyException

    with pytest.raises(ProxyException) as exc:
        await _drive_team_write(
            "patch",
            existing_metadata={"team_member_max_budget_alert_emails": {"50": []}},
            raw_body={"metadata": {"team_member_max_budget_alert_emails": {"150": []}}},
        )

    assert "team_member_max_budget_alert_emails" in str(exc.value.message)
