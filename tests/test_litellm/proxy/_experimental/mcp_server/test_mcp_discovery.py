    ...
    def test_stdio_servers_have_command(self, registry_path):
        ...
        for server in data["servers"]:
            if server["transport"] == "stdio":
                assert "command" in server and server["command"], f"stdio server {server['name']} missing 'command'"
    ...
    def test_http_servers_have_url(self, registry_path):
        ...
        for server in data["servers"]:
            if server["transport"] in ("http", "sse"):
                assert "url" in server and server["url"], f"HTTP/SSE server {server['name']} missing 'url'"
