...
# ---------------------------------------------------------------------------
# SemanticToolFilterHook — anthropic_messages call type, MCP/non-MCP split,
# and original-tool-ordering preservation.
# ---------------------------------------------------------------------------
class _MockSemanticFilter:
    ...
    def extract_user_query(self, messages):
        # Walk messages to find the latest user message — supports both the
        # plain-string content (chat completion) and list-of-blocks content
        # (anthropic_messages) shapes.
    ...
    async def filter_tools(self, query, available_tools):
        # Capture inputs for later assertions, then return a stable subset
        # that respects top_k.
...
async def test_semantic_filter_hook_runs_for_anthropic_messages():
    """
    Hook must run for ``anthropic_messages`` requests so that Claude Code
    and other Anthropic SDK clients (which hit /v1/messages) get the
    semantic filtering treatment instead of being silently skipped.
    """
    from litellm.proxy.hooks.mcp_semantic_filter import SemanticToolFilterHook
...
async def test_semantic_filter_hook_skips_non_allowlisted_call_type():
    """Hook must remain a no-op for unsupported call types."""
    from litellm.proxy.hooks.mcp_semantic_filter import SemanticToolFilterHook
    ...
    data = {
        "messages": [{"role": "user", "content": "hello"}],
        "tools": [{"type": "function", "function": {"name": "mcp__t1"}}],
        "metadata": {},
    }
    result = await hook.async_pre_call_hook(
        user_api_key_dict=Mock(),
