...
class TestPreemptive401ModeAware:
    ...
    async def test_aggregate_challenge_preserves_admission_mode(self, monkeypatch, prefix, authorized, keyless):
        ...
        monkeypatch.setenv("LITELLM_SALT_KEY", "keyed-mode-test-salt")
        ...
        caller: Final = UserAPIKeyAuth(api_key=None if keyless else "sk-test", user_id="owner")
        caller.mcp_admitted_user_subject = keyless
        caller.via_virtual_key = not keyless
        scope: Final = {
            "type": "http",
            "method": "POST",
            "scheme": "http",
            "path": f"{prefix}/mcp",
            "root_path": prefix,
            "headers": [(b"host", b"testserver"), (b"x-litellm-api-key", b"Bearer sk-test")],
        }
        if not authorized:
        ...
        metadata_url: Final = exc.value.headers["www-authenticate"].split('resource_metadata="')[1].rstrip('"')
        parsed: Final = urlsplit(metadata_url)
        assert parsed.path == f"{prefix}/.well-known/oauth-protected-resource/mcp" + ("/interactive" if keyless else "")
        if keyless:
            assert parsed.query == ""
        else:
    ...
    @pytest.mark.asyncio
    async def test_keyed_aggregate_challenge_carries_authorization_identity(self, monkeypatch):
        from urllib.parse import parse_qs, urlsplit

        from litellm.proxy._experimental.mcp_server import server as server_module

        monkeypatch.setenv("LITELLM_SALT_KEY", "keyed-flow-test-salt")
        monkeypatch.delenv("PROXY_BASE_URL", raising=False)
        monkeypatch.delenv("SERVER_ROOT_PATH", raising=False)
        server: Final = _make_oauth2_server("interactive", oauth2_flow="authorization_code")
        server_module.global_mcp_server_manager.registry[server.server_id] = server
        caller: Final = UserAPIKeyAuth(api_key="sk-test", user_id="flow-owner")
        caller.via_virtual_key = True
        scope: Final = {
            "type": "http", "method": "POST", "scheme": "http", "path": "/mcp",
            "headers": [(b"host", b"localhost:4000"), (b"x-litellm-api-key", b"Bearer sk-test")],
        }
        with pytest.raises(HTTPException) as error:
            await server_module._raise_preemptive_401_for_unauthenticated_servers(
                scope, [server.alias], None, None, caller, None, allowed_server_ids={server.server_id},
            )
        assert error.value.status_code == 401
        metadata: Final = error.value.headers["www-authenticate"].split('resource_metadata="')[1].rstrip('"')
        query: Final = parse_qs(urlsplit(metadata).query)
        assert query.get("flow"), "OAuth discovery must retain the original key identity for headerless exchange"
        assert "sk-test" not in metadata and "flow-owner" not in metadata
