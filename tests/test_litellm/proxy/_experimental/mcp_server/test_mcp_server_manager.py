...
class TestMCPServerManager:
    ...
    async def test_check_tool_permission_for_key_team_allows_all_when_no_restrictions(
        ...
        # An unconverted (version 0) permission row places no restriction
        user_auth = UserAPIKeyAuth(
            api_key="sk-test-key",
            user_id="user-456",
            object_permission=LiteLLM_ObjectPermissionTable(
    ...
    @pytest.mark.asyncio
    async def test_check_tool_permission_converted_row_denies_discovered_delete_tool(self):
        """A converted grant (mcp_permission_version=1, no overrides) admits the
        discovered non-delete tools and refuses discovered delete tools and any
        name the inventory never produced."""
        from litellm.proxy._experimental.mcp_server.mcp_server_manager import (
        ...
        object_permission = LiteLLM_ObjectPermissionTable(
            object_permission_id="perm_conv",
        ...
        inventory = {"read_repo": "reads repositories", "delete_repo": "removes a repository"}
        with patch.object(global_mcp_server_manager, "discovered_inventory", return_value=inventory):
            await manager.check_tool_permission_for_key_team(
                tool_name="read_repo",
                server=server,
                user_api_key_auth=user_auth,
            ...
            with pytest.raises(HTTPException) as exc_info:
                await manager.check_tool_permission_for_key_team(
                    tool_name="never_discovered_tool",
                    server=server,
                    user_api_key_auth=user_auth,
                )
            assert exc_info.value.status_code == 403
...
def _unrestricted_auth() -> MagicMock:
    """A caller whose only object_permission row is unconverted (version 0),
    which places no tool restriction, so only server-level checks apply."""
    unrestricted_row = MagicMock()
    unrestricted_row.mcp_tool_permissions = None
    unrestricted_row.mcp_tool_overrides = None
    unrestricted_row.mcp_permission_version = 0
    unrestricted_row.mcp_toolsets = None
    user_api_key_auth = MagicMock()
    user_api_key_auth.object_permission = unrestricted_row
    ...
    user_api_key_auth.agent_id = None
    user_api_key_auth.org_id = None
    user_api_key_auth.mcp_admitted_user_subject = False
