...
async def _complete_page(response, scoped_server=None, vendor=None, cache=None, **overrides):
...
async def _describe_page(response, scoped_server=None, vendor=None, session_user_id="u1", cookies=None):
    from unittest.mock import patch
...
async def test_scoped_flow_deny_and_stale_server_never_need_the_vendor_credential():
    ...
    with patch(_MANAGER_PATCH) as manager:
        ...
        response = _scoped_authorize(client_id, SCOPED_RESOURCE)
    cache = DualCache()
    stale = await _complete_page(response, scoped_server=None, cache=cache)
    assert stale.status_code == 400
    assert json.loads(stale.body)["error_description"] == "the requested MCP server is no longer available"
    ...
    vendor = _VendorCredential("absent")
    denied = await _complete_page(response, scoped_server=github, vendor=vendor, cache=cache, decision="deny")
