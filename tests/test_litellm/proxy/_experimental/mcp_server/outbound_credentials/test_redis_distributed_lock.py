...
async def test_acquire_sets_token_with_nx_px_and_reports_acquired():
    ...
    assert await RedisDistributedLock(redis).acquire("k", "tok-1", 10.0) is (
...
async def test_acquire_reports_held_when_key_already_held():
    ...
    assert (
        await RedisDistributedLock(_FakeRedis(set_returns=None)).acquire(
            "k", "tok", 10.0
        )
        is LockAcquisition.HELD
    )
...
async def test_acquire_reports_error_on_redis_error_distinct_from_held():
    ...
    assert (
        await RedisDistributedLock(_FakeRedis(raise_on=["set"])).acquire(
            "k", "tok", 10.0
        )
        is LockAcquisition.ERROR
    )
...
async def test_release_deletes_only_when_the_token_matches():
    ...
    assert (
        redis.deleted == [] and redis.store.get("k") == "owner-B"
    )  # B's lock survives
...
async def test_keys_are_namespaced_before_reaching_redis():
    ...
    assert redis.eval_calls[0][1][0] == "ns:k"  # release (EVAL KEYS[1]) namespaced
