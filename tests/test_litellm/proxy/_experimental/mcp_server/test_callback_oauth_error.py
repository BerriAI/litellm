"""LIT-2750: /callback must handle IdP error responses gracefully.

Per RFC 6749 section 4.1.2.1, when the user denies an OAuth authorization request
(or the IdP otherwise cannot grant it), the IdP redirects to the configured
``redirect_uri`` with ``error`` / ``error_description`` / ``error_uri`` query
params and no ``code``. The MCP loopback flow funnels that response through this
``/callback`` endpoint, so the handler must accept either a successful
(``code``+``state``) or an RFC-shaped error response. Pre-fix, FastAPI rejected
the error response with a 422 because ``code`` and ``state`` were declared as
required — leaving the MCP client waiting on the loopback until timeout.
"""
...
def _mock_mcp_client_ip():
    """Mock IPAddressUtils.get_mcp_client_ip to bypass IP-based access control
    in tests, mirroring the convention used by the sibling test file."""
    with patch(
...
def callback_client(monkeypatch):
    """Build a FastAPI TestClient mounted with the discoverable router.

    Uses a deterministic ``LITELLM_SALT_KEY`` so encoded states minted in-test
    can be decrypted by the handler.
    """
...
class TestCallbackOAuthErrorHandling:
    """Coverage for LIT-2750: IdP error responses to /callback."""

    def test_idp_error_with_no_state_returns_400_html(self, callback_client):
        # Pre-fix: 422 from Pydantic because ``code`` is missing.
        # Post-fix: 400 HTML page surfacing the IdP error.
        resp = callback_client.get(
            "/callback",
            params={
                "error": "access_denied",
        ...
        # Sanity: must not have leaked into a Pydantic validation error blob.
        assert "Field required" not in body

    def test_idp_error_html_escapes_user_controlled_fields(self, callback_client):
        ...
        assert resp.status_code == 400
        body = resp.text
        # Raw <script> / onerror tags must be escaped, not present verbatim.
    ...
    def test_idp_error_with_trusted_state_propagates_to_client_redirect_uri(
        ...
        # error_description should be percent-encoded but present.
        assert "error_description=User" in location
        # Our wrapped/encrypted state must NOT leak to the client.
    ...
    def test_bare_callback_with_no_params_returns_400_not_422(self, callback_client):
        ...
        resp = callback_client.get("/callback", follow_redirects=False)
        assert resp.status_code == 400
        assert "invalid_request" in resp.text
        assert "Field required" not in resp.text

    def test_success_path_still_redirects_with_code_and_state(self, callback_client):
