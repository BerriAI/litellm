...
class TestProxyBaseLLMRequestProcessing:
    ...
    @pytest.mark.asyncio
    async def test_common_processing_pre_call_logic_checks_guardrail_tag_owner_without_budget_for_zero_cost_model(
        self, monkeypatch
    ):
        processing_obj, mock_request, mock_proxy_logging_obj, mock_proxy_config, tag_budget_check = (
            self._guardrail_tag_budget_harness(
        ...
        tag_budget_check.assert_awaited_once()
        assert tag_budget_check.call_args.kwargs["tags"] == ("guardrail-tag",)
        assert tag_budget_check.call_args.kwargs["check_budgets"] is False

    @pytest.mark.asyncio
    async def test_common_processing_pre_call_logic_checks_guardrail_tag_owner_without_budget_on_budget_exempt_route(
        self, monkeypatch
    ):
        processing_obj, mock_request, mock_proxy_logging_obj, mock_proxy_config, tag_budget_check = (
            self._guardrail_tag_budget_harness(
