...
class TestPreCallWithFallbacksOnLocalRateLimit:
    ...
    async def test_cross_model_scoped_rejection_mid_chain_stops_further_fallback_attempts(self):
        """
        The check above only covers the ORIGINAL exception, before the
        fallback loop starts. A later fallback attempt's own rejection was
        never checked the same way -- the loop's ``except
        ProxyRateLimitError: continue`` swallowed it and moved on to the next
        fallback model. If a chain-wide ``apply_to_models`` cap covers both
        the primary model and the first fallback, and a second fallback model
        isn't covered, this let the second fallback silently serve the
        request the cap was meant to block. The original (non-scoped)
        rejection enters the loop normally; the FIRST fallback's own
        rejection carries ``cross_model_scope=True`` and must stop the loop
