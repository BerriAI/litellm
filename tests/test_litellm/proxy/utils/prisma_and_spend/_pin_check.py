#!/usr/bin/env python3
"""Pin-list gate for PR2 (PrismaClient + spend) of the proxy/utils.py
behavior-pinning project.

For each identifier in the pin list, asserts that the test directory contains:
  1. At least one happy-path test that references the identifier and uses
     a real assertion (normalize(...) call, .model_validate(...), or a
     dict / object literal equality with >= 3 keys).
  2. At least one error-path test (name hint, ``pytest.raises``, ``raises(``
     context manager, or an assertion that checks an exception).
...
def _has_strong_assertion(node: ast.AST) -> bool:
    """True if an assert subtree contains normalize(), model_validate(), or
    a dict/object equality with >= 3 keys/keyword args.
    """
        ...
        if (
            ...
            for side in (sub.left, sub.comparators[0]):
                if isinstance(side, ast.Dict) and len(side.keys) >= 3:
                    return True
                if isinstance(side, ast.Call) and len(side.keywords) >= 3:
                    return True
    return False
...
def main() -> int:
    ...
    parser.add_argument(
        "--list",
        required=True,
        help="Path to pin list file (markdown bullets in '- `<symbol>`' format).",
    )
    parser.add_argument(
        "--test-dir",
        default=str(HERE),
