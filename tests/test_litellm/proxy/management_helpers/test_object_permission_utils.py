...
async def test_validate_stale_ids_in_mcp_tool_denied_tools_silently_dropped(mock_access_groups, mock_allow_all):
...
async def test_denied_tool_keys_are_validated_against_team_scope(mock_access_groups, mock_allow_all):
...
@pytest.mark.asyncio
async def test_set_object_permission_serializes_mcp_tool_approved_tools():
    """mcp_tool_approved_tools must persist exactly like mcp_tool_denied_tools: JSON
    serialized into the create payload so Prisma stores the map, not a repr."""
    mock_prisma_client = MagicMock()
    mock_created_permission = MagicMock()
    mock_created_permission.object_permission_id = "perm_id"
    ...
    data_json = {
        "object_permission": {
            "mcp_servers": ["server_a"],
            "mcp_tool_approved_tools": {"server_a": ["tool1", "tool2"]},
        },
    }
    ...
    created_data = mock_prisma_client.db.litellm_objectpermissiontable.create.call_args.kwargs["data"]
    assert isinstance(created_data["mcp_tool_approved_tools"], str)
    assert json.loads(created_data["mcp_tool_approved_tools"]) == {"server_a": ["tool1", "tool2"]}
...
async def test_validate_stale_ids_in_mcp_tool_approved_tools_silently_dropped(mock_access_groups, mock_allow_all):
    """An approval-map key naming a deleted server is silently stripped like a
    denylist key, not rejected with a 403."""
    team_obj = _make_team_obj(mcp_servers=["s3", "s4"])
    object_permission = {"mcp_tool_approved_tools": {"s1-stale": ["tool1"]}}
    await validate_key_mcp_servers_against_team(
        object_permission=object_permission,
        team_obj=team_obj,
    )  # Must not raise
    assert object_permission["mcp_tool_approved_tools"] == {}
...
async def test_set_object_permission_rejects_shared_alias_approved_tool_key(identifier, colliding_ids):
    """A shared alias or server_name cannot key mcp_tool_approved_tools either: the
    write is rejected with 400 naming the field and both servers."""
    mock_prisma = _make_ambiguity_prisma()
    data_json = {"object_permission": {"mcp_tool_approved_tools": {identifier: ["delete_tool"]}}}
...
@pytest.mark.asyncio
async def test_prepare_object_permission_upsert_rejects_shared_alias_approved_tool_key():
    """The update seam rejects a new shared-alias approval-map key when the existing
    row does not already hold it, naming mcp_tool_approved_tools in the error."""
    mock_prisma = _make_ambiguity_prisma()

    with pytest.raises(HTTPException) as exc_info:
        await prepare_object_permission_upsert(
            new_object_permission={"mcp_tool_approved_tools": {"wiki": ["delete_tool"]}},
            existing_object_permission_id="perm-id",
            prisma_client=mock_prisma,
        )
...
async def test_approved_tool_keys_are_validated_against_team_scope(mock_access_groups, mock_allow_all):
    """An approval-map key naming a server outside the team's scope fails the same
    write validation a denylist key does, since the key still scopes this
    subject's grants to that server."""
    team_obj = _make_team_obj(mcp_servers=["server-1"])
    with pytest.raises(HTTPException) as exc_info:
        await validate_key_mcp_servers_against_team(
            object_permission={"mcp_tool_approved_tools": {"server-outside": ["tool1"]}},
            team_obj=team_obj,
        )
    assert exc_info.value.status_code == 403
    assert "server-outside" in str(exc_info.value.detail)
