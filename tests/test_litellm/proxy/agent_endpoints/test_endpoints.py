...
class TestAgentRBACProxyAdminViewOnly:
    """Read-only proxy admins share the admin READ path on GET /v1/agents, but
    secret unredaction stays gated on full PROXY_ADMIN."""
    ...
    def test_should_give_view_only_admin_the_unfiltered_agent_list(self):
        """The key/team allowlist here excludes every registered agent, so a
        viewer routed through the non-admin branch would come back empty."""
        resp = self._list_agents(self.viewer_client)

        assert resp.status_code == 200
        assert {agent["agent_id"] for agent in resp.json()} == {"agent-1", "agent-2"}
        self.allowed_agents_spy.assert_not_awaited()

    def test_should_still_redact_secrets_for_view_only_admin(self):
        """Read parity stops at redaction: the same payload a full admin gets
        unredacted comes back with keys stripped and litellm_params masked."""
