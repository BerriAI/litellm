...
SCRIPT_PATH = Path(__file__).resolve().parents[2] / ".github" / "scripts" / "close_low_quality_prs.py"
...
class TestExtractGreptileScore:
    def test_should_extract_score_from_html_header(self, closer_module):
        comments = [_greptile_comment("<h3>Confidence Score: 3/5</h3>\nSome body text.")]
    ...
    def test_should_accept_both_greptile_login_variants(self, closer_module):
        ...
        for login in ("greptile-apps", "greptile-apps[bot]"):
            comments = [_greptile_comment("<h3>Confidence Score: 2/5</h3>", login=login)]
            result = closer_module.extract_greptile_score(comments)
    ...
    def test_should_pick_most_recent_comment_when_rereview_happens(self, closer_module):
        comments = [
            _greptile_comment("Confidence Score: 2/5", updated_at="2026-05-01T00:00:00Z"),
            _greptile_comment("Confidence Score: 5/5", updated_at="2026-05-12T00:00:00Z"),
        ]
        result = closer_module.extract_greptile_score(comments)
        assert result is not None
        score, _ = result
...
class TestEvaluatePr:
    ...
    def _make_pr(
        ...
        *,
        ...
        created = dt.datetime(2026, 5, 17, tzinfo=dt.timezone.utc) - dt.timedelta(days=created_days_ago)
        return {
            "number": number,
            "title": f"PR #{number}",
    ...
    def _external_author(self, closer_module, monkeypatch):
        ...
        monkeypatch.setattr(closer_module, "is_external_pr_author", lambda pr, repo: True)

    def test_should_warn_drafts_when_score_low_first_time(self, closer_module, _now, monkeypatch):
    ...
    def test_should_warn_brand_new_pr_when_min_age_zero(self, closer_module, _now, monkeypatch):
    ...
    def test_should_skip_optout_label_case_insensitive(self, closer_module, _now, monkeypatch):
        monkeypatch.setattr(
            closer_module,
            "fetch_pr_comments",
    ...
    def test_should_skip_too_young_when_min_age_set(self, closer_module, _now, monkeypatch):
    ...
    def test_should_not_skip_when_min_age_is_zero(self, closer_module, _now, monkeypatch):
    ...
    def test_should_skip_when_greptile_has_not_reviewed(self, closer_module, _now, monkeypatch):
    ...
    def test_should_skip_when_score_meets_threshold(self, closer_module, _now, monkeypatch):
        monkeypatch.setattr(
            closer_module,
            "fetch_pr_comments",
    ...
    def test_should_warn_when_old_and_low_score_no_prior_warning(self, closer_module, _now, monkeypatch):
    ...
    def test_should_close_when_grace_warning_aged_out_and_score_still_low(self, closer_module, _now, monkeypatch):
        ...
        old_warning = {
            ...
            "body": ("you have 2 hours to fix this\n\n" + closer_module.GRACE_COMMENT_MARKER),
            "created_at": (_now - dt.timedelta(seconds=closer_module.GRACE_PERIOD_SECONDS + 60))
            .isoformat()
            .replace("+00:00", "Z"),
    ...
    def test_should_skip_when_grace_warning_within_window(self, closer_module, _now, monkeypatch):
        ...
        recent_warning = {
            "user": {"login": "github-actions[bot]"},
            "body": "warning text\n\n" + closer_module.GRACE_COMMENT_MARKER,
            "created_at": (_now - dt.timedelta(hours=1)).isoformat().replace("+00:00", "Z"),
        }
        monkeypatch.setattr(
            closer_module,
            "fetch_pr_comments",
    ...
    def test_should_warn_grace_for_swiftwinds_not_close_immediately(self, closer_module, _now, monkeypatch):
    ...
    def test_should_skip_internal_authors(self, closer_module, _now, monkeypatch):
        ...
        monkeypatch.setattr(closer_module, "is_external_pr_author", lambda pr, repo: False)
        monkeypatch.setattr(
            closer_module,
            "fetch_pr_comments",
...
class TestMainOptoutLabelDefault:
    ...
    def test_should_use_canonical_defaults_when_flag_omitted(self, closer_module, monkeypatch):
    ...
    def test_should_replace_defaults_when_flag_provided(self, closer_module, monkeypatch):
        captured = self._patch_no_op(closer_module, monkeypatch)
        monkeypatch.setattr(
            closer_module,
            "fetch_open_prs",
...
class TestListOpenItemsNoCap:
    ...
    def test_list_open_items_passes_no_cap_limit_not_1000(self, closer_module, monkeypatch):
    ...
    def test_list_open_items_uses_dedicated_command_state_and_fields(self, closer_module, monkeypatch):
        shared, captured = self._capture_gh_args(closer_module, monkeypatch)
    ...
    def test_list_open_items_parses_json_array(self, closer_module, monkeypatch):
        shared, _ = self._capture_gh_args(closer_module, monkeypatch, returns='[{"number": 1}, {"number": 2}]')
...
class TestEvaluatePrAllowlist:
    ...
    def test_should_skip_author_not_on_allowlist(self, closer_module, _now, monkeypatch):
        monkeypatch.setattr(
            closer_module,
            "fetch_pr_comments",
    ...
    def test_should_act_on_allowlisted_internal_author(self, closer_module, _now, monkeypatch):
        monkeypatch.setattr(closer_module, "is_external_pr_author", lambda pr, repo: False)
        monkeypatch.setattr(
            closer_module,
            "fetch_pr_comments",
    ...
    def test_empty_allowlist_restores_internal_skip(self, closer_module, _now, monkeypatch):
        monkeypatch.setattr(closer_module, "is_external_pr_author", lambda pr, repo: False)
        monkeypatch.setattr(
            closer_module,
            "fetch_pr_comments",
    ...
    def test_allowlist_constant_is_the_two_dogfood_accounts(self, closer_module):
        assert closer_module.ALLOWLIST_LOGINS == frozenset({"mateo-berri", "swiftwinds"})
...
class TestDryRunGateOnClose:
    ...
    def test_dry_run_sweep_detects_but_does_not_close(self, closer_module, monkeypatch, capsys):
        aged_out_warning = {
            "user": {"login": "github-actions[bot]"},
            "body": "warned\n\n" + closer_module.GRACE_COMMENT_MARKER,
        ...
        monkeypatch.setattr(closer_module, "fetch_open_prs", lambda repo: [self._closeable_pr()])
        monkeypatch.setattr(
            closer_module,
            "fetch_pr_comments",
            lambda *a, **kw: [
