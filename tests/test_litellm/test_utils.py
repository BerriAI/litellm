...
class TestStreamOptionsNonStreaming:
    """include_usage in stream_options is only valid for streaming requests;
    OpenAI-compatible backends (e.g. vLLM) 400 if it's sent without stream=True.
    Other keys (e.g. include_obfuscation) must survive. See #29431."""

    def test_include_usage_dropped_when_stream_not_set(self):
        from litellm.utils import get_optional_params
    ...
    def test_include_usage_dropped_when_stream_false(self):
        from litellm.utils import get_optional_params
    ...
    def test_other_stream_options_keys_survive_when_not_streaming(self):
        from litellm.utils import get_optional_params
        ...
        assert result["stream_options"] == {"include_obfuscation": False}

    def test_include_usage_kept_when_streaming(self):
        from litellm.utils import get_optional_params
