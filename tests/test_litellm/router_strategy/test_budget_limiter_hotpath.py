...
@pytest.mark.asyncio
async def test_async_filter_deployments_filters_provider_over_max_budget(
    disable_budget_sync, monkeypatch
):
    provider_budget = RouterBudgetLimiting(
        dual_cache=DualCache(),
    ...
    await provider_budget.dual_cache.async_set_cache(
        key="provider_spend:openai:1d", value=150.0
    )
    ...
    with pytest.raises(ValueError, match="Exceeded budget for provider openai"):
        await provider_budget.async_filter_deployments(
            model="gpt-4o-mini",
            healthy_deployments=healthy_deployments,
            messages=[],
            request_kwargs={},
