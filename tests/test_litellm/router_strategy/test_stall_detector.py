...
class TestDetectStalledTask:
    def test_repeated_identical_anthropic_calls_are_stalled(self):
        ...
        assert detect_stalled_task(messages, window=6, repeat_threshold=3) == "repeated_tool_call"
    ...
    def test_repeated_errors_are_stalled_even_with_varied_arguments(self):
        ...
        assert detect_stalled_task(messages, window=6, repeat_threshold=3) == "repeated_tool_error"
    ...
    def test_varied_successful_calls_are_not_stalled(self):
        ...
        assert detect_stalled_task(messages, window=6, repeat_threshold=3) is None
    ...
    def test_evidence_older_than_the_window_does_not_count(self):
        ...
        assert detect_stalled_task(messages, window=2, repeat_threshold=2) is None
    ...
    def test_a_repeating_call_that_also_errors_reports_the_repeat_branch(self):
        """Both branches hold here. The repeat is the more specific fact and the one the
        feature exists for, so a stuck model is never filed under 'the tool is broken'."""
        messages = [
            *_anthropic_call("t1", "bash", {"cmd": "make build"}, is_error=True),
            *_anthropic_call("t2", "bash", {"cmd": "make build"}, is_error=True),
            *_anthropic_call("t3", "bash", {"cmd": "make build"}, is_error=True),
        ]
        assert detect_stalled_task(messages, window=6, repeat_threshold=3) == "repeated_tool_call"

    def test_no_messages_is_not_stalled(self):
        assert detect_stalled_task(None, window=6, repeat_threshold=3) is None
        assert detect_stalled_task([], window=6, repeat_threshold=3) is None
    ...
    def test_zero_threshold_never_flags_stalled(self):
        ...
        assert detect_stalled_task(messages, window=6, repeat_threshold=0) is None
