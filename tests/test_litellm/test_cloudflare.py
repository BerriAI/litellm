...
async def test_parameter_validation_cloudflare_llama4_scout(
    ...
    try:
        # Mock 400 response for top_k validation error
        respx_mock.post(
            "https://api.cloudflare.com/client/v4/accounts/test-account-id/ai/run/@cf/meta/llama-4-scout-17b-16e-instruct"
        ).respond(
            status_code=400,
            json={"error": {"message": "top_k must be between 1 and 50"}},
        )
        ...
        pytest.fail("Should have raised error for top_k > 50")
    except (ValueError, litellm.APIConnectionError) as e:
        assert "top_k must be between 1 and 50" in str(e)
    ...
    try:
        # Mock 400 response for repetition_penalty validation error
        respx_mock.post(
            "https://api.cloudflare.com/client/v4/accounts/test-account-id/ai/run/@cf/meta/llama-4-scout-17b-16e-instruct"
        ).respond(
            status_code=400,
            json={"error": {"message": "repetition_penalty must be between 0 and 2"}},
        )
        ...
        pytest.fail("Should have raised error for repetition_penalty > 2")
    except (ValueError, litellm.APIConnectionError) as e:
        assert "repetition_penalty must be between 0 and 2" in str(e)
