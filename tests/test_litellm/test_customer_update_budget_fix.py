"""
Unit tests for customer update budget fix.

Tests the specific changes made to fix the customer update endpoint's budget_id handling:
1. budget_id routing to end user table data (not budget table data)
2. Correct Prisma include field usage ("end_users" not "litellm_endusertable")
3. Required fields (created_by, updated_by) in budget creation
"""
...
class TestCustomerUpdateBudgetFix:
    """Test class for customer update budget fix functionality."""
    ...
    async def test_budget_id_routes_to_end_user_data_not_budget_data(
        self, 
        mock_prisma_client,
        ...
        """
        Test that budget_id is correctly routed to end user table data, not budget table data.
        
        This was the main bug - budget_id was being treated as budget creation data
        instead of budget linking data.
        """
        # Arrange
        ...
        # Act
        result = await update_end_user(update_request, mock_user_api_key_dict)
    ...
    @patch('litellm.proxy.proxy_server.prisma_client')
    @patch('litellm.proxy.proxy_server.litellm_proxy_admin_name', 'admin')
    async def test_budget_creation_uses_correct_include_field(
        self,
        mock_prisma_client,
        mock_user_api_key_dict,
        mock_existing_customer
        ...
        """
        Test that budget creation uses correct Prisma include field "end_users".
        
        The bug was using "litellm_endusertable" instead of "end_users".
        """
        # Arrange
        mock_existing_customer.model_dump.return_value = {
            "user_id": "test-user",
        ...
        # Check that include uses correct field name "end_users"
        ...
        assert 'litellm_endusertable' not in include_param

    @pytest.mark.asyncio
    @patch('litellm.proxy.proxy_server.prisma_client')
    @patch('litellm.proxy.proxy_server.litellm_proxy_admin_name', 'admin')
    async def test_budget_creation_includes_required_fields(
        self,
        mock_prisma_client,
        mock_user_api_key_dict,
        mock_existing_customer
        ...
        """
        Test that budget creation includes required created_by and updated_by fields.
        
        These fields were missing and causing creation failures.
        """
        # Arrange
        mock_existing_customer.model_dump.return_value = {
            "user_id": "test-user",
    ...
    @patch('litellm.proxy.proxy_server.prisma_client')
    @patch('litellm.proxy.proxy_server.litellm_proxy_admin_name', 'admin')
    async def test_budget_creation_fallback_to_admin_name(
        self,
        mock_prisma_client,
        mock_existing_customer
    ):
        """
        Test that budget creation falls back to litellm_proxy_admin_name when user_id is None.
        """
    ...
    @patch('litellm.proxy.proxy_server.prisma_client')
    @patch('litellm.proxy.proxy_server.litellm_proxy_admin_name', 'admin')
    async def test_budget_id_and_budget_fields_combination(
        self,
        mock_prisma_client,
        mock_user_api_key_dict,
        mock_existing_customer
        ...
        """
        Test behavior when both budget_id and budget creation fields are provided.
        
        budget_id should go to end user data, budget fields should trigger budget creation.
        """
        # Arrange
        mock_existing_customer.model_dump.return_value = {
            "user_id": "test-user",
