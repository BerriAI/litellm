...
class TestToolTransformation:
    ...
    def test_transform_computer_use_tools_anthropic(self):
        """Test that computer_use tools are preserved in Anthropic format"""
        computer_use_tool = {
        ...
        # Assert — Anthropic format: type stays computer_use_preview, function.parameters contains config
        assert len(result_tools) == 1
        assert result_tools[0]["type"] == "computer_use_preview"
        assert result_tools[0]["function"]["parameters"]["display_width_px"] == 1024
        assert result_tools[0]["function"]["parameters"]["display_height_px"] == 768
        assert result_tools[0]["function"]["parameters"]["environment"] == "browser"
        assert web_search_options is None

    def test_transform_computer_use_tools_gemini(self):
        """Test that computer_use tools are preserved in Gemini flat-dict format"""
        computer_use_tool = {
            "type": "computer_use_preview",
            "display_width_px": 1024,
            "display_height_px": 768,
        ...
        # Assert — Gemini format: flat dict with type="computer_use"
        assert len(result_tools) == 1
        assert result_tools[0]["type"] == "computer_use"
        assert result_tools[0]["display_width_px"] == 1024
        assert result_tools[0]["display_height_px"] == 768
        assert web_search_options is None
