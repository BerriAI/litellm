...
def test_create_litellm_model_response_uses_wrapper_chunks_for_reasoning_tokens():
    """create_litellm_model_response must pull usage from litellm_custom_stream_wrapper.chunks.

    CustomStreamWrapper stores pre-stripping chunks (with usage) in its .chunks list.
    Yielded chunks have usage stripped; collected_chat_completion_chunks are snapshots of
    those stripped chunks and therefore lack completion_tokens_details.  The fix uses
    litellm_custom_stream_wrapper.chunks so that reasoning_tokens from Gemini 2.5
    thinking models survive end-of-stream assembly and reach the response.completed event.
    """
    from unittest.mock import MagicMock
    ...
    mock_wrapper = MagicMock(spec=litellm.CustomStreamWrapper)
    mock_wrapper.chunks = [chunk_with_usage]
    mock_logging_obj = MagicMock()
    mock_wrapper.logging_obj = mock_logging_obj

    iterator = LiteLLMCompletionStreamingIterator(
        model="gemini-2.5-flash",
        litellm_custom_stream_wrapper=mock_wrapper,
        request_input="What is 6 * 7?",
        responses_api_request={},
        custom_llm_provider="gemini",
    )

    result = iterator.create_litellm_model_response()
