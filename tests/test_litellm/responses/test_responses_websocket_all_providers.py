...
class TestNativeWebSocketGuardrails:
    ...
    @pytest.mark.asyncio
    async def test_completed_event_with_non_list_content_and_summary_passes_through(
        self,
    ):
        """Regression test: an output item whose "content"/"summary" is a truthy
        non-iterable value (malformed upstream payload) must be skipped, not raise."""
        from unittest.mock import MagicMock
        ...
        event = json.dumps(
            {
                "type": "response.completed",
                "response": {
                    "output": [{"type": "message", "content": 42, "summary": True}]
                },
            }
        )
        guardrail = Guardrail()
