...
class TestStreamingIteratorOutputRecovery:
    ...
    def test_dict_shaped_terminal_response_is_backfilled(self):
        """
        When a terminal event fails validation, the provider transformation falls back to
        model_construct and leaves response as a raw dict, which must still be backfilled.
        """
        from litellm.types.llms.openai import ResponseCompletedEvent

        iterator = _make_iterator()
        terminal = ResponseCompletedEvent.model_construct(
            type="response.completed", response={**_RESPONSE_BASE, "output": []}
        )
        ...
        with patch.object(iterator, "_handle_logging_completed_response"):
            ...
            with patch.object(
                iterator.responses_api_provider_config, "transform_streaming_response", return_value=terminal
            ):
