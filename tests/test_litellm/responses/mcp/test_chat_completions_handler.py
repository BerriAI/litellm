...
async def test_acompletion_with_mcp_auto_exec_performs_follow_up(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        staticmethod(
            lambda **_: [{"id": "call-1", "type": "function", "function": {"name": "tool", "arguments": "{}"}}]
        ),
    )
...
async def test_acompletion_with_mcp_adds_metadata_to_streaming(monkeypatch):
    ...
    if hasattr(first_chunk, "choices") and first_chunk.choices:
        ...
        if hasattr(choice, "delta") and choice.delta:
            ...
            assert "mcp_list_tools" in provider_fields, (
...
async def test_acompletion_with_mcp_streaming_initial_call_is_streaming(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "_extract_tool_calls_from_chat_response",
        staticmethod(
            lambda **_: [{"id": "call-1", "type": "function", "function": {"name": "local_search", "arguments": "{}"}}]
        ),
    )
        ...
        staticmethod(
            lambda **_: [
                {"role": "user", "content": "hello"},
                {
                    "role": "assistant",
                    "tool_calls": [
                        {"id": "call-1", "type": "function", "function": {"name": "local_search", "arguments": "{}"}}
                    ],
                },
...
async def test_acompletion_with_mcp_streaming_metadata_in_correct_chunks(monkeypatch):
    ...
    # Patch litellm.acompletion at module level to catch function-level imports
    # NOTE: The follow-up LLM call is triggered while iterating the stream.
    # Keep the patch context open while consuming the stream to prevent real network calls.
    with (
        patch("litellm.acompletion", mock_acompletion_func),
        patch.object(
...
async def test_execute_tool_calls_sets_proxy_server_request_arguments(monkeypatch):
    ...
    mcp_handler_module = importlib.import_module("litellm.responses.mcp.litellm_proxy_mcp_handler")
    ...
    assert proxy_server_request["body"]["arguments"] == {"param1": "value1", "param2": 123}, (
