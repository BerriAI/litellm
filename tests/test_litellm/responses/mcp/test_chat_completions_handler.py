...
_REPLAY_UPSTREAM = "https://upstream.example/v1"


def _counting_ledger_mcp_tool(monkeypatch) -> AsyncMock:
    """Serve one auto-approved MCP tool through the real manager and registry, returning the tool itself."""
    from mcp.types import Tool
    ...
    manager.tool_name_to_mcp_server_name_mapping = {"ledger-commit_write": "ledger"}
    tool = AsyncMock(return_value={"written": True})
    registry = tool_registry.MCPToolRegistry()
    registry.register_tool("ledger-commit_write", "Write one ledger entry", {"type": "object"}, tool)
    monkeypatch.setattr(tool_registry, "global_mcp_tool_registry", registry)
    monkeypatch.setattr(mcp_server_manager, "global_mcp_server_manager", manager)
    monkeypatch.setattr(proxy_server, "proxy_logging_obj", ProxyLogging(user_api_key_cache=DualCache()))
    monkeypatch.setattr(
        ...
        AsyncMock(
            return_value=AggregateToolListing(
                tools=[Tool(name="ledger-commit_write", inputSchema={"type": "object"})], outcomes={}
            )
        ),
    )
    monkeypatch.setattr(litellm, "disable_aiohttp_transport", True)
...
def _is_follow_up(request) -> bool:
    return any(message.get("role") == "tool" for message in json.loads(request.content)["messages"])


def _upstream_call_kind(call) -> str:
    return "follow_up" if _is_follow_up(call.request) else "initial"


def _chat_completion_response(message: dict, finish_reason: str) -> Response:
    return Response(
        200,
        json={
            "id": "chatcmpl-replay",
...
def _ledger_tool_call_response() -> Response:
            ...
            "tool_calls": [
                {
                    "id": "call_1",
                    "type": "function",
                    "function": {"name": "ledger-commit_write", "arguments": '{"entry": "alpha"}'},
                }
            ],
        },
        "tool_calls",
...
def _server_error_response() -> Response:
    return Response(500, json={"error": {"message": "upstream failed", "type": "server_error"}})


async def _router_mcp_completion(router: litellm.Router) -> ModelResponse:
    from litellm.proxy._types import LiteLLM_ObjectPermissionTable, UserAPIKeyAuth

    return await router.acompletion(
        model="repro-model",
        messages=[{"role": "user", "content": "Record entry alpha"}],
        tools=[{"type": "mcp", "server_url": "litellm_proxy/ledger", "require_approval": "never"}],
        metadata={
            "user_api_key_auth": UserAPIKeyAuth(
...
def _replay_router(num_retries: int, fallbacks: list | None = None) -> litellm.Router:
    deployment = {"model": "openai/gpt-4.1-mini", "api_key": "sk-test", "api_base": _REPLAY_UPSTREAM}
...
@respx.mock
async def test_router_does_not_replay_an_executed_mcp_tool_after_the_follow_up_fails(monkeypatch, router_kwargs):
    """
    Regression test for #43153: an auto-approved MCP tool must run once per request.

    Given: The model asks for the tool, the tool runs, and the follow-up model call returns a 500
    When:  The router has retries or a fallback group configured
    Then:  The client gets that 500 after one tool execution and one follow-up call

    A retry or fallback re-runs the whole MCP loop, so before the fix every replay executed
    the tool again (three writes with num_retries=2, two with one fallback) and the client
    still received the same 500.
    """
    tool = _counting_ledger_mcp_tool(monkeypatch)
    upstream = respx.post(f"{_REPLAY_UPSTREAM}/chat/completions").mock(
        side_effect=lambda request: _server_error_response() if _is_follow_up(request) else _ledger_tool_call_response()
    )

    with pytest.raises(litellm.InternalServerError) as exc_info:
        await _router_mcp_completion(_replay_router(**router_kwargs))

    assert exc_info.value.status_code == 500
    assert tool.await_count == 1
    assert [_upstream_call_kind(call) for call in upstream.calls] == ["initial", "follow_up"]


@pytest.mark.asyncio
@respx.mock
async def test_router_still_retries_a_failure_before_any_mcp_tool_ran(monkeypatch):
    """
    Given: The initial model call fails once with a 500, then asks for the tool
    When:  The router retries
    Then:  The retry succeeds, the tool runs once, and the follow-up answer reaches the client
    """
    tool = _counting_ledger_mcp_tool(monkeypatch)
    upstream = respx.post(f"{_REPLAY_UPSTREAM}/chat/completions").mock(
        side_effect=[
            _server_error_response(),
            _ledger_tool_call_response(),
            _chat_completion_response({"role": "assistant", "content": "Recorded alpha"}, "stop"),
        ]
    )

    response = await _router_mcp_completion(_replay_router(num_retries=2))

    assert response.choices[0].message.content == "Recorded alpha"
    assert tool.await_count == 1
    assert [_upstream_call_kind(call) for call in upstream.calls] == ["initial", "initial", "follow_up"]
