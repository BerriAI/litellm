...
def test_streaming_multiple_partial_tool_calls():
    ...
    for call_id, function_name, args_chunk in test_chunks:
        ...
        # Create mock streaming tool call delta
        mock_tool_call_delta = ChatCompletionDeltaToolCall(
            id=call_id, type="function", function=mock_function, index=0
        )
...
def test_api_base_and_api_key_passthrough(function_name, is_async, is_stream):
    ...
    with unittest.mock.patch(mock_target) as mock_completion:
        ...
        if is_stream:
            ...
            ), f"Expected stream=True for {function_name}"
        else:
...
async def test_google_generate_content_with_openai():
    ...
    with unittest.mock.patch(
        "litellm.acompletion", new_callable=unittest.mock.AsyncMock
    ) as mock_completion:
        ...
        assert passed_fields == set(
            ["model", "messages"]
        ), f"Expected only model, contents, systemInstruction, and safetySettings to be passed through, got {passed_fields}"
...
async def test_agenerate_content_x_goog_api_key_header():
    ...
    with unittest.mock.patch.dict(
        os.environ, {"GEMINI_API_KEY": test_api_key}, clear=False
    ):
        ...
        with unittest.mock.patch(
            ...
            ), f"x-goog-api-key header not found in headers: {list(headers.keys())}"
            ...
            ), f"Expected x-goog-api-key to be {test_api_key}, got {api_key_value}"
            ...
            print(
                f"✓ Test passed: x-goog-api-key header correctly set to {api_key_value}"
            )
