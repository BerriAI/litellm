...
def debug_capture_buf():
    ...
    _secret_filter attached, then call _turn_on_debug() so all three
    are at DEBUG. Restore prior state on teardown."""
    buf = StringIO()
...
def test_matrix_catches_regression_when_redaction_disabled():
    ...
    handler = logging.StreamHandler(buf)
    handler.addFilter(_secret_filter)
    saved = (verbose_logger.handlers[:], verbose_logger.level,
             verbose_logger.propagate, verbose_logger.disabled)
    ...
    try:
        ...
        handlers, level, propagate, disabled = saved
        verbose_logger.handlers.clear()
        for old in handlers:
            verbose_logger.addHandler(old)
        verbose_logger.setLevel(level)
        verbose_logger.propagate = propagate
        verbose_logger.disabled = disabled
