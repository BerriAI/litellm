...
def test_responses_api_bridge_check_sap_gpt_tools_with_default_reasoning_routes_to_responses(
    monkeypatch, model_name
):
    """
    SAP direct-connect gpt with function tools and UNSET reasoning_effort must bridge to
    Responses. SAP's /chat/completions rejects the combination just like OpenAI/Azure
    ("Function tools with reasoning_effort are not supported for gpt-5.6-sol in
    /v1/chat/completions. To use function tools, use /v1/responses or set reasoning_effort
    to 'none'."), so the sap provider counts as a constraint-enforcing endpoint and bridges
    without needing an explicit effort. Regression for the deployed gpt-5.6-sol 400.
    """
    import litellm
    from litellm.main import responses_api_bridge_check
...
def test_responses_api_bridge_check_sap_gpt_tools_plus_reasoning_routes_to_responses():
    """SAP gpt with both tools and an explicit reasoning_effort bridges to Responses."""
    from litellm.main import responses_api_bridge_check
...
def test_responses_api_bridge_check_sap_gpt_tools_with_reasoning_none_stays_chat():
    """
    reasoning_effort "none" is the escape hatch SAP's own error names, so a SAP gpt call
    that sets it keeps its function tools servable on chat and must not bridge.
    """
    from litellm.main import responses_api_bridge_check
...
def test_responses_api_bridge_check_sap_claude_tools_stays_chat():
    """
    SAP Claude runs on a Bedrock invoke executable with no native Responses surface, so a
    Claude call with tools must NOT bridge; it keeps flowing through chat completions.
    """
    from litellm.main import responses_api_bridge_check
