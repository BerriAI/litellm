...
# ==============================================================
# SRE-3573: S3 403 SignatureDoesNotMatch when the team alias (used
# as an S3 key prefix via s3_use_team_prefix) contains special
# characters (space, #, +, &, unicode, ...).
#
# Root cause: the signing flow signed a path that the generic
# botocore SigV4Auth re-quoted ("My%20Team" -> canonical
# "My%2520Team"), while httpx put "My%20Team" on the wire. The
# SigV4 canonical URI therefore did not match what S3 verifies,
# producing 403 SignatureDoesNotMatch. The fix percent-encodes the
# object key once and signs with S3SigV4Auth (which does not
# re-encode the path), so the signed URI matches the wire URL.
#
# These tests model S3's server-side verification: they re-derive
# the SigV4 signature over the URL that was actually sent on the
# wire and assert it equals the signature in the Authorization
# header the client sent. With the pre-fix code these differ.
# ==============================================================

# Object keys whose team-alias prefix contains characters that are
# percent-encoded by URL quoting and therefore exposed the double-
# encoding bug.
SPECIAL_CHAR_OBJECT_KEYS = [
    "My Team/2025-09-14/test-key.json",
    "R&D + Ops/2025-09-14/test-key.json",
    "tëam#1/2025-09-14/test-key.json",
    "a@b/2025-09-14/test-key.json",
]


def _assert_signature_valid_for_wire_url(
    request_url, data, signed_headers, access_key, secret_key, region="us-east-1"
):
    """
    Re-derive the SigV4 signature over the exact URL + headers that were sent
    on the wire (the way S3 verifies an incoming request) and assert it matches
    the Authorization header the client produced.

    A mismatch is exactly the SRE-3573 403 SignatureDoesNotMatch condition:
    the client signed a different (double-encoded) canonical URI than the path
    ...
    # Rebuild the request S3 would verify: same method, same wire URL, same
    # signed header values, without the client's Authorization header.
    verify_headers = {
        k: v for k, v in signed_headers.items() if k.lower() != "authorization"
    }
    verify_req = AWSRequest(
        method="PUT", url=request_url, data=data, headers=verify_headers
    )
    # Reuse the client's signing timestamp so the two signatures are comparable.
    ...
    assert verify_req.headers["Authorization"] == signed_headers["Authorization"], (
        ...
        "S3 would reject this with 403 SignatureDoesNotMatch (SRE-3573).\n"
...
def test_async_upload_signature_matches_wire_url_special_chars(
    ...
    """SRE-3573: async upload must sign the same (single-encoded) URL it sends."""
    from unittest.mock import AsyncMock
    ...
    request_url = call[0][0]
    data = call.kwargs["data"]
    signed_headers = call.kwargs["headers"]
...
@pytest.mark.parametrize("s3_object_key", SPECIAL_CHAR_OBJECT_KEYS)
def test_sync_upload_signature_matches_wire_url_special_chars(s3_object_key):
    """SRE-3573: sync upload must sign the same (single-encoded) URL it sends."""
    from litellm.types.integrations.s3_v2 import s3BatchLoggingElement
    ...
    with patch(
        "litellm.integrations.s3_v2._get_httpx_client", return_value=mock_client
    ):
