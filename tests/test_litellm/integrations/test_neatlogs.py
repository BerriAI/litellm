...
def test_payload_data_types(sample_payload):
    ...
    for field in string_fields:
        if sample_payload[field] is not None:
            assert isinstance(sample_payload[field], str), f"Field {field} should be string"

    # Integer fields
    int_fields = ["prompt_tokens", "completion_tokens", "total_tokens"]
    for field in int_fields:
        assert isinstance(sample_payload[field], int), f"Field {field} should be integer"

    # Float fields
    float_fields = ["cost", "start_time", "end_time", "duration"]
    for field in float_fields:
        assert isinstance(sample_payload[field], float), f"Field {field} should be float"

    # List fields
    list_fields = ["messages", "tags"]
    for field in list_fields:
        assert isinstance(sample_payload[field], list), f"Field {field} should be list"
