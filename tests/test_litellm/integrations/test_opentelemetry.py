...
class TestOpenTelemetryNonMappingResponseObj(unittest.TestCase):
    """set_attributes is called for every traced call, including non-LLM call
    types whose response_obj is not a mapping: management list endpoints return
    a list, MCP tool calls return a CallToolResult. Those have no .get(), so the
    response-derived attributes used to raise AttributeError and abort the whole
    tracing callback (once per such request — the flood seen in prod). The
    response_obj must be tolerated so the rest of the span is still annotated."""
    ...
    def _assert_survives(self, response_obj):
        otel = OpenTelemetry()
        span = MagicMock()
        otel.set_attributes(span=span, kwargs=self._kwargs(), response_obj=response_obj)
        calls = {c.args[0]: c.args[1] for c in span.set_attribute.call_args_list if c.args}
        # litellm.call_id is set AFTER the first response_obj.get(); before the fix
        # the AttributeError aborted set_attributes before reaching it.
        assert calls.get("litellm.call_id") == "call-id-456"
        # response id falls back to the standard logging payload id, not a crash.
        assert calls.get("gen_ai.response.id") == "std-id-123"

    def test_list_response_obj_does_not_abort_span(self):
        self._assert_survives([{"team_id": "t1"}, {"team_id": "t2"}])

    def test_non_mapping_object_response_obj_does_not_abort_span(self):
        class CallToolResultLike:
            def __init__(self):
                self.content = "ok"

        self._assert_survives(CallToolResultLike())

    def test_dict_response_obj_still_sets_response_attributes(self):
        otel = OpenTelemetry()
        span = MagicMock()
        otel.set_attributes(
            span=span,
            kwargs=self._kwargs(),
            response_obj={"id": "resp-id", "model": "gpt-4", "choices": []},
        )
        calls = {c.args[0]: c.args[1] for c in span.set_attribute.call_args_list if c.args}
        # A real dict response still wins over the fallback.
        assert calls.get("gen_ai.response.id") == "resp-id"


class TestOpenTelemetrySafeSetAttributeEndedSpan(unittest.TestCase):
    """safe_set_attribute must not write to an already-ended span. Async
    success/failure logging can run after the request span end()s; writing then
    is a no-op that the OTel SDK logs as a "Setting attribute on ended span"
    warning, flooding logs (and burning CPU) under load."""

    def test_skips_write_when_span_not_recording(self):
        otel = OpenTelemetry()
        span = MagicMock()
        span.is_recording.return_value = False
        otel.safe_set_attribute(span=span, key="k", value="v")
        span.set_attribute.assert_not_called()

    def test_writes_when_span_recording(self):
        otel = OpenTelemetry()
        span = MagicMock()
