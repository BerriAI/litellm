...
class TestDataDogLLMObsLogger:
    ...
    def test_apm_id_included(self, mock_env_vars, mock_response_obj):
        ...
        with patch(
            ...
            with patch(
                "litellm.integrations.datadog.datadog_llm_obs.tracer", fake_tracer
            ):
                logger = DataDogLLMObsLogger()
...
async def test_dd_llms_obs_redaction(mock_env_vars):
    ...
    print(
        "logged DD LLM Obs payload",
        json.dumps(
            dd_llms_obs_logger.logged_standard_logging_payload, indent=4, default=str
        ),
    )
    print(
        "\n\nlogged S3 payload",
        json.dumps(
            test_s3_logger.logged_standard_logging_payload, indent=4, default=str
        ),
    )
