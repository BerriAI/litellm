...
class TestEventTypeLogging:
    ...
    def test_list_event_hook_fallback_preserves_configured_shape(self):
        """When event_type is None and event_hook is a list, the logged
        guardrail_mode keeps the configured list verbatim — the fallback must
        stay backward compatible with pre-existing consumers of the field."""
        from litellm.types.guardrails import GuardrailEventHooks

        guardrail = CustomGuardrail(
            guardrail_name="test_fallback",
            event_hook=[GuardrailEventHooks.pre_call, GuardrailEventHooks.post_call],
        )
        ...
        assert mode == [GuardrailEventHooks.pre_call, GuardrailEventHooks.post_call]
