...
group's single deployment (``model_id`` and ``custom_llm_provider``). The
deployment health metrics must keep treating such rejects as "no deployment
picked": a key rate limit or guardrail block never reached the deployment, so
it must not flip ``litellm_deployment_state`` to partial outage or count as a
...
def _attributed_failure_kwargs(exception: Exception) -> dict:
    return {
...
def test_attributed_proxy_reject_leaves_deployment_healthy(rejection: Exception):
    ...
    logger.set_llm_deployment_failure_metrics(_attributed_failure_kwargs(rejection))
...
def test_attributed_provider_failure_still_marks_deployment_partial_outage():
    logger = PrometheusLogger()

    logger.set_llm_deployment_failure_metrics(_attributed_failure_kwargs(_ProviderError("upstream 500")))
