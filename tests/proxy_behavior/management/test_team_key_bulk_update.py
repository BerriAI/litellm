...
async def _seed_team_key(prisma, proxy_client, prefix: str, world, shape: str) -> str:
    ...
    if shape == "admin":
        await create_scratch_team(prisma, prefix, organization_id=world.org_a_id, admin_user_ids=[internal])
        key_owner = internal
        ...
        await create_scratch_team(prisma, prefix, organization_id=world.org_a_id, admin_user_ids=[owner])
        key_owner = owner
    else:
...
async def test_team_key_bulk_update_requires_team_id(proxy_client, prisma, scratch, world):
...
async def test_team_key_bulk_update_all_keys_in_team(proxy_client, prisma, scratch, world):
    ...
    for key in keys:
        row = await prisma.db.litellm_verificationtoken.find_unique(where={"token": hash_token(key)})
        assert row is not None and row.max_budget == _MARKER_BUDGET


async def test_team_key_bulk_update_no_keys_found_is_404(proxy_client, prisma, scratch, world):
...
async def test_team_key_bulk_update_missing_key_is_isolated(proxy_client, prisma, scratch, world):
    """A key_id absent from the team lands in failed_updates; the batch still
    returns 200 and the real key is updated."""
    admin = world.keys[Actor.PROXY_ADMIN].cleartext
    real = await _seed_team_key(prisma, proxy_client, scratch.prefix, world, "nonmember")
    ...
    row = await prisma.db.litellm_verificationtoken.find_unique(where={"token": hash_token(real)})
    assert row is not None and row.max_budget == _MARKER_BUDGET
