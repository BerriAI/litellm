...
async def test_bulk_update_specific_team_only_mutates_listed(proxy_client, prisma, scratch, world):
    ...
    target_row = await prisma.db.litellm_teamtable.find_unique(where={"team_id": target})
    assert perm in (target_row.team_member_permissions or []), (
        f"target team did not gain perm; got={target_row.team_member_permissions}"
    )

    for tid, before_perms in before.items():
        after_row = await prisma.db.litellm_teamtable.find_unique(where={"team_id": tid})
        assert list(after_row.team_member_permissions or []) == before_perms, (
            f"untouched team {tid} mutated: {before_perms} → {list(after_row.team_member_permissions or [])}"
        )
...
async def test_bulk_update_specific_team_idempotent(proxy_client, prisma, scratch, world):
    target = await create_scratch_team(
        prisma,
        team_id=scratch.tag("target"),
...
async def test_bulk_update_unknown_team_id_rejected(proxy_client, prisma, scratch, world):
    ...
    before = list(
        (await prisma.db.litellm_teamtable.find_unique(where={"team_id": existing})).team_member_permissions or []
    )
    seeder = world.keys[Actor.PROXY_ADMIN].cleartext
    resp = await proxy_client.post(
        "/team/permissions_bulk_update",
    ...
    after_real = await prisma.db.litellm_teamtable.find_unique(where={"team_id": existing})
    assert list(after_real.team_member_permissions or []) == before, (
...
async def test_bulk_update_selector_validation(body, expected_substring: str, proxy_client, world):
    seeder = world.keys[Actor.PROXY_ADMIN].cleartext
    resp = await proxy_client.post(
        "/team/permissions_bulk_update",
        headers={"Authorization": f"Bearer {seeder}"},
...
async def test_bulk_update_apply_to_all_mutates_every_team(proxy_client, prisma, scratch, world):
    # Two scratch teams so we can assert "every" includes our own targets.
    a = await create_scratch_team(prisma, team_id=scratch.tag("a"), team_member_permissions=[])
    b = await create_scratch_team(prisma, team_id=scratch.tag("b"), team_member_permissions=["other"])
    ...
    try:
        ...
        for team in post:
            ...
            for prior in snapshot.get(team.team_id, []):
                assert prior in perms, f"team {team.team_id} lost prior perm {prior!r}: {perms}"
    finally:
        ...
        for team_id, prior_perms in snapshot.items():
            current = await prisma.db.litellm_teamtable.find_unique(where={"team_id": team_id})
            if current is None:
                continue
