...
async def test_team_bulk_member_add_proxy_admin_adds_explicit_members(proxy_client, prisma, scratch, world):
...
async def test_team_bulk_member_add_empty_members_is_400(proxy_client, prisma, scratch, world):
...
async def test_team_bulk_member_add_over_max_batch_is_400(proxy_client, prisma, scratch, world):
    """A member list larger than the 500-member cap is rejected 400."""
    await create_scratch_team(prisma, scratch.prefix, organization_id=world.org_a_id)
    members = [{"user_id": f"{scratch.prefix}-u{i}", "role": "user"} for i in range(501)]
    resp = await proxy_client.post(
        "/team/bulk_member_add",
        headers={"Authorization": f"Bearer {world.keys[Actor.PROXY_ADMIN].cleartext}"},
...
async def test_team_bulk_member_add_non_admin_is_401(actor: Actor, proxy_client, prisma, scratch, world):
...
async def test_team_bulk_member_add_all_users_proxy_admin(proxy_client, prisma, scratch, world):
