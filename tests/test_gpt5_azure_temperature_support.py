...
def test_azure_gpt5_supports_temperature():
    ...
    config = ProviderConfigManager.get_provider_responses_api_config(provider=LlmProviders.AZURE, model="gpt-5")
...
def test_azure_o_series_does_not_support_temperature():
    ...
    for model in test_models:
        config = ProviderConfigManager.get_provider_responses_api_config(provider=LlmProviders.AZURE, model=model)
        ...
        assert "temperature" not in supported_params, f"Azure {model} should NOT support temperature parameter"


def test_openai_gpt5_supports_temperature():
    """Test that OpenAI GPT-5 supports temperature parameter."""
    config = ProviderConfigManager.get_provider_responses_api_config(provider=LlmProviders.OPENAI, model="gpt-5")
...
def test_azure_gpt5_variants_support_temperature():
    ...
    for model in gpt5_variants:
        ...
        supported_params = config.get_supported_openai_params(model)
        assert "temperature" in supported_params, f"Model '{model}' should support temperature parameter"
