...
class TestVertexAILivePassthroughLoggingHandler:
    ...
    def test_extract_usage_metadata_multiple_messages(self, handler):
        """Each message repeats the session's running totals, so the widest one is the session."""
        messages = [
            {
                "type": "response.create",
                "usageMetadata": {
            ...
            },
            {
                ...
                "usageMetadata": {
                    ...
                    "candidatesTokensDetails": [{"modality": "TEXT", "tokenCount": 23}],
        ...
        assert result is not None
        assert result["promptTokenCount"] == 15
        assert result["candidatesTokenCount"] == 23
        assert result["totalTokenCount"] == 38
        assert result["promptTokensDetails"] == [{"modality": "TEXT", "tokenCount": 15}]
        assert result["candidatesTokensDetails"] == [{"modality": "TEXT", "tokenCount": 23}]
        assert result["trafficType"] == "ON_DEMAND", "sibling fields must come from the winning snapshot"
    ...
    CUMULATIVE_SESSION = (
        {"prompt": (14, 122), "candidates": (10, 88)},
        {"prompt": (21, 182), "candidates": (14, 140)},
        {"prompt": (24, 203), "candidates": (16, 171)},
    )

    @staticmethod
    def _live_messages(snapshots):
        ...
        return [{"type": "session.created", "session": {"id": "s"}}] + [
            {
                "type": "response.done",
                "usageMetadata": {
                    "promptTokenCount": sum(snapshot["prompt"]),
                    "candidatesTokenCount": sum(snapshot["candidates"]),
                    "totalTokenCount": sum(snapshot["prompt"]) + sum(snapshot["candidates"]),
                    "promptTokensDetails": [
                        {"modality": "TEXT", "tokenCount": snapshot["prompt"][0]},
                        {"modality": "AUDIO", "tokenCount": snapshot["prompt"][1]},
                    ],
                    "candidatesTokensDetails": [
                        {"modality": "TEXT", "tokenCount": snapshot["candidates"][0]},
                        {"modality": "AUDIO", "tokenCount": snapshot["candidates"][1]},
                    ],
                },
            }
    ...
    @staticmethod
    def _session_cost(handler, mock_logging_obj, messages, model):
        ...
        return completion_cost(
            completion_response=result["result"],
            model=f"vertex_ai/{model}",
            custom_llm_provider="vertex_ai",
            call_type="acompletion",
        )

    @pytest.mark.parametrize("order", ["as sent", "reversed"])
    def test_multi_turn_session_bills_the_widest_snapshot_not_the_sum(self, handler, mock_logging_obj, order):
        """Live reports the session's running totals on every usageMetadata message, not that turn's delta.

        Summing them charged the same audio once per message, so this three-message
        session billed 2.4x the audio it actually carried, and per-modality pricing puts
        that over-count on the audio rate rather than the 6x cheaper text rate. Order is
        parametrized because the fix must not rely on the messages arriving in sequence.
        """
        ...
        def cost_of(text_in, audio_in, text_out, audio_out):
            return (
                text_in * info["input_cost_per_token"]
                + audio_in * info["input_cost_per_audio_token"]
                + text_out * info["output_cost_per_token"]
                + audio_out * info["output_cost_per_audio_token"]
            )

        messages = self._live_messages(self.CUMULATIVE_SESSION)
        cost = self._session_cost(handler, mock_logging_obj, messages if order == "as sent" else messages[::-1], model)

        widest = cost_of(24, 203, 16, 171)
        summed = cost_of(14 + 21 + 24, 122 + 182 + 203, 10 + 14 + 16, 88 + 140 + 171)

        assert cost == pytest.approx(widest, rel=1e-9), "the session must cost what its final running total says"
        assert cost < summed, "summing cumulative snapshots over-bills every modality"

    def test_single_snapshot_session_is_unchanged(self, handler, mock_logging_obj):
        """A one-message session has nothing to aggregate, so it must bill exactly as before."""
        from litellm.utils import get_model_info

        model = "gemini-live-2.5-flash-preview-native-audio-09-2025"
        info = get_model_info(model=model, custom_llm_provider="vertex_ai")

        cost = self._session_cost(handler, mock_logging_obj, self._live_messages(self.CUMULATIVE_SESSION[-1:]), model)

        expected = (
            24 * info["input_cost_per_token"]
            + 203 * info["input_cost_per_audio_token"]
            + 16 * info["output_cost_per_token"]
            + 171 * info["output_cost_per_audio_token"]
        )
    ...
    def test_server_side_tool_use_prompt_tokens_are_reported(self, handler, mock_logging_obj):
        ...
        """
        model = "gemini-live-2.5-flash-preview-native-audio-09-2025"
        messages = self._live_messages(self.CUMULATIVE_SESSION[-1:])
        ...
        usage = handler._create_usage_object_from_metadata(usage_metadata=grounded[-1]["usageMetadata"], model=model)
        assert usage.prompt_tokens_details.tool_use_tokens == 500

        plain_cost = self._session_cost(handler, mock_logging_obj, messages, model)
        grounded_cost = self._session_cost(handler, mock_logging_obj, grounded, model)
