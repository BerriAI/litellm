"""Tests for LiteLLMA2ACardResolver: well-known path probing at the httpx boundary."""
...
async def test_card_resolver_fallback_from_new_to_old_path():
    httpx_client = _FakeHttpxClient(
        ...
        responses={
            ...
            "/.well-known/agent.json": (200, dict(_FOUNDRY_CARD_JSON)),
        },
    )
    ...
    assert httpx_client.calls == ["/.well-known/agent-card.json", "/.well-known/agent.json"]
