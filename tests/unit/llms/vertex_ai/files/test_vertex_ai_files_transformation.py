...
from litellm.llms.vertex_ai.files.transformation import (
    ...
    is_passthrough_managed_gcs_url,
)
...
def _upload_body_bytes(config, file, litellm_params) -> bytes:
    ...
    return b"".join(body["streaming_media_upload"]["body_stream"].iter_bytes())
...
class TestPassthroughBatchUpload:
    ...
    @pytest.mark.parametrize(
        ...
        ids=["bytes-tuple", "bare-bytes", "handle-tuple"],
    )
    ...
    def test_passthrough_upload_streams_a_large_file_in_bounded_chunks(self, config):
        content = NATIVE_VERTEX_ROW * (3 * 1024 * 1024 // len(NATIVE_VERTEX_ROW) + 1)
        body = config.transform_create_file_request(
            model="",
            create_file_data={"file": ("batch.jsonl", io.BytesIO(content), "application/jsonl"), "purpose": "batch"},
            optional_params={},
            litellm_params={"passthrough": True},
        )
        chunks = list(body["streaming_media_upload"]["body_stream"].iter_bytes())
        assert len(chunks) >= 3
