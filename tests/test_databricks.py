...
def test_completions_with_sync_databricks_client(monkeypatch, set_base_key):
    ...
    with patch(
        "databricks.sdk.WorkspaceClient", wraps=WorkspaceClient
    ) as mock_workspace_client, patch.object(
        ApiClient, "do", return_value=mock_chat_response()
    ) as mock_api_request:
        response = litellm.completion(
            model="databricks/dbrx-instruct-071224",
            messages=messages,
...
def test_completions_streaming_with_sync_databricks_client(monkeypatch, set_base_key):
    ...
    with patch(
        ...
        ApiClient, "do", return_value=mock_response
    ) as mock_api_request:
        response_stream: CustomStreamWrapper = litellm.completion(
            model="databricks/dbrx-instruct-071224",
            messages=messages,
...
def test_embeddings_with_sync_databricks_client(monkeypatch, set_base_key):
    ...
    with patch(
        ...
        ApiClient, "do", return_value=mock_embedding_response()
    ) as mock_api_request:
        response = litellm.embedding(
            model="databricks/bge-large-en-v1.5",
            input=inputs,
