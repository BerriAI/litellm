...
class TeamAPI:
    ...
    def create_team(self, team_alias: str, models: List[str] = None) -> Dict:
        ...
        response = requests.post(f"{self.base_url}/team/new", headers=self.headers, json=data)
    ...
    def add_team_member(self, team_id: str, user_email: str, role: str) -> Dict:
        ...
        response = requests.post(f"{self.base_url}/team/member_add", headers=self.headers, json=data)
        response.raise_for_status()
        return response.json()
    ...
    def delete_team_member(self, team_id: str, user_id: str) -> Dict:
        ...
        response = requests.post(f"{self.base_url}/team/member_delete", headers=self.headers, json=data)
        response.raise_for_status()
        return response.json()
...
def verify_member_in_team(team_info: Dict, user_email: str) -> bool:
    ...
    return any(member["user_id"] == user_email for member in team_info["team_info"]["members_with_roles"])
...
def test_add_single_member(api_client, new_team):
    ...
    # Assertions
    assert verify_member_in_team(updated_info, test_email), f"Member {test_email} not found in team"
    assert updated_size == initial_size + 1, f"Team size did not increase by 1 (was {initial_size}, now {updated_size})"
...
def test_add_multiple_members(api_client, new_team):
    ...
    for i in range(10):
        ...
        assert verify_member_in_team(current_info, email), f"Member {email} not found in team"
        assert current_size == initial_size + i + 1, f"Team size incorrect after adding {email}"
    ...
    for email in added_emails:
        assert verify_member_in_team(final_info, email), f"Member {email} not found in final team check"
...
def test_duplicate_user_addition(api_client, new_team):
    ...
    user_count = sum(
        1 for member in team_info_after_second["team_info"]["members_with_roles"] if member["user_id"] == test_email
    )
...
def test_member_deletion(api_client, new_team):
    ...
    assert verify_member_in_team(team_info_before, user_id), "User was not added successfully"
    ...
    for i in range(5):
        ...
        if i == 0:
            ...
            try:
                ...
                logger.info(f"Expected error received on deletion attempt {i + 1}: {str(e)}")
    ...
    assert not verify_member_in_team(final_info, user_id), "User still exists in team after deletion"

    # Verify only one member was removed
    assert final_size == initial_size - 1, f"Team size changed unexpectedly (was {initial_size}, now {final_size})"
...
def test_delete_nonexistent_member(api_client, new_team):
    ...
    team_info = api_client.get_team_info(new_team)
    assert not verify_member_in_team(team_info, nonexistent_user), "Test setup error: nonexistent user somehow exists"
