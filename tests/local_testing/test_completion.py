...
async def test_re_use_azure_async_client():
    try:
        ...
        for _ in range(3):
            response = await litellm.acompletion(
                model="azure/gpt-4o-new-test", messages=messages, client=client
            )
            print(f"response: {response}")
    except Exception as e:
...
def test_azure_openai_ad_token():
    ...
    try:
        response = litellm.completion(
            model="azure/gpt-4o-new-test",  # e.g. gpt-35-instant
            messages=[
                {
                    "role": "user",
...
def test_completion_response_ratelimit_headers(model, stream):
    ...
    if model == "azure/gpt-4o-new-test":
        # Azure OpenAI header
