...
#### Test Nebius Chat Completions ####

def test_nebius_completion_call():
    completion_model = "Qwen/Qwen3-30B-A3B-fast"
    try:
        litellm.set_verbose = False
        start_time = time.time()
        response = completion(
            model=f"nebius/{completion_model}",
            messages=messages,
            logger_fn=logger_fn,
            stream=True,
        )
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.asyncio
async def test_nebius_completion_streaming(sync_mode):
    completion_model = "Qwen/Qwen3-30B-A3B-fast"
    try:
        litellm.set_verbose = False
        if sync_mode:
...
def test_nebius_completion_call_bad_key():
    completion_model = "Qwen/Qwen3-30B-A3B-fast"
    try:
        api_key = "bad-key"
        start_time = time.time()
