...
@pytest.mark.parametrize(
    "team_id, key_team_id, user_role, is_team_admin, expected_result",
    [
        ("1234", "1234", LitellmUserRoles.PROXY_ADMIN.value, False, True),
        (
            "1234",
            "1235",
        ...
        (None, "1234", LitellmUserRoles.PROXY_ADMIN.value, False, True),
        (None, None, LitellmUserRoles.PROXY_ADMIN.value, False, True),
        (
            "1234",
            "1234",
            ...
            False,
            False,
        ),  # internal users cannot add models for other teams
        (
            "1234",
            "1234",
            LitellmUserRoles.INTERNAL_USER.value,
            True,
            True,
        ),  # team admins can add models for their team
        ("1234", "1235", LitellmUserRoles.INTERNAL_USER.value, False, False),
        (
            "1234",
            "1235",
            LitellmUserRoles.INTERNAL_USER.value,
            True,
            True,
        ),  # team admins can add models for teams they are admin for, even if key is not team key
        (None, "1234", LitellmUserRoles.INTERNAL_USER.value, False, False),
        (
            None,
            None,
            LitellmUserRoles.INTERNAL_USER.value,
...
def test_can_add_model(team_id, key_team_id, user_role, is_team_admin, expected_result):
    ...
    team_admin_id = current_user_id if is_team_admin else non_user_id
    if team_id is not None:
        team_obj = LiteLLM_TeamTable(
            team_id=team_id,
            members_with_roles=[
                Member(user_id=team_admin_id, role="admin"),
            ],
        )
    else:
        team_obj = None
    ...
    assert (
        check_if_team_id_matches_key(**args) is expected_result
    ), f"expected {expected_result} but got {check_if_team_id_matches_key(**args)}"
