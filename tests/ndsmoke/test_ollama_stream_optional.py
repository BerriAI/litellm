...
def test_ollama_stream_optional():
    ...
    async def run():
        ...
        stream = await r.acompletion(model='m', messages=[{"role":"user","content":"Explain transformers in one short sentence."}], stream=True, timeout=15)
        collected = []
        async for chunk in stream:
            try:
                s = getattr(getattr(chunk.choices[0],'delta',{}),'content',None)
                if s:
                    collected.append(s)
            except Exception:
                pass
