...
async def test_integration_ephemeral_real_e2b():
    result = await litellm.acode_interpreter_tool(provider="e2b", code="print(sum(range(10)))")
...
async def test_integration_lifecycle_roundtrip_real_e2b():
    ...
    try:
        result = await litellm.arun_code(provider="e2b", container=container, code="print(6*7)")
