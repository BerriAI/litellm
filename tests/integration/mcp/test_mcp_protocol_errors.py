...
def test_jsonrpc_error_and_malformed_tool_result_remain_errors(gateway: Gateway) -> None:
    def provider(request: Request) -> Reply:
        ...
        if method == "initialize":
            return Reply(body=json.dumps({**base, "result": {"protocolVersion": body["params"]["protocolVersion"], "capabilities": {"tools": {}}, "serverInfo": {"name": "synthetic-protocol-peer", "version": "1"}}}).encode())
        if method == "tools/list":
            return Reply(body=json.dumps({**base, "result": {"tools": [{"name": name, "inputSchema": {"type": "object"}} for name in ("add", "multiply", "fail")]}}).encode())
        assert method == "tools/call"
        name: Final = body["params"]["name"]
        if name == "fail":
            return Reply(body=json.dumps({**base, "error": {"code": -32042, "message": "synthetic JSON-RPC error"}}).encode())
        result: Final = {"content": "synthetic malformed content"} if name == "multiply" else {"content": [{"type": "text", "text": "8"}], "isError": False}
        return Reply(body=json.dumps({**base, "result": result}).encode())

    with wire_server(provider) as wire, gateway.scenario() as scenario:
        identity: Final = register_mcp(scenario, McpPeer(wire.url + "/mcp", queue.Queue()), "integration" + uuid.uuid4().hex)
        ...
        for name, expected in (("fail", "synthetic JSON-RPC error"), ("multiply", "validation")):
            ...
            assert len(tuple(item for item in wire.drain() if item.method == "POST" and json.loads(item.body).get("method") == "tools/call")) == 1
