...
def test_repeat_requests_under_one_session_name_assume_role_once_per_session_name(gateway: Gateway, tmp_path: Path) -> None:
    role: Final = "arn:aws:iam::123456789012:role/integration-" + uuid.uuid4().hex
    ...
    with wire_server(sts) as authority, wire_server(bedrock) as provider:
        ...
        empty.write_text("")
        overrides: Final = {
            "AWS_ACCESS_KEY_ID": "AKIAINTEGRATION000002", "AWS_SECRET_ACCESS_KEY": "synthetic-source-secret-key-for-testing",
            "AWS_CONFIG_FILE": str(empty), "AWS_SHARED_CREDENTIALS_FILE": str(empty), "AWS_EC2_METADATA_DISABLED": "true",
            "AWS_ENDPOINT_URL_STS": authority.url, "AWS_DEFAULT_REGION": "us-east-1", "LITELLM_RUST": "false",
        }
        with owned_proxy(gateway, tmp_path, overrides, remove_environment=tuple(name for name in os.environ if name.startswith("AWS_"))) as candidate, candidate.scenario() as scenario:
            parameters: Final = {
                "model": MODEL, "api_key": None, "aws_region_name": "us-east-1", "aws_role_name": role,
                "aws_bedrock_runtime_endpoint": provider.url, "aws_sts_endpoint": authority.url,
            }
            ...
            for model in (first_model, first_model, second_model, second_model):
                response: Final = candidate.request("POST", "/v1/chat/completions", {"model": model, "messages": [{"role": "user", "content": "synthetic cached role request"}], "cache": {"no-cache": True}})
                assert response.status_code == 200, response.text
                assert response.json()["choices"][0]["message"]["content"] == "bedrock wire control"
                assert len(provider.drain()) == 1
            assumed: Final = tuple(parse_qs(request.body.decode()) for request in authority.drain() if parse_qs(request.body.decode())["Action"] == ["AssumeRole"])
