...
def test_semantic_auto_router_traffic_is_counted_by_the_benchmarks_endpoint(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        routes: Final = json.dumps(
            {"routes": [{"name": default_model, "utterances": ["write a poem about the sea"], "score_threshold": 0.3}]}
        )
        ...
        for turn in range(2):
            body: Final = gateway.post(
                ...
                {
                    "model": router_alias,
                    "messages": [{"role": "user", "content": f"semantic turn {turn}"}],
        ...
        group: Final = next(
            object_value(entry) for entry in groups if object_value(entry)["router_name"] == router_alias
        )
