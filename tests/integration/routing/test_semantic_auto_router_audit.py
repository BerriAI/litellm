...
def _wait_for_workers(gateway: Gateway, alias: str, marker: str) -> None:
    ...
    def probe() -> int:
        ...
        ok: Final = sum(
            ...
            for _ in range(5)
            if httpx.post(f"{proxy_url}{endpoint}", json=body, headers=headers, timeout=30).status_code == 200
        )
        return ok

    count: Final = eventually(probe, lambda value: value == 15, seconds=60)
    assert count == 15, count


def _waited_post(gateway: Gateway, body: dict[str, JsonValue]) -> dict[str, JsonValue]:
    response: Final = eventually(
        lambda: gateway.request("POST", "/v1/chat/completions", body),
...
def test_a_matched_semantic_route_records_a_tier_decision_and_a_session_turn(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        body: Final = _waited_post(gateway, _chat_body(alias, session_id))
...
def test_a_streamed_semantic_fallback_records_a_default_fallback_decision(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        response_id: Final = eventually(lambda: turn(), lambda value: value is not None, seconds=30)
...
def test_an_anthropic_messages_turn_records_a_semantic_decision(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        message: Final = eventually(lambda: turn(), lambda result: getattr(result, "id", None) is not None, seconds=30)
...
def test_a_responses_turn_records_a_semantic_fallback_decision(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        body: Final = eventually(
            lambda: asyncio.run(turn()), lambda result: str(result["id"]).startswith("resp_"), seconds=30
        )
...
def test_streamed_semantic_turns_feed_the_benchmarks_and_session_views(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        async def turn(number: int) -> str | None:
            try:
        ...
        for number in range(2):
            response_id: Final = eventually(
                lambda number=number: asyncio.run(turn(number)),
        ...
        assert group is not None, f"semantic router missing after traffic: {gateway.get('/auto_router/benchmarks')}"
        assert group["router_type"] == "semantic", group
        assert (group["sessions"], group["turns"]) == (1, 2) and float(str(group["spend"])) > 0, group
        assert group["tier_turns"] == {route_model: 2}, group
...
def test_a_broken_embedding_call_falls_back_and_is_counted(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        warm: Final = _waited_post(gateway, _chat_body(alias, f"audit-s1-warm-{uuid.uuid4().hex}"))
        assert string_value(warm["id"])
        delete_scenario(handle)

        body: Final = _waited_post(
...
def test_session_id_edge_values_never_break_a_semantic_turn(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        control: Final = _waited_post(gateway, _chat_body(alias, f"audit-s2-control-{marker}"))
...
def test_a_failed_semantic_turn_is_not_counted_but_the_next_success_is(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        warm_session: Final = f"audit-s4-warm-{uuid.uuid4().hex}"
        warm: Final = _waited_post(gateway, _chat_body(alias, warm_session))
        assert string_value(warm["id"])
        ...
        failure: Final = eventually(
            ...
            lambda result: result.status_code == 500,
            seconds=30,
        )
        ...
        body: Final = _waited_post(gateway, _chat_body(alias, success_session, text="audit s4 success turn"))
        assert string_value(body["id"])
...
def test_a_shared_session_counts_only_the_auto_routed_turn(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        routed: Final = _waited_post(gateway, _chat_body(alias, session_id))
        assert string_value(routed["id"])
        plain: Final = _waited_post(gateway, _chat_body(plain_model, session_id))
...
def test_complexity_router_decisions_are_unchanged_by_the_semantic_fix(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        body: Final = _waited_post(gateway, _chat_body(alias, session_id, "what is 2+2"))
        assert string_value(body["id"])
...
def test_a_mixed_endpoint_burst_records_every_turn_once(gateway: Gateway) -> None:
    with gateway.scenario() as scenario:
        ...
        marker: Final = uuid.uuid4().hex
        warm: Final = _waited_post(gateway, _chat_body(alias, f"audit-c1-warm-{marker}-{uuid.uuid4().hex}"))
        assert string_value(warm["id"])
        _wait_for_workers(gateway, alias, marker)
        ...
        async def burst() -> dict[str, int]:
            ...
            async with httpx.AsyncClient(timeout=60, trust_env=False) as client:
                ...
                async def call(index: int) -> None:
                    ...
                    stream: Final = index % 2 == 0 and endpoint == "/v1/chat/completions"
                ...
                await asyncio.gather(*(call(index) for index in range(30)))
            return results
        ...
        messages_ok: Final = {sid for sid in ok_ids if int(sid.rsplit("-", 1)[-1]) % 3 == 1}
        logged_ok: Final = sorted(sid for sid in ok_ids if sid not in messages_ok)
        ...
        decision_rows: Final = eventually(
            ...
            lambda values: len(values) == len(logged_ok),
            seconds=70,
        )
        by_session: Final = {row["session_id"]: row for row in decision_rows}
        assert set(by_session) == set(logged_ok), (sorted(by_session), logged_ok)
        for session_id in logged_ok:
        ...
        def warmed_group() -> int:
            warmup_turns: Final = sum(
                ...
                for row in read_rows(
                    'SELECT turns FROM "LiteLLM_AutoRouterSession" WHERE session_id LIKE %s OR session_id LIKE %s',
                    (f"audit-probe-{marker}-%", f"audit-c1-warm-{marker}-%"),
                )
            )
            group: Final = _benchmarks_group(gateway, alias)
