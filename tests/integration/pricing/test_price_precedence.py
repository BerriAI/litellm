...
def test_generated_zero_null_and_omitted_prices_follow_independent_arithmetic(gateway: Gateway) -> None:
    ...
    @given(rates=st.one_of(st.sampled_from((("omitted", "omitted"), ("null", "null"))), st.tuples(st.integers(0, 25), st.integers(0, 25))))
    def check(rates: tuple[str | int, str | int]) -> None:
        if rates[0] in ("omitted", "null"):
            parameters: Final = {} if rates[0] == "omitted" else {"input_cost_per_token": None, "output_cost_per_token": None}
            input_rate, output_rate = 0.00000015, 0.0000006
        else:
            assert isinstance(rates[0], int) and isinstance(rates[1], int)
            input_rate, output_rate = rates[0] / 1_000_000, rates[1] / 1_000_000
            parameters = {"input_cost_per_token": input_rate, "output_cost_per_token": output_rate}
        with gateway.scenario() as scenario:
            model: Final = scenario.model(**parameters)
            response: Final = gateway.request("POST", "/v1/chat/completions", {
                "model": model, "messages": [{"role": "user", "content": f"independent price {uuid.uuid4().hex}"}],
            })
            ...
            rows: Final = eventually(lambda: read_rows(
                ...
                (response.json()["id"],),
            ), lambda values: len(values) == 1, seconds=70)
...
def test_same_upstream_aliases_keep_distinct_prices_after_reload(gateway: Gateway) -> None:
    for order in (("free", "paid"), ("paid", "free")):
        with gateway.scenario() as scenario:
            rates: Final = {"free": {"input_cost_per_token": 0, "output_cost_per_token": 0}, "paid": {"input_cost_per_token": 0.001, "output_cost_per_token": 0.003}}
            ...
            for generation in range(2):
                for kind in order if generation == 0 else reversed(order):
                    ...
                    response: Final = gateway.request("POST", "/v1/chat/completions", {"model": model, "messages": [{"role": "user", "content": f"alias price {model} {generation}"}]})
                    assert response.status_code == 200, response.text
                    assert response.json()["usage"]["total_tokens"] == 40
                    if cost:
                        assert float(response.headers["x-litellm-response-cost"]) == pytest.approx(cost)
                    rows: Final = eventually(lambda response=response: read_rows('SELECT spend, metadata FROM "LiteLLM_SpendLogs" WHERE request_id=%s', (response.json()["id"],)), lambda values: len(values) == 1, seconds=70)
