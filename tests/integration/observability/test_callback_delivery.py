...
@pytest.mark.covers("other.observability.callbacks.credentials_stay_out_of_event_bodies", "other.observability.callbacks.concurrent_results_join_complete_events_and_rows")
def test_concurrent_success_and_failure_join_callbacks_and_rows_without_credentials(gateway: Gateway, tmp_path: Path) -> None:
    ...
    def upstream(request: Request) -> Reply:
        ...
        if text.endswith("failure"):
            return Reply(status=400, body=json.dumps({"error": {"type": "invalid_request_error", "code": "synthetic_failure", "message": "synthetic callback failure"}}).encode())
        return Reply(body=json.dumps({"id": text, "object": "chat.completion", "created": 1, "model": "gpt-4o-mini", "choices": [{"index": 0, "message": {"role": "assistant", "content": text}, "finish_reason": "stop"}], "usage": {"prompt_tokens": 11, "completion_tokens": 4, "total_tokens": 15}}).encode())
    ...
    with wire_server(upstream) as provider, wire_server(sink) as endpoint:
        ...
        with owned_proxy(gateway, tmp_path, {"GENERIC_LOGGER_ENDPOINT": endpoint.url, "GENERIC_LOGGER_HEADERS": f"Authorization=Bearer {sink_secret}"}, config=path) as candidate, candidate.scenario() as scenario:
            model: Final = scenario.model(api_base=provider.url + "/v1", api_key=secret, input_cost_per_token=0.001, output_cost_per_token=0.002)
            key: Final = scenario.key(models=[model])
            tags: Final = tuple(f"{marker}-{index}-{'failure' if index % 2 else 'success'}" for index in range(4))
            def request(tag: str):
                return candidate.request("POST", "/v1/chat/completions", {"model": model, "messages": [{"role": "user", "content": tag}], "metadata": {"tags": [tag]}, "cache": {"no-cache": True}}, key=key)
            ...
            def delivered() -> tuple[dict, ...]:
                ...
                return tuple(event for batch in batches for event in json.loads(batch.body) if any(tag in event.get("request_tags", []) for tag in tags))
            ...
            assert {tuple(tag for tag in event["request_tags"] if tag in tags) for event in events} == {(tag,) for tag in tags}
            for tag, response in zip(tags, responses, strict=True):
                ...
                rows: Final = eventually(lambda identity=event["id"]: read_rows('SELECT request_id, spend, prompt_tokens, completion_tokens, request_tags FROM "LiteLLM_SpendLogs" WHERE request_id=%s', (identity,)), lambda values: len(values) == 1, seconds=70)
                saved_tags: Final = json.loads(rows[0]["request_tags"]) if isinstance(rows[0]["request_tags"], str) else rows[0]["request_tags"]
