...
class TestBedrockGovCloudSupport:
    ...
    def test_govcloud_model_pricing_verification(self):
        ...
        assert (
            gov_east_pricing["input_cost_per_token"] == 3.6e-06
        )  # 0.0000036 (20% higher)
        assert (
            gov_east_pricing["output_cost_per_token"] == 1.8e-05
        )  # 0.000018 (20% higher)
        assert (
            gov_west_pricing["input_cost_per_token"] == 3.6e-06
        )  # 0.0000036 (20% higher)
        assert (
            gov_west_pricing["output_cost_per_token"] == 1.8e-05
        )  # 0.000018 (20% higher)

        # Verify the pricing difference is exactly 20%
        assert (
            gov_east_pricing["input_cost_per_token"]
            == base_pricing["input_cost_per_token"] * 1.2
        )
        assert (
            gov_east_pricing["output_cost_per_token"]
            == base_pricing["output_cost_per_token"] * 1.2
        )
        assert (
    ...
    def test_govcloud_completion_cost_calculation(self, mock_completion):
        ...
        assert (
            abs(gov_east_cost - base_cost * 1.2) < 1e-10
        ), f"Gov East cost should be 20% higher than base: got {gov_east_cost}, expected {base_cost * 1.2}"
        assert (
            abs(gov_west_cost - base_cost * 1.2) < 1e-10
        ), f"Gov West cost should be 20% higher than base: got {gov_west_cost}, expected {base_cost * 1.2}"
        ...
        assert (
            abs(large_gov_cost - large_base_cost * 1.2) < 1e-10
        ), f"Large gov cost should be 20% higher than base: got {large_gov_cost}, expected {large_base_cost * 1.2}"
    ...
    def test_govcloud_cost_per_token_with_region(self):
        ...
        assert (
            abs(gov_east_prompt_cost - base_prompt_cost * 1.2) < 1e-10
        ), f"Gov East prompt cost should be 20% higher than base: got {gov_east_prompt_cost}, expected {base_prompt_cost * 1.2}"
        assert (
            abs(gov_east_completion_cost - base_completion_cost * 1.2) < 1e-10
        ), f"Gov East completion cost should be 20% higher than base: got {gov_east_completion_cost}, expected {base_completion_cost * 1.2}"
        assert (
            abs(gov_west_prompt_cost - base_prompt_cost * 1.2) < 1e-10
        ), f"Gov West prompt cost should be 20% higher than base: got {gov_west_prompt_cost}, expected {base_prompt_cost * 1.2}"
        assert (
            abs(gov_west_completion_cost - base_completion_cost * 1.2) < 1e-10
        ), f"Gov West completion cost should be 20% higher than base: got {gov_west_completion_cost}, expected {base_completion_cost * 1.2}"
        ...
        expected_base_total = (
            expected_base_prompt_cost + expected_base_completion_cost
        )  # 0.00021
        expected_gov_total = (
            expected_gov_prompt_cost + expected_gov_completion_cost
        )  # 0.000252
        ...
        assert (
            abs(gov_east_total_cost - base_total_cost * 1.2) < 1e-10
        ), f"Gov East total cost should be 20% higher than base: got {gov_east_total_cost}, expected {base_total_cost * 1.2}"
        assert (
            abs(gov_west_total_cost - base_total_cost * 1.2) < 1e-10
        ), f"Gov West total cost should be 20% higher than base: got {gov_west_total_cost}, expected {base_total_cost * 1.2}"
