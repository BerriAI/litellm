...
async def test_openai_realtime_direct_call_no_intent():
    ...
    ), "Failed to establish successful connection to OpenAI"
    assert (
        websocket_client.received_session_created
    ), "Did not receive session.created response from OpenAI"
    assert (
        len(websocket_client.messages_received) > 0
    ), "No messages received from OpenAI"
...
async def test_openai_realtime_direct_call_with_intent():
    ...
    class RealTimeWebSocketClient:
        ...
        async def send_text(self, message):
            ...
            try:
                msg_data = json.loads(message)
                print(
                    f"Sent to OpenAI (with intent): {msg_data.get('type', 'unknown')}"
                )
            except json.JSONDecodeError:
                pass
    ...
    assert (
        websocket_client.connection_successful
    ), "Failed to establish successful connection to OpenAI (with intent)"
    assert (
        websocket_client.received_session_created
    ), "Did not receive session.created response from OpenAI (with intent)"
    assert (
        len(websocket_client.messages_received) > 0
    ), "No messages received from OpenAI (with intent)"

    # Validate the structure of the session.created response
    session_message = websocket_client.messages_received[0]
    assert (
        session_message["type"] == "session.created"
    ), f"Expected session.created, got {session_message.get('type')} (with intent)"
    assert (
        "session" in session_message
    ), "session.created response missing session object (with intent)"
    assert (
        "id" in session_message["session"]
    ), "Session object missing id field (with intent)"
    assert (
        "model" in session_message["session"]
    ), "Session object missing model field (with intent)"

    print(
        f"✅ Successfully validated OpenAI realtime API response structure (with intent=chat)"
    )
