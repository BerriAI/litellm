...
export default function SSOSettings() {
  ...
  const renderSimpleValue = (value?: string | null) =>
    value ? value : <span className="text-gray-400 italic">{t("settingsPages.sSOSettings.notConfigured")}</span>;

  const renderTeamMappingsField = (values: SSOSettingsValues) => {
    if (!values.team_mappings?.team_ids_jwt_field) {
      return <span className="text-gray-400 italic">{t("settingsPages.sSOSettings.notConfigured")}</span>;
    }
  ...
  const providerConfigs = {
    ...
    microsoft: {
      ...
      fields: [
        ...
        {
          label: t("settingsPages.sSOSettings.tenant"),
    ...
    okta: {
      ...
      fields: [
        ...
        isTeamMappingsEnabled
          ? {
              label: t("settingsPages.sSOSettings.teamIdsJwtField"),
  ...
  const renderSSOSettings = () => {
    ...
    return (
      <Descriptions bordered {...descriptionsConfig}>
        <Descriptions.Item label={t("settingsPages.sSOSettings.provider")}>
                  ...
                  {isSSOConfigured && (
                    <>
                      <Button icon={<Edit className="w-4 h-4" />} onClick={() => setIsEditModalVisible(true)}>
                        {t("settingsPages.sSOSettings.editSSOSettings")}
                      </Button>
                      <Button
                        danger
                      ...
                      >
                        {t("settingsPages.sSOSettings.deleteSSOSettings")}
                      </Button>
                    </>
                  )}
                </div>
