...
export function MCPServerPermissions({
  ...
  const grantsAllTeamMcps = mcpServers.includes(ALL_TEAM_MCPS_SENTINEL);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <Badge
          color={blocksAllMcpServers ? "red" : grantsAllProxyMcps ? "green" : grantsAllTeamMcps ? "cyan" : "blue"}
          size="xs"
        >
          {blocksAllMcpServers ? "Blocked" : grantsAllProxyMcps ? "All" : grantsAllTeamMcps ? "Team" : totalCount}
        </Badge>
      </div>
      ...
      {blocksAllMcpServers ? (
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-cyan-50 border border-cyan-200">
          ...
          <Text className="text-cyan-700 text-sm">
            All team MCP servers — access to every MCP server this key&apos;s team can reach, tracking the team as its
            grant changes
          </Text>
        </div>
      ) : totalCount > 0 ? (
