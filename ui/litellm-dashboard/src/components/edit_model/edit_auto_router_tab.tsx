...
import { Card, Form, Button, Tooltip, Typography, Select as AntdSelect, message } from "antd";
...
const EditAutoRouterTab: React.FC<EditAutoRouterTabProps> = ({
  ...
  useEffect(() => {
    const loadModels = async () => {
      try {
        ...
        console.log("Fetched models for auto router edit:", uniqueModels);
  ...
  // Initialize form values and router config when localModelData changes
  useEffect(() => {
    if (localModelData) {
      ...
      if (autoRouterConfig) {
        try {
          ...
          parsedConfig = typeof autoRouterConfig === 'string' 
      ...
      const defaultModel = localModelData.litellm_params?.auto_router_default_model;
      const embeddingModel = localModelData.litellm_params?.auto_router_embedding_model;
      
      setShowCustomDefaultModel(defaultModel && !modelInfo.some(m => m.model_group === defaultModel));
      setShowCustomEmbeddingModel(embeddingModel && !modelInfo.some(m => m.model_group === embeddingModel));
    }
  }, [localModelData, modelInfo]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <Title level={3}>Auto Router Configuration</Title>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {isEditing ? (
            <Form.Item name="auto_router_name" className="mb-0">
              <TextInput 
