...
describe("SurveyModal", () => {
  ...
  it("should render nothing when isOpen is false", () => {
    renderWithProviders(<SurveyModal isOpen={false} onClose={vi.fn()} onComplete={vi.fn()} />);
    expect(screen.queryByText(/Are you using LiteLLM at your company\?/i)).not.toBeInTheDocument();
  });

  it("should render step 1 when the modal is opened", () => {
    renderWithProviders(<SurveyModal isOpen={true} onClose={vi.fn()} onComplete={vi.fn()} />);
    expect(screen.getByText(/Are you using LiteLLM at your company\?/i)).toBeInTheDocument();
  });
  ...
  it("should navigate to the company name step when Yes is selected and Next is clicked", async () => {
    ...
    expect(screen.getByText(/What company are you using LiteLLM at\?/i)).toBeInTheDocument();
  });
  ...
  describe("when step 4 (reasons) is reached", () => {
    ...
    it("should enable Next when a standard reason is selected", async () => {
      ...
      await navigateToStep4(user);
      await user.click(screen.getByRole("button", { name: /Stars, contributors, forks, community support/i }));
  ...
  it("should call onComplete after successfully submitting the form", async () => {
    ...
    const user = userEvent.setup();
    renderWithProviders(<SurveyModal isOpen={true} onClose={vi.fn()} onComplete={onComplete} />);
  ...
  it("should call onClose when the close button is clicked", async () => {
    const onClose = vi.fn();
    const user = userEvent.setup();
    renderWithProviders(<SurveyModal isOpen={true} onClose={onClose} onComplete={vi.fn()} />);
