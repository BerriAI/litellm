...
// Keys the backend rejects alongside tier_definitions. Mirrors
// validate_complexity_router_config_write (litellm/router_utils/auto_router_model_naming.py);
// add new backend rejections here. Drift shows inline via the pre-save dry-run of that same
// validator, but only this list can auto-drop a stored key so the save succeeds.
...
// A plain map: rows are the definition list; a blank definition on a built-in name inherits it.
...
// The inverse, for the edit modal. A built-in name keeps its canonical key as the row id so
// Restore and the explicit exit recognize it; other rows get positional ids.
...
// Everything an edited tier set forces onto the wire, shared by both builders so the invariant
// set has one owner; the floor resolves from row id to the row's current name.
...
import { scopeTierParamsToRows } from "./custom_tier_set";
...
export const buildComplexityRouterConfig = ({
  ...
  const serializedTierModelConfigs = serializeTierModelConfigs(
    customTierSet
      ...
      : tiers,
    customTierSet ? scopeTierParamsToRows(tierModelParams, customTierSet.tiers) : tierModelParams,
  );
