...
export { activeTierNames, getCustomTierRowsError, isBuiltInTierName } from "./custom_tier_set";
...
interface ComplexityRouterConfigProps {
  ...
  /** Parent-owned: this component unmounts on section collapse and editor mode must survive. */
...
// Row IDS the floor may point at: the backend rejects a floor whose tier has no models.
...
const ComplexityRouterConfig: React.FC<ComplexityRouterConfigProps> = ({
  ...
  // The single owner of every tier-set write: reconciles both row-id pointers (fallback
  // re-points, floor clears) and issues the one onChange. Mode transitions are explicit.
  const commitTierRows = (rows: TierDraft[], fallbackTierId: string, base: ComplexityRouterConfigValue = value) => {
    ...
    onChange({ ...base, plan_mode_min_tier, custom_tier_set: { tiers: rows, fallback_tier_id } });
  };
  ...
  // The one way back to built-in mode; models map back by row id.
  const exitToBuiltInTiers = () => {
    ...
    onChange({
      ...rest,
      plan_mode_min_tier: (TIER_KEYS as string[]).includes(value.plan_mode_min_tier ?? "")
        ? value.plan_mode_min_tier
        : undefined,
      tier_model_params: scopeTierParamsToRows(
        value.tier_model_params,
        TIER_KEYS.map((tier) => ({ name: tier })),
      ),
      tiers: {
            ...
            children: (
              <ClassificationMethodConfig
                ...
                onFallbackTierChange={
                  customTierSet ? (fallbackTierId) => commitTierRows(customTierSet.tiers, fallbackTierId) : undefined
                }
              />
            ),
          },
