...
const AttachmentTable: React.FC<AttachmentTableProps> = ({
  ...
  const columns: ColumnDef<PolicyAttachment>[] = [
    {
      header: t("policies.attachmentTable.attachmentId"),
    ...
    {
      header: t("policies.attachmentTable.policy"),
    ...
    {
      header: t("policies.attachmentTable.scope"),
      accessorKey: "scope",
      cell: ({ row }) => {
        const attachment = row.original;
        if (attachment.scope === "*") {
          return (
            <Badge color="amber" size="xs">
              {t("policies.attachmentTable.globalScope")}
            </Badge>
          );
        }
    ...
    {
      header: t("policies.attachmentTable.teams"),
      accessorKey: "teams",
      cell: ({ row }) => {
        const attachment = row.original;
    ...
    {
      header: t("policies.attachmentTable.keys"),
      accessorKey: "keys",
      cell: ({ row }) => {
        const attachment = row.original;
    ...
    {
      header: t("policies.attachmentTable.models"),
      accessorKey: "models",
      cell: ({ row }) => {
        const attachment = row.original;
    ...
    {
      header: t("policies.attachmentTable.tags"),
      accessorKey: "tags",
      cell: ({ row }) => {
        const attachment = row.original;
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex space-x-2">
            ...
            {isAdmin && (
              <Tooltip title={t("policies.attachmentTable.deleteAttachment")}>
                <Icon
                  icon={TrashIcon}
                  size="sm"
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-8 text-center">
                  <div className="text-center text-gray-500">
                    <p>{t("policies.attachmentTable.noAttachmentsFound")}</p>
                  </div>
                </TableCell>
              </TableRow>
            )}
