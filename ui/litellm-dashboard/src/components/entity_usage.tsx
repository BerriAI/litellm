                  ...
                  <Grid numItems={5} className="gap-4 mt-4">
                    <Card>
                      ...
                      <Text className="text-2xl font-bold mt-2">
                        {formatCurrency(spendData.metadata.total_spend.toFixed(2))}
                      </Text>
                    </Card>
                    <Card>
                          ...
                          customTooltip={({ payload, active }) => {
                            ...
                            return (
                              <div className="bg-white p-4 shadow-lg rounded-lg border">
                                ...
                                <p className="text-cyan-500">Total Spend: {formatCurrency(data.metrics.spend.toFixed(2))}</p>
                                  ...
                                  {Object.entries(data.breakdown.entities || {}).map(([entity, entityData]) => {
                                    ...
                                    return (
                                      <p key={entity} className="text-sm text-gray-600">
                                        {metrics.metadata.team_alias || entity}: {formatCurrency(metrics.metrics.spend.toFixed(2))}
                                      </p>
                                    );
                                  })}
                                </div>
                    ...
                    <div className="flex flex-col space-y-2">
                      ...
                      <Grid numItems={2}>
                        ...
                        <BarChart
                          ...
                          categories={["metrics.spend"]}
                          colors={["cyan"]}
                          valueFormatter={(value) => `${formatCurrency(value.toFixed(4))}`}
                          layout="vertical"
                          showLegend={false}
                          yAxisWidth={100}
                        />
                            ...
                            {getEntityBreakdown()
                              ...
                              .map((entity) => (
                                <TableRow key={entity.metadata.id}>
                                  ...
                                  <TableCell>{formatCurrency(entity.metrics.spend.toFixed(4))}</TableCell>
                          ...
                          <TableBody>
                            {getProviderSpend().map((provider) => (
                              <TableRow key={provider.provider}>
                                ...
                                <TableCell>{formatCurrency(provider.spend.toFixed(2))}</TableCell>
                                <TableCell className="text-green-600">
