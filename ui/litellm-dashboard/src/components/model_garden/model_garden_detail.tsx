...
import { ProviderGroup, ModelInfo, formatCost, formatContextWindow, getCapabilities, getModeLabel, getProviderLogo } from "./model_garden_utils";
...
const LogoWithFallback: React.FC<{ src: string; name: string; size?: number }> = ({ src, name, size = 40 }) => {
  ...
  if (hasError || !src) {
    return (
      <div style={{
        ...
        fontSize: size * 0.4, fontWeight: 600, color: "#6b7280", flexShrink: 0,
      }}>
        {name?.charAt(0) || "?"}
      </div>
    );
...
const ModelGardenDetailView: React.FC<ModelGardenDetailViewProps> = ({ group, onBack }) => {
  const [activeTab, setActiveTab] = useState("overview");
  const [modelSearch, setModelSearch] = useState("");

  const chatModels = useMemo(() => {
    return group.models
      .filter((m) => m.mode === "chat" || m.mode === "responses" || m.mode === "completion")
      .filter((m) => !modelSearch || m.key.toLowerCase().includes(modelSearch.toLowerCase()));
  ...
  return (
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      <div onClick={onBack} style={{
        ...
        color: "#5f6368", cursor: "pointer", fontSize: 14, marginBottom: 24,
      }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 8 }}>
        <LogoWithFallback src={group.logo} name={group.displayName} size={40} />
        ...
        <div style={{ display: "flex", gap: 0 }}>
          {tabs.map((tab) => (
            <div key={tab.key} onClick={() => setActiveTab(tab.key)} style={{
              ...
              cursor: "pointer", fontWeight: activeTab === tab.key ? 500 : 400, marginBottom: -1,
            }}>
              {tab.label}
            </div>
          ))}
        ...
        <div style={{ display: "flex", gap: 64 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #dadce0" }}>
                  ...
                  <th style={{ textAlign: "left", padding: "12px 0", color: "#5f6368", fontWeight: 500 }}>{group.displayName}</th>
                </tr>
              </thead>
              <tbody>
          ...
          <div style={{ width: 240, flexShrink: 0 }}>
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ fontSize: 13, color: "#202124", wordBreak: "break-all" }}>{group.provider}</div>
            </div>
            <div style={{ marginBottom: 28 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {group.capabilities.map((cap) => (
                    <span key={cap} style={{
                      ...
                      border: "1px solid #dadce0", color: "#3c4043", backgroundColor: "#fff",
                    }}>{cap}</span>
                  ))}
                </div>
              </div>
      ...
      {activeTab === "models" && (
        <div>
          <div style={{ marginBottom: 16 }}>
            <Input placeholder="Search models..." prefix={<SearchOutlined style={{ color: "#9ca3af" }} />}
      ...
      {models.slice(0, 100).map((model) => {
        ...
        return (
          <tr key={model.key} style={{ borderBottom: "1px solid #f1f3f4" }}>
            <td style={{ padding: "10px 12px", color: "#202124", fontFamily: "monospace", fontSize: 12 }}>{model.key}</td>
            <td style={{ padding: "10px 12px", color: "#3c4043" }}>{model.mode}</td>
            <td style={{ padding: "10px 12px", color: "#202124", textAlign: "right" }}>{formatCost(model.input_cost_per_token)}</td>
            <td style={{ padding: "10px 12px", color: "#202124", textAlign: "right" }}>{formatCost(model.output_cost_per_token)}</td>
            <td style={{ padding: "10px 12px", color: "#202124", textAlign: "right" }}>{formatContextWindow(model.max_input_tokens)}</td>
            <td style={{ padding: "10px 12px" }}>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                {caps.slice(0, 3).map((c) => (
                  <span key={c} style={{ fontSize: 10, padding: "1px 6px", borderRadius: 8, backgroundColor: "#f0f9ff", color: "#1d4ed8" }}>{c}</span>
                ))}
              </div>
            </td>
          </tr>
