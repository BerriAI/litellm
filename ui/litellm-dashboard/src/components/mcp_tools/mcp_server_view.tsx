...
export const MCPServerView: React.FC<MCPServerViewProps> = ({
  ...
  const getTransportBadge = (transport: string) => {
    ...
    return <span className={`inline-flex items-center text-sm font-medium px-2.5 py-0.5 rounded border ${colorClass}`}>{label}</span>;
  };
  ...
  const getAuthBadge = (authType: string) => {
    ...
    return <span className={`inline-flex items-center text-sm font-medium px-2.5 py-0.5 rounded border ${colorClass}`}>{authType}</span>;
  };
