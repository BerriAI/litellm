...
describe("ShadowEvalSection", () => {
  ...
  it("gives every active job its own card with a stop button, with the form still offered", () => {
    mockHooks({
      jobs: [
        ...
        job({ job_id: "job-b", status: "pending", api_key_id: "key-b" }),
      ],
    });
    render(<ShadowEvalSection accessToken="token" />);
  ...
  it("shows a failure line instead of loading forever when the detail fetch errors", () => {
    mockHooks({
      jobs: [job({ status: "completed", completed_count: 12, results: null })],
      detailsById: {},
      detailError: true,
    });
    render(<ShadowEvalSection accessToken="token" />);
  ...
  it("never claims no verdicts for a judged job whose results have not loaded yet", () => {
    mockHooks({ jobs: [job({ status: "completed", completed_count: 12, results: null })], detailsById: {} });
    render(<ShadowEvalSection accessToken="token" />);
