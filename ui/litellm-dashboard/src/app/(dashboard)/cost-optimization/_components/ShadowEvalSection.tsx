...
const isActive = (job: ShadowEvalJob): boolean => job.status === "pending" || job.status === "running";
...
const emptyResultsText = (job: ShadowEvalJob, resultsError: boolean): string => {
  ...
  if (job.completed_count === 0) return "No verdicts were recorded for this job.";
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {usd(job.cost_actual)} judge spend
...
const KeySelect: React.FC<{ value: string; onChange: (token: string) => void }> = ({ value, onChange }) => {
  ...
  const { data, isPending, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteKeys(50, {
...
const StartForm: React.FC<{ accessToken: string | null }> = ({ accessToken }) => {
  ...
  const valid = Boolean(accessToken) && filled && percentageValid;
  const startBody = {
    api_key_id: apiKeyId,
  ...
  return (
    <Card size="sm">
      ...
      <CardContent className="space-y-3">
        ...
        <Button disabled={!valid || start.isPending} onClick={() => start.mutate(startBody)}>
...
const previousSummary = (job: ShadowEvalJob): string => {
  ...
  return job.completed_count > 0 ? "view results" : "no verdicts";
};

const PreviousJob: React.FC<{ accessToken: string | null; job: ShadowEvalJob }> = ({ accessToken, job }) => {
  const [expanded, setExpanded] = useState(false);
  const { data: detail, isError } = useShadowEvalJob(accessToken, expanded ? job.job_id : null);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {usd(shown.cost_actual)} judge spend · {new Date(shown.created_at).toLocaleDateString()}
            </p>
          </div>
        </div>
...
const PreviousJobs: React.FC<{ accessToken: string | null; jobs: readonly ShadowEvalJob[] }> = ({
  accessToken,
  jobs,
}) => {
...
const JobCard: React.FC<{ accessToken: string | null; job: ShadowEvalJob; readOnly: boolean }> = ({
  accessToken,
  job,
  readOnly,
}) => {
  const { data: detail, isError } = useShadowEvalJob(accessToken, job.job_id);
...
const ShadowEvalSection: React.FC<{ accessToken: string | null }> = ({ accessToken }) => {
  const { data: jobs, error, isPending } = useShadowEvalJobs(accessToken);
  const { isViewOnly } = useAuthorized();
  const active = (jobs ?? []).filter(isActive);
  const finished = (jobs ?? []).filter((job) => !isActive(job));
  const showcased = active.length > 0 ? active : finished.slice(0, 1);
  const listed = finished.filter((job) => !showcased.includes(job));
  ...
  return (
    <div className="space-y-4">
      ...
      {showcased.map((job) => (
        <JobCard key={job.job_id} accessToken={accessToken} job={job} readOnly={isViewOnly} />
      ))}

      {!isViewOnly && <StartForm accessToken={accessToken} />}

      <PreviousJobs accessToken={accessToken} jobs={listed} />
    </div>
  );
};
