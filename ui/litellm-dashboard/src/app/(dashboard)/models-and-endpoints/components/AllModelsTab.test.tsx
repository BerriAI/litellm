...
describe("AllModelsTab", () => {
  ...
  describe("URL state", () => {
    it("reads search, page, page size, sort, team, access group and view from the URL", async () => {
      ...
      renderTab(
        {},
        "?model_search=claude&page=2&page_size=25&sort_by=model_name&sort_order=desc&filter_team=team-1&access_group=sales-team&view_mode=all",
      );
    ...
    it("clamps a hand-edited ?page= and ?page_size= into the range the table offers", () => {
      renderTab({}, "?page=0&page_size=5000");

      expect(lastModelsInfoCall().page).toBe(1);
      expect(lastModelsInfoCall().size).toBe(50);
    });

    it("keeps the default page size when ?page_size= is not a number", () => {
      renderTab({}, "?page_size=lots");
    ...
    it("writes the view mode to ?view_mode= and drops the key for the default view", async () => {
      const user = userEvent.setup();
      ...
      await waitFor(() => expect(lastUrl(onUrlUpdate).get("view_mode")).toBe("all"));
      expect(screen.getByTestId("models-view-select")).toHaveTextContent("All Available Models");
    ...
    it("writes the access group filter to ?access_group= and drops it when the chip is removed", async () => {
      ...
      await waitFor(() => expect(lastUrl(onUrlUpdate).has("access_group")).toBe(false));
    ...
    it("clears every table key and the model group in a single URL update from the drawer reset", async () => {
      ...
      expect(lastUrl(onUrlUpdate).toString()).toBe("");
