...
const AgentInfoView: React.FC<AgentInfoViewProps> = ({ agentId, onClose, accessToken, isAdmin }) => {
  ...
  const fetchAgentInfo = async () => {
    ...
    try {
      ...
      MessageManager.error(t("agents.details.loadFailed"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleUpdate = async (values: any) => {
    ...
    try {
      ...
      MessageManager.success(t("agents.details.updated"));
      setIsEditing(false);
      fetchAgentInfo();
    } catch (error) {
      console.error("Error updating agent:", error);
      MessageManager.error(t("agents.details.updateFailed"));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (!agent) {
    return (
      <div className="p-4">
        <div className="text-center">{t("agents.details.notFound")}</div>
        <TremorButton onClick={onClose} className="mt-4">
          {t("agents.details.backToList")}
        </TremorButton>
      </div>
    );
  }
  ...
  if (selectedKey) {
    return (
      <KeyInfoView
        ...
        teams={null}
        backButtonText={t("agents.details.backToAgent")}
      />
    );
  }
  ...
  return (
    <div className="p-4">
      <div>
        ...
        <Title>{agent.agent_name || t("agents.details.unnamed")}</Title>
        <Text className="text-gray-500 font-mono">{agent.agent_id}</Text>
      </div>

      <TabGroup>
        <TabList className="mb-4">
          <Tab key="overview">{t("agents.details.overview")}</Tab>
          {isAdmin ? <Tab key="settings">{t("agents.details.settings")}</Tab> : <></>}
        </TabList>
        ...
        <TabPanels>
          ...
          <TabPanel>
            <Descriptions bordered column={1}>
              <Descriptions.Item label={t("agents.details.id")}>{agent.agent_id}</Descriptions.Item>
              <Descriptions.Item label={t("agents.details.name")}>{agent.agent_name}</Descriptions.Item>
              <Descriptions.Item label={t("agents.details.displayName")}>
                {agent.agent_card_params?.name || "-"}
              </Descriptions.Item>
              <Descriptions.Item label={t("agents.details.description")}>
                {agent.agent_card_params?.description || "-"}
              </Descriptions.Item>
              <Descriptions.Item label="URL">{agent.agent_card_params?.url || "-"}</Descriptions.Item>
              <Descriptions.Item label={t("agents.details.version")}>
                {agent.agent_card_params?.version || "-"}
              </Descriptions.Item>
              <Descriptions.Item label={t("agents.details.protocolVersion")}>
                {agent.agent_card_params?.protocolVersion || "-"}
              </Descriptions.Item>
              <Descriptions.Item label={t("agents.details.streaming")}>
                {agent.agent_card_params?.capabilities?.streaming ? t("agents.details.yes") : t("agents.details.no")}
              </Descriptions.Item>
              {agent.agent_card_params?.capabilities?.pushNotifications && (
                <Descriptions.Item label={t("agents.details.pushNotifications")}>
                  {t("agents.details.yes")}
                </Descriptions.Item>
              )}
              {agent.agent_card_params?.capabilities?.stateTransitionHistory && (
                <Descriptions.Item label={t("agents.details.stateTransitionHistory")}>
                  {t("agents.details.yes")}
                </Descriptions.Item>
              )}
              <Descriptions.Item label={t("agents.details.skills")}>
                {t("agents.details.configuredSkills", { count: agent.agent_card_params?.skills?.length || 0 })}
              </Descriptions.Item>
              {agent.litellm_params?.model && (
                <Descriptions.Item label={t("agents.details.model")}>{agent.litellm_params.model}</Descriptions.Item>
              )}
              {agent.litellm_params?.make_public !== undefined && (
                <Descriptions.Item label={t("agents.details.makePublic")}>
                  {agent.litellm_params.make_public ? t("agents.details.yes") : t("agents.details.no")}
                </Descriptions.Item>
              )}
              {agent.agent_card_params?.iconUrl && (
                <Descriptions.Item label={t("agents.details.iconUrl")}>
                  {agent.agent_card_params.iconUrl}
                </Descriptions.Item>
              )}
              {agent.agent_card_params?.documentationUrl && (
                <Descriptions.Item label={t("agents.details.documentationUrl")}>
                  {agent.agent_card_params.documentationUrl}
                </Descriptions.Item>
              )}
              <Descriptions.Item label={t("agents.details.tpmLimit")}>
                {agent.tpm_limit ?? t("agents.details.unlimited")}
              </Descriptions.Item>
              <Descriptions.Item label={t("agents.details.rpmLimit")}>
                {agent.rpm_limit ?? t("agents.details.unlimited")}
              </Descriptions.Item>
              <Descriptions.Item label={t("agents.details.sessionTpmLimit")}>
                {agent.session_tpm_limit ?? t("agents.details.unlimited")}
              </Descriptions.Item>
              <Descriptions.Item label={t("agents.details.sessionRpmLimit")}>
                {agent.session_rpm_limit ?? t("agents.details.unlimited")}
              </Descriptions.Item>
              <Descriptions.Item label={t("agents.details.createdAt")}>
                {formatDate(agent.created_at)}
              </Descriptions.Item>
              <Descriptions.Item label={t("agents.details.updatedAt")}>
            ...
            {agent.object_permission &&
              (agent.object_permission.mcp_servers?.length ||
                ...
                <div style={{ marginTop: 24 }}>
                  <Title>{t("agents.details.mcpPermissions")}</Title>
                  <Descriptions bordered column={1} style={{ marginTop: 16 }}>
                    {agent.object_permission.mcp_servers && agent.object_permission.mcp_servers.length > 0 && (
                      <Descriptions.Item label={t("agents.details.mcpServers")}>
                    ...
                    {agent.object_permission.mcp_access_groups &&
                      agent.object_permission.mcp_access_groups.length > 0 && (
                        <Descriptions.Item label={t("agents.details.mcpAccessGroups")}>
                          {agent.object_permission.mcp_access_groups.join(", ")}
                        </Descriptions.Item>
                      )}
                    {agent.object_permission.mcp_tool_permissions &&
                      Object.keys(agent.object_permission.mcp_tool_permissions).length > 0 && (
                        <Descriptions.Item label={t("agents.details.toolsPerServer")}>
            ...
            {agent.agent_card_params?.skills && agent.agent_card_params.skills.length > 0 && (
              <div style={{ marginTop: 24 }}>
                <Title>{t("agents.details.skills")}</Title>
                <Descriptions bordered column={1} style={{ marginTop: 16 }}>
                  {agent.agent_card_params.skills.map((skill: any, index: number) => (
                    <Descriptions.Item
                      label={skill.name || t("agents.details.skill", { number: index + 1 })}
                      key={index}
                    >
                      <div>
                        <div>
                          <strong>{t("agents.details.skillId")}</strong> {skill.id}
                        </div>
                        <div>
                          <strong>{t("agents.details.skillDescription")}</strong> {skill.description}
                        </div>
                        <div>
                          <strong>{t("agents.details.skillTags")}</strong>{" "}
                          {Array.isArray(skill.tags) ? skill.tags.join(", ") : skill.tags}
                        </div>
                        {skill.examples && skill.examples.length > 0 && (
                          <div>
                            <strong>{t("agents.details.skillExamples")}</strong>{" "}
            ...
            <TabPanel>
              <Card>
                <div className="flex justify-between items-center mb-4">
                  <Title>{t("agents.details.agentSettings")}</Title>
                  {!isEditing && (
                    <TremorButton
                      onClick={() => {
                ...
                {isEditing ? (
                  <Form form={form} layout="vertical" onFinish={handleUpdate}>
                    <Form.Item label={t("agents.details.id")}>
                    ...
                    <Divider />
                    <Title className="mb-4">{t("agents.details.rateLimits")}</Title>
                    <div className="grid grid-cols-2 gap-4">
                      <Form.Item label={t("agents.details.tpmLimit")} name="tpm_limit">
                        <InputNumber className="w-full" min={0} placeholder={t("agents.details.unlimited")} />
                      </Form.Item>
                      <Form.Item label={t("agents.details.rpmLimit")} name="rpm_limit">
                        <InputNumber className="w-full" min={0} placeholder={t("agents.details.unlimited")} />
                      </Form.Item>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <Form.Item label={t("agents.details.sessionTpmLimit")} name="session_tpm_limit">
                        <InputNumber className="w-full" min={0} placeholder={t("agents.details.unlimited")} />
                      </Form.Item>
                      <Form.Item label={t("agents.details.sessionRpmLimit")} name="session_rpm_limit">
                        <InputNumber className="w-full" min={0} placeholder={t("agents.details.unlimited")} />
                      </Form.Item>
                    </div>
                    ...
                    <div className="flex justify-end gap-2 mt-6">
                      ...
                      >
                        {t("agents.details.cancel")}
                      </AntButton>
                      <TremorButton loading={isSaving}>{t("agents.details.save")}</TremorButton>
                    </div>
                  </Form>
                ) : (
                  <Text>{t("agents.details.editHint")}</Text>
                )}
              </Card>
            </TabPanel>
          )}
