...
class PoliciesClient(metaclass=PoliciesClientMeta):
    ...
    def _get_api_endpoint(
        ...
        if api_override is not None:
            ...
            api_endpoint = PoliciesClient.DEFAULT_MTLS_ENDPOINT
        else:
            api_endpoint = PoliciesClient._DEFAULT_ENDPOINT_TEMPLATE.format(
    ...
    def _get_universe_domain(
        ...
        """
        universe_domain = PoliciesClient._DEFAULT_UNIVERSE
    ...
    def __init__(
        ...
        *,
        ...
        ) = PoliciesClient._read_environment_variables()
        self._client_cert_source = PoliciesClient._get_client_cert_source(
            self._client_options.client_cert_source, self._use_client_cert
        )
        self._universe_domain = PoliciesClient._get_universe_domain(
        ...
        transport_provided = isinstance(transport, PoliciesTransport)
        if transport_provided:
            ...
            self._transport = cast(PoliciesTransport, transport)
            self._api_endpoint = self._transport.host

        self._api_endpoint = self._api_endpoint or PoliciesClient._get_api_endpoint(
        ...
        if "async" not in str(self._transport):
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(
                ...
                _LOGGER.debug(
                    "Created client `google.iam_v2beta.PoliciesClient`.",
                    extra={
                        "serviceName": "google.iam.v2beta.Policies",
