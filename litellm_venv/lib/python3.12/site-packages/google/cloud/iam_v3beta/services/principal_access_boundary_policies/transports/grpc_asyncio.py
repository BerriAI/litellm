...
class PrincipalAccessBoundaryPoliciesGrpcAsyncIOTransport(
    ...
    def create_principal_access_boundary_policy(
        ...
        Creates a principal access boundary policy, and
        returns a long running operation.

        Returns:
            Callable[[~.CreatePrincipalAccessBoundaryPolicyRequest],
                    Awaitable[~.Operation]]:
    ...
    def get_principal_access_boundary_policy(
        ...
        Returns:
            Callable[[~.GetPrincipalAccessBoundaryPolicyRequest],
                    Awaitable[~.PrincipalAccessBoundaryPolicy]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def update_principal_access_boundary_policy(
        ...
        Returns:
            Callable[[~.UpdatePrincipalAccessBoundaryPolicyRequest],
                    Awaitable[~.Operation]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
    ...
    def delete_principal_access_boundary_policy(
        ...
        Returns:
            Callable[[~.DeletePrincipalAccessBoundaryPolicyRequest],
                    Awaitable[~.Operation]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
    ...
    def list_principal_access_boundary_policies(
        ...
        Returns:
            Callable[[~.ListPrincipalAccessBoundaryPoliciesRequest],
                    Awaitable[~.ListPrincipalAccessBoundaryPoliciesResponse]]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        # Generate a "stub function" on-the-fly which will actually make
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.create_principal_access_boundary_policy: self._wrap_method(
                self.create_principal_access_boundary_policy,
                default_timeout=30.0,
                client_info=client_info,
            ),
            self.get_principal_access_boundary_policy: self._wrap_method(
                self.get_principal_access_boundary_policy,
            ...
            self.update_principal_access_boundary_policy: self._wrap_method(
                self.update_principal_access_boundary_policy,
                default_timeout=30.0,
                client_info=client_info,
            ),
            self.delete_principal_access_boundary_policy: self._wrap_method(
                self.delete_principal_access_boundary_policy,
                default_timeout=30.0,
                client_info=client_info,
            ),
            self.list_principal_access_boundary_policies: self._wrap_method(
                self.list_principal_access_boundary_policies,
                default_retry=retries.AsyncRetry(
                    initial=1.0,
                    maximum=10.0,
                    multiplier=1.3,
            ...
            self.search_principal_access_boundary_policy_bindings: self._wrap_method(
                self.search_principal_access_boundary_policy_bindings,
                default_retry=retries.AsyncRetry(
                    initial=1.0,
                    maximum=10.0,
                    multiplier=1.3,
...
__all__ = ("PrincipalAccessBoundaryPoliciesGrpcAsyncIOTransport",)
