...
TuningExampleOptions = Union[
    TuningExampleDict, glm.TuningExample, tuple[str, str], list[str]
]
...
def _convert_dict(data, input_key, output_key):
    ...
    try:
        ...
        raise KeyError(
            f'input_key is "{input_key}", but data has keys: {sorted(data.keys())}'
        )
    ...
    try:
        ...
        raise KeyError(
            f'output_key is "{output_key}", but data has keys: {sorted(data.keys())}'
        )
...
def encode_tuning_example(example: TuningExampleOptions, input_key, output_key):
    if isinstance(example, glm.TuningExample):
        ...
        example = glm.TuningExample(
            text_input=example[input_key], output=example[output_key]
        )
    return example
...
def make_model_name(name: AnyModelNameOptions):
    ...
    if not (name.startswith("models/") or name.startswith("tunedModels/")):
        raise ValueError(
            "Model names should start with `models/` or `tunedModels/`, got: {name}"
        )
