...
class KeyVaultClientOperationsMixin(  # pylint: disable=too-many-public-methods
    ...
    async def import_key(
        self,
        key_name: str,
        parameters: Union[_models.KeyImportParameters, JSON, IO[bytes]],
        **kwargs: Any
    ) -> _models.KeyBundle:
        """Imports an externally created key, stores it, and returns key parameters and attributes to the
        client.
    ...
    @distributed_trace_async
    async def get_key(
        self, key_name: str, key_version: str, **kwargs: Any
    ) -> _models.KeyBundle:
    ...
    def get_key_versions(
        ...
        async def extract_data(pipeline_response):
            ...
            list_of_elem = _deserialize(
                List[_models.KeyItem], deserialized.get("value", [])
            )
            if cls:
    ...
    async def restore_key(
        self,
        parameters: Union[_models.KeyRestoreParameters, JSON, IO[bytes]],
        **kwargs: Any
    ) -> _models.KeyBundle:
        """Restores a backed up key to a vault.
    ...
    def get_deleted_keys(
        ...
        async def extract_data(pipeline_response):
            deserialized = pipeline_response.http_response.json()
            list_of_elem = _deserialize(
                List[_models.DeletedKeyItem], deserialized.get("value", [])
            )
            if cls:
                list_of_elem = cls(list_of_elem)  # type: ignore
            return deserialized.get("nextLink") or None, AsyncList(list_of_elem)
    ...
    async def update_key_rotation_policy(
        self,
        key_name: str,
        key_rotation_policy: Union[_models.KeyRotationPolicy, JSON, IO[bytes]],
        **kwargs: Any
    ) -> _models.KeyRotationPolicy:
        """Updates the rotation policy for a key.
    ...
    async def get_random_bytes(
        self,
        parameters: Union[_models.GetRandomBytesRequest, JSON, IO[bytes]],
        **kwargs: Any
    ) -> _models.RandomBytes:
        """Get the requested number of bytes containing random values.
