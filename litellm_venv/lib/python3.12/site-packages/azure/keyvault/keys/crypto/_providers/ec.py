...
_PRIVATE_KEY_OPERATIONS = frozenset(
    (KeyOperation.decrypt, KeyOperation.sign, KeyOperation.unwrap_key)
)
...
class EllipticCurveCryptographyProvider(LocalCryptographyProvider):
    ...
    def supports(self, operation: KeyOperation, algorithm: "Algorithm") -> bool:
        if (
            operation in _PRIVATE_KEY_OPERATIONS
            and not self._internal_key.is_private_key()
        ):
            return False
