    ...
    if ch == "{":
        ...
        if source.match("}"):
            ...
            prop = lookup_property(
                prop_name, name, positive != negate, source, encoding=encoding
            )
...
class Property(RegexBase):
    ...
    def __init__(
        self, value, positive=True, case_flags=NOCASE, zerowidth=False, encoding=0
    ):
        RegexBase.__init__(self)
        self.value = value
        self.positive = bool(positive)
    ...
    def rebuild(self, positive, case_flags, zerowidth):
        return Property(self.value, positive, case_flags, zerowidth, self.encoding)
