...
class AllExpression(object):
    ...
    def validate_range(self, field_name):
        ...
        if self.step and self.step > value_range:
            raise ValueError(
                "the step value ({}) is higher than the total range of the "
...
class RangeExpression(AllExpression):
    ...
    def validate_range(self, field_name):
        ...
        if self.first < MIN_VALUES[field_name]:
            raise ValueError(
                "the first value ({}) is lower than the minimum value ({})".format(
                    self.first, MIN_VALUES[field_name]
                )
            )
        if self.last is not None and self.last > MAX_VALUES[field_name]:
            raise ValueError(
                "the last value ({}) is higher than the maximum value ({})".format(
                    self.last, MAX_VALUES[field_name]
                )
            )
...
class WeekdayPositionExpression(AllExpression):
    ...
    def __eq__(self, other):
        return (
            super(WeekdayPositionExpression, self).__eq__(other)
