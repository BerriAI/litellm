...
class AioRefreshableCredentials(RefreshableCredentials):
    ...
    def access_key(self):
        ...
        raise NotImplementedError(
            "missing call to self._refresh. " "Use get_frozen_credentials instead"
        )
    ...
    async def load(self):
        if "AWS_CREDENTIAL_FILE" in self._environ:
            ...
            if self.ACCESS_KEY in creds:
                ...
                return AioCredentials(access_key, secret_key, method=self.METHOD)
        else:
            return None
    ...
    async def load(self):
        ...
        if self._profile_name in available_creds:
            ...
            if self.ACCESS_KEY in config:
                ...
                return AioCredentials(access_key, secret_key, token, method=self.METHOD)
...
class AioAssumeRoleProvider(AssumeRoleProvider):
    ...
    async def _resolve_credentials_from_source(self, credential_source, profile_name):
