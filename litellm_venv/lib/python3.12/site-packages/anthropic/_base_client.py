...
class BaseClient(Generic[_HttpxClientT, _DefaultStreamT]):
    ...
    def _calculate_nonstreaming_timeout(
        ...
        if expected_time > default_time or (
            max_nonstreaming_tokens and max_tokens > max_nonstreaming_tokens
        ):
            raise ValueError(
...
class _DefaultHttpxClient(httpx.Client):
    def __init__(self, **kwargs: Any) -> None:
        ...
        if "transport" not in kwargs:
            ...
            transport_kwargs = {
                arg: kwargs[arg]
                for arg in ("verify", "cert", "trust_env", "http1", "http2", "limits")
                if arg in kwargs
            }
            ...
            proxy_mounts = {
                key: None
                if proxy is None
                else HTTPTransport(proxy=proxy, **transport_kwargs)
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if "transport" not in kwargs:
            ...
            proxy_mounts = {
                key: None
                if proxy is None
                else AsyncHTTPTransport(proxy=proxy, **transport_kwargs)
                for key, proxy in proxy_map.items()
            }
