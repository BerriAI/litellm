...
| litellm-gateway-auth | Shared axum authentication extractors and credential hashing, decoupled from concrete gateway state. |
...
Dependency direction is acyclic: `litellm-config` depends on `litellm-core`, the gateway depends on both plus `litellm-gateway-auth`, and `litellm-python-bridge` depends on the domain layers and `litellm-python-interop`. The auth and interop foundations depend on no LiteLLM domain crate.
