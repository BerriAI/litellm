...
litellm-rust has eight crates. A crate is a layer or shared foundation, not a route. Routes (ocr, realtime, chat) and providers (mistral, openai) are modules inside the layers.
...
| litellm-auth | Shared authentication contracts and secret-safe values. |
| litellm-auth-python | Python bindings for reusable authentication callbacks. Depends only on `litellm-auth` among LiteLLM crates. |
| litellm-auth-aws | Reusable AWS credential discovery, cache state, STS operations, role comparison, and SigV4 signing. |
...
Dependency direction is acyclic: `litellm-core` may depend on `litellm-auth-aws`, `litellm-auth-python` depends on `litellm-auth`, `litellm-config` depends on `litellm-core`, the gateway depends on both, and `litellm-python-bridge` depends on the domain layers and `litellm-python-interop`. The interop foundation depends on no LiteLLM domain crate.
