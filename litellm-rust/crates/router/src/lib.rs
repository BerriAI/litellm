...
//!
//! Mirrors the Python spec: a `Router` is built from a `model_list` of
//! deployments (`{ model_name, litellm_params: { model, api_key, api_base } }`)
//! and selects one per request. For now the only routing strategy is
//! `simple-shuffle` — a uniform random pick among the deployments registered
//! under the requested `model_name`. The router then calls the matching route
//! function (here: realtime) with the chosen deployment's params.
...
/// Routing strategy. Only `simple-shuffle` is implemented for now.
...
impl Router {
    ...
    /// Pick a deployment for `model` per the routing strategy.
    ///
    /// `simple-shuffle`: uniform random choice among deployments whose
    /// `model_name` matches. Returns `None` when no deployment is registered.
...
mod tests {
    ...
    #[test]
    fn simple_shuffle_stays_within_the_model_group() {
        let router = Router::new(vec![
            deployment("gpt-realtime", "key-a"),
            deployment("gpt-realtime", "key-b"),
        ]);
        // Whatever the shuffle picks, it must belong to the requested group.
        for _ in 0..20 {
            let chosen = router.get_available_deployment("gpt-realtime").unwrap();
            assert_eq!(chosen.model_name, "gpt-realtime");
            assert!(matches!(
