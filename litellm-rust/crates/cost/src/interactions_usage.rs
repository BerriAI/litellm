...
fn add(existing: Option<u64>, value: u64) -> Result<Option<u64>, UsageError> {
    existing
        .unwrap_or(0)
        .checked_add(value)
        .map(Some)
...
pub fn transform_interactions_usage_object(usage: &Value) -> Result<ChatUsage, UsageError> {
