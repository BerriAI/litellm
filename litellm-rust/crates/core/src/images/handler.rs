...
use super::types::{ImagesEditResponse, ImagesGenerationResponse, ProviderImagesEditRequest, ProviderImagesGenerationRequest};
...
pub(super) async fn execute_images_edit_provider_call(
    ...
    let mut form = reqwest::multipart::Form::new()
        .text("prompt", request.body.get("prompt").and_then(|v| v.as_str()).unwrap_or("").to_string());
    ...
    if let Some(mask) = request.mask {
        let mask_part = reqwest::multipart::Part::bytes(mask)
            .file_name("mask.png")
            .mime_str("image/png")
            .map_err(|err| CoreError::InvalidRequest(format!("failed to create mask part: {err}")))?;
    ...
    serde_json::from_value(transformed).map_err(|err| {
        CoreError::InvalidResponse(format!("invalid images edit response: {err}"))
    })
}
