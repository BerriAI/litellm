...
fn rejects_an_unknown_provider() {
    assert_eq!(
        ...
        CoreError::invalid_provider("openai".to_string())
    );
}
...
fn rejects_an_empty_or_malformed_message_list() {
    assert_eq!(
        ...
        CoreError::invalid_request("chat completions requires at least one message".to_string())
    );
    assert!(matches!(
        decline(request(
            "anthropic/claude-sonnet-4-5",
...
async fn a_forwarded_header_the_signer_computes_declines_to_python() {
    ...
    for forwarded in [
        ...
        assert!(
            matches!(error, CoreError::Request(RequestError::Unsupported(_))),
...
mod round_trip {
    ...
    async fn a_response_it_cannot_normalize_is_reported_as_already_sent() {
        ...
        assert!(
            matches!(err, CoreError::Upstream(UpstreamError::InvalidResponse(_))),
    ...
    async fn an_upstream_error_status_keeps_its_code() {
        ...
        assert!(
            matches!(
                err,
                CoreError::Upstream(UpstreamError::Http { status: 429, .. })
            ),
    ...
    async fn a_connection_that_is_never_established_declines_instead_of_failing() {
        ...
        assert!(
            matches!(err, CoreError::Request(RequestError::Connect(_))),
