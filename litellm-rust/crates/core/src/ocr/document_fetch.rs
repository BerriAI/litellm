...
async fn validate_safe_fetch_url(url: &Url) -> CoreResult<()> {
    ...
    let addresses = tokio::net::lookup_host((host, port))
        .await
        .map_err(|err| CoreError::connect(err.to_string()))?;
...
fn redirect_location(response: &reqwest::Response, url: &Url) -> CoreResult<Url> {
    ...
    url.join(location)
        .map_err(|err| CoreError::invalid_response(format!("invalid OCR document redirect: {err}")))
}
...
async fn safe_get_document_url(url: &str) -> CoreResult<(Url, reqwest::Response)> {
    ...
    let mut current_url = Url::parse(url)
        .map_err(|err| CoreError::invalid_request(format!("invalid OCR document URL: {err}")))?;
