...
use super::hooks::{OcrDuringCallRequest, OcrHooks, OcrLifecycleHooks, OcrRequestBody};
...
pub(crate) async fn perform_ocr_request(request: OcrRequest) -> Result<OcrResponseData, Error> {
    ...
    match request.provider {
        OcrProviderRequest::Mistral(params) => dispatch!(MistralOcrConfig, params),
        OcrProviderRequest::AzureAi(params) => dispatch!(AzureAiOcrConfig, params),
        OcrProviderRequest::AzureDocumentIntelligence(params) => {
            dispatch!(AzureDocumentIntelligenceOcrConfig, params)
        }
        OcrProviderRequest::VertexAi(params) => dispatch!(VertexAiOcrConfig, params),
        OcrProviderRequest::VertexAiDeepSeek(params) => {
            dispatch!(VertexAiDeepSeekOcrConfig, params)
        }
        OcrProviderRequest::ReductoV3(params) => dispatch!(ReductoParseV3Config, params),
        OcrProviderRequest::ReductoLegacy(params) => dispatch!(ReductoParseLegacyConfig, params),
    }
}
...
async fn perform_provider_ocr<C: OcrProviderConfig>(
    ...
    CallLifecycle::default()
        .run(context, request, &lifecycle_hooks, |request| {
            execute_ocr_provider_call(request, hooks.as_ref())
        })
        .await
}
...
async fn execute_ocr_provider_call<C: OcrProviderConfig>(
    ...
    let document = if config.guard_document_before_preparation() && hooks.has_guardrails() {
        let guarded = hooks
            .during_call(OcrDuringCallRequest {
                ...
                custom_llm_provider: config.provider_name().into(),
                url: url.clone(),
                body: OcrRequestBody::Document(request.document),
            })
            .await?;
        match guarded.body {
            OcrRequestBody::Document(document) => document,
            _ => return Err(request_error("guardrail.body")),
        }
    } else {
        request.document
    };
    let document = config
        ...
        .await?;
    let body = config.transform_ocr_request(&request.model, document, &request.params)?;
    let body = if !config.guard_document_before_preparation() && hooks.has_guardrails() {
        let guarded = hooks
            .during_call(OcrDuringCallRequest {
                ...
                custom_llm_provider: config.provider_name().into(),
                url: url.clone(),
                body: config.body_for_hook(body),
            })
            .await?;
        config.body_from_hook(guarded.body)?
    } else {
        body
    };
