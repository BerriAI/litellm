...
async fn bedrock_transcription_is_rejected_without_auth_feature() {
    let result = audio_transcription(AudioTranscriptionRequest {
    ...
    assert_eq!(
        result,
        Err(crate::CoreError::InvalidProvider("bedrock".to_string()))
    );
}
