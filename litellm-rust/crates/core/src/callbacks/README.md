# LiteLLM Rust callbacks

This directory contains Rust-native equivalents of LiteLLM integration hooks:
the traits, runners, and payload types. Host crates own the I/O
implementations (e.g. the gateway's `integrations/litellm_python_proxy_api`
shipper). The first supported surfaces are terminal custom loggers and
pre/during-call custom guardrails.

## File layout

Every callback surface is a folder:

- `mod.rs` contains the trait and runner
- `types.rs` contains the surface-local request, response, error, and future
  types

Do not add new flat files such as `custom_logger.rs`. Shared wire contracts
that are used by multiple surfaces can stay in `callbacks/types.rs`.
