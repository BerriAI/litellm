...
#[tokio::test]
async fn public_client_bounds_tls_handshake_with_connect_timeout() {
    ...
    let mut request = wire_request("mistral/mistral-ocr-latest", &base, json!({}));
    request.connection.timeout = Duration::from_secs(60);
    let started = Instant::now();
    let result = tokio::time::timeout(Duration::from_secs(15), super::ocr(request)).await;
    server.abort();
    let error = result
        .expect("connect deadline precedes the full request deadline")
        .unwrap_err();
    assert!(matches!(error, crate::Error::Network(_)), "{error}");
    assert!(started.elapsed() >= Duration::from_secs(9));
}

#[tokio::test]
async fn public_client_bounds_stalled_response_with_request_timeout() {
    use std::time::{Duration, Instant};
    ...
    let mut request = wire_request("mistral/mistral-ocr-latest", &base, json!({}));
    request.connection.timeout = Duration::from_millis(100);
    let started = Instant::now();
    let result = tokio::time::timeout(Duration::from_secs(3), super::ocr(request)).await;
    server.abort();
    let error = result
        .expect("request deadline bounds a connected but stalled provider")
        .unwrap_err();
    assert!(matches!(error, crate::Error::Network(_)), "{error}");
