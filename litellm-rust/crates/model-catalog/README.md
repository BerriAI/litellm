...
Each entry also deserializes into `ModelInfo`, a typed mirror of the registry entry shape, reachable via `ModelEntry::info()`. All modeled fields are optional on `ModelInfo`, including `litellm_provider` which the checked-in registry requires, so small synthetic catalogs still parse. Unknown fields are not part of `ModelInfo`; they remain on `fields()`. The repository tests require checked-in entries to use recognized fields, while the parser remains forward compatible with callers' custom fields. Building with the `schema` feature adds `schemars` derives and exposes `model_entry_json_schema()` for emitting the entry's JSON Schema. Parse and validation failures are reported by the `Error` enum in `error.rs`, while catalog logic lives in `catalog.rs`

The integration tests read the repository's catalog and backup at test time, assert every entry round-trips through `ModelInfo`, check required provider and nonnegative prices, and verify that both files match
