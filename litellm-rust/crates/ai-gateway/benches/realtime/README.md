...
The load generator is the `realtime` Cargo benchmark in this crate. The API key
is read from an environment variable so it does not appear in the process list.

```bash
# Direct to OpenAI (baseline)
cargo bench -p litellm-ai-gateway --bench realtime -- \
  --host api.openai.com --api-key-env OPENAI_API_KEY --model gpt-realtime \
  --connections 5000 --concurrency 500 --timeout 60

# Through the gateway, run once with pool ON and once with REALTIME_POOL_SIZE=0
cargo bench -p litellm-ai-gateway --bench realtime -- \
  --host gateway.example.com --api-key-env LITELLM_MASTER_KEY --model gpt-realtime \
  --connections 5000 --concurrency 500 --timeout 60
```
...
used here for 10 instances). Run 500-concurrency legs from a hosted runner with
about 8 vCPUs to avoid measuring laptop CPU, NIC, or ephemeral-port limits.
**Never commit keys.**
