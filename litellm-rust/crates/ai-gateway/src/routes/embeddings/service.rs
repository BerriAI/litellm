...
pub(crate) enum EmbeddingsResponseEnum {
    Json(Value),
}
...
pub async fn run(
    ...
    let model = body
        ...
        .ok_or_else(|| CoreError::InvalidRequest("embeddings body requires a model".to_string()))?
        .to_string();
    ...
    for (deployment_idx, deployment) in deployments.iter().enumerate() {
        ...
        let input = match body.get("input") {
            Some(Value::String(s)) => EmbeddingsInput::Single(s.clone()),
        ...
        for attempt in 0..=max_retries {
            match embeddings(request).await {
                Ok(response) => {
                    if let Some(provider) = custom_llm_provider {
                        let breaker = state.circuit_breakers.get_or_create(provider);
        ...
        tracing::warn!(
            ...
            hashed_token = %hashed_token.as_hex_str(),
            error = %err,
            event = "embeddings.deployment_failed",
    ...
    tracing::error!(
        ...
        error = %err,
        duration_secs = duration,
        event = "embeddings.all_deployments_failed",
    ...
    let cost_request = CostRequest {
        ...
        usage: cost_calculator::types::Usage {
            ...
            prompt_tokens_details: Some(cost_calculator::types::PromptTokensDetails {
                ...
                text_tokens: usage.prompt_tokens as u64,
                audio_tokens: 0,
                image_tokens: 0,
            }),
