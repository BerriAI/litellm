...
//! the connection pool ([`crate::routes::realtime::pool`]) can pre-establish an upstream,
...
/// Splice a pre-warmed upstream (taken from [`crate::routes::realtime::pool`]) to the
...
pub async fn realtime_warm<In, Out>(
    model: &str,
    handoff: crate::routes::realtime::pool::WarmHandoff,
    idle_timeout: Option<Duration>,
    observe: impl FnMut(&RealtimeEvent) + Send,
    client_in: In,
    client_out: Out,
