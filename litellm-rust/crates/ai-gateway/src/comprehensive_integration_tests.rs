...
mod tests {
    ...
    use litellm_core::router::{
        Deployment, HealthConfig, HealthMonitor, HealthStatus, LatencyTracker,
        LiteLLMParams, LoadTracker, Router, RoutingStrategy,
    };
    ...
    async fn test_latency_based_routing_full_lifecycle() {
        ...
        for _ in 0..15 {
            ...
            monitor.record_request_success(&dep1, Duration::from_millis(300)).await;
            monitor.record_request_start(&dep2).await;
            monitor.record_request_success(&dep2, Duration::from_millis(100)).await;
            monitor.record_request_start(&dep3).await;
            monitor.record_request_success(&dep3, Duration::from_millis(200)).await;
        }
        ...
        assert_eq!(monitor.get_health_status(&dep2).await, HealthStatus::Healthy);
    }
    ...
    async fn test_cost_based_routing_pricing() {
        let dep1 = create_deployment("gpt-4", "openai/gpt-4", Some(true), None, Some(0.00003), Some(0.00006));
        let dep2 = create_deployment("gpt-4", "azure/gpt-4", Some(true), None, Some(0.00002), Some(0.00004));
        let dep3 = create_deployment("gpt-4", "anthropic/gpt-4", Some(true), None, Some(0.000025), Some(0.00005));
    ...
    async fn test_health_monitoring_request_lifecycle() {
        ...
        assert_eq!(monitor.get_health_status(&deployment).await, HealthStatus::Healthy);

        // Record some failures to trigger Degraded status
        for _ in 0..3 {
            monitor.record_request_start(&deployment).await;
            monitor.record_request_failure(&deployment, Duration::from_millis(2000)).await;
        }
    ...
    async fn test_fallback_routing_unhealthy_deployments() {
        ...
        // Record latencies
        tracker.record_latency(&dep1, Duration::from_millis(100)).await;
        tracker.record_latency(&dep2, Duration::from_millis(200)).await;
        tracker.record_latency(&dep3, Duration::from_millis(300)).await;

        // Mark dep1 as unhealthy
        for _ in 0..20 {
            monitor.record_request_start(&dep1).await;
            monitor.record_request_failure(&dep1, Duration::from_millis(5000)).await;
        }

        // Verify dep1 is unhealthy
        assert_eq!(monitor.get_health_status(&dep1).await, HealthStatus::Unhealthy);

        // Filter healthy deployments
        let healthy = monitor.filter_healthy(vec![&dep1, &dep2, &dep3]).await;
        assert_eq!(healthy.len(), 2);
        assert!(healthy.iter().all(|d| d.litellm_params.model != "openai/gpt-4"));
    }

    /// Test routing strategy switching
    #[tokio::test]
    async fn test_routing_strategy_switching() {
        let dep1 = create_deployment("gpt-4", "openai/gpt-4", Some(true), Some(50), Some(0.00003), Some(0.00006));
        let dep2 = create_deployment("gpt-4", "azure/gpt-4", Some(true), Some(50), Some(0.00002), Some(0.00004));
    ...
    async fn test_empty_deployment_list() {
        ...
        assert!(router.get_available_deployment_with_latency("gpt-4").await.is_none());
        assert!(router.get_available_deployment_with_load("gpt-4").await.is_none());
        assert!(router.get_available_deployment_with_cost("gpt-4", 1000, 500).is_none());
        assert!(router.get_available_deployment_with_weight("gpt-4").is_none());
    }

    /// Test edge case: single deployment
    #[tokio::test]
    async fn test_single_deployment() {
        let dep = create_deployment("gpt-4", "openai/gpt-4", Some(true), Some(100), Some(0.00003), Some(0.00006));
    ...
    async fn test_concurrent_routing_access() {
        ...
        let deployments = vec![dep1.clone(), dep2.clone()];
        let router = Arc::new(Router::with_strategy(deployments, RoutingStrategy::LatencyBased));
        ...
        for i in 0..100 {
            ...
            let dep = if i % 2 == 0 { dep1.clone() } else { dep2.clone() };
            
            join_set.spawn(async move {
                tracker.record_latency(&dep, Duration::from_millis(100 + i)).await;
            });
        }
    ...
    async fn test_health_monitor_concurrent_access() {
        ...
        for i in 0..100 {
            ...
            join_set.spawn(async move {
                monitor.record_request_start(&deployment).await;
                if i % 10 == 0 {
                    monitor.record_request_failure(&deployment, Duration::from_millis(100)).await;
                } else {
                    monitor.record_request_success(&deployment, Duration::from_millis(100)).await;
                }
            });
