...
pub(crate) async fn execute_audio_transcription_provider_call(
    ...
    let request = sign_request(&request, &request.optional_params).await?;
    let body = serde_json::to_vec(&request.body).map_err(|error| {
        CoreError::invalid_request(format!("invalid audio request body: {error}"))
    })?;
    ...
    let response_json: Value = serde_json::from_str(&text).map_err(|error| {
        CoreError::invalid_response(format!("invalid audio response JSON: {error}"))
    })?;
    Ok(request
        .config
