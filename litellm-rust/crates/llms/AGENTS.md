...
For non-OCR pairs, order corresponding methods as parameter support/mapping, environment validation, URL construction, request transformation, and response transformation, followed by Rust-only runtime hooks. Auth resolution remains split between configs and route preparation in litellm-core. The runtime `ChatExecutionConfig::supported_openai_param_mappings` describes accepted OpenAI/provider name pairs, unlike Python's `get_supported_openai_params` name list. Audio `map_transcription_params` remains a Rust filtering helper
...
The expanded declaration-only contracts retain `BaseConfig`, `BaseAnthropicMessagesConfig`, and `BaseResponsesApiConfig`. Existing object-safe execution subsets use `ChatExecutionConfig`, `MessagesExecutionConfig`, and `ResponsesWebSocketProviderConfig` until callers migrate. Keep those runtime interfaces separate from unimplemented declarations. Provider macros emit explicit placeholder methods, not inherited default behavior. Uninhabited associated payload types mark schemas that still need translation. `translation_inventory.tsv` distinguishes declared, existing partial, and planned counterparts, with no parity claim for any of those statuses
