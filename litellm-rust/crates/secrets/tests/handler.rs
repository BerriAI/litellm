...
mod common_read_contract {
    ...
    #[tokio::test]
    async fn reads_preserve_values_and_distinguish_absence_from_failure(
        ...
        let missing = match provider {
            Provider::Aws => ResponseTemplate::new(400).set_body_json(
                json!({"__type": "ResourceNotFoundException", "Message": "missing"}),
            ),
