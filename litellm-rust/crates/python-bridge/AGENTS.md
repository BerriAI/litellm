...
- Keep this crate the product-specific PyO3 consumer of `litellm-host-python`
  ...
  - Legacy callback sharing (the caller's args, kwargs and request object, body/header roots, re-aliasing unchanged body keys) lives in `litellm-callbacks-legacy-python` behind `PublicCall` and `LegacyPythonLifecycle`; the bridge hands the public call over and keeps no copy
  - Routes never select a callback contract: `callbacks::run_python_call` is the only way a route starts a call, and `src/callbacks.rs` is the only place that names a contract crate
    - Every call runs the legacy `Logging` lifecycle, then the v1 lifecycle when anything is subscribed; with no subscriber the call is exactly the legacy-only call
    - A route describes itself once as a `CallSurface`; fields only the legacy contract reads are marked so and go when it goes
    - Retiring legacy for a route, or for all of them, is a change to that one composition, never to a route, the driver or an adapter
