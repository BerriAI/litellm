...
use litellm_core::ocr::OcrDocument;
use pyo3::prelude::*;
...
fn former_json_roundtrip_from_py(py: Python<'_>, value: &Bound<'_, PyAny>) -> OcrDocument {
    let json = py.import("json").expect("Python json module should import");
    let encoded: String = json
        .call_method1("dumps", (value,))
        .expect("OCR document should serialize")
        .extract()
        .expect("json.dumps should return a string");
    let value = serde_json::from_str::<Value>(&encoded).expect("serialized JSON should parse");
    serde_json::from_value(value).expect("OCR document should deserialize")
}

fn pythonize_from_py(value: &Bound<'_, PyAny>) -> OcrDocument {
    pythonize::depythonize(value).expect("OCR document should depythonize")
}

fn former_json_roundtrip_to_py(py: Python<'_>, value: &Value) -> Py<PyAny> {
    let json = py.import("json").expect("Python json module should import");
    let encoded = serde_json::to_string(value).expect("OCR response should serialize");
...
fn pythonize_to_py(py: Python<'_>, value: &Value) -> Py<PyAny> {
    pythonize::pythonize(py, value)
        .expect("OCR response should pythonize")
        .unbind()
}
...
fn serialization(c: &mut Criterion) {
    ...
    Python::attach(|py| {
        ...
        let mut inbound = c.benchmark_group("python_to_rust_ocr_document");
        ...
        let mut outbound = c.benchmark_group("rust_to_python_ocr_response");
