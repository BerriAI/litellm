...
| `postgresql.image.*`      | Docker image for the Postgres pods (both primary and read replicas). **Warning:** `bitnamilegacy/` images are frozen — no security patches. Override with `bitnami/postgresql` for production.                                                                 | `bitnamilegacy/postgresql:16.2.0-debian-12-r6`                                             |
| `redis.enabled`           | Deploy a Redis instance using the Bitnami redis chart.                                                                                                                                                                                                                                | `false`                                                                                    |
| `redis.image.*`           | Docker image for the Redis pods (both master and replicas). **Warning:** `bitnamilegacy/` images are frozen — no security patches. Override with `bitnami/redis` for production.                                                               | `bitnamilegacy/redis:7.2.4-debian-12-r9`                                                   |
