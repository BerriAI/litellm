...
EXPECT = {
    "00000000000000000000000000000001": {
        "label": "T01 chat 200 (team)",
        "code": 200,
        "otel": "OK",
        "team_on": [SERVER, "litellm_request", "raw_gen_ai_request"],
    },
    "00000000000000000000000000000002": {
        "label": "T02 chat 400 invalid-json",
        "code": 400,
        "otel": "ERROR",
    },
    "00000000000000000000000000000003": {
        "label": "T03 chat 401 bad-key",
        "code": 401,
        "otel": "ERROR",
    },
    "00000000000000000000000000000004": {
        "label": "T04 chat 400 unknown-model",
        "code": 400,
        "otel": "ERROR",
    },
    "00000000000000000000000000000005": {
        "label": "T05 chat 429 (team)",
        "code": 429,
        "otel": "ERROR",
        "team_on": [SERVER, FAILED],
    },
    "00000000000000000000000000000006": {
        "label": "T06 chat 500 (team)",
        "code": 500,
        "otel": "ERROR",
    },
    "00000000000000000000000000000007": {
        "label": "T07 messages 200 (team)",
        "code": 200,
        "otel": "OK",
    },
    "00000000000000000000000000000008": {
        "label": "T08 admin 200 key/generate",
        "code": 200,
        "otel": "OK",
    },
    "00000000000000000000000000000009": {
        "label": "T09 admin 500 key/generate",
        "code": 500,
        "otel": "ERROR",
    },
    "00000000000000000000000000000010": {
        "label": "T10 admin 422 key/generate",
        "code": 422,
        "otel": "ERROR",
    },
    "00000000000000000000000000000011": {
        "label": "T11 guardrail block 400",
        "code": 400,
        "otel": "ERROR",
        "guardrail": {
...
def main():
    ...
    for tid, spec in EXPECT.items():
        ...
        if server is None:
            ...
            checks.append(
                (
                    "#28405 http.response.status_code",
                    code == spec["code"] and isinstance(code, int),
                    f"{code!r}",
                )
            )
            checks.append(
                ("#28405 url.path present", bool(a.get("url.path")), a.get("url.path"))
            )
            checks.append(
                (
                    "#28405 http.route present",
                    bool(a.get("http.route")),
                    a.get("http.route"),
                )
            )
            d = duration_ms(server)
            checks.append(
                ("#28405 duration>0", d is not None and d > 0, f"{d:.1f}ms" if d else d)
            )
        ...
        for name in spec.get("team_on", []):
            ...
            ok = (
                a.get("metadata.user_api_key_team_id") == TEAM_ID
                and a.get("metadata.user_api_key_team_alias") == TEAM_ALIAS
            )
            checks.append(
                (
                    f"#28273 team attrs on {name}",
                    ok,
        ...
        if g is not None:
            ...
            if gs is None:
                ...
                checks.append(
                    (
                        "#28364 guardrail_status",
                        a.get("guardrail_status") == g["guardrail_status"],
                        a.get("guardrail_status"),
                    )
                )
                ...
                if gr is not None:
                    try:
                        ...
                        detail = (
                            "valid JSON dict" if json_ok else f"not-json: {gr[:40]}"
                        )
                    except Exception:
                ...
                if "guardrail_action" in g:
                    checks.append(
                        (
                            "#28364 guardrail_action",
                            a.get("guardrail_action") == g["guardrail_action"],
                            a.get("guardrail_action"),
                        )
                    )
                if "categories" in g:
                    ...
                    cats_ok = False
                    try:
                        cats_ok = sorted(json.loads(cats_raw)) == sorted(
                            g["categories"]
                        )
                    except Exception:
                        pass
