...
class Logging:
    ...
    def pre_call(self, input, api_key, model=None, additional_args={}):
        ...
        print_verbose(
            f"Logging Details Pre-API Call for call id {self.litellm_call_id}"
        )
            ...
            for callback in litellm.success_callback:
                try:
                    ...
                    if callback == "cache":
                        ...
                        if (
                            litellm.cache != None
                            and self.model_call_details.get("optional_params", {}).get(
                                "stream", False
                            )
                            == True
                        ):
                            ...
                            if (
                                litellm_call_id
                                in self.litellm_params["stream_response"]
                            ):
                                # append for the given call_id
                                if (
                                    self.litellm_params["stream_response"][
                                        litellm_call_id
                                ...
                                new_model_response = ModelResponse(
                                    choices=[
                                        Choices(message=Message(content="default"))
                                    ]
                                )
                            ...
                            litellm.cache.add_cache(
                                self.litellm_params["stream_response"][litellm_call_id],
                                **self.model_call_details,
                            )
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            threading.Thread(
                ...
                args=(e, traceback_exception, start_time, end_time),
            ).start()
            my_thread = threading.Thread(
...
def get_optional_params(  # use the openai defaults
    ...
    if model in litellm.anthropic_models:
        ...
        if frequency_penalty != 0:
            optional_params[
                "frequency_penalty"
            ] = frequency_penalty  # TODO: Check if should be repetition penalty
        if stop != None:
            optional_params[
                "stop"
            ] = stop  # TG AI expects a list, example ["\n\n\n\n","&lt;|endoftext|&gt;"]
        ...
        if "ai21" in model:
            ...
            if max_tokens != float("inf"):
                ...
                ] = 256  # anthropic fails without max_tokens_to_sample
            if temperature != 1:
                optional_params["temperature"] = temperature
            if top_p != 1:
                optional_params["top_p"] = top_p
        ...
        if max_tokens != float("inf"):
            ...
            optional_params[
                "max_new_tokens"
            ] = 256  # petals always needs max_new_tokens
        if temperature != 1:
            optional_params["temperature"] = temperature
        if top_p != 1:
            optional_params["top_p"] = top_p
...
def get_max_tokens(model: str):
    try:
        ...
        raise Exception(
            "This model isn't mapped yet. Add it here - https://github.com/BerriAI/litellm/blob/main/cookbook/community-resources/max_tokens.json"
        )
        ...
        try:
            ...
            for log_key in litellm.error_logs:
                ...
                if type(current_logs) == dict:
                    filtered_error_logs = {
                        key: str(value) for key, value in current_logs.items()
                    }
        ...
        if isinstance(original_exception, OriginalError):
            ...
            if custom_llm_provider == "anthropic":  # one of the anthropics
                ...
                if "length limit exceeded" in error_str:
                    exception_mapping_worked = True
                    raise ContextWindowExceededError(
                        message=error_str, model=model, llm_provider="huggingface"
                    )
                elif "A valid user token is required" in error_str:
                    exception_mapping_worked = True
                    raise InvalidRequestError(
                        message=error_str, llm_provider="huggingface", model=model
                    )
                if hasattr(original_exception, "status_code"):
                    if original_exception.status_code == 401:
                        exception_mapping_worked = True
        ...
        try:
            while True:  # loop until a non-empty string is found
                ...
                if self.custom_llm_provider and self.custom_llm_provider == "anthropic":
                    ...
                    self.model == "replicate" or self.custom_llm_provider == "replicate"
                ):
                    chunk = next(self.completion_stream)
                    ...
                    and self.custom_llm_provider == "together_ai"
                ):
                    chunk = next(self.completion_stream)
                    ...
                    self.custom_llm_provider
                    and self.custom_llm_provider == "huggingface"
                ):
                    chunk = next(self.completion_stream)
                    ...
                    self.custom_llm_provider
                    and self.custom_llm_provider == "aleph_alpha"
                    ...
                    self.custom_llm_provider
                    and self.custom_llm_provider == "text-completion-openai"
                ):
                    chunk = next(self.completion_stream)
                    completion_obj[
                        "content"
                    ] = self.handle_openai_text_completion_chunk(chunk)
                elif (
                    self.model in litellm.nlp_cloud_models
                    or self.custom_llm_provider == "nlp_cloud"
                ):
                    try:
                        chunk = next(self.completion_stream)
                    ...
                    self.custom_llm_provider and self.custom_llm_provider == "vertex_ai"
                ):
                    try:
                        chunk = next(self.completion_stream)
                    ...
                    # LOGGING
                    threading.Thread(
                        target=self.logging_obj.success_handler, args=(completion_obj,)
                    ).start()
...
def get_model_split_test(models, completion_call_id):
    ...
    try:
        ...
        response = requests.post(
            ...
            headers={"content-type": "application/json"},
            data=json.dumps(
                {"completion_call_id": completion_call_id, "models": models}
            ),
        )
        print_verbose(f"get_model_list response: {response.text}")
        data = response.json()
...
def completion_with_split_tests(
    models={}, messages=[], use_client=False, override_client=False, **kwargs
):
    """
    Example Usage:
    ...
    if use_client and not override_client:
        ...
        models, model_configs = get_model_split_test(
            models=models, completion_call_id=kwargs["id"]
        )

    try:
        selected_llm = random.choices(
            list(models.keys()), weights=list(models.values())
        )[0]
    except:
        traceback.print_exc()
        raise ValueError(
            """models does not follow the required format - {'model_name': 'split_percentage'}, e.g. {'gpt-4': 0.7, 'huggingface/wizard-coder': 0.3}"""
        )
    ...
    if model_configs != {}:
        ...
        if (
            "prompt" in selected_model_configs
        ):  # special case, add this to messages as system prompt
            messages.append(
                {"role": "system", "content": selected_model_configs["prompt"]}
            )
    ...
    return litellm.completion(
        model=selected_llm, messages=messages, use_client=use_client, **kwargs
    )
