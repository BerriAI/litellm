...
class ComplexityRouter(CustomLogger):
    ...
    async def _pick_model_for_tier(
        ...
        if not self.config.plugins:
            ...
            pool_value: Final = self.config.tiers.get(_tier_name(tier), ())
            filtered: Final = tuple(
                entry
                for entry in ((pool_value,) if isinstance(pool_value, str) else tuple(pool_value))
                if entry in eligible_models
            )
            if not filtered:
                raise ValueError(f"Empty eligible model pool for tier {_tier_name(tier)}")
    ...
    async def _gate_response_modality(
        ...
        elif self._modality_default_model_usable(decision, eligible):
            new_tier = None
    ...
    def _modality_default_model_usable(
        self, decision: StandardLoggingRoutingDecision | None, eligible: frozenset[str]
    ) -> bool:
        """default_model may serve a gated request only when it is configured, plugin-free
        (it is never checked against the plugin pipeline), capability-eligible, and the
        decision was not plan-floored (default_model carries no tier guarantee)."""
        plan_floored: Final = decision is not None and (
            decision.get("cause") == "plan_mode" or "plan_mode_floor" in (decision.get("signals") or ())
        )
        return (
            bool(self.config.default_model)
            and not self.config.plugins
            and self.config.default_model in eligible
            and not plan_floored
        )
