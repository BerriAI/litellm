...
AutoSetupObjective = Literal["cost", "task_completion_speed", "balanced"]
AutoSetupSelectionMode = Literal["snapshot_ranked", "runtime_response_latency"]


class AutoSetupCandidate(BaseModel):
    """One benchmark-backed model group in an Auto-generated tier policy."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    model_name: str = Field(min_length=1)
    benchmark_model_id: str = Field(min_length=1)
    quality_lower_bound: float = Field(ge=0, le=1)
    cost_per_completed_task_usd: float = Field(ge=0)


class AutoSetupTierPolicy(BaseModel):
    """How an Auto-generated tier chooses from its quality-admitted model groups."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    selection_mode: AutoSetupSelectionMode
    candidates: tuple[AutoSetupCandidate, ...] = Field(min_length=1)
    cold_start_model: str | None = None
    evidence_status: str = Field(min_length=1)

    @model_validator(mode="after")
    def _validate_candidates(self) -> "AutoSetupTierPolicy":
        names: Final = tuple(candidate.model_name for candidate in self.candidates)
        if len(names) != len(frozenset(names)):
            raise ValueError("Auto setup tier candidates must use distinct model_name values")
        if self.selection_mode == "runtime_response_latency":
            if self.cold_start_model not in names:
                raise ValueError("A runtime response-latency policy needs a cold_start_model in candidates")
        elif self.cold_start_model is not None:
            raise ValueError("cold_start_model is only valid for runtime_response_latency")
        return self


class AutoSetupConfig(BaseModel):
    """Provenance and selection policy stamped only by zero-setup Auto creation."""
    ...
    tier_policies: Mapping[str, AutoSetupTierPolicy]

    @field_validator("tier_policies", mode="before")
    @classmethod
    def _freeze_tier_policies(cls, value: Mapping[str, object]) -> Mapping[str, object]:
        return MappingProxyType(dict(value))

    @field_serializer("tier_policies")
    def _serialize_tier_policies(self, value: Mapping[str, AutoSetupTierPolicy]) -> Mapping[str, AutoSetupTierPolicy]:
        return dict(value)  # mutable-ok: Pydantic JSON serialization requires a concrete mapping

    @model_validator(mode="after")
    def _validate_tier_policies(self) -> "AutoSetupConfig":
        expected: Final = frozenset(tier.value for tier in TIER_SEVERITY_ORDER)
        actual: Final = frozenset(self.tier_policies)
        if actual != expected:
            raise ValueError(
                f"Auto setup tier_policies must define exactly {', '.join(tier.value for tier in TIER_SEVERITY_ORDER)}"
            )
        if self.optimize_for == "cost" and any(
            policy.selection_mode != "snapshot_ranked" for policy in self.tier_policies.values()
        ):
            raise ValueError("Cost-optimized Auto setup cannot enable runtime response-latency selection")
        return self
...
class ComplexityRouterConfig(BaseModel):
    ...
    auto_setup: AutoSetupConfig | None = Field(
        default=None,
        description=(
            "Benchmark snapshot and per-tier selection policy generated by the zero-setup Auto flow. "
            "Absent on manually configured routers, whose pool selection behavior is unchanged"
        ),
    )
    ...
    @model_validator(mode="after")
    def _validate_auto_setup(self) -> "ComplexityRouterConfig":
        setup: Final = self.auto_setup
        if setup is None:
            return self
        if self.adaptive:
            raise ValueError("auto_setup and adaptive=True cannot both select from the same tier pool")
        if self.tier_definitions is not None:
            raise ValueError("auto_setup requires the built-in four-tier complexity ladder")
        for tier_name, policy in setup.tier_policies.items():
            configured = self.tiers.get(tier_name)
            configured_names = frozenset((configured,)) if isinstance(configured, str) else frozenset(configured or ())
            policy_names = frozenset(candidate.model_name for candidate in policy.candidates)
            if not policy_names.issubset(configured_names):
                missing = ", ".join(sorted(policy_names - configured_names))
                raise ValueError(f"auto_setup tier {tier_name} references models not present in tiers: {missing}")
        return self
