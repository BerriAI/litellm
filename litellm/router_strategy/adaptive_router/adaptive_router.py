...
class AdaptiveRouter:
    ...
    async def record_efficiency(
        ...
        call. Returns the reward that was applied.
        """
        reward: Final = (
            0.0
            if is_failure
        ...
        async with self._lock:
            ...
            before: Final = self._cells[cell_key]
            after: Final = apply_efficiency_delta(before, d_alpha_eff, d_beta_eff)
            self._cells[cell_key] = after
            if after is before:
                return reward
            await self.queue.add_state_delta(
                self.router_name,
