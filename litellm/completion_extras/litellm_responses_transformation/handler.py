...
class ResponsesToCompletionBridgeHandler:
    ...
    def completion(
        ...
        # request_data already carries `custom_llm_provider` via the spread of
        # `sanitized_litellm_params`; overwriting it on the dict (rather than
        # adding an explicit kwarg) avoids the duplicate-keyword TypeError that
        # would otherwise fire on the real bridge path.
        request_data["custom_llm_provider"] = custom_llm_provider
        result = responses(
            **request_data,
            _provider_already_resolved=True,
        )
    ...
    async def acompletion(self, *args, **kwargs) -> Union["ModelResponse", "CustomStreamWrapper"]:
        ...
        # Set on request_data rather than passed as a separate kwarg to avoid the
        # duplicate-keyword TypeError when `sanitized_litellm_params` already
