...
class LiteLLMResponsesTransformationHandler(CompletionTransformationBridge):
    ...
    def _parse_raw_sse_chunk(cls, chunk: str) -> Optional[Dict[str, Any]]:
        ...
        if not stripped_chunk or stripped_chunk == "[DONE]" or stripped_chunk.startswith("event:"):
            return None
        try:
    ...
    def _update_recovered_text_only_items(
        ...
        item = recovered_output_items.get(output_index) or recovered_text_only_items.get(output_index)
        if item is None:
            item = {
                "type": "message",
