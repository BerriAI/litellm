...
class Router:
    ...
    async def _collect_stream_with_ttft_timeout(
        ...
        try:
            ...
            if stream_idle_timeout is not None:
                await self._collect_until_idle_timeout(
                    aiter, response, chunks, loop, stream_idle_timeout
                )
            else:
    ...
    async def _collect_until_first_token(
        self,
        aiter: AsyncIterator[ModelResponseStream],
        response: CustomStreamWrapper,
        chunks: list[ModelResponseStream],
        deadline: float | None,
        ttft_timeout: float | None,
            ...
            try:
                if deadline is None:
                    ...
                    chunk = await asyncio.wait_for(
                        aiter.__anext__(), timeout=deadline - loop.time()
                    )
    ...
    async def _collect_until_idle_timeout(
        self,
        aiter: AsyncIterator[ModelResponseStream],
        response: CustomStreamWrapper,
        chunks: list[ModelResponseStream],
        ...
        def _reset_stream_idle_timer() -> None:
            ...
            idle_timeout_handle = loop.call_later(
                stream_idle_timeout, _cancel_on_stream_idle_timeout
            )
