...
try:
    ...
    EnterpriseStandardLoggingPayloadSetupVAR: (
        type[EnterpriseStandardLoggingPayloadSetup] | None
    ) = EnterpriseStandardLoggingPayloadSetup
except Exception as e:
    verbose_logger.debug(
        "[Non-Blocking] Unable to import GenericAPILogger - LiteLLM Enterprise Feature - %s",
        e,
    )
...
_STANDARD_LOGGING_METADATA_KEYS: Final[frozenset] = frozenset(
...
_CUSTOM_PRICING_KEYS: Final[frozenset] = frozenset(
...
class Logging(LiteLLMLoggingBaseClass):
    ...
    def __init__(
        ...
        dynamic_async_success_callbacks: list[str | Callable | CustomLogger]
        | None = None,
        dynamic_failure_callbacks: list[str | Callable | CustomLogger] | None = None,
        dynamic_async_failure_callbacks: list[str | Callable | CustomLogger]
        | None = None,
        ...
        self.dynamic_input_callbacks: list[str | Callable | CustomLogger] | None = (
            dynamic_input_callbacks
        )
        self.dynamic_success_callbacks: list[str | Callable | CustomLogger] | None = (
            dynamic_success_callbacks
        )
        self.dynamic_async_success_callbacks: (
            list[str | Callable | CustomLogger] | None
        ) = dynamic_async_success_callbacks
        self.dynamic_failure_callbacks: list[str | Callable | CustomLogger] | None = (
        ...
        self._trusted_callback_vars: tuple[tuple[str, str], ...] = (
            get_trusted_callback_params(kwargs)
        )
            ...
            if (
                ...
                if callback == "datadog":
                    ...
                    _custom_logger_init_args = {
                        k: v
                        for k, v in self._trusted_callback_vars
                        if k.startswith("dd_")
                    }
    ...
    def _auto_detect_prompt_management_logger(
        ...
        """
        prompt_management_loggers: Final = (
    ...
    def pre_call(self, input, api_key, model=None, additional_args={}):
        ...
        try:
            ...
            if (
                ...
                _litellm_params: Final = self.model_call_details.get(
                    "litellm_params", {}
                )
            ...
            if getattr(self, "logger_fn", None) and callable(self.logger_fn):
                try:
                    ...
                    verbose_logger.exception(
                        "LiteLLM.LoggingError: [Non-Blocking] Exception occurred while logging %s",
                        e,
                    )
            ...
            callbacks: Final = litellm.input_callback + (
                self.dynamic_input_callbacks or []
            )
            for callback in callbacks:
                try:
                    ...
                    verbose_logger.exception(
                        "litellm.Logging.pre_call(): Exception occured - %s", e
                    )
                    verbose_logger.debug(
                        "LiteLLM.Logging: is sentry capture exception initialized %s",
                        capture_exception,
                    )
    ...
    def _get_request_curl_command(
        self, api_base: str, headers: dict | None, additional_args: dict, data: dict
    ) -> str:
    ...
    async def async_post_mcp_tool_call_hook(
        ...
        post_mcp_tool_call_response_obj: Final[MCPPostCallResponseObject] = (
    ...
    def _response_cost_calculator(
        ...
        transformed_result: Final = self._generate_content_result_as_model_response(
            result
        )
        ...
        try:
            ...
            verbose_logger.debug(
                "response_cost_failure_debug_information: %s", debug_info
            )
        ...
        try:
            response_cost: Final = litellm.response_cost_calculator(
                **response_cost_calculator_kwargs
            )

            verbose_logger.debug("response_cost: %s", response_cost)
            additional_response_cost: Final[object] = self.model_call_details.get(
                "additional_response_cost"
            )
            if (
                isinstance(additional_response_cost, (int, float))
                and additional_response_cost > 0
            ):
    ...
    def _generate_content_result_as_model_response(
        self, result: object
    ) -> ModelResponse | None:
        """
        ...
        if self.call_type not in (
            ...
            return None
        if isinstance(result, ModelResponse) or not isinstance(
        ...
        try:
            ...
            verbose_logger.debug(
                "generate_content response cost normalization failed: %s", e
            )
            return None
    ...
    def _is_sync_litellm_request(litellm_params: dict) -> bool:
        ...
        return (
            ...
            and litellm_params.get(CallTypes.allm_passthrough_route.value, False)
            is not True
            and litellm_params.get(CallTypes.aanthropic_messages.value, False)
            is not True
            and litellm_params.get(CallTypes.agenerate_content.value, False) is not True
            and litellm_params.get(CallTypes.agenerate_content_stream.value, False)
            is not True
        )
    ...
    def should_run_callback(
        ...
        if litellm_params.get("no-log", False) is True:
            ...
            if not (
                ...
                verbose_logger.debug(
                    "no-log request, skipping logging for %s event", event_hook
                )
                return False
        ...
        if (
            ...
            verbose_logger.debug(
                "Callback %s disabled via x-litellm-disable-callbacks header for %s event",
                callback,
                event_hook,
            )
            return False
    ...
    def normalize_logging_result(self, result: Any) -> Any:
        ...
        if self.call_type == CallTypes.arealtime.value and isinstance(result, list):
            combined_usage_object: Final = RealtimeAPITokenUsageProcessor.collect_and_combine_usage_from_realtime_stream_results(
                results=result
            )
            logging_result = (
            ...
            provider_config: Final = (
                ProviderConfigManager.get_provider_passthrough_config(
    ...
    def _flush_passthrough_collected_chunks_helper(
        ...
        complete_streaming_response: Final = (
            provider_config.handle_logging_collected_chunks(
    ...
    def success_handler(
        ...
        verbose_logger.debug(
            "Logging Details LiteLLM-Success Call: Cache_hit=%s", cache_hit
        )
            ...
            for callback in callbacks:
                try:
                    ...
                    if (
                        ...
                        print_verbose(
                            f"success callbacks: Running Custom Callback Function - {callback}"
                        )
    ...
    async def async_success_handler(
        ...
        """
        print_verbose(
            f"Logging Details LiteLLM-Async Success Call, cache_hit={cache_hit}"
        )
        ...
        if self.call_type == CallTypes.aretrieve_batch.value and isinstance(
            ...
            is_base64_unified_file_id: Final = _is_base64_encoded_unified_file_id(
                result.id
            )
            ...
            has_explicit_batch_data: Final = all(
        ...
        if complete_streaming_response is not None:
            ...
            try:
                ...
                verbose_logger.warning(
                    "Model=%s not found in completion cost map. Setting 'response_cost' to None",
                    self.model,
                )
    ...
    def failure_handler(
        ...
        verbose_logger.debug(
            "Logging Details LiteLLM-Failure Call: %s", litellm.failure_callback
        )
        if not self.should_run_logging(
    ...
    def get_combined_callback_list(
        self, dynamic_success_callbacks: list | None, global_callbacks: list
    ) -> list:
    ...
    def _handle_anthropic_messages_response_logging(self, result: Any) -> ModelResponse:
        ...
        if (
            ...
            or isinstance(result, ModelResponse)
        ):
            return result
...
def _init_custom_logger_compatible_class(
    ...
    llm_router: Any
    | None,  # expect litellm.Router, but typing errors due to circular import
    ...
    try:
        ...
        if logging_integration == "agentops":  # Add AgentOps initialization
            ...
            _arize_otel_logger: Final = ArizeLogger(
                config=otel_config, callback_name="arize"
            )
            ...
            _levo_otel_logger: Final = LevoLogger(
                config=otel_config, callback_name="levo"
            )
            ...
            logfire_base_url: Final = os.getenv(
            ...
            if internal_usage_cache is None:
                raise Exception(
                    f"Internal Error: Cache cannot be empty - internal_usage_cache={internal_usage_cache}"
                )

            dynamic_rate_limiter_obj: Final = _PROXY_DynamicRateLimitHandler(
            ...
            bitbucket_logger: Final = BitBucketPromptManager(
        ...
        verbose_logger.exception(
            "[Non-Blocking Error] Error initializing custom logger: %s", e
        )
        return None
    return None
...
def _maybe_auto_initialize_arize_phoenix(_in_memory_loggers: list) -> None:
    ...
    try:
        ...
        phoenix_logger: Final = ArizePhoenixLogger(
            config=otel_config, callback_name="arize_phoenix"
        )
...
def get_custom_logger_compatible_class(
    ...
    try:
        ...
        verbose_logger.exception(
            "[Non-Blocking Error] Error getting custom logger: %s", e
        )
        return None
...
class StandardLoggingPayloadSetup:
    ...
    def append_system_prompt_messages(
        kwargs: dict | None = None, messages: Any | None = None
    ):
        """
        ...
        if messages is None:
            ...
            if (
                ...
                and messages[0].get("content") == system
            ):
                return messages
    ...
    def get_standard_logging_metadata(
        ...
        if litellm_params is not None:
            ...
            prompt_variables: Final = cast(
                dict | None, litellm_params.get("prompt_variables", None)
            )
    ...
    def get_usage_from_response_obj(
        response_obj: dict | None, combined_usage_object: Usage | None = None
    ) -> Usage:
        ## BASE CASE ##
    ...
    def _generate_cold_storage_object_key(
        ...
        try:
            ...
            s3_file_name: Final = (
                f"time-{start_time.strftime('%H-%M-%S-%f')}_{response_id}"
            )
    ...
    def get_error_information(
        ...
        rate_limit_category: Final = validate_rate_limit_category(
            getattr(original_exception, "category", None)
        )
        rate_limit_type: Final = validate_rate_limit_type(
            getattr(original_exception, "rate_limit_type", None)
        )
        budget_error: Final = (
            original_exception
            if isinstance(original_exception, BudgetExceededError)
            else None
        )
    ...
    def _get_request_tags(
        ...
        if metadata.get("tags", []):
            ...
            request_tags = []
        user_agent_tags: Final = StandardLoggingPayloadSetup._get_user_agent_tags(
            proxy_server_request
        )
        additional_header_tags: Final = (
            StandardLoggingPayloadSetup._get_extra_header_tags(proxy_server_request)
        )
...
def get_standard_logging_object_payload(
    ...
    try:
        ...
        metadata: Final[dict] = StandardLoggingPayloadSetup.merge_litellm_metadata(
            litellm_params
        )
        ...
        raw_usage_dict: Final = StandardLoggingPayloadSetup.get_usage_as_dict(
            ...
            combined_usage_object=cast(
                Usage | None, kwargs.get("combined_usage_object")
            ),
        )
        usage_dict: Final = (
        ...
        end_user_id: Final = clean_metadata[
            "user_api_key_end_user_id"
        ] or _request_body.get(
            "user", None
        ...
        clean_hidden_params: Final = StandardLoggingPayloadSetup.get_hidden_params(
            hidden_params
        )
        if (
        ...
        payload: Final[StandardLoggingPayload] = StandardLoggingPayload(
            ...
            system_prompt=StandardLoggingPayloadSetup.get_system_prompt_from_kwargs(
                kwargs
            ),
        )
...
def create_dummy_standard_logging_payload() -> StandardLoggingPayload:
    ...
    model_info: Final = StandardLoggingModelInformation(
    ...
    response: Final[dict[str, list[dict[str, dict[str, str]]]]] = {
