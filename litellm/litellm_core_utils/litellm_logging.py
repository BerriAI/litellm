...
class Logging(LiteLLMLoggingBaseClass):
    ...
    async def async_get_chat_completion_prompt(
        ...
        # The Anthropic cache-control hook and the vector store hook both claim the single
        # prompt-management slot; a request that seeds cache_control_injection_points (e.g. via
        # `enable_anthropic_prompt_caching`) would otherwise starve knowledge-base retrieval, so
        # run the vector store hook first when both apply to the same request.
