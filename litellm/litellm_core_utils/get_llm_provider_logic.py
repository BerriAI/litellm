...
def _is_openai_compatible_provider(model: str, custom_llm_provider: str) -> bool:
    if custom_llm_provider in litellm.openai_compatible_providers:
        return True

    if custom_llm_provider == "ai21" and model in litellm.ai21_chat_models:
        return True
    return False


def get_dynamic_openai_compatible_provider_params(  # ruff: noqa: PLR0915
    model: str,
    ...
    if custom_llm_provider == "perplexity":
        ...
        api_base = api_base or get_secret_str("DEEPINFRA_API_BASE") or "https://api.deepinfra.com/v1/openai"  # type: ignore
        dynamic_api_key = api_key or get_secret_str("DEEPINFRA_API_KEY")
    elif custom_llm_provider == "empower":
        api_base = (
            api_base
            or get_secret_str("EMPOWER_API_BASE")
        ...
        api_base = (
            api_base
            or get_secret_str("NVIDIA_NIM_API_BASE")
        ...
        api_base = (
            api_base
            or get_secret_str("CEREBRAS_API_BASE")
        ...
        api_base = (
            api_base
            or get_secret_str("AI21_API_BASE")
        ...
        api_base = (
            api_base
            or get_secret_str("CODESTRAL_API_BASE")
        ...
        api_base = api_base or get_secret_str("AZURE_AI_API_BASE")  # type: ignore
        ...
        api_base = api_base or get_secret_str("GITHUB_API_BASE") or "https://models.inference.ai.azure.com"  # type: ignore
        dynamic_api_key = api_key or get_secret_str("GITHUB_API_KEY")
    elif custom_llm_provider == "litellm_proxy":
        api_base = api_base or get_secret_str("LITELLM_PROXY_API_BASE") or "https://models.inference.ai.azure.com"  # type: ignore
        ...
        api_base = (
            api_base
            or get_secret_str("FRIENDLI_API_BASE")
...
def get_llm_provider(
    ...
    try:
        ...
        is_openai_compatible_provider = False
        ...
        if (
            ...
            model = model.split("/", 1)[1]
            is_openai_compatible_provider = _is_openai_compatible_provider(
                model, custom_llm_provider
            )

            if not is_openai_compatible_provider:
                if api_base is not None and not isinstance(api_base, str):
                    raise Exception(
            ...
            for endpoint in litellm.openai_compatible_endpoints_dict:
                if endpoint in api_base:
                    custom_llm_provider = litellm.openai_compatible_endpoints_dict[
                        endpoint
                    ]
        ...
        if is_openai_compatible_provider and custom_llm_provider:
            return get_dynamic_openai_compatible_provider_params(
