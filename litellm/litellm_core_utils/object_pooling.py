...
class GenericPooledObjectFactory(PooledObjectFactory):
    ...
    def __init__(
        self,
        object_class: Type[T],
        pooled_maxsize: Optional[
            int
        ] = None,  # None = unlimited pool with eviction-based memory control
...
def get_object_pool(
    ...
    factory = GenericPooledObjectFactory(
        object_class=object_class, pooled_maxsize=pooled_maxsize, least_one=least_one
    )
