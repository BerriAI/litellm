...
``Ok`` and ``Error`` are separate classes joined by a ``Union`` alias, so
reaching for ``result.ok`` before eliminating the ``Error`` arm (via
``isinstance`` or a ``match`` pattern) is a pyright error, not a runtime
``AttributeError``. The single-class ``expression.Result`` this replaces
declared both payload fields on one class, which made unguarded access
invisible to tooling.

Both variants are covariant and frozen; the absent side defaults to
``Never`` so a bare ``Ok(value)`` or ``Error(err)`` infers fully and is
assignable to any ``Result`` whose matching side fits.

``is_ok``/``is_error`` are runtime predicates only; they do not narrow the
union for the type checker. Inside the strictly-checked translation tree,
discriminate with ``match`` or ``isinstance``.
"""
