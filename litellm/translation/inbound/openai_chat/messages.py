...
def _convert_turn(message: WireMessage) -> Union[_Turn, TranslationError, None]:
...
def _user_blocks(
...
) -> Union[List[ContentBlock], TranslationError]:
    if message.content is None:
        return []
    if isinstance(message.content, str):
    ...
    for part in message.content:
        if isinstance(part, TextPartIn):
            block = ContentBlock.of_text(Text(text=part.text, cache=cache_of(part.cache_control)))
        else:
...
def _image_block(part: ImagePartIn) -> Union[ContentBlock, TranslationError]:
    ...
    if url.startswith("https://"):
        return ContentBlock.of_image(Image(source=ImageSource.of_url(UrlSource(url=url)), cache=cache))
    if url.startswith("http://"):
        return TranslationError.of_unsupported("http:// image URLs require a download; v1 handles them")
...
def _base64_source(url: str, fmt: Optional[str]) -> Union[ImageSource, TranslationError]:
...
def _assistant_blocks(
    ...
    if message.function_call is not None:
        return TranslationError.of_unsupported("legacy assistant.function_call; v1 handles it")
...
def _thinking_ir(part: Union[ThinkingPartIn, RedactedThinkingPartIn]) -> ContentBlock:
...
def _assistant_content(message: AssistantMessageIn) -> List[ContentBlock]:
    ...
    if message.content is None:
        return []
    return [_assistant_part_ir(part) for part in message.content if _keeps_assistant_part(part)]
...
def _tool_use(call: ToolCallIn) -> Union[ContentBlock, TranslationError]:
    ...
    if call.id.startswith("srvtoolu_"):
        return TranslationError.of_unsupported("server-tool call reconstruction (srvtoolu_*); v1 handles it")
...
def _arguments_of(raw: Optional[str]) -> Union[JsonBlob, TranslationError]:
    if raw is None or not raw.strip():
        return JsonBlob(value={})
    try:
        value: PlainJson = json.loads(raw)
    except ValueError:
        return TranslationError.of_unsupported("malformed tool_call arguments need v1's JSON repair")
