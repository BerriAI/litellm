...
async def exec_code(req: ExecReq):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if lang in ("py", "python"):
            ...
            return ExecResp(ok=(rc == 0), rc=rc, stdout=out, stderr=err, error=None if rc == 0 else err).model_dump() | {"t_ms": t_ms}
        ...
        if lang in ("asm", "nasm"):
            ...
            if rc2 != 0:
                return ExecResp(ok=False, rc=rc2, stdout=out2, stderr=err2, error=err2)
            rc, out, err, t_ms = await _run([str(td_path / "a.out")], None, timeout)
            return ExecResp(ok=(rc == 0), rc=rc, stdout=out, stderr=err, error=None if rc == 0 else err).model_dump() | {"t_ms": t_ms}
