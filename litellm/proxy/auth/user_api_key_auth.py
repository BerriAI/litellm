...
def update_valid_token_with_end_user_params(
    ...
    # end_user_max_budget must be written back on every cache-hit path so that
    # the freshly-resolved DB value is never shadowed by a stale cached budget
    # from a different end-user who previously shared the same virtual key.
