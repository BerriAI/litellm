        ...
        if valid_token is not None:
            ...
            if not skip_budget_checks:
                with tracer.trace("litellm.proxy.auth.budget_checks"):
                    ...
                    key_spend_requests: Dict[str, float] = {}
                    if valid_token.max_budget is not None:
                        key_spend_requests[f"spend:key:{valid_token.token}"] = (
                            valid_token.spend or 0.0
                        )
                    _append_budget_window_spend_counter_requests(
                        counter_requests=key_spend_requests,
                        prefix=f"spend:key:{valid_token.token}",
                        budget_limits=valid_token.budget_limits,
                    )
                    await prefetch_spend_counters(key_spend_requests)
