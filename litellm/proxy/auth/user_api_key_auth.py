...
from litellm.proxy.auth.auth_checks import (
    ...
    can_key_call_model, common_checks, get_end_user_object, get_key_object,
...
async def _execute_post_auth_filters(
    request: Request,
    api_key: str,
    user_api_key_auth: UserAPIKeyAuth,
) -> UserAPIKeyAuth:
    """
    Execute auth_filter policies after standard authentication.

    This function runs all registered auth_filter guardrails, passing them:
    - The original request object
    - The API key used for authentication
    - The validated UserAPIKeyAuth object from standard auth
    ...
    Args:
        ...
        user_api_key_auth: The validated UserAPIKeyAuth from standard auth

    Returns:
        UserAPIKeyAuth: The potentially enriched/modified auth object

    Raises:
        HTTPException: If an auth filter blocks the request
    """
    from litellm.proxy.guardrails.guardrail_registry import \
        IN_MEMORY_GUARDRAIL_HANDLER

    try:
        # Get all auth_filter guardrails from memory
        auth_filters = []
        for (
            guardrail_id,
            guardrail_obj,
        ) in IN_MEMORY_GUARDRAIL_HANDLER.guardrail_id_to_custom_guardrail.items():
            # Check if this is an auth_filter guardrail
            from litellm.proxy.guardrails.guardrail_hooks.auth_filter import \
                AuthFilterGuardrail

            if isinstance(guardrail_obj, AuthFilterGuardrail):
                auth_filters.append(guardrail_obj)

        verbose_proxy_logger.debug(
            f"Found {len(auth_filters)} auth_filter guardrails to execute"
        )

        # Execute each auth filter in order
        current_auth = user_api_key_auth
        for auth_filter in auth_filters:
            verbose_proxy_logger.debug(
                f"Executing auth_filter: {auth_filter.guardrail_name}"
            )

            result = await auth_filter.execute_auth_filter(
                request=request,
                api_key=api_key,
                user_api_key_auth=current_auth,
            )

            # If filter returned a modified auth object, use it for next filter
            if result is not None:
                current_auth = result
                verbose_proxy_logger.debug(
                    f"Auth_filter '{auth_filter.guardrail_name}' modified auth object"
                )

        return current_auth

    except HTTPException:
        # Re-raise HTTP exceptions from auth filters (blocks)
        raise
    except Exception as e:
        verbose_proxy_logger.error(
            f"Error executing post-auth filters: {e}", exc_info=True
        )
        # Don't block on errors - continue with original auth
...
async def _user_api_key_auth_builder(  # noqa: PLR0915
    ...
    try:
        ...
        if valid_token is not None:
            ...
            ### POST-DB AUTH FILTER POLICIES ###
            # Execute auth_filter guardrails after standard authentication
            # Auth filters can enrich, validate, or block based on custom logic
            valid_token = await _execute_post_auth_filters(
                request=request,
                api_key=api_key,
                user_api_key_auth=valid_token,
            )
