...
class OrganizationRoleBasedAccessChecks:
    ...
    def organization_role_based_access_check(
        ...
        passed_organization_id: Optional[str] = request_body.get(
        ...
        if route in LiteLLMRoutes.org_admin_only_routes.value:
            _user_organizations, _user_organization_role_mapping = (
                OrganizationRoleBasedAccessChecks.get_user_organization_info(
                    user_object
                )
            )
    ...
    @staticmethod
    def _user_is_org_admin_in_requested_org(
        request_data: dict,
        user_object: Optional[LiteLLM_UserTable] = None,
    ) -> bool:
        """
        Helper function to check if user is an org admin for the passed organization_id in request_data

        Args:
            request_data (dict): The request data containing the organization_id
            user_object (LiteLLM_UserTable): The user object containing organization memberships

        Returns:
            bool: True if user is an org admin for the passed organization_id, False otherwise
        """
        if request_data.get("organization_id", None) is None:
            return False

        _user_is_org_admin = OrganizationRoleBasedAccessChecks._user_is_admin_in_org(
            user_object=user_object,
            organization_id=request_data.get("organization_id", None),
        )

        return _user_is_org_admin

    @staticmethod
    def _user_is_admin_in_org(
        user_object: Optional[LiteLLM_UserTable] = None,
        organization_id: Optional[str] = None,
    ) -> bool:
        """
        Returns True if user is an admin in the passed organization_id

        Args:
            user_object (LiteLLM_UserTable): The user object containing organization memberships
            organization_id (str): The organization_id to check if the user is an admin in

        Returns:
            bool: True if user is an admin in the passed organization_id, False otherwise
        """
        if user_object is None:
            return False
        ...
        for _membership in user_object.organization_memberships:
            if _membership.organization_id == organization_id:
