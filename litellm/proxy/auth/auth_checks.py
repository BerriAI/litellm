...
def _copy_user_api_key_auth_for_cache(
    ...
    # Strip per-request end-user fields so they are not reused by a
    # different end-user whose request later hits the same cache entry.
    # Without this, the first request to populate the cache for a given
    # virtual key contaminates the cached UserAPIKeyAuth with that
    # end-user's budget limits (end_user_max_budget, etc.).  Subsequent
    # requests sharing the same virtual key then have their spend checked
    # against the wrong budget, causing spurious BudgetExceededErrors.
    # See https://github.com/BerriAI/litellm/issues/29142
    ...
    copied_key_obj.end_user_object_permission = None
