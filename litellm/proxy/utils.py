...
async def _patch_eval_spend_log(
    prisma_client: Any,
    request_id: str,
    eval_info: Any,
    _retried: bool = False,
) -> None:
    """Patch eval_information into an existing spend log row.

    The DB-logger callback that writes spend logs runs concurrently with
    async_post_call_success_hook. If the row isn't committed yet, we sleep
    500 ms and retry once — covering the common timing gap without busy-looping.
    """
    import asyncio
    import json as _json

    try:
        existing = await prisma_client.db.litellm_spendlogs.find_unique(
        ...
        if existing is None:
            if not _retried:
                ...
                await _patch_eval_spend_log(
                    prisma_client, request_id, eval_info, _retried=True
                )
                return
            verbose_proxy_logger.warning(
                f"[Evals] Spend log row {request_id} not found after retry; eval_information not persisted"
            )
            return
        meta = existing.metadata or {}
        if isinstance(meta, str):
            meta = _json.loads(meta)
        meta["eval_information"] = eval_info
        await prisma_client.db.litellm_spendlogs.update(
            where={"request_id": request_id},
            data={"metadata": _json.dumps(meta)},
        )
    except Exception as _e:
        verbose_proxy_logger.warning(
            f"[Evals] Failed to patch spend log {request_id}: {_e}"
        )
            ...
            try:
                ...
                if _agent_id:
                    ...
                    if _prisma_client is not None:
                        ...
                        # Patch the spend log — eval runs after the spend logger fires,
                ...
                if isinstance(e, HTTPException):
                    # Even on block, patch the spend log so Logs UI shows the failed eval scores
                    _eval_info = (data.get("metadata") or {}).get("eval_information")
                    _request_id = (
                        getattr(response, "id", None)
                        or data.get("id")
