...
async def _create_spend_logs_with_poison_isolation(
    ...
    try:
        ...
        if len(rows) == 1:
            ...
            return attempts_left - 1
        mid = len(rows) // 2
        remaining = await _create_spend_logs_with_poison_isolation(
            repo, rows[:mid], attempts_left - 1
        )
        return await _create_spend_logs_with_poison_isolation(
            repo, rows[mid:], remaining
        )
...
async def _resolve_db_access_group_model_names_for_utils(
    user_api_key_dict: "UserAPIKeyAuth",
    llm_router: Optional["Router"],
    prisma_client: Optional["PrismaClient"],
    team_id: Optional[str] = None,
) -> set[str]:
    """Resolve actual model names from DB-based access groups.

    Used by get_available_models_for_user to complement config-level
    model_access_groups resolution.
    """
    if prisma_client is None:
        return set()

    # Use explicit team_id if provided, otherwise get from user key
    target_team_ids = []
    if team_id:
        target_team_ids = [team_id]
    elif user_api_key_dict.team_id:
        target_team_ids.append(user_api_key_dict.team_id)

    # Also check user_teams
    if not target_team_ids:
        user_teams = getattr(user_api_key_dict, "user_teams", None)
        if user_teams:
            for t in user_teams:
    ...
    from litellm.repositories.team_repository import TeamRepository
    from litellm.db_models import LiteLLM_TeamTable

    teams = await TeamRepository(prisma_client).table.find_many(
        where={"team_id": {"in": target_team_ids}}
    )
    teams_typed = [LiteLLM_TeamTable(**t.model_dump()) for t in teams]

    all_ag_ids: set[str] = set()
    for team in teams_typed:
        if team.access_group_ids:
            all_ag_ids.update(team.access_group_ids)

    if not all_ag_ids:
        return set()

    ag_rows = await AccessGroupRepository(prisma_client).table.find_many(
        where={"access_group_id": {"in": list(all_ag_ids)}}
    )

    model_names: set[str] = set()
    for row in ag_rows:
        if row.access_model_names:
            model_names.update(row.access_model_names)
...
async def get_available_models_for_user(
    ...
    # Also resolve DB-based access group models (complements config-level
    # model_access_groups which are already resolved above).
    db_ag_names = await _resolve_db_access_group_model_names_for_utils(
        user_api_key_dict=user_api_key_dict,
        llm_router=llm_router,
        prisma_client=prisma_client,
        team_id=effective_team_id,
    ...
    if db_ag_names:
        all_models = list(set(all_models + list(db_ag_names)))
