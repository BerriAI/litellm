...
The token minted by `lite login` is a real virtual key, scoped to the user and team you authenticated as. It inherits that user's and team's models and budgets, and is enforced on the proxy exactly like any other key on the same team (guardrails, routing, logging, spend). Spend is tracked against the shared team and user budgets, so running several agents (or logging in more than once) does not hand each session its own separate budget; they all draw down the same team/user allowance. There is no separate per-session cap, so sustained agent use is not capped at a small chat-session limit.

The key has a bounded duration (default 24h, configurable via `LITELLM_CLI_JWT_EXPIRATION_HOURS`). Because it's a real virtual key, it shows up in the Admin UI's Keys page (alias `cli-{user_id}`) and an admin can revoke or regenerate it at any time, same as any other key; `lite logout` also revokes it directly. On an Enterprise deployment, `lite auth print-token` (usable as Claude Code's `apiKeyHelper`) silently rotates the key's secret in place via `/sso/cli/refresh` before it expires, so a long-running session doesn't need a fresh browser login. On a non-Enterprise deployment silent rotation isn't available; run `lite login` again once the key expires, which also re-reads your latest team and user settings. `lite claude`, `lite codex`, and `lite opencode` work with it on a default deployment; `EXPERIMENTAL_UI_LOGIN` is not required. If you need a separately managed key that isn't tied to your SSO login (for example to avoid sharing the team-wide budget), create a dedicated virtual key in the dashboard and pass it via `--api-key` or `LITELLM_PROXY_API_KEY` instead.
