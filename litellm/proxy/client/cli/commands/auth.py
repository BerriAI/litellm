    ...
    if teams:
        ...
        if selected_team:
            ...
            if team_id:
                ...
                if success:
                    click.echo(
                        f"✅ Your CLI key is now associated with team: {selected_team.get('team_alias', team_id)}"
                    )
                    click.echo(
                        f"🎯 You can now access models: {', '.join(selected_team.get('models', ['All models']))}"
                    )
                else:
                    click.echo(
                        "⚠️ Key assignment failed, but you can still use the CLI"
                    )
            else:
                click.echo(
                    "ℹ️ Continuing without team assignment. You can assign a team later using the CLI."
                )
    else:
        click.echo(
                ...
                if response.status_code == 200:
                    ...
                    if data.get("status") == "ready":
                        ...
                        if api_key:
                            ...
                            _handle_team_assignment(
                                base_url, api_key, data.get("user_id")
                            )
