...
token_cache = DualCache(default_in_memory_ttl=2 * 60 * 60 - 30)


def request_sign(
    client_id, client_secret, base_url, method, path, url, params, json_body, headers
):
...
def get_token(client_id, client_secret, base_url):
    ...
    if (
        response_dict.get("error_response") is None
        and response_dict["code"] == "0"
        and response_dict["msg"] == "SUCCESS"
    ):
...
def _build_params(
    client_id, client_secret, path, biz_model, params_model, app_token=None
):
    """构建所有的请求参数
