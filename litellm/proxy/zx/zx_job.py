...
def data_to_nocobase(datas, table, filter):
    ...
    params = {"pageSize": 1, "page": 1, "filter": filter}
    req = urllib.request.Request(
        f"{NOCOBASE_API_URL}/{table}:list?{urllib.parse.urlencode(params)}",
        headers=headers,
        method="GET",
    )
    with urllib.request.urlopen(req) as response:
...
async def litellm_get_user_daily_activity_aggregated(start_date, end_date):
    ...
    return await internal_user_endpoints.get_user_daily_activity_aggregated(
...
async def litellm_user_daily_activity(start_date, end_date):
                ...
                if b.metadata and b.metadata.key_alias and "@" in b.metadata.key_alias:
                    ...
                    if key in data:
                        data[key] = {
                            ...
                            "cost": data[key]["cost"]
                            + key_entry["cost"],  # Cumulative spend
                            "api_key_reqs": data[key]["api_key_reqs"]
                            + key_entry["api_key_reqs"],  # Cumulative api_requests
                        }
                    else:
                        data[key] = key_entry
    ...
    user_api_key_dict = UserAPIKeyAuth(user_role=LitellmUserRoles.PROXY_ADMIN)
    res = await internal_user_endpoints.get_user_daily_activity_aggregated(
        start_date=start_date,
        end_date=end_date,
        user_api_key_dict=user_api_key_dict,
        model=None,
...
async def litellm_get_users(start_date, end_date):
    ...
    while page < total_pages:
        page = page + 1
        res = await internal_user_endpoints.get_users(
            sort_by="created_at",
            sort_order="desc",
            page=page,
            page_size=page_size,
        ...
        data = [
            ...
            for a in res["users"]
            if a.user_id != "default_user_id" and start_time <= a.created_at <= end_time
        ]
...
async def ai_usage_to_nocobase(
    ...
    try:
        verbose_proxy_logger.info(
            f"[{datetime.now()}] LiteLLM用户采集开始，时间范围: [{start_date}, {end_date}]"
        )
        datas = await litellm_get_users(start_date, end_date)
        verbose_proxy_logger.info(
            f"[{datetime.now()}] LiteLLM用户采集结束，推送nocobase开始: len={len(datas)}"
        )
        data_to_nocobase(
            datas=datas,
            table="ai_user_litellm",
            filter='{"$and":[{"createdAt":{"$dateBetween":["'
            + start_date
            + '","'
            + end_date
            + '"]}}]}',
        ...
        verbose_proxy_logger.exception(
            f"LiteLLM用户采集失败: 日期[{start_date},{end_date}] {e}"
        )

    verbose_proxy_logger.info(
        f"[{datetime.now()}] LiteLLM用量采集开始，时间范围: [{start_date}, {end_date}]"
    )
    ...
    verbose_proxy_logger.info(
        f"[{datetime.now()}] LiteLLM用量采集结束，len={len(res.results)}"
    )
    ...
    try:
        ...
        verbose_proxy_logger.info(
            f"[{datetime.now()}] 推送nocobase开始: ai_usage_litellm 用户用量 len={len(datas)}"
        )
        data_to_nocobase(
            datas=datas,
            table="ai_usage_litellm",
            filter='{"$and":[{"date":{"$dateBetween":["'
            + start_date
            + '","'
            + end_date
            + '"]}}]}',
        ...
        verbose_proxy_logger.info(
            f"[{datetime.now()}] 推送nocobase结束: ai_usage_litellm 用户用量"
        )
    except Exception as e:
        verbose_proxy_logger.exception(
            f"推送nocobase失败: ai_usage_litellm 用户用量 日期[{start_date}, {end_date}] {e}"
        )
    ...
    try:
        ...
        verbose_proxy_logger.info(
            f"[{datetime.now()}] 推送nocobase开始: ai_usage_litellm_detail 模型 len={len(datas)}"
        )
        data_to_nocobase(
            datas=datas,
            table="ai_usage_litellm_detail",
            filter='{"$and":[{"catalog":{"$eq":"model"}},{"date":{"$dateBetween":["'
            + start_date
            + '","'
            + end_date
            + '"]}}]}',
        ...
        verbose_proxy_logger.info(
            f"[{datetime.now()}] 推送nocobase结束: ai_usage_litellm_detail 模型"
        )
    except Exception as e:
        verbose_proxy_logger.exception(
            f"推送nocobase失败: ai_usage_litellm_detail 模型 日期[{start_date}, {end_date}] {e}"
        )
    ...
    try:
        ...
        verbose_proxy_logger.info(
            f"[{datetime.now()}] 推送nocobase开始: ai_usage_litellm_detail mcp len={len(datas)}"
        )
        data_to_nocobase(
            datas=datas,
            table="ai_usage_litellm_detail",
            filter='{"$and":[{"catalog":{"$eq":"mcp_server"}},{"date":{"$dateBetween":["'
            + start_date
            + '","'
            + end_date
            + '"]}}]}',
        ...
        verbose_proxy_logger.info(
            f"[{datetime.now()}] 推送nocobase结束: ai_usage_litellm_detail mcp"
        )
    except Exception as e:
        verbose_proxy_logger.exception(
            f"推送nocobase失败: ai_usage_litellm_detail mcp 日期[{start_date}, {end_date}] {e}"
        )
