...
CacheCounterValue = int | float | str | bytes

CacheCounterValues = Sequence[CacheCounterValue | None]

ParallelGaugeCacheValue = dict[str, object] | int | float | str | bytes
...
DescriptorAtomicGroup = tuple[list[str], list[int], list[AtomicCounterMeta]]
...
_request_stash: ContextVar[RequestRateLimiterStash | None] = ContextVar(
...
class _PROXY_MaxParallelRequestsHandler_v3(CustomLogger):
    ...
    async def in_memory_cache_sliding_window(
        ...
        """
        results: list[CacheCounterValue | None] = []
    ...
    async def _execute_redis_batch_rate_limiter_script(
        ...
        all_cache_values: list[CacheCounterValue | None] = []
    ...
    async def should_rate_limit(
        ...
        if keys_to_fetch:
            ...
            cache_values = await self._batch_get_counter_values(
                keys=keys_to_fetch,
                parent_otel_span=parent_otel_span,
                local_only=True,
            )
    ...
    async def _check_parallel_request_gauges(
        ...
        if read_only:
            if self.parallel_count_script is not None:
                try:
                    raw_counts: list[CacheCounterValue] = await self.parallel_count_script(
                        keys=gauge_keys,
        ...
        if self.parallel_acquire_script is not None:
            try:
                raw: list[CacheCounterValue] = await self.parallel_acquire_script(
                    keys=gauge_keys,
                    args=[
    ...
    async def _read_local_gauge_counts(
        ...
        values = await self._batch_get_gauge_values(
            keys=gauge_keys,
            parent_otel_span=parent_otel_span,
        )
        if values is None:
