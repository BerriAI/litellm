    ...
    async def async_set_cache_sadd(self, model: str, value: list):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "litellm.proxy.hooks.dynamic_rate_limiter.py::async_set_cache_sadd(): Exception occurred - %s", e
            )
            raise e
...
class _PROXY_DynamicRateLimitHandler(CustomLogger):
    ...
    async def async_post_call_success_hook(self, data: dict, user_api_key_dict: UserAPIKeyAuth, response):
        try:
            ...
            verbose_proxy_logger.exception(
                "litellm.proxy.hooks.dynamic_rate_limiter.py::async_post_call_success_hook(): Exception occurred - %s", e
            )
            return response
