...
class SemanticToolFilterHook(CustomLogger):
    ...
    async def async_pre_call_hook(
        ...
        if not tools:
            verbose_proxy_logger.debug(
                "No tools in request, skipping semantic filter"
            )
            return None
        ...
        try:
            ...
            verbose_proxy_logger.debug(
                f"Applying semantic filter to {len(tools)} tools with query: '{user_query[:50]}...'"
            )
            ...
            # Ensure metadata container exists so we can attach filter stats safely.
    ...
    async def async_post_call_response_headers_hook(
        ...
        if tool_names_csv:
            if (
                ...
                tool_names_csv = (
                    tool_names_csv[
                        : MAX_MCP_SEMANTIC_FILTER_TOOLS_HEADER_LENGTH - 3
                    ]
                    + "..."
                )
    ...
    async def initialize_from_config(
        ...
        if not config or not config.get("enabled", False):
            verbose_proxy_logger.debug(
                "Semantic tool filter not enabled in config"
            )
            return None
