...
async def list_vector_stores(
    ...
    try:
        ...
        if litellm.vector_store_registry is not None:
            # Vector stores loaded from config.yaml never had a DB row. Treat
            # their ids as present in the DB for the reconciliation below so
            # the "in memory but not in DB → deleted" branch never fires for
            # config-sourced entries.
            db_vector_store_ids.update(litellm.vector_store_registry.config_loaded_vector_store_ids)
