...
class TeamMemberBudgetHandler:
    ...
    async def backfill_team_member_budget_entries(
        ...
        """
        Sync every team member's TeamMembership row to the team's default budget.

        Called after team_member_budget is set/updated on a team. Creates rows
        for members who don't have one, and points ALL existing rows (including
        those with per-member budget overrides) at the new team-default budget.
        This implements the "auto-sync" behaviour: when a team admin updates the
        team member budget, every member is moved onto that budget immediately.
        """
        ...
        # Sync ALL existing membership rows to the team's default budget.
        # This includes rows that previously pointed at a per-member override so
        # that a team-level budget update is immediately reflected for every member.
        ...
        if updated:
            verbose_proxy_logger.info(
                "Synced budget_id on %d team_memberships for team %s to budget %s",
                updated,
                _sanitize_for_log(team_id),
                _sanitize_for_log(team_member_budget_id),
            )
