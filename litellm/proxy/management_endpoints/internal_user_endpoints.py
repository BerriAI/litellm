...
@management_endpoint_wrapper
async def get_user_daily_activity_model_top_keys(
    ...
    """
    Top keys by spend on one model or model group, ranked across every key.
    ...
    try:
        ...
        verbose_proxy_logger.exception("/user/daily/activity/aggregated/model_top_keys: Exception occured - %s", e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": f"Failed to fetch analytics: {e}"},  # mutable-ok: FastAPI detail payload shape
        )
