...
async def new_user(
    ...
    This creates a new user and generates a new api key for the new user. 
    ...
    - user_role: Optional[str] - Specify a user role - "proxy_admin", "proxy_admin_viewer", "internal_user", "internal_user_viewer", "team", "customer". Info about each role can be found in the [documentation.](https://docs.litellm.ai/docs/proxy/self_serve#available-roles)
...
async def user_info(
    ...
    Example request
    ```bash
    curl -X GET 'http://localhost:4000/user/info?user_id=krrish7@berri.ai' \\
...
async def get_users(
    ...
    Parameters:
    - role: Optional[str] - Filter users by role. Can be one of:
        - proxy_admin
        - proxy_admin_viewer
        - internal_user
        - internal_user_viewer
    - user_ids: Optional[str] - Get list of users by user_ids. Comma separated list of user_ids.
...
async def delete_user(
    ...
    ```bash
    curl -X POST 'http://0.0.0.0:4000/user/delete' \\
    --header 'Authorization: Bearer sk-1234' \\
    --header 'Content-Type: application/json' \\
    --data-raw '{
