...
async def delete_model(
    ...
    try:
        ...
        if model_params.model_info.team_id is not None:
            # For team-scoped (BYOK) models, `model_params.model_name` is the
            # internal unique name (`model_name_<team_id>_<uuid>`) generated by
            # `_add_team_model_to_db`; the user-visible name is stored in
            # `model_info.team_public_model_name`. Without this lookup,
            # `delete_team_model_alias` searches `model_aliases.values()` for
            # the internal name, finds nothing, and `team.models` (which holds
            # the public name) is never trimmed — leaving a "ghost" entry in
            # /models (GH #22594).
            ...
            public_model_name_for_alias_lookup = (
                team_public_model_name or model_params.model_name
            )
            ...
            if existing_team_row is not None:
                # Defensively drop the public name from `team.models` even if
                # the alias lookup returned no entries — covers BYOK models
                # (where `model_aliases` is never populated) and the case
                # where a prior team-alias update overwrote the alias map
                # (root cause of GH #22594). Backward-compatible with rows
                # that do have an alias map.
                names_to_drop = set(valid_team_model_aliases)
                if team_public_model_name:
                    names_to_drop.add(team_public_model_name)

                existing_team_row.models = [
                    model
                    for model in existing_team_row.models
                    if model not in names_to_drop
                ]
                ...
                # Refresh in-memory team cache so callers stop seeing the
                # deleted public name on subsequent /models requests instead
                ...
                except Exception as cache_refresh_exc:  # pragma: no cover - defensive
                    verbose_proxy_logger.exception(
                        "delete_model: team cache refresh failed for team_id=%s: %s",
                        model_params.model_info.team_id,
                        str(cache_refresh_exc),
                    )
