...
class _ProxyModelTableLike(Protocol):
    def find_unique(self, *, where: Mapping[str, object]) -> Awaitable[LiteLLM_ProxyModelTable | None]: ...

    def find_many(self, *, where: Mapping[str, object]) -> Awaitable[Sequence[LiteLLM_ProxyModelTable]]: ...

    def create(self, *, data: Mapping[str, object]) -> Awaitable[LiteLLM_ProxyModelTable]: ...

    def update(
        self, *, where: Mapping[str, object], data: Mapping[str, object]
    ) -> Awaitable[LiteLLM_ProxyModelTable]: ...

    def delete(self, *, where: Mapping[str, object]) -> Awaitable[LiteLLM_ProxyModelTable | None]: ...

    def delete_many(self, *, where: Mapping[str, object]) -> Awaitable[int]: ...


class _ModelRepositoryLike(Protocol):
    @property
    def table(self) -> _ProxyModelTableLike: ...


class _TeamRowLike(Protocol):
    @property
...
class _TeamTableLike(Protocol):
    def find_unique(self, *, where: Mapping[str, object]) -> Awaitable[_TeamRowLike | None]: ...
...
class _ModelAliasRowLike(Protocol):
    @property
    def id(self) -> int: ...
    ...
    @property
    def team(self) -> _TeamAliasRefLike | None: ...


class _ModelAliasTableLike(Protocol):
    def find_many(self, *, include: Mapping[str, bool]) -> Awaitable[Sequence[_ModelAliasRowLike]]: ...

    def update(self, *, where: Mapping[str, object], data: Mapping[str, object]) -> Awaitable[_ModelAliasRowLike]: ...


class _ModelTableRepositoryLike(Protocol):
    @property
    def table(self) -> _ModelAliasTableLike: ...


def _model_table(repo: _ModelRepositoryLike) -> _ProxyModelTableLike:
    return repo.table


def _tx_model_table(table: _ProxyModelTableLike) -> _ProxyModelTableLike:
    return table
...
def _db_team_table(table: _TeamTableLike) -> _TeamTableLike:
    return table


def _model_alias_table(repo: _ModelTableRepositoryLike) -> _ModelAliasTableLike:
    return repo.table


def _row_model_info(row: LiteLLM_ProxyModelTable) -> object:
    return row.model_info


async def get_db_model(model_id: str, prisma_client: PrismaClient) -> Deployment | None:
    db_model: Final = await _model_table(ModelRepository(prisma_client)).find_unique(where={"model_id": model_id})
...
async def patch_model(
    ...
    try:
        ...
        updated_model: Final = await _model_table(ModelRepository(prisma_client)).update(
        ...
        raise_if_reload_degraded_serving(
            ...
            written_models=[(model_id, _row_model_info(updated_model))],
            action="update",
...
async def _set_model_blocked_status(
    ...
    try:
        ...
        raise_if_reload_degraded_serving(
            before=live_before_reload,
            written_models=[(data.model_id, _row_model_info(updated_model))],
            action=action,
            still_desired=still_desired_ids,
        )
...
async def _add_model_to_db(
    ...
    _litellm_params_dict: Final = model_params.litellm_params.model_dump(exclude_none=True)
    ...
    if should_create_model_in_db:
        model_response = await _model_table(ModelRepository(prisma_client)).create(data=_data)
    else:
...
async def _get_team_deployments(
    team_id: str, prisma_client: PrismaClient, table: _ProxyModelTableLike | None = None
    ...
    resolved_table: Final = table if table is not None else _model_table(ModelRepository(prisma_client))
    ...
    result: Final = []
    for row in response:
        model_info = model_info_as_mapping(_row_model_info(row))
...
async def delete_team_models(
    ...
    async with prisma_client.db.tx() as tx:
        tx_table: Final = _tx_model_table(tx.litellm_proxymodeltable)
        for team_id in team_ids:
            rows = await _get_team_deployments(team_id, prisma_client, table=tx_table)
            model_ids = [row.model_id for row in rows]
            if model_ids:
                await tx_table.delete_many(where={"model_id": {"in": model_ids}})
...
async def _remove_unbacked_team_models(
    ...
    team_table: Final = _db_team_table(prisma_client.db.litellm_teamtable)
    existing_team_row: Final = await team_table.find_unique(where={"team_id": team_id})
    if existing_team_row is None:
        return

    updated_team_row: Final[LiteLLM_TeamTable] = await team_table.update(
...
async def _update_existing_team_model_assignment(
    ...
    if old_public_name and public_model_name != old_public_name:
        ...
        other_deployments_with_old_name: Final = [
            ...
            and _get_team_public_model_name(_row_model_info(d)) == old_public_name
        ]
...
class ModelManagementAuthChecks:
    ...
    async def allow_team_model_action(
        ...
        _existing_team_row: Final = await _team_table(TeamRepository(prisma_client)).find_unique(
    ...
    async def can_user_make_model_call(
        ...
        if model_params.model_info is not None and model_params.model_info.team_id is not None:
            team_obj_row: Final = await _team_table(TeamRepository(prisma_client)).find_unique(
                where={"team_id": model_params.model_info.team_id}
            )
...
async def delete_model(
    ...
    try:
        ...
        model_in_db: Final = await _model_table(ModelRepository(prisma_client)).find_unique(
        ...
        if store_model_in_db is True:
            ...
            """
            # encrypt litellm params #
            result: Final = await _model_table(ModelRepository(prisma_client)).delete(where={"model_id": model_info.id})
...
async def delete_team_model_alias(
    ...
    """
    team_model_aliases: Final = await _model_alias_table(ModelTableRepository(prisma_client)).find_many(
        include={"team": True}
    )
    tasks: Final = []
    ...
    for team_model_alias in team_model_aliases:
        ...
        if public_model_name in model_aliases.values():
            ...
            remaining_aliases = {alias: target for alias, target in model_aliases.items() if alias != key}
            tasks.append(
...
async def add_new_model(
    ...
    try:
        ...
        raise_if_reload_degraded_serving(
            before=live_before_reload,
            written_models=[(model_response.model_id, _row_model_info(model_response))],
            action="create",
            still_desired=still_desired_ids,
        )
...
async def update_model(
    ...
    try:
        ...
        _existing_litellm_params = await _model_table(ModelRepository(prisma_client)).find_unique(
        ...
        if store_model_in_db is True:
            ...
            model_response: Final = await _model_table(ModelRepository(prisma_client)).update(
                where={"model_id": _model_id},
                data=_data,
            )
            ...
            asyncio.create_task(
                create_object_audit_log(
                    ...
                    user_api_key_dict=user_api_key_dict,
                    table_name=LitellmTableNames.PROXY_MODEL_TABLE_NAME,
                    before_value=_existing_litellm_params.model_dump_json(exclude_none=True),
                    after_value=model_response.model_dump_json(exclude_none=True),
                    litellm_changed_by=user_api_key_dict.user_id,
                    litellm_proxy_admin_name=LITELLM_PROXY_ADMIN_NAME,
                )
            ...
            raise_if_reload_degraded_serving(
                before=live_before_reload,
                written_models=[(_model_id, _row_model_info(model_response))],
                action="update",
                still_desired=still_desired_ids,
            )
...
def _deduplicate_litellm_router_models(models: Sequence[dict[str, object]]) -> list[dict[str, object]]:
    """
    ...
    """
    seen_ids: Final[set[object]] = set()
    unique_models: Final = []
    for model in models:
        model_id = (model_info_as_mapping(model.get("model_info")) or {}).get("id")
...
async def clear_cache() -> frozenset[str] | None:
    ...
    try:
        ...
        current_models: Final = cast(list[dict[str, object]], llm_router.model_list.copy())
        config_models: Final = []
        db_model_ids: Final = []

        for model in current_models:
            model_info = model_info_as_mapping(model.get("model_info")) or {}
            if not model_info.get("db_model", False):
                # This is a config model, preserve it
                config_models.append(model)
            elif isinstance(candidate_id := model_info.get("id"), str):
                # This is a DB model, mark for deletion
                db_model_ids.append(candidate_id)
        ...
        db_router_names: Final = {
            model_name
            for model in current_models
            if isinstance(model_name := model.get("model_name"), str)
            and (model_info_as_mapping(model.get("model_info")) or {}).get("db_model", False)
            and str((model_info_as_mapping(model.get("litellm_params")) or {}).get("model", "")).startswith(
                "auto_router/"
            )
        }
