...
def with_config_updated_at(data: Mapping[str, object]) -> dict[str, object]:
    ...
    spend flush, which makes it useless for auditing; ``config_updated_at`` is
    written only from key-management write paths.
    """
    return {**data, "config_updated_at": datetime.now(timezone.utc)}
