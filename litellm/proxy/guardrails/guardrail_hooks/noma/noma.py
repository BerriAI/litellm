...
class NomaGuardrail(CustomGuardrail):
    ...
    def _create_background_noma_check(
        ...
        try:
            ...
            verbose_proxy_logger.error(f"Failed to create background Noma task: {e}")
    ...
    async def _process_user_message_check(
        ...
        if self._should_anonymize(response_json, USER_ROLE):
            anonymized_content = self._extract_anonymized_content(response_json, USER_ROLE)
    ...
    def _determine_guardrail_status(self, response_json: dict) -> GuardrailStatus:
        ...
        try:
            ...
            verbose_proxy_logger.error(f"Error determining NOMA guardrail status: {e}")
            return "guardrail_failed_to_respond"
    ...
    async def _check_user_message_background(
        ...
        try:
            ...
            verbose_proxy_logger.error(f"Noma background user message check failed: {e}")
    ...
    async def _check_llm_response_background(
        ...
        try:
            ...
            verbose_proxy_logger.error(f"Noma background response check failed: {e}")
    ...
    async def _handle_verdict_background(
        ...
        try:
            ...
            verbose_proxy_logger.error(f"Noma background verdict handling failed: {e}")
    ...
    async def async_pre_call_hook(
        ...
        if self.monitor_mode:
            try:
                ...
                verbose_proxy_logger.error(f"Failed to start background Noma pre-call check: {e}")
            return data
        ...
        try:
            ...
            verbose_proxy_logger.error(f"Noma pre-call hook failed: {e}")
    ...
    async def async_moderation_hook(
        ...
        if self.monitor_mode:
            try:
                ...
                verbose_proxy_logger.error(f"Failed to start background Noma moderation check: {e}")
            return data
        ...
        try:
            ...
            verbose_proxy_logger.error(f"Noma moderation hook failed: {e}")
    ...
    async def async_post_call_success_hook(
        ...
        if self.monitor_mode:
            try:
                ...
                verbose_proxy_logger.error(f"Failed to start background Noma post-call check: {e}")
            return response
        ...
        try:
            ...
            verbose_proxy_logger.error(f"Noma post-call hook failed: {e}")
            if self.block_failures:
                raise
            return response
    ...
    async def _check_user_message(
        ...
        user_message = await self._process_user_message_check(request_data, user_auth, event_type)
        if not user_message:
            return request_data
    ...
    async def _check_llm_response(
        ...
        content = await self._process_llm_response_check(request_data, response, user_auth, event_type)
        if not content:
            return response
    ...
    async def _call_noma_api(
        ...
        endpoint = urljoin(self.api_base or "https://api.noma.security/", NomaGuardrail._AIDR_ENDPOINT)
    ...
    async def async_post_call_streaming_iterator_hook(
        ...
        if isinstance(assembled_model_response, ModelResponse):
            try:
                ...
                if self.block_failures:
                    raise
                verbose_proxy_logger.error(f"Noma streaming post-call hook failed: {e}")
