    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            app_name: Application name (reads from ALICE_APP_NAME env var if not provided)
            ...
            fail_open: When True, proceed if WonderFence is unreachable
        """
        try:
        ...
        client_kwargs = {
            ...
            # WonderFence SDK expects an integer timeout
        ...
        try:
            ...
            if action == "BLOCK":
                ...
                if hasattr(result, "detections") and result.detections:
                    # Convert SDK detection objects to JSON-serializable dicts
                    detail["detections"] = [
                ...
                # Note: structured_messages is intentionally not updated here —
                # the framework uses only `texts` from the return value to rebuild the request.
                texts = inputs.get("texts", [])
                if texts:
                    texts[-1] = masked_text
                    inputs["texts"] = texts
