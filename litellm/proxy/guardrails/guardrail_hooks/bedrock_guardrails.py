...
class BedrockGuardrail(CustomGuardrail, BaseAWSLLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #
        # SECURITY NOTE: When this flag is True, tool/function messages remain
        # in `data["messages"]` and are still forwarded to the downstream LLM
        # provider — only Bedrock skips scanning them. Only enable this in
        # deployments where you trust the source of `role="tool"` /
        # `role="function"` / `assistant.tool_calls` content (e.g. tool
        # results generated by your own server-side tool runtime). If your
        # proxy accepts these message types directly from untrusted clients,
        # a client could submit blocked text under a tool role and bypass
        # the input guardrail. In that threat model leave this flag at the
        # default (False).
