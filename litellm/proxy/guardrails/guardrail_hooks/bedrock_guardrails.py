...
class BedrockGuardrail(CustomGuardrail, BaseAWSLLM):
    ...
    async def _build_image_content_item(self, image_url: str) -> BedrockContentItem:
        ...
        # base64 carries 3 bytes per 4 characters, less whatever the padding stands in
        # for, so the size follows from the string without decoding the whole image a
        # second time. Counting the padding matters at the boundary: without it a 4 MiB
        # image measures 2 bytes over and gets refused.
        decoded_size: Final = len(image_bytes) // 4 * 3 - image_bytes.count("=")
