...
class GenericGuardrailAPI(CustomGuardrail):
    ...
    def _build_guardrail_return_inputs(
        ...
        *,
        ...
        returned_tool_calls: Final = guardrail_response.tool_calls
        if returned_tool_calls and not all(_returned_tool_call_is_usable(call) for call in returned_tool_calls):
            verbose_proxy_logger.warning(
                "Generic Guardrail API returned tool calls missing a function name or arguments; keeping the ones already sent",
            )
            returned_tool_calls = None  # rebind-ok: an unusable reply is discarded
        if returned_tool_calls:
            return_inputs["tool_calls"] = returned_tool_calls
        elif tool_calls:
