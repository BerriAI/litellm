...
# What a too-large block is replaced with, for an observe-only guardrail. An
# enforcing guardrail is sent the block unstripped instead: replacing it there
# would let a caller pad prohibited content past the ceiling and have the
# guardrail rule on a placeholder while the original still reaches the model.
...
class PayloadPolicy:
    ...
    # True when the guardrail cannot act on what it is sent, which decides how an
    # oversized text block is handled: see OVERSIZED_TEXT_PLACEHOLDER.
...
def _shape_text(text: str, policy: PayloadPolicy) -> str:
    if policy.strip_patterns and len(text) > MAX_STRIP_INPUT_CHARS:
        # Past the ceiling the patterns are not run at all, since re has no match
        # timeout. What is sent instead depends on whether the guardrail can act:
        # an observer gets a placeholder, an enforcing guardrail gets the block
        # unstripped, because hiding content from it is how padding turns into a
        # policy bypass.
        verbose_proxy_logger.warning(
            "Generic Guardrail API: a %d character text block exceeds the %d character strip_patterns "
            "limit, so the patterns were not applied to it. Sent %s.",
            len(text),
            MAX_STRIP_INPUT_CHARS,
            "as a placeholder (observe-only guardrail)" if policy.observe_only else "unstripped so it is still scanned",
        )
        return OVERSIZED_TEXT_PLACEHOLDER if policy.observe_only else text
