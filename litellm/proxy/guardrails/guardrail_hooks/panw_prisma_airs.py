...
class PanwPrismaAirsHandler(CustomGuardrail):
    ...
    def __init__(
        ...
        **kwargs
        ...
        verbose_proxy_logger.info(f"Initialized PANW Prisma AIRS Guardrail: {guardrail_name}")
    ...
    def _extract_response_text(self, response: ModelResponse) -> str:
        ...
        try:
            ...
            verbose_proxy_logger.error("PANW Prisma AIRS: Error extracting response text")
        return ""
    ...
    async def _call_panw_api(
        ...
        transaction_id = f"litellm-{'resp' if is_response else 'req'}-{uuid.uuid4().hex[:8]}"
        
        payload = {
            "tr_id": transaction_id,
            "ai_profile": {"profile_name": self.profile_name},
            "metadata": {
                "app_user": metadata.get("user", "litellm_user") if metadata else "litellm_user",
    ...
    def _build_error_detail(self, scan_result: Dict[str, Any], is_response: bool = False) -> Dict[str, Any]:
        ...
        optional_fields = ["scan_id", "report_id", "profile_name", "profile_id", "tr_id"]
    ...
    async def async_pre_call_hook(
        ...
        if not prompt_text:
            verbose_proxy_logger.warning("PANW Prisma AIRS: No user prompt found in request")
            return None
...
# Initializer function (required for built-in guardrails)
def initialize_panw_prisma_airs(litellm_params: Dict[str, Any], guardrail: Dict[str, Any]) -> PanwPrismaAirsHandler:
    """
    Initialize PANW Prisma AIRS guardrail handler.
    
    This function is called by LiteLLM's guardrail system when loading
    guardrails from configuration.
    
    Args:
        litellm_params: LiteLLM configuration parameters
        guardrail: Guardrail-specific configuration
        
    Returns:
        Initialized PanwPrismaAirsHandler instance
    """
    
    # Extract configuration
    guardrail_name = guardrail.get("guardrail_name", "panw_prisma_airs")
    api_key = litellm_params.get("api_key")
    api_base = litellm_params.get("api_base", "https://service.api.aisecurity.paloaltonetworks.com/v1/scan/sync/request")
    profile_name = litellm_params.get("profile_name")
    default_on = litellm_params.get("default_on", True)
    ...
    panw_handler = PanwPrismaAirsHandler(
        guardrail_name=guardrail_name,
        api_key=api_key,
        api_base=api_base,
    ...
    litellm.logging_callback_manager.add_litellm_callback(panw_handler)
