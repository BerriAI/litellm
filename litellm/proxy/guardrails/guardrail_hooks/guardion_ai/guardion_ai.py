...
class GuardionAIGuardrail(CustomGuardrail):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        """
        Initialize the GuardionAIGuardrail class.

        This calls: https://api.guardion.ai/v1/guard
        ...
        self.Guardion_api_key = api_key or os.environ["GUARDION_API_KEY"]
        self.application_id = application_id
        self.api_base = (
            api_base or get_secret_str("GUARDION_API_BASE") or "https://api.guardion.ai"
        )
    ...
    async def call_guard(
        ...
        request_data: Dict,
    ) -> Tuple[GuardionAIResponse, Dict]:
        """
        Call the GuardionAI API.
        """
        status: GuardrailStatus = "success"
        exception_str: str = ""
        start_time: datetime = datetime.now()
        guardion_response: Optional[GuardionAIResponse] = None
        request: Dict = {}
        ...
        try:
            ...
            verbose_proxy_logger.debug("GuardionAI request: %s", request)
            response = await self.async_handler.post(
                url=f"{self.api_base}/v1/guard",
                headers={"Authorization": f"Bearer {self.guardion_api_key}"},
                json=request,
            )
            verbose_proxy_logger.debug(
                "GuardionAI response: %s", response.json()
            )
            guardion_response = GuardionAIResponse(**response.json())
            return guardion_response, masked_entity_count
            ...
            if status == "success":
                copy_guardion_response_dict = (
                    dict(copy.deepcopy(guardion_response)) if guardion_response else {}
                )
                # payload contains PII, we don't want to log it
                copy_guardion_response_dict.pop("payload")
                guardrail_json_response = copy_guardion_response_dict
            else:
    ...
    def _mask_pii_in_messages(
        self,
        messages: List[AllMessageValues],
        guardion_response: Optional[GuardionAIResponse],
        masked_entity_count: Dict,
        ...
        “[MASKED <TYPE>]” tokens.
        """
        payload = guardion_response.get("payload") if guardion_response else None
        if not payload:
            return messages
        ...
        for idx, msg in enumerate(messages):
            ...
            # We only support masking content strings
    ...
    async def async_pre_call_hook(
        ...
        verbose_proxy_logger.debug("GuardionAI: pre_call_hook")

        event_type: GuardrailEventHooks = GuardrailEventHooks.pre_call
        if self.should_run_guardrail(data=data, event_type=event_type) is not True:
            verbose_proxy_logger.debug(
                "GuardionAI: not running guardrail. Guardrail is disabled."
            )
            return data
        ...
        if new_messages is None:
            verbose_proxy_logger.warning(
                "GuardionAI: not running guardrail. No messages in data"
            )
            return data

        #########################################################
        ########## 1. Make the GuardionAI API request ##########
        #########################################################
        guardion_guardrail_response, masked_entity_count = await self.call_guard(
        ...
        #########################################################
        if guardion_guardrail_response.get("flagged") is True:
            # If only PII violations exist, mask the PII
            if self._is_only_pii_violation(guardion_guardrail_response):
                data["messages"] = self._mask_pii_in_messages(
                    messages=new_messages,
                    guardion_response=guardion_guardrail_response,
                    masked_entity_count=masked_entity_count,
                )
                verbose_proxy_logger.debug(
                    "GuardionAI: Masked PII in messages instead of blocking request"
                )
            else:
                # If there are other violations or not set to mask PII, raise exception
                raise self._get_http_exception_for_blocked_guardrail(
                    guardion_guardrail_response
                )
    ...
    async def async_moderation_hook(
        ...
        ########## 1. Make the GuardionAI API request ##########
        #########################################################
        guardion_guardrail_response, masked_entity_count = await self.call_v2_guard(
            messages=new_messages,
            request_data=data,
        )
    ...
    def _is_only_pii_violation(
        self, guardion_response: Optional[GuardionAIResponse]
    ) -> bool:
        """
        Returns True if there are only PII violations in the response.
        """
        if not guardion_response:
            return False

        # Check breakdown field for detected violations
        breakdown = guardion_response.get("breakdown", []) or []
        if not breakdown:
            return False
    ...
    def _get_http_exception_for_blocked_guardrail(
        ...
        return HTTPException(
            ...
            detail={
                ...
                "guardion_guardrail_response": guardion_response,
            },
        )
