...
api_key_header = APIKeyHeader(name="Authorization", auto_error=False, description="Bearer token")
...
async def user_api_key_auth(
    ...
    try:
            ...
            if is_jwt:
                ...
                if is_admin:
                    ...
                    if is_allowed:
                        ...
                        allowed_routes = jwt_handler.litellm_jwtauth.admin_allowed_routes
                ...
                user_object = None
                user_id = jwt_handler.get_user_id(token=jwt_valid_token, default_value=None)
        ...
        # note: never string compare api keys, this is vulenerable to a time attack. Use secrets.compare_digest instead
        ## Check CACHE
        valid_token: Optional[UserAPIKeyAuth] = user_api_key_cache.get_cache(key=hash_token(api_key))
        if (
        ...
        if is_master_key_valid:
            ...
            await user_api_key_cache.async_set_cache(key=hash_token(master_key), value=_user_api_key_obj)
        ...
        if valid_token is None:
            ...
            if prisma_client is not None:
                ...
                if _valid_token is not None:
                    valid_token = UserAPIKeyAuth(**_valid_token.model_dump(exclude_none=True))
        ...
        if valid_token is not None:
            ...
            if len(valid_token.models) == 0:  # assume an empty model list means all models are allowed to be called
                ...
                if model is not None and model not in filtered_models and "*" not in filtered_models:
                    raise ValueError(
            ...
            if valid_token.user_id is not None:
                ...
                if user_id_information is None or (
                    isinstance(user_id_information, list) and len(user_id_information) < 1
                ):
                    if prisma_client is not None:
                ...
                verbose_proxy_logger.debug(f"user_id_information: {user_id_information}")
                    ...
                    if isinstance(user_id_information, list):
                        ...
                        if user_max_budget is not None and user_current_spend is not None:
                            call_info = CallInfo(
                                ...
                                user_email=getattr(user_id_information, "user_email", None),
                                key_alias=valid_token.key_alias,
                            )
                            asyncio.create_task(
                                proxy_logging_obj.budget_alerts(
            ...
            if (
                ...
                if len(model_spend) > 0 and max_budget_per_model.get(current_model, None) is not None:
                    if (
                        model_spend[0]["model"] == current_model
                        and model_spend[0]["_sum"]["spend"] >= max_budget_per_model[current_model]
                    ):
            ...
            if not _is_user_proxy_admin(user_id_information):  # if non-admin
                ...
                elif _user_role == LitellmUserRoles.INTERNAL_USER and route in LiteLLMRoutes.internal_user_routes.value:
                    pass
                else:
                    user_role = "unknown"
                    user_id = "unknown"
        ...
        verbose_proxy_logger.error(
            "litellm.proxy.proxy_server.user_api_key_auth(): Exception occured - {}".format(str(e))
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, litellm.BudgetExceededError):
            raise ProxyException(message=e.message, type="auth_error", param=None, code=400)
...
async def _PROXY_failure_handler(
    ...
    if prisma_client is not None:
        verbose_proxy_logger.debug("inside _PROXY_failure_handler kwargs=", extra=kwargs)
...
async def _PROXY_track_cost_callback(
    ...
    try:
        ...
        end_user_max_budget = kwargs["litellm_params"]["metadata"].get("user_api_end_user_max_budget", None)
        if kwargs.get("response_cost", None) is not None:
            ...
            if kwargs.get("cache_hit", False) == True:
                response_cost = 0.0
                verbose_proxy_logger.info(f"Cache Hit: response_cost {response_cost}, for user_id {user_id}")
            ...
            if user_api_key is not None or user_id is not None or team_id is not None:
                ...
                raise Exception("User API key and team id and user id missing from custom callback.")
        else:
            if kwargs["stream"] != True or (kwargs["stream"] == True and "complete_streaming_response" in kwargs):
                raise Exception(
...
def _set_spend_logs_payload(payload: dict, prisma_client: PrismaClient, spend_logs_url: Optional[str] = None):
    ...
    try:
                    ...
                    for _id in user_ids:
                        if _id is not None:
                            prisma_client.user_list_transactons[_id] = (
                                response_cost + prisma_client.user_list_transactons.get(_id, 0)
                            )
                    if end_user_id is not None:
                        prisma_client.end_user_list_transactons[end_user_id] = (
                            response_cost + prisma_client.end_user_list_transactons.get(end_user_id, 0)
                        )
            except Exception as e:
                verbose_proxy_logger.info(
                    "\033[91m" + f"Update User DB call failed to execute {str(e)}\n{traceback.format_exc()}"
                )
            ...
            try:
                ...
                if prisma_client is not None:
                    prisma_client.key_list_transactons[hashed_token] = (
                        response_cost + prisma_client.key_list_transactons.get(hashed_token, 0)
                    )
            except Exception as e:
                verbose_proxy_logger.info(f"Update Key DB Call failed to execute - {str(e)}\n{traceback.format_exc()}")
                raise e
        ...
        async def _update_team_db():
            try:
                ...
                if team_id is None:
                    verbose_proxy_logger.debug("track_cost_callback: team_id is None. Not tracking spend for team")
                    return
                if prisma_client is not None:
                    ...
                    try:
                        ...
                        prisma_client.team_member_list_transactons[team_member_key] = (
                            response_cost + prisma_client.team_member_list_transactons.get(team_member_key, 0)
                        )
                    except:
                        pass
            except Exception as e:
                verbose_proxy_logger.info(f"Update Team DB failed to execute - {str(e)}\n{traceback.format_exc()}")
                raise e
        ...
        async def _update_org_db():
            try:
                verbose_proxy_logger.debug(
                    "adding spend to org db. Response cost: {}. org_id: {}.".format(response_cost, org_id)
                )
                if org_id is None:
                    verbose_proxy_logger.debug("track_cost_callback: org_id is None. Not tracking spend for org")
                    return
                if prisma_client is not None:
                    prisma_client.org_list_transactons[org_id] = response_cost + prisma_client.org_list_transactons.get(
                        org_id, 0
                    )
            except Exception as e:
                verbose_proxy_logger.info(f"Update Org DB failed to execute - {str(e)}\n{traceback.format_exc()}")
                raise e
        ...
        verbose_proxy_logger.debug(f"Error updating Prisma database: {traceback.format_exc()}")
...
async def update_cache(
    ...
    async def _update_key_cache(token: str, response_cost: float):
        ...
        verbose_proxy_logger.debug(f"_update_key_cache: existing spend: {existing_spend_obj}")
    ...
    async def _update_user_cache():
        ...
        try:
            ...
            verbose_proxy_logger.debug(f"An error occurred updating user cache: {str(e)}\n\n{traceback.format_exc()}")
        ...
        try:
            ...
            if existing_spend_obj is None:
                ...
                existing_spend_obj = LiteLLM_EndUserTable(
                    ...
                    spend=0,
                    blocked=False,
                    litellm_budget_table=LiteLLM_BudgetTable(max_budget=max_end_user_budget),
                )
            verbose_proxy_logger.debug(
...
class ProxyConfig:
    ...
    async def get_config(self, config_file_path: Optional[str] = None) -> dict:
        ...
        ## DB
        if prisma_client is not None and (general_settings.get("store_model_in_db", False) == True):
            _tasks = []
            keys = [
                "general_settings",
                "router_settings",
    ...
    async def load_config(self, router: Optional[litellm.Router], config_file_path: str):
        ...
        if litellm_settings:
            ...
            for key, value in litellm_settings.items():
                if key == "cache" and value == True:
                    ...
                    if (cache_type in ["redis", "redis-semantic", "dual-semantic"]) and len(cache_params.keys()) == 0:
                    ...
                    if isinstance(value, list):
                        ...
                        known_compatible_callbacks = list(get_args(litellm._custom_logger_compatible_callbacks_literal))
        ...
        if general_settings:
            ...
            ### MASTER KEY ###
            master_key = general_settings.get("master_key", litellm.get_secret("LITELLM_MASTER_KEY", None))

            if master_key and master_key.startswith("os.environ/"):
                master_key = litellm.get_secret(master_key)
                if not isinstance(master_key, str):
                    raise Exception("Master key must be a string. Current type - {}".format(type(master_key)))
            ...
            if database_type is not None and (database_type == "dynamo_db" or database_type == "dynamodb"):
                ...
                for k, v in database_args.items():
                    ...
                    if isinstance(k, str) and k == "aws_web_identity_token":
                        ...
                        verbose_proxy_logger.debug(f"Loading AWS Web Identity Token from file: {value}")
                        if os.path.exists(value):
                            with open(value, "r") as file:
                                token_content = file.read()
                                database_args[k] = token_content
                        else:
                            verbose_proxy_logger.info(f"DynamoDB Loading - {value} is not a valid file path")
    ...
    def _add_deployment(self, db_models: list) -> int:
        ...
        if master_key is None or not isinstance(master_key, str):
            raise Exception(f"Master key is not initialized or formatted. master_key={master_key}")
                ...
                for k, v in _litellm_params.items():
                    if isinstance(v, str):
                        ...
                        try:
                            ...
                            verbose_proxy_logger.error("Error decoding value - {}".format(v))
                            continue
                        # decrypt value
    ...
    async def _update_llm_router(
        ...
        try:
            ...
            verbose_proxy_logger.error(f"Error adding/deleting model to llm_router: {str(e)}")
        ...
        if "alerting" in _general_settings:
            if (
                ...
                proxy_logging_obj.slack_alerting_instance.alerting = general_settings["alerting"]
    ...
    async def add_deployment(
        ...
        try:
            if master_key is None or not isinstance(master_key, str):
                raise ValueError(f"Master key is not initialized or formatted. master_key={master_key}")
...
async def generate_key_helper_fn(
    ...
    try:
        ...
        if isinstance(saved_token["permissions"], str):
            if "get_spend_routes" in saved_token["permissions"] and premium_user != True:
        ...
        if prisma_client is not None:
            ...
            if user_id == litellm_proxy_budget_name or (table_name is not None and table_name == "user"):
    ...
    try:
        if prisma_client:
            ...
            # else
                ...
                deleted_tokens = await prisma_client.delete_data(tokens=tokens, user_id=user_id)
                _num_deleted_tokens = deleted_tokens.get("deleted_keys", 0)
                if _num_deleted_tokens != len(tokens):
                    raise Exception(
                        "Failed to delete all tokens. Tried to delete tokens that don't belong to user: " + str(user_id)
                    )
        else:
            raise Exception("DB not connected. prisma_client is None")
    except Exception as e:
        verbose_proxy_logger.error(
            "litellm.proxy.proxy_server.delete_verification_token(): Exception occured - {}".format(str(e))
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        raise e
...
async def startup_event():
    ...
    if "daily_reports" in proxy_logging_obj.slack_alerting_instance.alert_types:
        asyncio.create_task(
            proxy_logging_obj.slack_alerting_instance._run_scheduled_daily_report(llm_router=llm_router)
    ...
    if prisma_client is not None and master_key is not None:
        ...
        if os.getenv("PROXY_ADMIN_ID", None) is not None:
            litellm_proxy_admin_name = os.getenv("PROXY_ADMIN_ID", litellm_proxy_admin_name)
        asyncio.create_task(
    ...
    verbose_proxy_logger.debug(f"custom_db_client client {custom_db_client}. Master_key: {master_key}")
    ...
    if prisma_client is not None:
        ...
        ### ADD NEW MODELS ###
        store_model_in_db = litellm.get_secret("STORE_MODEL_IN_DB", store_model_in_db) or store_model_in_db  # type: ignore
...
def model_list(
    ...
    key_models = get_key_models(user_api_key_dict=user_api_key_dict, proxy_model_list=proxy_model_list)
    team_models = get_team_models(user_api_key_dict=user_api_key_dict, proxy_model_list=proxy_model_list)
...
async def chat_completion(
    ...
    try:
        ...
        if "api_key" in data:
            ...
            if data.get("fastest_response", None) is not None and data["fastest_response"] == True:
            ...
            raise HTTPException(
                ...
                detail={"error": "chat_completion: Invalid model name passed in model=" + data.get("model", "")},
            )
        ...
        verbose_proxy_logger.error(
            "litellm.proxy.proxy_server.chat_completion(): Exception occured - {}".format(get_error_message_str(e=e))
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        await proxy_logging_obj.post_call_failure_hook(
...
async def completion(
    ...
    try:
        ...
        if "api_key" in data:
            ...
            llm_response = asyncio.create_task(llm_router.atext_completion(**data, specific_deployment=True))
        elif (
            llm_router is not None
            and data["model"] not in router_model_names
            and llm_router.default_deployment is not None
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.completion(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        error_msg = f"{str(e)}"
        raise ProxyException(
            message=getattr(e, "message", error_msg),
...
async def embeddings(
    ...
    try:
        ...
        if "api_key" in data:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error": "embeddings: Invalid model name passed in model=" + data.get("model", "")},
            )
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.embeddings(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def image_generation(
    ...
    try:
        ...
        if "api_key" in data:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error": "image_generation: Invalid model name passed in model=" + data.get("model", "")},
            )
...
async def audio_speech(
    ...
    try:
        ...
        if "api_key" in data:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error": "audio_speech: Invalid model name passed in model=" + data.get("model", "")},
            )
        ...
        return StreamingResponse(generate(response), media_type="audio/mpeg", headers=custom_headers)
...
async def audio_transcriptions(
    ...
    try:
        ...
        assert file.filename is not None  # make sure filename passed in (needed for type)
            ...
            try:
                ...
                if "api_key" in data:
                    ...
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail={
                            "error": "audio_transcriptions: Invalid model name passed in model=" + data.get("model", "")
                        },
                    )
...
async def create_batch(
    ...
    try:
        ...
        # for now use custom_llm_provider=="openai" -> this will change as LiteLLM adds more providers for acreate_batch
        response = await litellm.acreate_batch(custom_llm_provider="openai", **_create_batch_data)
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.create_batch(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def retrieve_batch(
    ...
    try:
        ...
        # for now use custom_llm_provider=="openai" -> this will change as LiteLLM adds more providers for acreate_batch
        response = await litellm.aretrieve_batch(custom_llm_provider="openai", **_retrieve_batch_request)
...
async def create_file(
    ...
    try:
        ...
        # for now use custom_llm_provider=="openai" -> this will change as LiteLLM adds more providers for acreate_batch
        response = await litellm.acreate_file(custom_llm_provider="openai", **_create_file_request)
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.create_file(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def moderations(
    ...
    try:
        ...
        if "api_key" in data:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error": "moderations: Invalid model name passed in model=" + data.get("model", "")},
            )
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.moderations(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e)),
    ...
    try:
        ...
        if litellm.upperbound_key_generate_params is not None:
            for elem in data:
                ...
                key, value = elem
                if value is not None and getattr(litellm.upperbound_key_generate_params, key, None) is not None:
...
async def update_key_fn(
    ...
    try:
        ...
        existing_key_row = await prisma_client.get_data(token=data.key, table_name="key", query_type="find_unique")
...
async def delete_key_fn(
    ...
    try:
        ...
        if user_api_key_dict.user_role is not None and user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN:
        ...
        number_deleted_keys = await delete_verification_token(tokens=keys, user_id=user_id)
...
async def info_key_fn_v2(
    ...
    **New endpoint**. Currently admin only.
    ...
    try:
        ...
        key_info = await prisma_client.get_data(token=data.keys, table_name="key", query_type="find_all")
        filtered_key_info = []
        for k in key_info:
            try:
                k = k.model_dump()  # noqa
...
async def _get_spend_report_for_time_range(
    ...
    try:
        ...
        spend_per_tag = await prisma_client.db.query_raw(sql_query, start_date, end_date)
...
async def calculate_spend(request: Request):
    ...
    if "completion_response" in data:
        data["completion_response"] = litellm.ModelResponse(**data["completion_response"])
        ...
        if (
            ...
            if isinstance(response, list) and len(response) > 0 and isinstance(response[0], dict):
                ...
                for record in response:
                    ...
                    user_id = record["user"]
                    model = record["model"]
                    result[date]["spend"] = result[date].get("spend", 0) + record["_sum"]["spend"]
                    result[date][api_key] = result[date].get(api_key, 0) + record["_sum"]["spend"]
                    result[date]["users"][user_id] = result[date]["users"].get(user_id, 0) + record["_sum"]["spend"]
                    result[date]["models"][model] = result[date]["models"].get(model, 0) + record["_sum"]["spend"]
                return_list = []
                final_date = None
...
async def global_spend_reset():
    ...
    await prisma_client.db.litellm_verificationtoken.update_many(data={"spend": 0.0}, where={})
...
async def user_auth(request: Request):
    ...
    response = await prisma_client.get_generic_data(key="user_email", value=user_email, table_name="users")
...
async def user_info(
    ...
    try:
        ...
        teams_1 = await prisma_client.get_data(user_id=user_id, table_name="team", query_type="find_all")
        ...
        if user_info is not None:
            ...
            # *NEW* get all teams in user 'teams' field
            if getattr(caller_user_info, "user_role", None) == LitellmUserRoles.PROXY_ADMIN:
                teams_2 = await prisma_client.get_data(
                    table_name="team",
                    query_type="find_all",
                    team_id_list=None,
        ...
        for key in keys:
            ...
            try:
                ...
                # if using pydantic v1
                key = key.dict()
            if "team_id" in key and key["team_id"] is not None and key["team_id"] != "litellm-dashboard":
                team_info = await prisma_client.get_data(team_id=key["team_id"], table_name="team")
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.user_info(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "detail", f"Authentication Error({str(e)})"),
...
async def user_update(data: UpdateUserRequest):
    ...
    try:
        ...
        if data.user_id is not None and len(data.user_id) > 0:
            ...
            if existing_user_rows is None or (isinstance(existing_user_rows, list) and len(existing_user_rows) == 0):
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.user_update(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "detail", f"Authentication Error({str(e)})"),
...
async def user_request_model(request: Request):
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "litellm.proxy.proxy_server.user_request_model(): Exception occured - {}".format(str(e))
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def user_get_requests():
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "litellm.proxy.proxy_server.user_get_requests(): Exception occured - {}".format(str(e))
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def get_users(
    ...
    all_users = await prisma_client.get_data(table_name="user", query_type="find_all", key_val={"user_role": role})
...
async def end_user_info(
    ...
    user_info = await prisma_client.db.litellm_endusertable.find_first(where={"user_id": end_user_id})
    ...
    try:
        ...
        if data.user_id is not None and len(data.user_id) > 0:
            ...
            response = await prisma_client.db.litellm_endusertable.update(
                ...
                data=non_default_values,  # type: ignore
            )
            if response is None:
...
async def delete_end_user(
    ...
    try:
        ...
        if data.user_ids is not None and isinstance(data.user_ids, list) and len(data.user_ids) > 0:
            response = await prisma_client.db.litellm_endusertable.delete_many(where={"user_id": {"in": data.user_ids}})
            if response is None:
                raise ValueError(
...
async def new_team(
    ...
    team_row = await prisma_client.insert_data(data=complete_team_data.json(exclude_none=True), table_name="team")
...
async def update_team(
    ...
    existing_team_row = await prisma_client.get_data(team_id=data.team_id, table_name="team", query_type="find_unique")
...
async def team_info(team_id: str = fastapi.Query(default=None, description="Team ID in the request parameters")):
    """
    ...
    try:
        ...
        team_info = await prisma_client.get_data(team_id=team_id, table_name="team", query_type="find_unique")
...
async def add_new_model(
    ...
    try:
        ...
        if store_model_in_db == True:
            ...
            for k, v in _litellm_params_dict.items():
                if isinstance(v, str):
                    ...
                    model_params.litellm_params[k] = base64.b64encode(encrypted_value).decode("utf-8")
            _data: dict = {
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.add_new_model(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "detail", f"Authentication Error({str(e)})"),
...
async def update_model(
    ...
    try:
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.update_model(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "detail", f"Authentication Error({str(e)})"),
...
async def model_info_v2(
    ...
    if llm_model_list is None or not isinstance(llm_model_list, list):
        raise HTTPException(
            status_code=500,
            detail={"error": f"Invalid llm model list. llm_model_list={llm_model_list}"},
        )
    ...
    for _model in all_models:
        ...
        if debug == True:
            _openai_client = "None"
            if llm_router is not None:
                _openai_client = llm_router._get_client(deployment=_model, kwargs={}, client_type="async") or "None"
            else:
...
async def model_metrics_slow_responses(
    ...
    alerting_threshold = proxy_logging_obj.slack_alerting_instance.alerting_threshold or 300
...
async def model_info_v1(
    ...
    if llm_model_list is None:
        raise HTTPException(status_code=500, detail={"error": "LLM Model List not loaded in"})
...
async def model_group_info(
    ...
    if llm_model_list is None:
        raise HTTPException(status_code=500, detail={"error": "LLM Model List not loaded in"})
    if llm_router is None:
        raise HTTPException(status_code=500, detail={"error": "LLM Router is not loaded in"})
    all_models: List[dict] = []
    ## CHECK IF MODEL RESTRICTIONS ARE SET AT KEY/TEAM LEVEL ##
    if llm_model_list is None:
        proxy_model_list = []
...
@router.get("/ollama_logs", dependencies=[Depends(user_api_key_auth)], tags=["experimental"])
...
async def login(request: Request):
    ...
    _user_row = None
    if prisma_client is not None:
        _user_row = await prisma_client.db.litellm_usertable.find_first(where={"user_email": {"equals": username}})
    """
    To login to Admin UI, we support the following 
    - Login with UI_USERNAME and UI_PASSWORD
    - Login with Invite Link `user_email` and `password` combination
    """
    if secrets.compare_digest(username, ui_username) and secrets.compare_digest(password, ui_password):
        user_role = "app_owner"
        user_id = username
        key_user_id = user_id
        if (
        ...
        if secrets.compare_digest(password, _password) or secrets.compare_digest(hash_password, _password):
            if os.getenv("DATABASE_URL") is not None:
                response = await generate_key_helper_fn(
                    request_type="key",
                    **{  # type: ignore
...
async def auth_callback(request: Request):
    ...
    if google_client_id is not None:
        ...
        result = await generic_sso.verify_and_process(request, params={"include_client_id": generic_include_client_id})
    ...
    try:
        ...
        if prisma_client is not None:
            ...
            elif litellm.default_user_params is not None and isinstance(litellm.default_user_params, dict):
                user_defined_values = {
                    "models": litellm.default_user_params.get("models", user_id_models),
                    "user_id": litellm.default_user_params.get("user_id", user_id),
                    "user_email": litellm.default_user_params.get("user_email", user_email),
                }
    except Exception as e:
        pass
    ...
    if os.getenv("PROXY_ADMIN_ID", None) is not None and os.environ["PROXY_ADMIN_ID"] == user_id:
        # checks if user is admin
        user_role = "app_admin"
    ...
    if ui_access_mode == "admin_only" and "admin" not in user_role:
        ...
        raise HTTPException(
            status_code=401,
            detail={"error": f"User not allowed to access proxy. User role={user_role}, proxy mode={ui_access_mode}"},
        )
    ...
    try:
        ...
        if config_info.environment_variables is not None:
            ...
            for k, v in _updated_environment_variables.items():
                if isinstance(v, str):
                    encrypted_value = encrypt_value(value=v, master_key=master_key)  # type: ignore
                    _updated_environment_variables[k] = base64.b64encode(encrypted_value).decode("utf-8")
            ...
            if "success_callback" in updated_litellm_settings and "success_callback" in config["litellm_settings"]:
                ...
                if isinstance(config["litellm_settings"]["success_callback"], list) and isinstance(
                    ...
                    combined_success_callback = (
                        config["litellm_settings"]["success_callback"] + updated_litellm_settings["success_callback"]
                    )
...
async def get_config():
    ...
    try:
            ...
            if _callback == "openmeter":
                ...
                for _var in env_vars:
                    ...
                    if env_variable is None:
                        ...
                        _decrypted_value = decrypt_value(value=decoded_b64, master_key=master_key)
                ...
                _data_to_return.append({"name": _callback, "variables": _langfuse_env_vars})
        ...
        verbose_proxy_logger.error("litellm.proxy.proxy_server.get_config(): Exception occured - {}".format(str(e)))
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "detail", f"Authentication Error({str(e)})"),
...
async def health_services_endpoint(
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    service: Literal["slack_budget_alerts", "langfuse", "slack", "openmeter", "webhook", "email"] = fastapi.Query(
    ...
    try:
        ...
        if service not in [
            ...
            raise HTTPException(
                status_code=400,
                detail={"error": f"Service must be in list. Service={service}. List={['slack_budget_alerts']}"},
            )
...
async def health_endpoint(
    ...
    try:
        ...
        if use_background_health_checks:
            ...
            healthy_endpoints, unhealthy_endpoints = await perform_health_check(_llm_model_list, model)
...
async def get_routes():
    ...
    for route in app.routes:
        route_info = {
            ...
            "endpoint": (getattr(route, "endpoint", None).__name__ if getattr(route, "endpoint", None) else None),
        }
...
def _has_user_setup_sso():
    ...
    sso_setup = (microsoft_client_id is not None) or (google_client_id is not None) or (ui_username is not None)
