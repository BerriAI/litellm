...
async def model_info_v1(  # noqa: PLR0915
    ...
    # Resolve DB-based access groups (LiteLLM_AccessGroupTable) from team.access_group_ids.
    # Only config YAML model_access_groups are resolved by get_key_models/get_team_models;
    # team-level access_group_ids stored in DB are separate and must be resolved here
    # so that /v1/models returns the full model list for keys with "all-team-models".
    if (
        ...
        try:
            ...
            if team_object and team_object.access_group_ids:
                db_access_models = await _get_models_from_db_access_groups(
                    access_group_ids=team_object.access_group_ids,
                )
                if db_access_models:
                    key_models = list(dict.fromkeys(key_models + db_access_models))
                    team_models = list(dict.fromkeys(team_models + db_access_models))
        except Exception:
            pass
