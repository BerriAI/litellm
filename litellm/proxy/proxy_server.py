...
def cleanup_router_config_variables():
    global master_key, user_config_file_path, otel_logging, user_custom_auth, user_custom_auth_path, user_custom_key_generate, user_custom_sso, user_custom_ui_sso_sign_in_handler, use_background_health_checks, use_shared_health_check, use_background_agent_health_checks, health_check_interval, health_check_concurrency, prisma_client
    ...
    use_background_agent_health_checks = None
...
async def proxy_startup_event(app: FastAPI):  # noqa: PLR0915
    ...
    if use_background_agent_health_checks:
        asyncio.create_task(
            _run_background_agent_health_check()
        )
...
async def _run_background_agent_health_check():
    """Periodically run health checks on registered A2A agents."""
    ...
    while True:
        try:
            ...
            verbose_proxy_logger.exception(
                "background agent health check failed: %s", e
            )
        await asyncio.sleep(health_check_interval or 300)
...
class ProxyConfig:
    ...
    async def load_config(  # noqa: PLR0915
        ...
        if general_settings:
            ...
            use_background_agent_health_checks = general_settings.get(
                "background_agent_health_checks", False
            )
