...
@click.option(
    ...
    is_flag=True,
    type=bool,
    help="Get a deployed proxy endpoint - api.litellm.ai",
)
...
@click.option(
    "--config", is_flag=True, help="Create and open .env file from .env.template"
)
@click.option(
    "--test", default=None, help="proxy chat completions url to make a test request to"
)
...
def run_server(
    ...
    if deploy == True:
        ...
        url = deploy_proxy(
            model, api_base, debug, temperature, max_tokens, telemetry, deploy
        )
        print(f"\033[32mLiteLLM: Deploy Successfull\033[0m\n")
        print(f"\033[32mLiteLLM: Your deployed url: {url}\033[0m\n")

        print(
            f'\033[32mLiteLLM: Test your URL using the following: "litellm --test {url}"\033[0m'
        )
        return
    if test != None:
        ...
        print(
            f'\033[32mLiteLLM: Test your URL using the following: "litellm --test http://0.0.0.0:{port}" [In a new terminal tab]\033[0m\n'
        )
        print(
            f'\033[32mLiteLLM: Deploy your proxy using the following: "litellm --model claude-instant-1 --deploy" Get an https://api.litellm.ai/chat/completions endpoint \033[0m\n'
        )
