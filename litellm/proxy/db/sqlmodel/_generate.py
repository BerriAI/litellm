...
_SCALAR_SA_TYPE = {
    ...
    # ``JSONB`` is Postgres-specific; fall back to portable ``JSON`` on
    # SQLite so the test environment can run without a real database.
...
def _collect_used_symbols(schema: PrismaSchema) -> Set[str]:
    ...
    if flags["jsonb"]:
        # JSONB columns use ``with_variant(JSON(), 'sqlite')`` -- pull JSON
        # in from sqlalchemy.types so SQLite test envs compile cleanly.
        sa_imports.add("JSON")
