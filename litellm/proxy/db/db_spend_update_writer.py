...
def _get_local_date_from_start_time(start_time: datetime) -> str:
    from zoneinfo import ZoneInfo

    from litellm.proxy.common_utils.timezone_utils import get_report_timezone

    tz_name = get_report_timezone()
    if not start_time.tzinfo:
        start_time = start_time.replace(tzinfo=timezone.utc)
    return start_time.astimezone(ZoneInfo(tz_name)).date().isoformat()
...
class DBSpendUpdateWriter:
            ...
            for i in range(n_retry_times + 1):
                try:
                    ...
                    try:
                        ...
                        spend_log_error(
                            "Daily %s spend batch upsert failed. Table: %s, Constraint: %s, Batch size: %d, Error: %s",
                            entity_type,
                            table_name,
                            unique_constraint_name,
    ...
    async def _common_add_spend_log_transaction_to_daily_transaction(
        ...
        if isinstance(payload["startTime"], datetime):
            date = _get_local_date_from_start_time(payload["startTime"])
        elif isinstance(payload["startTime"], str):
            try:
                parsed = datetime.fromisoformat(payload["startTime"])
            except ValueError:
                verbose_proxy_logger.debug(
                    f"Invalid start time: {payload['startTime']}, skipping from daily_user_spend_transactions"
                )
                return None
            date = _get_local_date_from_start_time(parsed)
        else:
            verbose_proxy_logger.debug(
                f"Invalid start time: {payload['startTime']}, skipping from daily_user_spend_transactions"
            )
