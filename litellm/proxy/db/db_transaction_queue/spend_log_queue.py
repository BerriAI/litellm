...
class SpendLogQueue:
    ...
    def __delitem__(self, index: Union[int, slice]) -> None:
        """Support deleting by index/slice for list-compatible queue operations."""
        if isinstance(index, int):
            del self._buf[index]
            return
        ...
        delete_positions = set(range(start, stop, step))
        kept_items = [
            item for i, item in enumerate(self._buf) if i not in delete_positions
        ]
        self._buf = deque(kept_items, maxlen=self._buf.maxlen)
