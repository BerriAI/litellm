...
This manager only acts when the table is already partitioned (set up via the
db_scripts/partition_spend_logs.sql runbook). On a normal, non-partitioned
install every method is a safe no-op, so existing deployments are untouched.
"""
