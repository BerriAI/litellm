...
client = openai.OpenAI(
	api_key="${v||"YOUR_LITELLM_API_KEY"}",
	base_url="${I}"
)`;switch(_){case i.CHAT:{let e=Object.keys(x).length>0,a="";if(e){let e=JSON.stringify({metadata:x},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();a=`,
    extra_body=${e}`}let o=C.length>0?C:[{role:"user",content:y}];t=`
import base64
...
`;break}case i.RESPONSES:{let e=Object.keys(x).length>0,a="";if(e){let e=JSON.stringify({metadata:x},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();a=`,
    extra_body=${e}`}let o=C.length>0?C:[{role:"user",content:y}];t=`
import base64
