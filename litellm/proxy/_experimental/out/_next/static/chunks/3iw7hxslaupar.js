...
client = openai.OpenAI(
	api_key="${x||"YOUR_LITELLM_API_KEY"}",
	base_url="${_}"
)`;switch(u){case r.CHAT:{let e=Object.keys(w).length>0,i="";if(e){let e=JSON.stringify({metadata:w},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();i=`,
    extra_body=${e}`}let a=j.length>0?j:[{role:"user",content:y}];t=`
import base64
