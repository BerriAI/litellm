...
client = openai.OpenAI(
	api_key="${I||"YOUR_LITELLM_API_KEY"}",
	base_url="${y}"
)`;switch(f){case o.CHAT:{let e=Object.keys($).length>0,i="";if(e){let e=JSON.stringify({metadata:$},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();i=`,
...
`;break}case o.RESPONSES:{let e=Object.keys($).length>0,i="";if(e){let e=JSON.stringify({metadata:$},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();i=`,
    extra_body=${e}`}let a=S.length>0?S:[{role:"user",content:v}];t=`
import base64
