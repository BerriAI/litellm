...
client = openai.OpenAI(
	api_key="${b||"YOUR_LITELLM_API_KEY"}",
	base_url="${y}"
)`;switch(h){case s.CHAT:{let e=Object.keys(k).length>0,a="";if(e){let e=JSON.stringify({metadata:k},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();a=`,
    extra_body=${e}`}let i=E.length>0?E:[{role:"user",content:w}];t=`
import base64
...
`;break}case s.RESPONSES:{let e=Object.keys(k).length>0,a="";if(e){let e=JSON.stringify({metadata:k},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();a=`,
    extra_body=${e}`}let i=E.length>0?E:[{role:"user",content:w}];t=`
import base64
