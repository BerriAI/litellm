...
client = openai.OpenAI(
	api_key="${_||"YOUR_LITELLM_API_KEY"}",
	base_url="${b}"
)`;switch(m){case r.CHAT:{let e=Object.keys(j).length>0,i="";if(e){let e=JSON.stringify({metadata:j},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();i=`,
...
`;break}case r.RESPONSES:{let e=Object.keys(j).length>0,i="";if(e){let e=JSON.stringify({metadata:j},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();i=`,
    extra_body=${e}`}let a=w.length>0?w:[{role:"user",content:y}];t=`
import base64
