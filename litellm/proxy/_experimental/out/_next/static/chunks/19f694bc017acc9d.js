...
client = openai.OpenAI(
	api_key="${v||"YOUR_LITELLM_API_KEY"}",
	base_url="${C}"
)`;switch(h){case i.CHAT:{let e=Object.keys(k).length>0,a="";if(e){let e=JSON.stringify({metadata:k},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();a=`,
    extra_body=${e}`}let r=E.length>0?E:[{role:"user",content:w}];t=`
import base64
...
`;break}case i.RESPONSES:{let e=Object.keys(k).length>0,a="";if(e){let e=JSON.stringify({metadata:k},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();a=`,
    extra_body=${e}`}let r=E.length>0?E:[{role:"user",content:w}];t=`
import base64
