...
client = openai.OpenAI(
	api_key="${_||"YOUR_LITELLM_API_KEY"}",
	base_url="${x}"
)`;switch(m){case a.CHAT:{let e=Object.keys(j).length>0,i="";if(e){let e=JSON.stringify({metadata:j},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();i=`,
    extra_body=${e}`}let r=w.length>0?w:[{role:"user",content:y}];t=`
import base64
...
`;break}case a.RESPONSES:{let e=Object.keys(j).length>0,i="";if(e){let e=JSON.stringify({metadata:j},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();i=`,
    extra_body=${e}`}let r=w.length>0?w:[{role:"user",content:y}];t=`
import base64
