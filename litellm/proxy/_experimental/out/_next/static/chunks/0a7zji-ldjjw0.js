...
client = openai.OpenAI(
	api_key="${y||"YOUR_LITELLM_API_KEY"}",
	base_url="${v}"
)`;switch(h){case r.CHAT:{let e=Object.keys(I).length>0,i="";if(e){let e=JSON.stringify({metadata:I},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();i=`,
    extra_body=${e}`}let a=k.length>0?k:[{role:"user",content:j}];t=`
import base64
...
`;break}case r.RESPONSES:{let e=Object.keys(I).length>0,i="";if(e){let e=JSON.stringify({metadata:I},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();i=`,
    extra_body=${e}`}let a=k.length>0?k:[{role:"user",content:j}];t=`
import base64
