...
if MCP_AVAILABLE:
    ...
    def _get_gateway_oauth2_challenge(scope: Scope, server_name: str) -> str:
        ...
        well_known_path = f"mcp/{server_name}" if path.startswith(f"/mcp/{server_name}") else server_name
        ...
        for server_name in mcp_servers or []:
            ...
            if server and server.auth_type == MCPAuth.oauth2 and not oauth2_headers:
                ...
                raise HTTPException(
                    status_code=401,
                    detail="Unauthorized",
                    headers={"www-authenticate": _get_gateway_oauth2_challenge(scope, server_name)},
                )
    ...
    async def _check_oauth2_upstream_auth(
        ...
        probe_results = await asyncio.gather(*(_probe_oauth2_server(server) for server in candidate_servers))
