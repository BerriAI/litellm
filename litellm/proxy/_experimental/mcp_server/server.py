...
_byok_cred_cache: dict[tuple[str, str], tuple[str | None, float]] = {}
...
try:
    ...
    active_mcp_session_var: contextvars.ContextVar[_McpServerSession | None] = contextvars.ContextVar(
...
if MCP_AVAILABLE:
    ...
    async def handle_list_tools() -> "ListToolsResult | list[Tool]":
        ...
        try:
            ...
            verbose_logger.exception(f"Error in list_tools endpoint: {e}")
    ...
    async def list_prompts() -> list[Prompt]:
        ...
        try:
            ...
            verbose_logger.exception(f"Error in list_prompts endpoint: {e}")
            # Return empty list instead of failing completely
            # This prevents the HTTP stream from failing and allows the client to get a response
            return []
        finally:
    ...
    async def list_resources() -> list[Resource]:
        ...
        try:
            ...
            verbose_logger.exception(f"Error in list_resources endpoint: {e}")
            return []
        finally:
            if _session_reset_token is not None:
                active_mcp_session_var.reset(_session_reset_token)
    ...
    async def list_resource_templates() -> list[ResourceTemplate]:
        ...
        try:
            ...
            verbose_logger.exception(f"Error in list_resource_templates endpoint: {e}")
            return []
        finally:
            if _session_reset_token is not None:
                active_mcp_session_var.reset(_session_reset_token)
        ...
        try:
            ...
            async def _fetch_and_filter_server_tools(
                ...
                try:
                    ...
                    verbose_logger.exception(f"Error getting tools from server {server.name}: {e}")
                    return [], classify_list_exception(e)
    ...
    async def _get_prompts_from_mcp_servers(
        ...
        for server in allowed_mcp_servers:
            ...
            try:
                ...
                verbose_logger.exception(f"Error getting prompts from server {server.name}: {e}")
    ...
    async def _get_resources_from_mcp_servers(
        ...
        for server in allowed_mcp_servers:
            ...
            try:
                ...
                verbose_logger.exception(f"Error getting resources from server {server.name}: {e}")
    ...
    async def _list_mcp_tools(
        ...
        try:
            ...
            verbose_logger.exception(f"Error getting tools from managed MCP servers: {e}")
    ...
    async def _list_mcp_resources(
        ...
        try:
            ...
            verbose_logger.exception(f"Error getting resources from managed MCP servers: {e}")
    ...
    async def execute_mcp_tool(
        ...
        **kwargs: Any,
        ...
        requested_server_id: str | None = kwargs.get("requested_server_id")
    ...
    async def _handle_local_mcp_tool(
        ...
        try:
            ...
            verbose_logger.exception(f"Error executing local tool {name}: {e}")
