        ...
        *,
        ...
        new_server = MCPServer(
            ...
            token_endpoint_auth_method=(
                credentials_dict.get("token_endpoint_auth_method")
                if credentials_dict
                else None
            ),
    ...
    async def get_allowed_mcp_servers(
        ...
        try:
            ...
            if submitter_user_id:
                ...
                submitted_server_ids = await user_api_key_cache.async_get_cache(
                    key=byom_cache_key
                )
                if submitted_server_ids is None:
                    submitted_server_ids = (
                        await get_active_submitted_mcp_server_ids_for_user(
        ...
        try:
            ...
            if should_surface_upstream_auth:
                ...
                if auth_info is not None:
                    ...
                    verbose_logger.info(
                        f"Upstream auth failure from MCP server {server_name}: HTTP {status_code}"
                    )
