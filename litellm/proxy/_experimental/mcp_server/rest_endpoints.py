...
if MCP_AVAILABLE:
    ...
    def _tools_list_cancelled_response(
        server_name: Optional[str] = None,
    ) -> Dict[str, Any]:
        """Structured REST response when the client aborts tools/list."""
        if server_name:
            verbose_logger.warning(
                "tools/list cancelled for server %s (client disconnected)",
                server_name,
            )
        else:
            verbose_logger.warning(
                "tools/list request cancelled (client disconnected before completion)"
            )
        return {
            "tools": [],
            "error": "request_cancelled",
            "message": (
                "The request was cancelled before tools could be loaded. "
                "Connect OAuth for this server and try again."
            ),
        }
    ...
    async def _list_tools_for_single_server(
        ...
        user_oauth_extra_headers = await _get_user_oauth_extra_headers_from_db(
            server=server,
        ...
        try:
            ...
            return _tools_list_cancelled_response(server.name)
        except Exception as e:
    ...
    async def list_tool_rest_api(
        ...
        try:
            ...
            return _tools_list_cancelled_response()
        except Exception as e:
            verbose_logger.exception(
