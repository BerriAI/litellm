...
def _resolve_oauth2_server_for_root_endpoints(
    ...
    Root-level endpoints (e.g., /register, /authorize, /token) should NOT
    auto-resolve to any server. Clients must use server-specific paths
    (e.g., /{server_name}/authorize) to avoid returning OAuth metadata from
    an unrelated server, which pollutes non-OAuth server responses.

    Always returns None. Callers fall through to the appropriate default
    behaviour (404 or dummy response) when no server name is present.

    See: https://github.com/BerriAI/litellm/issues/22075
    """
    return None
