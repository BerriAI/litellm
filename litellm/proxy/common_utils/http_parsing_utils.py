...
async def get_form_data(request: Request) -> dict[str, Any]:
    ...
    form_items = tuple(form.multi_items() if hasattr(form, "multi_items") else form.items())
    array_keys = frozenset(key[:-2] for key, _ in form_items if key.endswith("[]"))
    normalized_items = tuple((key.removesuffix("[]"), value) for key, value in form_items)
    normalized_keys = frozenset(key for key, _ in normalized_items)
