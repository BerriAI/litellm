...
def _update_litellm_params_for_health_check(model_info: dict, litellm_params: dict) -> dict:
    ...
    - for Bedrock models with region routing (bedrock/region/model), strips the litellm routing prefix but preserves the model ID, carries that region into `aws_region_name` unless the deployment already sets one, and pins `custom_llm_provider` to `bedrock` (only when the deployment hasn't already set one, so an explicit `bedrock_converse` survives) so the bare model id still resolves to the provider (e.g. cross-region ids like `us.cohere.embed-v4:0`)
    """
    mode: Final = _resolve_health_check_mode(
        model_info,
        litellm_params,  # any-ok: untyped router config dict
    ...
    if litellm_params["model"].startswith("bedrock/"):
        ...
        parts: Final = litellm_params["model"].removeprefix("bedrock/").split("/")
        regions_in_path: Final = tuple(part for part in parts if part in BedrockModelInfo.all_global_regions)
        litellm_params["model"] = "/".join(part for part in parts if part not in BedrockModelInfo.all_global_regions)
        if regions_in_path and not litellm_params.get("aws_region_name"):  # any-ok: untyped router dict
            litellm_params["aws_region_name"] = regions_in_path[0]  # any-ok: untyped router dict
