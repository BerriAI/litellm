...
def read_model_list(config_path: str) -> List[dict]:
    """Load ``config_path`` and return its resolved ``model_list``.

    `os.environ/...` markers and secret-manager references are resolved using the
    proxy's own reader, so the result matches what the proxy itself would use.
    """
