...
def _resolve_openai_project(
    optional_params: GenericLiteLLMParams,
    extra_headers: Mapping[str, str] | None,
) -> str | None:
    """
    Resolve the OpenAI project id for batches/files calls.

    Precedence: explicit param > `OpenAI-Project` extra header > litellm.project
    global > OPENAI_PROJECT env var.
    """
    if optional_params.get("project"):
        return optional_params.get("project")
    if extra_headers:
        for header_name, header_value in extra_headers.items():
            if header_name.lower() == "openai-project":
                return header_value
    return litellm.project or os.getenv("OPENAI_PROJECT", None) or None
    ...
    **kwargs,
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            project: Final = _resolve_openai_project(optional_params, extra_headers)
            # set API KEY
            api_key = (
