...
def cost_per_token(
    ...
    if call_type == "speech" or call_type == "aspeech":
        ...
        input_cost_per_second: Optional[float] = model_info.get(
            "input_cost_per_second", None
        )
        output_cost_per_second: Optional[float] = model_info.get(
        ...
        if (
        ...
        elif output_cost_per_second is not None and response_time_ms is not None:
            print_verbose(
            ...
            completion_tokens_cost_usd_dollar = (
                output_cost_per_second * response_time_ms / 1000
            )
        elif input_cost_per_second is not None and response_time_ms is not None:
            print_verbose(
                f"For model={model} - input_cost_per_second: {model_info.get('input_cost_per_second')}; response time: {response_time_ms}"
            )
            ## COST PER SECOND ##
            prompt_tokens_cost_usd_dollar = (
                input_cost_per_second * response_time_ms / 1000
            )
