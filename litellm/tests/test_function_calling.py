...
def test_parallel_function_call_dashscope():
    try:
        # Step 1: send the conversation and available functions to the model
        messages = [
            {
        ...
        print("length of tool calls", len(tool_calls))
        print("Expecting there to be 1 tool calls") # only one tool call
        assert (
            len(tool_calls) == 1
        )  # this has to call the function for SF, Tokyo and parise
        ...
        if tool_calls:
            ...
            print(f"messages: {messages}")
            second_response = litellm.completion(
                model="dashscope/qwen-plus", messages=messages, temperature=0.2, seed=22
            )  # get a new response from the model where it can see the function response
            print("second response\n", second_response)
            return second_response
    except Exception as e:
...
#test_parallel_function_call_dashscope()
