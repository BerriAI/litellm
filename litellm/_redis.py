...
def _credential_provider_from_connect_func(redis_connect_func: object | None) -> CredentialProvider | None:
    ...
    fresh. Sync clusters need the same swap because redis-py's ``RedisCluster.on_connect`` runs
    the static AUTH before the user callback. Any other ``redis_connect_func`` is left where it
    is, since redis-py awaits it itself when it is a coroutine function."""
...
def _credential_provider_auth_kwargs(redis_kwargs: dict) -> dict:
    ...
    The sync cluster path needs the same swap because ``RedisCluster.on_connect`` runs the
    static AUTH before the user callback."""
