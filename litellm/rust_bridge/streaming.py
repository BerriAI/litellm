...
def set_rust_streaming(
    *,
    ...
    """Override native stream bindings in tests; passing ``None`` restores discovery."""
...
def supports_streaming(api: StreamApi, provider: str, transport: StreamTransport = "http") -> bool:
    ...
    except Exception:  # noqa: BLE001  # extension-defined errors cannot be named statically
        return False
...
def _context(api: StreamApi, provider: str, request: Mapping[str, object]) -> BridgeErrorContext:
    model: Final = request.get("model")
    return BridgeErrorContext(
        route=f"{api.replace('_', ' ')} stream",
        provider=provider,
...
) -> TypedEventStreamAdapter:
    if not isinstance(stream, RustEventStream):
        raise TypeError("native stream opener returned an invalid stream")
    return TypedEventStreamAdapter(stream, context)


class TypedEventStreamAdapter:
    def __init__(self, stream: RustEventStream, context: BridgeErrorContext) -> None:
    ...
    def __next__(self) -> Event:
        self._claim("sync")
        event: Final = call(self._stream.next_event, self._context)
        if event is None:
            raise StopIteration
        return event
    ...
    async def __anext__(self) -> Event:
        self._claim("async")
        event: Final = await acall(self._stream.anext_event, self._context)
        if event is None:
            raise StopAsyncIteration
        return event

    def close(self) -> None:
        call(self._stream.close, self._context)

    async def aclose(self) -> None:
        await acall(self._stream.aclose, self._context)
...
def open_stream(
    *,
    ...
    context: Final = _context(api, provider, request)
    result: Final = attempt(
        ...
        adapt=lambda stream: _adapt_stream(stream, context=context),
        context=context,
    )
