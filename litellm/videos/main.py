...
async def avideo_delete(
    ...
    **kwargs,
    ...
    """
    Asynchronously calls the `video_delete` function with the given arguments and keyword arguments.
    ...
    Returns:
    - `response` (VideoObject): The response returned by the `video_delete` function.
    """
    local_vars = locals()
    try:
        loop = asyncio.get_event_loop()
...
# Overload for when avideo_delete=True (returns Coroutine)
@overload
def video_delete(
    video_id: str,
    model: Optional[str] = None,
...
def video_delete(
    ...
    custom_llm_provider=None,
    *,
    avideo_delete: Literal[False] = False,
    **kwargs,
) -> VideoObject:
    ...
...
def video_delete(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    """
    Maps the https://api.openai.com/v1/videos/{video_id} endpoint.
    ...
    try:
        ...
        # get provider config
        video_delete_provider_config: Optional[BaseVideoConfig] = (
            ProviderConfigManager.get_provider_video_config(
                model=model,
                provider=litellm.LlmProviders(custom_llm_provider),
            )
        ...
        local_vars.update(kwargs)
        # For video delete, we need the video_id
        video_delete_request_params: Dict = {
            "video_id": video_id,
        }
        ...
        # Set the correct call type for video delete
