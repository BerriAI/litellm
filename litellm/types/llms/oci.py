...
class CohereChatRequest(BaseModel):
    ...
    # Current-turn tool results for an agentic continuation. OCI carries these in
    # this top-level field (not as a chatHistory entry) and requires an empty
    # ``message`` when it is set.
    toolResults: Optional[List[CohereToolResult]] = None
...
class CohereCitation(BaseModel):
    ...
    text: str
    # OCI returns ``documentIds`` (camelCase) and omits it entirely for
    # tool-grounded citations (which reference tool calls, not documents). These
    # citations are never surfaced by the response handler, so keep the field
    # optional rather than failing the whole response parse.
