...
class AiohttpOpenAIChatConfig(OpenAILikeChatConfig):
    ...
    async def transform_response(  # type: ignore
        ...
        _json_response = OpenAILikeChatConfig._sanitize_usage_obj(_json_response)
        model_response.id = _json_response.get("id")
        model_response.choices = [Choices(**choice) for choice in (_json_response.get("choices") or [])]
        ...
        _usage = _json_response.get("usage")
        if _usage is not None:
            setattr(model_response, "usage", Usage(**_usage))
