...
def _translate_operation_failure(
    error: "OperationFailure", index_name: str, database: str, collection: str
) -> BadRequestError:
...
def _translate_configuration_error(error: "ConfigurationError") -> BadRequestError | Timeout:
    detail: Final = str(error).lower()
    ...
    return config_error(f"mongodb_connection_string is not a usable MongoDB connection string. Driver detail: {error}")
