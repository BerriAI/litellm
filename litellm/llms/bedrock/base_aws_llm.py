...
SIGV4_COMPUTED_HEADERS = frozenset(
    {"authorization", "x-amz-date", "x-amz-security-token", "date"}
)
        ...
        if aws_bearer_token:
            ...
            for header_name, header_value in headers.items():
                if (
                    ...
                    and header_name.lower() not in SIGV4_COMPUTED_HEADERS
                ):
    ...
    def _sign_request(
        ...
        incoming_authorization = next(
            (
                ...
                if name.lower() == "authorization" and value is not None
            ),
            None,
        )
        if (
            incoming_authorization is not None
            and not incoming_authorization.startswith("AWS4-HMAC-SHA256")
        ):  # prevent sigv4 from overwriting the auth header
