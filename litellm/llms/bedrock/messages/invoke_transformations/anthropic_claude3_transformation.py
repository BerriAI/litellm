...
class AmazonAnthropicClaudeMessagesConfig(
    ...
    async def _promote_message_stop_usage(
        ...
        Bedrock reports only non-cached tokens as input_tokens across all
        events (message_start, message_delta, message_stop), with the cache
        breakdown (cache_creation_input_tokens, cache_read_input_tokens)
        appearing on message_stop. Claude Code's SDK merges usage from
        message_start and message_delta but ignores message_stop. This
        method buffers message_delta and, when message_stop arrives with
        cache usage, merges those fields into message_delta and recomputes
        input_tokens = uncached + cache_creation + cache_read so clients
        see the true total.
        """
