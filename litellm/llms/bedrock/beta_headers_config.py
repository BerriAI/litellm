...
# Minor version is a single digit followed by a hyphen or end-of-string (to avoid
# capturing the date). The `(?:-|$)` lookahead handles both suffix-less IDs like
# `anthropic.claude-sonnet-4-6` and the more common `...-4-6-20250514-v1:0` form.
...
BETA_HEADER_MINIMUM_VERSION: Dict[str, float] = {
    ...
    # Latest computer use requires Claude Opus 4.5+ (see also family restriction)
    "computer-use-2025-11-24": 4.5,
...
BETA_HEADER_FAMILY_RESTRICTIONS: Dict[str, List[str]] = {
    "computer-use-2025-11-24": ["opus", "sonnet"],  # Opus 4.5+, Sonnet 4.6+
    "context-management-2025-06-27": ["sonnet", "haiku"],  # Not supported on Opus 4.5
...
class BedrockBetaHeaderFilter:
    ...
    def filter_beta_headers(
        ...
        # Filter: Keep only whitelisted headers that the model supports
        filtered = {
            beta
            for beta in beta_set
            if beta in self.supported_betas and self._model_supports_beta(model, beta)
        }
