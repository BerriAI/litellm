...
class BedrockFilesHandler(BaseAWSLLM):
    ...
    async def async_create_file(
        self, create_file_data: CreateFileRequest, **aws_params
    ) -> OpenAIFileObject:
        ...
        **aws_params,
        ...
        if _is_async:
            return self.async_create_file(
                create_file_data=create_file_data, **aws_params
            )
        else:
            import asyncio

            return asyncio.run(
                self.async_create_file(create_file_data=create_file_data, **aws_params)
            )
