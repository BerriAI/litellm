...
class AnthropicMessagesHandler(BaseTranslation):
    ...
    async def process_output_response(
        ...
        task_mappings: List[Tuple[int, Optional[int]]] = []
        response_content = self._get_output_response_content(response)

        if not response_content:
            return response

        self._extract_output_content_blocks(
        ...
        if texts_to_check or tool_calls_to_check:
            request_data = self._prepare_output_request_data(
                request_data=request_data,
                response=response,
                user_api_key_dict=user_api_key_dict,
            )
            inputs = self._build_output_guardrail_inputs(
                response=response,
                texts_to_check=texts_to_check,
                images_to_check=images_to_check,
                tool_calls_to_check=tool_calls_to_check,
    ...
    def _get_output_response_content(self, response: Any) -> List[Any]:
    ...
    def _extract_output_content_blocks(
        self,
        ...
        for content_idx, content_block in enumerate(response_content):
            block_dict, block_type = self._normalize_output_content_block(content_block)
            if block_dict is None or block_type not in ["text", "tool_use"]:
                continue
    ...
    def _normalize_output_content_block(
        self, content_block: Any
    ) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
        if isinstance(content_block, dict):
            return cast(Dict[str, Any], content_block), content_block.get("type")
        ...
        if hasattr(content_block, "model_dump"):
            return cast(Dict[str, Any], content_block.model_dump()), block_type
        return {
            "type": block_type,
            "text": getattr(content_block, "text", None),
        }, block_type

    def _prepare_output_request_data(
        self,
    ...
    def _build_output_guardrail_inputs(
        self,
        response: Any,
        texts_to_check: List[str],
        images_to_check: List[str],
