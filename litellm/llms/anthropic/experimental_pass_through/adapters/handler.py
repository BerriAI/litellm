    ...
    async def async_anthropic_messages_handler(
        ...
        **kwargs,
        ...
        if stream:
            ...
            # Use same post-response extraction as non-streaming path; fall back
            # to kwargs/metadata for backward compatibility.
            ...
            if (
                not litellm_call_id
                and hasattr(completion_response, "logging_obj")
            ):
                litellm_call_id = getattr(
                    completion_response.logging_obj, "litellm_call_id", None
                )
            if not litellm_call_id and completion_kwargs:
