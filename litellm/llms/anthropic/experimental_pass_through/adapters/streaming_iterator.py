...
class _CombinedChunkSplitter:
    ...
    def _is_combined(chunk: "ModelResponseStream") -> bool:
        ...
        choices = _optional_attr_sequence(chunk, "choices")
        if not choices:
            return False
        choice = choices[0]
        if _optional_attr(choice, "finish_reason") is None:
            return False
        delta = _optional_attr(choice, "delta")
        if delta is None:
            return False
        return bool(
    ...
    def _split_by_payload_kind(chunk: "ModelResponseStream") -> "tuple[ModelResponseStream, ...]":
        ...
        if len(choices) != 1:
            return (chunk,)
        delta = _optional_attr(choices[0], "delta")
        if delta is None:
            return (chunk,)
        tool_calls = _optional_attr_sequence(delta, "tool_calls")
...
class AnthropicStreamWrapper(AdapterCompletionStreamWrapper):
    ...
    def _augment_message_delta_usage(self, message_delta_chunk: MessageBlockDelta) -> MessageBlockDelta:
        ...
        augmented = message_delta_chunk.copy()
        augmented_usage: _UsageDeltaWithIterations = {**usage}
