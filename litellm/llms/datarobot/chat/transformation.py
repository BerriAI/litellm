...
class DataRobotConfig(OpenAILikeChatConfig):
    ...
    def _resolve_api_key(api_key: Optional[str] = None) -> str:
        ...
        over the secret manager key (``DATAROBOT_API_KEY``).

        If both are None, a fake API key is returned for testing.
        """
        return api_key or get_secret_str("DATAROBOT_API_KEY") or "fake-api-key"
    ...
    def _resolve_api_base(api_base: Optional[str] = None) -> Optional[str]:
        ...
        over the secret manager key (``DATAROBOT_API_BASE``).
        ...
        """
        api_base = api_base or get_secret_str("DATAROBOT_API_BASE")
    ...
    def _get_openai_compatible_provider_info(
        ...
        before falling back to secret manager values (``DATAROBOT_API_BASE`` and ``DATAROBOT_API_KEY``
        respectively).
