    ...
    async def post(
        ...
        try:
            ...
            if stream is True:
                ...
                tail = (body.decode(errors="ignore") if isinstance(body, (bytes, bytearray)) else str(body))
                tail = mask_sensitive_info(tail)[:256]
                setattr(e, "message", tail)
                setattr(e, "text", tail)
            else:
                tail = mask_sensitive_info(e.response.text)[:256]
                setattr(e, "message", tail)
                setattr(e, "text", tail)
