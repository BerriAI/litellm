            ...
            try:
                response = await async_httpx_client.post(
                    ...
                    data=(
                        ...
                        else json.dumps(data, **provider_config.get_json_dumps_params())
                    ),
                    timeout=timeout,
                    stream=stream,
    ...
    def embedding(
        ...
        try:
            response = sync_httpx_client.post(
                url=api_base,
                headers=headers,
                data=json.dumps(data, **provider_config.get_json_dumps_params()),
                timeout=timeout,
            )
        except Exception as e:
    ...
    async def arerank(
        ...
        try:
            response = await async_httpx_client.post(
                ...
                headers=headers,
                data=json.dumps(
                    request_data, **provider_config.get_json_dumps_params()
                ),
                timeout=timeout,
            )
        except Exception as e:
