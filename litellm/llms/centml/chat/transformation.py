...
class CentmlConfig(OpenAIGPTConfig):
    ...
    def get_supported_openai_params(self, model: str) -> list:
        ...
        try:
            ...
            full_model_name = f"centml/{model}" if not model.startswith(
                "centml/") else model
            # Use _get_model_info_helper to avoid circular dependency with get_supported_openai_params
            model_info = _get_model_info_helper(full_model_name,
                                                custom_llm_provider="centml")
            supports_function_calling = model_info.get(
                'supports_function_calling', False)
            supports_response_schema = model_info.get(
        ...
        if supports_function_calling is not True:
            verbose_logger.debug(
                "This CentML model does not support function calling")
        ...
        if supports_response_schema is not True:
            verbose_logger.debug(
                "This CentML model does not support response_format/JSON schema"
            )
