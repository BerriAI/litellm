"""
Transformation logic from Cohere's /v1/rerank format to BurnCloud's  `/v1/rerank` format.
"""
...
class BurnCloudRerankConfig(CohereRerankConfig):
    ...
    def transform_rerank_response(
        ...
        """
        Transform BurnCloud rerank response

        No transformation required, BurnCloud follows Cohere API response format
        """
        try:
            raw_response_json = raw_response.json()
        except Exception:
            raise BurnCloudError(
                status_code=raw_response.status_code,message=raw_response.text
            )
