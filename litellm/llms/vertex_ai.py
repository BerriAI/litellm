...
class VertexAi(BaseLLM):
    ...
    def _update_cache(self, cache_key: lambda: str, create_model: Callable, client_ttl: int = 3600):
        cached_model = self._cache.get_cache(cache_key)
        if cached_model is not None:
            return cached_model
        
        model = create_model()
        self._cache.set_cache(cache_key, model, ttl=client_ttl)
        return model
    ...
    def completion(
        ...
        try:
            ...
            if (
                ...
                mode = "vision"
                cache_key = f"{model}_{mode}"
                llm_model = self._update_cache(cache_key, lambda: GenerativeModel(model))
                request_str += f"llm_model = GenerativeModel({model})\n"
            elif model in litellm.vertex_chat_models:
                mode = "chat"
                cache_key = f"{model}_{mode}"
                llm_model = self._update_cache(cache_key, lambda: ChatModel.from_pretrained(model))
                request_str += f"llm_model = ChatModel.from_pretrained({model})\n"
            elif model in litellm.vertex_text_models:
                mode = "text"
                cache_key = f"{model}_{mode}"
                llm_model = self._update_cache(cache_key, lambda: TextGenerationModel.from_pretrained(model))
                request_str += f"llm_model = TextGenerationModel.from_pretrained({model})\n"
            elif model in litellm.vertex_code_text_models:
                mode = "text"
                cache_key = f"{model}_{mode}"
                llm_model = self._update_cache(cache_key, lambda: CodeGenerationModel.from_pretrained(model))
                request_str += f"llm_model = CodeGenerationModel.from_pretrained({model})\n"
            elif model in litellm.vertex_code_chat_models:  # vertex_code_llm_models
                mode = "chat"
                cache_key = f"{model}_{mode}"
                llm_model = self._update_cache(cache_key, lambda: CodeChatModel.from_pretrained(model))
                ...
                cache_key = f"{model}_{mode}"
                llm_model = self._update_cache(cache_key, lambda: aiplatform.PrivateEndpoint(
                    endpoint_name=model,
            ...
            if acompletion == True:
                ...
                if optional_params.get("stream", False) is True:
                    # async streaming
                    return self._async_streaming(**data)

                return self._async_completion(**data)
    ...
    def embedding(
        ...
        try:
            cache_key = f"{model}_embedding"
            llm_model = self._update_cache(cache_key, lambda: TextEmbeddingModel.from_pretrained(model))
        except Exception as e:
            raise VertexAIError(status_code=422, message=str(e))

        if aembedding == True:
            return VertexAi._async_embedding(
                model=model,
                client=llm_model,
                input=input,
                logging_obj=logging_obj,
