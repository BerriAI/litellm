...
class TinyfishSearchConfig(BaseSearchConfig):
        ...
        **kwargs: object,
        ...
        for param, value in optional_params.items():
            if param not in supported_perplexity and param not in request_data:
                # TinyFish Search's structured params (JSON objects / arrays)
                # arrive on the wire as JSON-encoded strings. Accept the
                # natural Python dict/list form and serialize here so callers
                # don't have to pre-stringify.
    ...
    def transform_search_response(
        ...
        **kwargs: object,
        ...
        - all other per-result fields (``position``, ``site_name``, and any
          others TinyFish returns) ride through as extras on ``SearchResult``
          via its ``extra="allow"`` config.
        ...
        # Truncate results in place and return the parsed instance. Preserves
        # every field pydantic populated — declared (results, object) AND
        # extras (query, total_results, page, parameter_warnings, and any
        # future TinyFish additions) — without depending on which storage
        # bucket they live in. Robust against upstream schema evolution: if
        # LiteLLM later promotes any current extra to a declared field on
        # SearchResponse, this code needs no change.
        parsed.results = list(parsed.results[:max_results])
        # Surface TinyFish's response headers on the success path. Matches the
        # convention used by Gemini/Volcengine/Manus/ChatGPT/OpenAI responses
        # providers. `headers` is the raw dict for debugging; `additional_headers`
        # is passed through process_response_headers, which prefixes any
        # x-litellm-* provider header with llm_provider- so downstream LiteLLM
        # code that trusts bare x-litellm-* markers can't be spoofed (the
        # values are preserved under the prefixed key for observability).
