...
# Mapping from Vertex AI Live API fields to OpenAI realtime events
...
class VertexAIRealtimeConfig(BaseRealtimeConfig, VertexBase):
    ...
    def __init__(self):
        ...
        self.vertex_project = get_secret_str("VERTEXAI_PROJECT")
        self.vertex_location = get_secret_str("VERTEXAI_LOCATION") or get_secret_str("VERTEX_LOCATION")
    ...
    def validate_environment(
        ...
        """
        # Extract project and location from headers or use environment defaults
        vertex_credentials = get_secret("VERTEXAI_CREDENTIALS")
        resolved_project = headers.get("x-goog-user-project") or self.vertex_project
        location = headers.get("x-goog-vertex-location") or self.vertex_location
        print(f"🔥Project ID: {self.vertex_project}")
        ...
        self.vertex_project = resolved_project
    ...
    def transform_realtime_request(
        ...
        messages: List[str] = []
        message_type: Optional[str] = json_message.get("type")
        if message_type == "session.update":
            client_session_configuration_request = self.map_openai_params(
                optional_params={}, non_default_params=json_message["session"]
            )

            vertex_project = self.vertex_project or get_secret_str("VERTEXAI_PROJECT")
            vertex_location = self.vertex_location or get_secret_str("VERTEXAI_LOCATION") or "us-central1"
            
            if vertex_project:
                client_session_configuration_request["model"] = f"projects/{vertex_project}/locations/{vertex_location}/publishers/google/models/{model}"
    ...
    def transform_session_created_event(
        ...
        vertex_modalities = generation_config.get("responseModalities", ["TEXT"])
        _modalities = [
            modality.lower() for modality in cast(List[str], vertex_modalities)
        ]
    ...
    def map_openai_event(
        ...
        if model_turn_event:  # check if model turn event
            ...
            for map_key, openai_event in MAP_VERTEX_FIELD_TO_OPENAI_EVENT.items():
                if map_key == key or (
                    "." in map_key
                    and VertexAIRealtimeConfig.get_nested_value(json_message, map_key)
                    is not None
                ):
    ...
    #         # Normalize type for easier handling
    #         message_type: Optional[str] = json_message.get("type")

    #         # 1) Session configuration → Vertex "setup"
    #         if message_type == "session.update":
    #             client_session_configuration_request = self.map_openai_params(
    #                 optional_params={}, non_default_params=json_message["session"]
    #             )
    #             # Convert model to full Vertex AI resource name format
    #             # Expected format: projects/{PROJECT_ID}/locations/{LOCATION}/publishers/google/models/{MODEL_ID}
    #             if not model.startswith("projects/"):
    #                 # Get project and location from the class instance
    #                 vertex_project = self.vertex_project or get_secret_str("VERTEXAI_PROJECT")
    #                 vertex_location = self.vertex_location or get_secret_str("VERTEXAI_LOCATION") or "us-central1"
    ...
    #                     client_session_configuration_request["model"] = f"projects/{vertex_project}/locations/{vertex_location}/publishers/google/models/{model}"
    #                 else:
    #                     # Fallback to simple format if project not available
    #                     client_session_configuration_request["model"] = f"models/{model}"
    #             else:
    #                 client_session_configuration_request["model"] = model
    ...
    #                 )
    #             )
    #             print(f"🔥Client session configuration request: {client_session_configuration_request}")
    #             return messages

    #         # 2) User content creation (text/audio) → Vertex "client_content"
    ...
    #                 for c in content:
    #                     ctype = c.get("type") or c.get("content_type")
    #                     # Handle text variants
    #                     if ctype in ("text", "input_text") or "text" in c:
    #                         text_val = c.get("text", c.get("value", ""))
    #                         parts.append({"text": text_val})
    #                     # Handle audio variants
    #                     elif ctype in ("input_audio", "audio") or "audio" in c or "data" in c:
    #                         audio_b64 = c.get("audio") or c.get("data") or ""
    #                         if audio_b64:
    #                             parts.append(
    #                                 {
    #                                     "inlineData": {
    ...
    #             else:
    #                 # Unknown structure, fallback to string
    ...
    #                 }
    #             }
    #             messages.append(json.dumps(vertex_message))
    #             return messages

    #         # 3) Audio streaming (append) → Vertex "realtime_input.audio"
    #         if message_type == "input_audio_buffer.append":
    #             audio_data = json_message.get("audio") or json_message.get("data") or ""
    #             if audio_data:
    #                 realtime_input_dict = {
    #                     "audio": HttpxBlobType(
    #                         mimeType=self._get_audio_mime_type(), data=audio_data
    #                     )
    #                 }
    #                 realtime_input_dict = cast(
    #                     Dict[str, object],
    ...
    #         # 4) End of input / trigger response → Vertex "client_content.turn_complete": true
    ...
    #         # 5) Simple JSON with {"text": ...}
    #         if "text" in json_message and isinstance(json_message["text"], str):
    #             vertex_message = {
    #                 "client_content": {
    #                     "turns": [
    #                         {"role": "user", "parts": [{"text": json_message["text"]}]}
    #                     ],
    #                     "turn_complete": True,
    #                 }
    #             }
    ...
    #         # Fallback: treat entire JSON as a text turn
    #         vertex_message = {
    #             "client_content": {
