...
def _iter_openai_jsonl_lines(openai_file_content: FileTypes) -> Iterator[str]:
    ...
    payload as a list. Large Gemini batch uploads (~1 GB) OOM-restart the proxy
    when the payload is copied into several full intermediate lists, so the hot
    path streams instead. Mirrors ``str.splitlines()`` + ``line.strip()``
    filtering for ``\\n`` / ``\\r\\n`` delimited JSONL.
    """
...
def _stream_openai_jsonl_to_vertex(
    ...
    Returns the joined Vertex JSONL payload (bytes for the async upload path so
