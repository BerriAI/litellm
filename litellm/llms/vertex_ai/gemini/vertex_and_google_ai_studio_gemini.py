...
def _gemini_convert_messages_with_history(
    ...
    """
    Gemini requires a specific message structure.
    - No system message. First user message can contain system prompt.
    - Roles must alternate between 'user' and 'model'.
    - 'tool' role is not supported. Tool responses are sent with 'function' role.
    """
    gemini_messages: List[ContentType] = []
    for message in messages:
        if message["role"] == "system":
            if gemini_messages and gemini_messages[0]["role"] == "user":
                if isinstance(message["content"], str):
                    gemini_messages[0]["parts"].insert(0, {"text": message["content"]})
            else:
                if isinstance(message["content"], str):
            ...
            gemini_message: ContentType = {"role": "model", "parts": []}
            ...
            if tool_calls:
                for tool_call in tool_calls:
                    ...
                    if (
                        "thought_signature" in tool_call
                        and tool_call["thought_signature"] is not None
                    ):
                        function_call_part["thoughtSignature"] = tool_call[
                            "thought_signature"
                        ]

                    gemini_message["parts"].append({"functionCall": function_call_part})

            if gemini_message["parts"]:
                gemini_messages.append(gemini_message)

        elif message["role"] == "tool":
            function_name = "not-used"
                ...
                if m["role"] == "assistant" and m.get("tool_calls"):
                    for tool_call in m["tool_calls"]:
                        if tool_call["id"] == tool_call_id:
                            ...
                            break
                    if function_name != "not-used":
                        break

            try:
                content_json = json.loads(message["content"])
            except (json.JSONDecodeError, TypeError):
                content_json = {"content": str(message["content"])}
