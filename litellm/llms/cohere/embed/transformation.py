...
class CohereEmbeddingConfig(BaseEmbeddingConfig):
    ...
    def _transform_request(
        self,
        model: str,
        input: list[str] | list[CohereEmbeddingContent],  # mutable-ok: matches the embedding request contract
        inference_params: dict,  # mutable-ok: request options are merged into the wire payload
    ) -> CohereEmbeddingRequestWithModel:
        if input and isinstance(input[0], dict):
            transformed_request = CohereEmbeddingRequestWithModel(
                model=model,
                inputs=cast(  # cast-ok: the dictionary branch identifies Cohere multimodal inputs
                    list[CohereEmbeddingContent], input
                ),
                input_type=COHERE_DEFAULT_EMBEDDING_INPUT_TYPE,
            )
        elif input and is_base64_encoded(
            cast(list[str], input)[-1]  # cast-ok: non-dictionary inputs follow the text/image contract
        ):
            transformed_request = CohereEmbeddingRequestWithModel(
                model=model,
                images=cast(  # cast-ok: the base64 branch identifies image strings
                    list[str], input
                ),
                input_type="image",
            )
            ...
            transformed_request = CohereEmbeddingRequestWithModel(
                model=model,
                texts=cast(  # cast-ok: remaining inputs follow the text embedding contract
                    list[str], input
                ),
                input_type=COHERE_DEFAULT_EMBEDDING_INPUT_TYPE,
            )
    ...
    def transform_embedding_request(
        ...
        if isinstance(input, list) and input and (isinstance(input[0], list) or isinstance(input[0], int)):
        ...
        return cast(
            ...
            self._transform_request(
                model=model,
                input=(
                    cast(  # cast-ok: LiteLLM's public input type does not yet include Cohere multimodal dictionaries
                        list[str] | list[CohereEmbeddingContent], input
                    )
    ...
    def _calculate_usage(self, input: list[str | CohereEmbeddingContent], encoding: Any, meta: dict) -> Usage:
        input_tokens = 0
                ...
                if isinstance(item, dict):
                    for part in item.get("content", []):
                        if isinstance(part, dict) and part.get("type") == "text":
                            input_tokens += len(encoding.encode(part.get("text", "")))
                else:
                    input_tokens += len(encoding.encode(item))
        else:
