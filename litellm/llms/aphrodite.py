...
class AphroditeError(Exception):
    def __init__(self, status_code, message):
        ...
        self.request = httpx.Request(method="POST", url="http://0.0.0.0:2242")
...
# check if aphrodite is installed
def validate_environment(model: str):
    global llm
    try: 
        from aphrodite import LLM, SamplingParams # type: ignore
        if llm is None:
            llm = LLM(model=model)
        return llm, SamplingParams
    except Exception as e:
        raise AphroditeError(status_code=0, message=str(e))
...
def completion(
    ...
    if llm:
        ...
        raise AphroditeError(status_code=0,
                             message="Need to pass in a model name "
                                     "to initialize Aphrodite Engine.")
    ...
    if "stream" in optional_params and optional_params["stream"] == True:
        ...
        model_response["created"] = int(time.monotonic())
...
def batch_completions(
    ...
    responses = batch_completion(
        model="aphrodite/EleutherAI/pythia-70m",
        messages = [
            [
                {
                    "role": "user",
    ...
    try:
        ...
        if "data parallel group is already initialized" in error_str:
            ...
            raise AphroditeError(status_code=0, message=error_str)
    sampling_params = SamplingParams(**optional_params)
    prompts = [] 
    if model in custom_prompt_dict:
        # check if the model has a registered custom prompt
        ...
        for message in messages:
            prompt = custom_prompt(
                ...
                initial_prompt_value=(
                    model_prompt_details["initial_prompt_value"],)
                final_prompt_value=model_prompt_details["final_prompt_value"],
                messages=message
            )
    ...
    if llm:
        ...
        raise AphroditeError(status_code=0,
                             message="Need to pass in a model name to "
