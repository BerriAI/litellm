...
def _get_commandcode_credentials(
    ...
    """
    resolved_base = (
        api_base or get_secret_str("COMMANDCODE_API_BASE") or get_secret_str("CMD_API_BASE") or COMMANDCODE_API_BASE
    ).rstrip("/")

    resolved_key = api_key or get_secret_str("COMMANDCODE_API_KEY") or get_secret_str("CMD_API_KEY")

    if resolved_key is None:
        raise CommandCodeError(
            "No CommandCode API key found. Set COMMANDCODE_API_KEY environment variable or pass api_key parameter."
        )
...
class CommandCodeOpenAIConfig(OpenAIGPTConfig):
    ...
    def _get_openai_compatible_provider_info(
        ...
        dynamic_api_key = api_key or get_secret_str("COMMANDCODE_API_KEY") or get_secret_str("CMD_API_KEY")
