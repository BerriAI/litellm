...
def prompt_factory(
    ...
    try:
        if "meta-llama/llama-2" in model and "chat" in model:
            ...
            chat_template = "<BOS_TOKEN>{%- if messages[0]['role'] == 'system' -%} {%- set loop_messages = messages[1:] -%} {%- set system_message = messages[0]['content'] -%} {%- else -%} {%- set loop_messages = messages -%} {%- set system_message = false -%} {%- endif -%} {%- if system_message -%} {{- '<|START_OF_TURN_TOKEN|><|SYSTEM_TOKEN|>' + system_message + '<|END_OF_TURN_TOKEN|>' -}} {%- endif -%} {%- for message in loop_messages -%} {%- if message['role'] == 'user' -%} {{- '<|START_OF_TURN_TOKEN|><|USER_TOKEN|>' + message['content'].strip() + '<|END_OF_TURN_TOKEN|>' -}} {%- elif message['role'] == 'assistant' -%} {{- '<|START_OF_TURN_TOKEN|><|CHATBOT_TOKEN|>' + message['content'].strip() + '<|END_OF_TURN_TOKEN|>' -}} {%- endif -%} {%- endfor -%}{{- '<|START_OF_TURN_TOKEN|><|CHATBOT_TOKEN|>' -}}"
            return hf_chat_template(
                model=model, messages=messages, chat_template=chat_template
            )
        else:
