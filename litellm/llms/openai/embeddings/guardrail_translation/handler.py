...
class OpenAIEmbeddingsHandler(BaseTranslation):
    ...
    async def _process_list_input(
        self,
        data: dict,
        input_data: list[str | int | list[int] | dict],
        guardrail_to_apply: "CustomGuardrail",
        litellm_logging_obj: Any | None,
    ) -> dict:
        ...
        if isinstance(first_item, dict):
            # Multimodal (Cohere-style) inputs: each item is
            # {"content": [{"type": "text", "text": ...} | {"type": "image_url", ...}]}.
            # Scan and replace the text parts in place; leave image parts untouched.
            text_positions: list[tuple[int, int]] = []
            texts_to_scan: list[str] = []
            for item_idx, item in enumerate(input_data):
                if not isinstance(item, dict):
                    continue
                for part_idx, part in enumerate(item.get("content") or []):
                    if isinstance(part, dict) and part.get("type") == "text" and isinstance(part.get("text"), str):
                        text_positions.append((item_idx, part_idx))
                        texts_to_scan.append(part["text"])

            if not texts_to_scan:
                verbose_proxy_logger.debug(
                    "OpenAI Embeddings: Multimodal input has no text parts, skipping guardrail processing"
                )
                return data

            multimodal_inputs: Final = GenericGuardrailAPIInputs(texts=texts_to_scan)
            if model := data.get("model"):
                multimodal_inputs["model"] = model

            guardrailed_multimodal: Final = await guardrail_to_apply.apply_guardrail(
            ...
            if guardrailed_multimodal_texts := guardrailed_multimodal.get("texts"):
                for (item_idx, part_idx), guardrailed_text in zip(text_positions, guardrailed_multimodal_texts):
                    input_data[item_idx]["content"][part_idx]["text"] = guardrailed_text
                data["input"] = input_data
                verbose_proxy_logger.debug(
                    "OpenAI Embeddings: Applied guardrail to %d multimodal text parts",
                    len(guardrailed_multimodal_texts),
                )
