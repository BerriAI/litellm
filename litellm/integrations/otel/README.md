...
1. **App creation** (`proxy_server` import): when the gate is on,
   ...
   By default the instrumentation emits the legacy `http.status_code` and no error
   attribute, so the SERVER span carries no machine-readable error marker on failures.
   Set `OTEL_SEMCONV_STABILITY_OPT_IN=http` to switch it to the new HTTP semantic
   conventions, which is what makes the span carry `http.response.status_code` and an
   `error.type` on 5xx (matching the new-semconv names the gen-ai spans already use).
   Use `http/dup` instead to keep the legacy `http.status_code` alongside the new
   attributes. The flag is comma-separated and shared with the gen-ai opt-in, so it
   can carry both, e.g. `OTEL_SEMCONV_STABILITY_OPT_IN=http,gen_ai_latest_experimental`.
