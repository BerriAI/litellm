...
class AsyncLangFuseLogger:
    ...
    def generate_headers(self):
        return {
            "Authorization": "Basic "
            + b64encode(f"{self.public_key}:{self.secret_key}".encode("utf-8")).decode(
                "ascii"
            ),
    ...
    async def async_log_success_event(
        ...
        try:
            ...
            trace_id, generation_id = await self._log_langfuse_v2(
                user_id,
                metadata,
                litellm_params,
                output,
    ...
    async def _log_langfuse_v2(
        ...
        try:
            ...
            # print("end_time in log_langfuse_v2: ", end_time)
            ...
            await self.log_to_langfuse_ingestions([_trace_to_log, _generation_to_log])
    ...
    async def log_to_langfuse_ingestions(self, batch: list):
        try:
            import json

            # print("about to log using httpx client - \n data=", batch, "\n host=", self.langfuse_host)

            _url = self.langfuse_host + "/api/public/ingestion"
            _headers = self.generate_headers()
            ...
            response = await self.async_http_handler.post(
                url=_url,
                headers=_headers,
                data=json.dumps(_data, default=str),
            )
            ...
            print("got exception in log_to_langfuse_ingestions", e)  # noqa
