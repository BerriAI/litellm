...
Flow:
  1. POST /metrics/agent/ai/{connection_id}          → register, get metricsMarker
  2. GET  /metrics/agent/ai/{connection_id}/upload-url → GCS signed URL
  3. PUT  <signed_url>                                → upload gzip CSV
  4. PATCH /metrics/agent/ai/{connection_id}          → advance metricsMarker
"""
...
class FocusMavvrikDestination(FocusDestination):
    ...
    async def update_metrics_marker(self, date_epoch: int) -> None:
        """PATCH the metricsMarker to date_epoch after a successful upload.

        Called after each successful deliver() so the Mavvrik API knows which
        date has been fully uploaded. Without this call the marker never
        advances and the catch-up logic re-exports the same dates every run.
        """
        resp = await self._http.client.request(
            method="PATCH",
            url=self._agent_url,
        ...
        if resp.status_code >= 400:
            verbose_logger.warning(
                ...
                "(%s): %s — export was uploaded but marker not advanced",
                resp.status_code,
                resp.text[:200],
            )
            return
        verbose_logger.debug(
            "Mavvrik FOCUS destination: metricsMarker updated to %s", date_epoch
        )
    ...
    async def deliver(
        ...
        *,
        ...
        After a successful upload, advances metricsMarker via PATCH so the
        catch-up logic on the next run does not re-export this date.
        """
        if not content:
            verbose_logger.debug(
        ...
        await self.update_metrics_marker(date_epoch)
