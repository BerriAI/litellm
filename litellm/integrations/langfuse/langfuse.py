...
class LangFuseLogger:
    ...
    def __init__(
        ...
        if os.getenv("UPSTREAM_LANGFUSE_SECRET_KEY") is not None:
            upstream_langfuse_debug = (
                str_to_bool(self.upstream_langfuse_debug) if self.upstream_langfuse_debug is not None else None
            )
    ...
    def _log_langfuse_v2(  # noqa: PLR0915
        ...
        try:
            ...
            # Add usage_object from standard_logging_object if available
            if standard_logging_object is not None and "metadata" in standard_logging_object:
                standard_metadata = standard_logging_object["metadata"]
                if "usage_object" in standard_metadata and standard_metadata["usage_object"] is not None:
                    clean_metadata["usage_object"] = standard_metadata["usage_object"]
            ...
            if standard_logging_object is not None:
                clean_metadata["hidden_params"] = standard_logging_object["hidden_params"]
...
def strip_null_values(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        ...
        return [strip_null_values(item) for item in data]
    else:
        return data
...
def log_requester_metadata(clean_metadata: dict):
    ...
    # Strip null values from the metadata before returning
    return strip_null_values(returned_metadata)
