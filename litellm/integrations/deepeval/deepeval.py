...
class DeepEvalLogger(CustomLogger):
    ...
    def __init__(self) -> None:
        try:
            ...
            # Make these imports available as class attributes
            self.ToolAttributes = ToolAttributes
            self.update_current_span_attributes = update_current_span_attributes
    ...
    def _handle_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        def send_litellm_message_failure_trace(input, output):
        ...
        send_litellm_message_failure_trace(
            input=_prepare_input_str(kwargs["input"]), output=error_str
        )
    ...
    def _handle_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        # Get cost per token
        try:
            _cost_per_input_token, _cost_per_output_token = self._get_cost_per_token(
                kwargs
            )
        ...
        @self.observe(
            ...
            cost_per_output_token=_cost_per_output_token,
            cost_per_input_token=_cost_per_input_token,
            metrics=metrics,
        )
        def send_litellm_message_success_trace(
            input, output, input_token_count=None, output_token_count=None
        ):
            self.update_current_span_attributes(
                self.LlmAttributes(
                    input=input,
    ...
    def _get_cost_per_token(self, kwargs):
        ...
        if litellm_params and litellm_params.get("input_cost_per_token") is not None:
            ...
            model_map_value and model_map_value.get("input_cost_per_token") is not None
        ):
        ...
        if litellm_params and litellm_params.get("output_cost_per_token") is not None:
            ...
            model_map_value and model_map_value.get("output_cost_per_token") is not None
        ):
