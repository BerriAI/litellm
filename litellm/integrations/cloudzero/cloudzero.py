...
class CloudZeroLogger(CustomLogger):
    ...
    async def _fetch_cbf_data_for_hour(
        ...
        verbose_logger.debug(
            f"CloudZero Logger: Loading spend logs for hour {target_hour}"
        )
        data = await database.get_usage_data_for_hour(
            target_hour=target_hour, limit=limit
        )

        if data.is_empty():
            verbose_logger.info(
                "CloudZero Logger: No usage data found for the specified hour"
            )
            ...
            while True:
                try:
                    ...
                    if lock_acquired:
                        try:
                            verbose_logger.info(
                                f"CloudZero Background Job: Starting export for hour {target_hour}"
                            )
                            await self.export_usage_data(target_hour)
                            verbose_logger.info(
                                f"CloudZero Background Job: Completed export for hour {target_hour}"
                            )
                        finally:
                        ...
                        verbose_logger.debug(
                            "CloudZero Background Job: Another instance is already running the export"
                        )
                    ...
                    verbose_logger.error(
                        f"CloudZero Background Job: Error in hourly export task: {str(e)}"
                    )
