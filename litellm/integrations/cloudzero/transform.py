...
class CBFTransformer:
    ...
    def transform(self, data: pl.DataFrame) -> pl.DataFrame:
        ...
        if filtered_data.is_empty():
            ...
            console = Console()
            console.print(
                f"[yellow]⚠️  Dropped all {original_count:,} records due to zero spend or missing team_id[/yellow]"
            )
            return pl.DataFrame()
        ...
        if zero_spend_dropped > 0:
            console.print(
                f"[yellow]⚠️  Dropped {zero_spend_dropped:,} of {original_count:,} records with zero spend or missing team_id[/yellow]"
            )

        if czrn_dropped_count > 0:
            console.print(
                f"[yellow]⚠️  Dropped {czrn_dropped_count:,} of {len(hourly_aggregated):,} aggregated records due to invalid CZRNs[/yellow]"
            )

        if len(cbf_data) > 0:
            console.print(
                f"[green]✓ Successfully transformed {len(cbf_data):,} hourly aggregated records[/green]"
            )
    ...
    def _aggregate_to_hourly(self, data: pl.DataFrame) -> pl.DataFrame:
        ...
        data_with_hour = data.with_columns(
            [
                ...
                .alias("parsed_tags"),  # Empty tags list for now
        ...
        aggregated = all_data.group_by(
            ...
            [
                ...
                pl.col("api_key")
                .first()
                .alias("api_key_sample"),  # Keep one for reference
                pl.col("status")
    ...
    def _create_cbf_record(self, row: dict[str, Any]) -> CBFRecord:
        ...
        total_prompt_tokens = int(
            extract_scalar(row.get("total_prompt_tokens", 0)) or 0
        )
        total_completion_tokens = int(
            extract_scalar(row.get("total_completion_tokens", 0)) or 0
        )
        request_count = int(extract_scalar(row.get("request_count", 0)) or 0)
        successful_requests = int(
            extract_scalar(row.get("successful_requests", 0)) or 0
        )
        ...
        cbf_record = {
            # Required CBF fields
            "time/usage_start": usage_time.isoformat()
            if usage_time
