...
def _is_wildcard_metrics(metrics: Sequence[str]) -> bool:
    """True if `metrics` is exactly the single-item wildcard sentinel `[PROMETHEUS_METRICS_WILDCARD]`."""
    return len(metrics) == 1 and metrics[0] == PROMETHEUS_METRICS_WILDCARD
...
class PrometheusLogger(CustomLogger):
    ...
    def _parse_prometheus_config(self) -> Dict[str, List[str]]:
        ...
        for group_config in config:
            ...
            if not _is_wildcard_metrics(parsed_config.metrics):
        ...
        for config in parsed_configs:
            if PROMETHEUS_METRICS_WILDCARD in config.metrics:
                ...
                if config.include_labels:
                    ...
                    unknown_labels = tuple(label for label in config.include_labels if label not in all_valid_labels)
                    if unknown_labels:
                        label_errors.append(
    ...
    def _build_label_filters(self, parsed_configs: List) -> Dict[str, List[str]]:
        ...
        wildcard_configs = tuple(c for c in parsed_configs if _is_wildcard_metrics(c.metrics))
        named_configs = tuple(c for c in parsed_configs if not _is_wildcard_metrics(c.metrics))
