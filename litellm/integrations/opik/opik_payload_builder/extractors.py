...
def extract_span_identifiers(
    ...
    if isinstance(current_span_data, dict):
        return (
            current_span_data.get("trace_id"),
            current_span_data.get("id")
        )
...
def extract_and_build_metadata(
    ...
    metadata = {
        k: v for k, v in opik_metadata.items()
        if k != "current_span_data"
    }
    ...
    if "response_cost_failure_debug_info" in litellm_kwargs:
        metadata["response_cost_failure_debug_info"] = (
            litellm_kwargs["response_cost_failure_debug_info"]
        )
