...
def completion(  # type: ignore # noqa: PLR0915
    ...
    **kwargs,
    ...
    try:
        ...
        elif custom_llm_provider == "apple_foundation_models":
            ## COMPLETION CALL - Apple Foundation Models (on-device, no API key needed)
            from litellm.llms.apple_foundation_models.chat import transformation

            try:
                response = transformation.completion(
                    model=model,
                    messages=messages,
                    model_response=model_response,
                    logging_obj=logging,
                    optional_params=optional_params,
                    litellm_params=litellm_params,
                    api_key=None,  # No API key for on-device models
                    api_base=None,  # No API base for on-device models
                    client=None,
                    acompletion=acompletion,
                    stream=stream,
                    timeout=timeout,
