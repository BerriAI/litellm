            ...
            for row in csv_reader:
                try:
                    ...
                    verification_token_data = {
                        ...
                        "soft_budget_cooldown": await parse_csv_value(row["soft_budget_cooldown"], "boolean"),
                        ...
                        "team_id": team_id,
                        "permissions": await parse_csv_value(row["permissions"], "json"),
                        "max_parallel_requests": await parse_csv_value(row["max_parallel_requests"], "int"),
                        ...
                        "budget_duration": await parse_csv_value(row["budget_duration"], "string"),
                        "budget_reset_at": await parse_csv_value(row["budget_reset_at"], "datetime"),
                        "allowed_cache_controls": await parse_csv_value(row["allowed_cache_controls"], "string_array"),
                        "model_spend": await parse_csv_value(row["model_spend"], "json"),
                        "model_max_budget": await parse_csv_value(row["model_max_budget"], "json"),
                        "budget_id": await parse_csv_value(row["budget_id"], "string"),
                        "blocked": await parse_csv_value(row["blocked"], "boolean"),
                        "created_at": await parse_csv_value(row["created_at"], "datetime"),
                        "updated_at": await parse_csv_value(row["updated_at"], "datetime"),
                        "allowed_routes": await parse_csv_value(row["allowed_routes"], "string_array"),
                        "object_permission_id": await parse_csv_value(row["object_permission_id"], "string"),
                        "created_by": await parse_csv_value(row["created_by"], "string"),
                        "updated_by": await parse_csv_value(row["updated_by"], "string"),
                        "organization_id": await parse_csv_value(row["organization_id"], "string"),
                    }

                    # Remove None values to use database defaults
                    verification_token_data = {k: v for k, v in verification_token_data.items() if v is not None}
                    ...
                    if existing_token:
                        print(f"Token {verification_token_data['token']} already exists, skipping...")
                        continue

                    # Insert the record
                    await prisma.litellm_verificationtoken.create(data=verification_token_data)

                    processed_count += 1
                    print(f"Successfully migrated token: {verification_token_data['token']}")

                except Exception as e:
                    error_count += 1
                    print(f"Error processing row with token {row.get('token', 'unknown')}: {str(e)}")
                    continue
